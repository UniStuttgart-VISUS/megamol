...
/*
 * misc::VolumeCache::VolumeCache
 */
misc::VolumeCache::VolumeCache(void) : Module(),
    ...
    this->outDataSlot.SetCallback(CallVolumeData::ClassName(), CallVolumeData::FunctionName(0), &VolumeCache::outDataCallback);
    this->outDataSlot.SetCallback(CallVolumeData::ClassName(), CallVolumeData::FunctionName(1), &VolumeCache::outExtentCallback);
    ...
    this->useCacheSlot << new param::BoolParam(true);
    this->MakeSlotAvailable(&this->useCacheSlot);

    this->forceAndSaveSlot << new param::ButtonParam();
    this->MakeSlotAvailable(&this->forceAndSaveSlot);

    this->saveSlot << new param::ButtonParam();
 ...
 * misc::VolumeCache::~VolumeCache
 */
misc::VolumeCache::~VolumeCache(void) {
    this->Release();
}
...
/*
 * misc::VolumeCache::create
 */
bool misc::VolumeCache::create(void) {
    // Intentionally empty
    return true;
}
...
/*
 * misc::VolumeCache::release
 */
void misc::VolumeCache::release(void) {
    // paranoia
    this->attr.Clear();
...
/*
 * misc::VolumeCache::outDataCallback
 */
bool misc::VolumeCache::outDataCallback(Call& caller) {
    CallVolumeData *o = dynamic_cast<CallVolumeData*>(&caller);
    if (o == NULL) return false;
    CallVolumeData *i = this->inDataSlot.CallAs<CallVolumeData>();

    if (this->useCacheSlot.Param<param::BoolParam>()->Value()) {
...
/*
 * misc::VolumeCache::outExtentCallback
 */
bool misc::VolumeCache::outExtentCallback(Call& caller) {
    CallVolumeData *o = dynamic_cast<CallVolumeData*>(&caller);
    if (o == NULL) return false;
    CallVolumeData *i = this->inDataSlot.CallAs<CallVolumeData>();
...
/*
 * misc::VolumeCache::loadCache
 */
bool misc::VolumeCache::loadCache(void) {
    ...
    for (unsigned int a = 0; a < static_cast<unsigned int>(this->attr.Count()); a++) {
        ...
        if (file.Read(&b, 1) != 1) return false;
        this->attr[a].SetType(static_cast<CallVolumeData::DataType>(b));
...
/*
 * misc::VolumeCache::saveCache
 */
void misc::VolumeCache::saveCache(void) {
    vislib::sys::File file;
    if (!file.Open(this->filenameSlot.Param<param::FilePathParam>()->Value(),
...
/*
 * misc::VolumeCache::buildCache
 */
void misc::VolumeCache::buildCache(CallVolumeData* inDat) {
    ASSERT(inDat != NULL);
    ...
    for (unsigned int a = 0; a < static_cast<unsigned int>(this->attr.Count()); a++) {
        ...
        case CallVolumeData::TYPE_BYTE: bpv = 1; break;
        case CallVolumeData::TYPE_FLOAT: bpv = 4; break;
        case CallVolumeData::TYPE_DOUBLE: bpv = 8; break;
