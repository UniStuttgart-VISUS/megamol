#include "stdafx.h"
#include "..\include\mmstd_datatools\MultiParticleDataAdaptor.h"
...
MultiParticleDataAdaptor::MultiParticleDataAdaptor(core::moldyn::MultiParticleDataCall& data)
    ...
    for (unsigned int pli = 0; pli < plc; pli++) {
        ...
        if (list[pli].count < 0) list[pli].count = 0;
        if (list[pli].count == 0) continue;
        ...
        if (pl.GetVertexDataType() == MultiParticleDataCall::Particles::VERTDATA_FLOAT_XYZ) {
            if (list[pli].pos_data_step < 12) list[pli].pos_data_step = 12;
        }
        else if (pl.GetVertexDataType() == MultiParticleDataCall::Particles::VERTDATA_FLOAT_XYZR) {
            if (list[pli].pos_data_step < 16) list[pli].pos_data_step = 16;
        }
    ...
    if (plc > 0) list[plc - 1].next = nullptr;
