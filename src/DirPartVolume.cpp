...
/*
 * volume::DirPartVolume::DirPartVolume
 */
volume::DirPartVolume::DirPartVolume(void) : core::Module(),
    ...
    this->xResSlot << new core::param::IntParam(64, 2);
    this->MakeSlotAvailable(&this->xResSlot);

    this->yResSlot << new core::param::IntParam(64, 2);
    this->MakeSlotAvailable(&this->yResSlot);

    this->zResSlot << new core::param::IntParam(64, 2);
    this->MakeSlotAvailable(&this->zResSlot);

    this->sampleRadiusSlot << new core::param::FloatParam(2.0f, 0.0f);
    this->MakeSlotAvailable(&this->sampleRadiusSlot);

    this->rebuildSlot << new core::param::ButtonParam();
 ...
 * volume::DirPartVolume::~DirPartVolume
 */
volume::DirPartVolume::~DirPartVolume(void) {
    this->Release();
}


/*
 * volume::DirPartVolume::create
 */
bool volume::DirPartVolume::create(void) {
    // intentionally empty
    return true;
}
...
/*
 * volume::DirPartVolume::release
 */
void volume::DirPartVolume::release(void) {
    ARY_SAFE_DELETE(this->data);
}


/*
 * volume::DirPartVolume::outExtend
 */
bool volume::DirPartVolume::outExtend(core::Call& caller) {
    ...
    if ((dpd == NULL) || (!(*dpd)(1))) {
        ...
        float sx = 0.5f / static_cast<float>(this->xResSlot.Param<core::param::IntParam>()->Value());
        float sy = 0.5f / static_cast<float>(this->yResSlot.Param<core::param::IntParam>()->Value());
        float sz = 0.5f / static_cast<float>(this->zResSlot.Param<core::param::IntParam>()->Value());
...
/*
 * volume::DirPartVolume::outData
 */
bool volume::DirPartVolume::outData(core::Call& caller) {
    core::CallVolumeData *cvd = dynamic_cast<core::CallVolumeData*>(&caller);
    if (cvd == NULL) return false;
    ...
    unsigned int sx = static_cast<unsigned int>(this->xResSlot.Param<core::param::IntParam>()->Value());
    unsigned int sy = static_cast<unsigned int>(this->yResSlot.Param<core::param::IntParam>()->Value());
    unsigned int sz = static_cast<unsigned int>(this->zResSlot.Param<core::param::IntParam>()->Value());
    ...
    if (rebuild) {
        ...
        float rad = this->sampleRadiusSlot.Param<core::param::FloatParam>()->Value();
        ...
        for (unsigned int pli = 0; pli < plCnt; pli++) {
            ...
            core::moldyn::SimpleSphericalParticles::VertexDataType vertDT = parts.GetVertexDataType();
        ...
        for (unsigned int pli = 0; pli < plCnt; pli++) {
            ...
            if (vertStp < sizeof(float) * ((vertDT == core::moldyn::SimpleSphericalParticles::VERTDATA_FLOAT_XYZ) ? 3 : 4)) {
                // everything between 0 and this is stupid anyway
                vertStp = sizeof(float) * ((vertDT == core::moldyn::SimpleSphericalParticles::VERTDATA_FLOAT_XYZ) ? 3 : 4);
            }
    ...
    cvd->Attribute(0).SetType(core::CallVolumeData::TYPE_FLOAT);
    cvd->Attribute(0).SetData(&this->data[sx * sy * sz * 0]);
    cvd->Attribute(1).SetName("s");
    cvd->Attribute(1).SetType(core::CallVolumeData::TYPE_FLOAT);
    cvd->Attribute(1).SetData(&this->data[sx * sy * sz * 1]);
    cvd->Attribute(2).SetName("a");
    cvd->Attribute(2).SetType(core::CallVolumeData::TYPE_FLOAT);
