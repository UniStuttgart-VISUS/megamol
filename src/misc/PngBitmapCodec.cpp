...
bool PngBitmapCodec::loadFromMemory(const void *mem, SIZE_T size) {
    ...
    png_structp png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, pngutil::pngErrorThrow, pngutil::pngWarningIgnore);
    ...
    if ((color_type & PNG_COLOR_MASK_PALETTE) == PNG_COLOR_TYPE_PALETTE) {
        ...
        unsigned int chanCnt
            = (((color_type & PNG_COLOR_MASK_COLOR) == PNG_COLOR_MASK_COLOR) ? 3 : 1)
...
bool PngBitmapCodec::saveToStream(vislib::sys::File& stream) const {
    ...
    png_structp png_ptr = ::png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL, pngutil::pngErrorThrow, pngutil::pngWarningIgnore);
    if (!png_ptr) return false;
    ...
    if (!src->EqualChannelLayout(hasAlpha ? BitmapImage::TemplateByteRGBA
            : BitmapImage::TemplateByteRGB)) {
        alt = new BitmapImage();
        alt->ConvertFrom(*src, hasAlpha ? BitmapImage::TemplateByteRGBA
            : BitmapImage::TemplateByteRGB);
        src = alt.operator->();
    }
