...
SplitMergeRenderer::SplitMergeRenderer( void ) : Renderer2DModule (),
        ...
        numVisibilityPropagationRounds("propagation rounds", "how many times visibility is propagated through transitions"),
        bounds(), selectionLevel(),
...
/*
    ...
    glPathGlyphRangeNV(fontBase,
        GL_SYSTEM_FONT_NAME_NV, "Verdana", GL_NONE,
        0, numChars,
...
void SplitMergeRenderer::calcExtents() {
    ...
    for (int i = 0; i < sortedSeries.Count(); i++) {
        SplitMergeCall::SplitMergeMappable *smm = diagram->GetSeries(sortedSeries[i])->GetMappable();
        ...
        if (flags & view::MOUSEFLAG_MODKEY_ALT_DOWN) {
            if (flags & view::MOUSEFLAG_BUTTON_LEFT_DOWN) {
                if (x > bounds.Left() && x < bounds.Right() && y > bounds.Bottom() && y < bounds.Top()) {
                    ...
                    if (series >= 0 && series < sortedSeries.Count() && (tmp-static_cast<int>(tmp)) < 0.5f) {
                        //Log::DefaultLog.WriteInfo(0, "I hit series %s", diagram->GetSeries(sortedSeries[series])->GetName());
                        consumeEvent = true;
...
void SplitMergeRenderer::closePath(SplitMergeCall::SplitMergeMappable *smm, int seriesIdx,
...
bool SplitMergeRenderer::Render(view::CallRender2D &call) {
    ...
    diagram = this->dataCallerSlot.CallAs<SplitMergeCall>();
    if( diagram == NULL ) return false;
    // execute the call
    if( !(*diagram)(SplitMergeCall::CallForGetData) ) return false;
    ...
    if (this->visibilityFromSelection.Param<param::BoolParam>()->Value()) {
        ...
        for (int i = 0; i < this->numVisibilityPropagationRounds.Param<param::IntParam>()->Value(); i++) {
            for(int t = 0; t < diagram->GetTransitionCount(); t++) {
                SplitMergeCall::SplitMergeTransition *smt = diagram->GetTransition(t);
    ...
    GLuint pathBase = glGenPathsNV(sortedSeries.Count());
    for (int i = 0; i < sortedSeries.Count(); i++) {
        SplitMergeCall::SplitMergeSeries *sms = diagram->GetSeries(sortedSeries[i]);
        SplitMergeCall::SplitMergeMappable *smm = sms->GetMappable();
        int increment = 50;
        ...
        glPathCommandsNV(pathBase + i, cmds.Count(), cmds.PeekElements(), coords.Count(), GL_FLOAT, coords.PeekElements());
    ...
    glDeletePathsNV(pathBase, sortedSeries.Count());
    ...
    pathBase = glGenPathsNV(diagram->GetTransitionCount());
    vislib::Array<GLubyte> cmds;
    ...
    for(int i = 0; i < diagram->GetTransitionCount(); i++) {
        ...
        SplitMergeCall::SplitMergeSeries *srcSeries = this->diagram->GetSeries( srcSeriesIdx);
        SplitMergeCall::SplitMergeMappable *srcSMM = srcSeries->GetMappable();
        ...
        SplitMergeCall::SplitMergeSeries *dstSeries = this->diagram->GetSeries( dstSeriesIdx);
        SplitMergeCall::SplitMergeMappable *dstSMM = dstSeries->GetMappable();
        ...
        glPathCommandsNV(pathBase + counter, cmds.Count(), cmds.PeekElements(), coords.Count(), GL_FLOAT, coords.PeekElements());
    ...
    //    counter++;
    //}
    glDeletePathsNV(pathBase, diagram->GetTransitionCount());
    ...
    for (int i = 0; i < sortedSeries.Count(); i++) {
        ...
        glGetPathSpacingNV(GL_ACCUM_ADJACENT_PAIRS_NV,
            theName.Length(), GL_UNSIGNED_BYTE, theName.PeekBuffer(),
            this->fontBase,
            1.0f, 1.0f,
            GL_TRANSLATE_X_NV,
            kerning + 1);
        ...
        glPushMatrix();
        glTranslatef(- kerning[theName.Length() - 1] - this->fontSize * 0.5f,
            -i * seriesSpacing - this->fontSize * 0.25f, 0.0f);

        glStencilFillPathInstancedNV(theName.Length() - 1,
            GL_UNSIGNED_BYTE, theName.PeekBuffer(), fontBase,
            GL_PATH_FILL_MODE_NV, 0xFF,
            GL_TRANSLATE_X_NV, kerning);

        glCoverFillPathInstancedNV(theName.Length() - 1, 
            GL_UNSIGNED_BYTE, theName.PeekBuffer(), fontBase,
    ...
    if (this->showGuidesParam.Param<param::BoolParam>()->Value()) {
        for (int i = 0; i < diagram->GetGuideCount(); i++) {
            SplitMergeCall::SplitMergeGuide *g = diagram->GetGuide(i);
            ::glDisable(GL_BLEND);
            ...
            float pos;
            switch(g->GetType()) {
                case SplitMergeCall::SPLITMERGE_GUIDE_HORIZONTAL:
                    ...
                    break;
                case SplitMergeCall::SPLITMERGE_GUIDE_VERTICAL:
