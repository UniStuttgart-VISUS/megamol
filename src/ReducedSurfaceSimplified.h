...
namespace protein_cuda {
	 ...
	 */
	class ReducedSurfaceSimplified {
	public:
		/**
		 * Simplified Chain class.
		 * Stores the simplified representation of an amino acid chain. The
		 * backbone and the side chain are represented as the minimal sphere
		 * which encloses all associated atoms (bounding sphere).
		 */
		...
		public:
			...
			const vislib::math::Vector<float, 3> BackbonePos( unsigned int idx) const;
			const vislib::math::Vector<float, 3> SidechainPos( unsigned int idx) const;
			float BackboneRadius( unsigned int idx) const;
			float SidechainRadius( unsigned int idx) const;
			unsigned int BackboneAtomIndex( unsigned int idx) const;
			unsigned int SidechainAtomIndex( unsigned int idx) const;
			/** The number of amino acids in this chain */
			unsigned int AminoAcidCount() const { return (unsigned int)backbonePos.size(); };
			/** The maximum radius of a backbone or sidechain */
			float MaxRadius() const { return maxRad; };
			/**
			 * Get the chain length, i.e. the total number of elements
			 * (backbones + sidechains) with radius > 0.
			 */
			unsigned int GetChainLength() const { return chainLength; };
		...
		private:
			...
			/** position of the backbones */
			std::vector<vislib::math::Vector<float, 3> > backbonePos;
			/** radius of the backbones */
			std::vector<float> backboneRadius;
			/** index of first atom in backbones */
			std::vector<unsigned int> backboneAtomIndex;
			/** position of the sidechains */
			std::vector<vislib::math::Vector<float, 3> > sidechainPos;
			/** radius of the sidechains */
			std::vector<float> sidechainRadius;
			/** index of first atom in sidechains */
			std::vector<unsigned int> sidechainAtomIndex;
			/** maximum radius of a backbone or sidechain */
			float maxRad;
			/** the total number of elements (backbones + sidechains) with radius > 0 */
			unsigned int chainLength;
		};
		
		// forward declaration of the RS-classes
		class RSVertex;
		class RSEdge;
		class RSFace;
		...
		public:
			...
			/** test for equality */
			bool operator==(const RSVertex& rhs) const { return ( idx == rhs.idx ); };
			/** getter for the atom index */
			const unsigned int GetIndex() const { return idx; };
			/** getter for the edge pointer list */
			RSEdge* GetEdge( unsigned int eIdx) const { return edgeList.at( eIdx); };
			/** getter for the edge list size */
			const unsigned int GetEdgeCount() const { return (unsigned int)edgeList.size(); };
			/** add edge */
			...
			/** set atom index */
			void SetAtomIdx( unsigned int atomIdx) { idx = atomIdx; edgeList.clear(); };
			/** set if the atom is burried */
			void SetAtomBuried( bool bury) { if( !edgeList.empty() ) return; else buried = bury; };
			/** returns true if the atom is burried, otherwise false */
			bool IsBuried() const { return buried; };
			void ClearEdgeList() { edgeList.clear(); buried = false; };
			/** Get the atom position */
			const vislib::math::Vector<float, 3> GetPosition() const { return position; };
			/** Set the atom position */
			void SetPosition( vislib::math::Vector<float, 3> pos) { position = pos; };
			/** Get the atom radius */
			float GetRadius() { return radius; };
			/** Get the status of the RS-vertex */
			bool Treated() { return treated; };
		...
		{
		public:
			RSEdge( RSVertex *v1, RSVertex *v2, 
			...
			void SetRSFaces( RSFace *f1, RSFace *f2) { face1 = f1; face2 = f2; };
			RSVertex* GetVertex1() const { return vert1; };
			RSVertex* GetVertex2() const { return vert2; };
			RSFace* GetFace1() const { return face1; };
			RSFace* GetFace2() const { return face2; };
			const vislib::math::Vector<float, 3>& GetTorusCenter() const { return torusCenter; };
			float GetTorusRadius() const { return torusRadius; };
		...
		public:
			...
			RSFace( RSVertex *v1, RSVertex *v2, RSVertex *v3,
			...
			const vislib::math::Vector<float, 3>& GetProbeCenter() const { return probeCenter; };
			const vislib::math::Vector<float, 3>& GetFaceNormal() const { return normal; };
			RSVertex* GetVertex1() const { return vert1; };
			RSVertex* GetVertex2() const { return vert2; };
			RSVertex* GetVertex3() const { return vert3; };
			RSEdge* GetEdge1() const { return edge1; };
			RSEdge* GetEdge2() const { return edge2; };
			RSEdge* GetEdge3() const { return edge3; };
			void SetEdge1( RSEdge *e) { edge1 = e; };
			void SetEdge2( RSEdge *e) { edge2 = e; };
			void SetEdge3( RSEdge *e) { edge3 = e; };
			void SetDualFace( RSFace *f) { dualFace = f; };
			RSFace* GetDualFace() { return dualFace; };
			void SetProbeIndex( unsigned int x, unsigned int y, unsigned int z) { probeIdx.Set( x, y, z); };
			const vislib::math::Vector<unsigned int, 3> GetProbeIndex() const { return probeIdx; };
		 ...
		 */
		unsigned int GetRSVertexCount() { return (unsigned int)rsVertex.size(); };
		
		/**
		 * Get the number of RS-edges.
		 * @return The number of RS-edges.
		 */
		unsigned int GetRSEdgeCount() { return (unsigned int)rsEdge.size(); };
		
		/**
		 * Get the number of RS-faces.
		 * @return The number of RS-faces.
		 */
		unsigned int GetRSFaceCount() { return (unsigned int)rsFace.size(); };
		...
		{ if( rsVertex.size() > idx ) return rsVertex[idx]; else return NULL; };
		...
		{ if( rsEdge.size() > idx ) return rsEdge[idx]; else return NULL; };
		...
		{ if( rsFace.size() > idx ) return rsFace[idx]; else return NULL; };

		/**
		 * Get the number of RS-edges, which are cut by at least one probe.
		 * @return The number of cut RS-edges.
		 */
		unsigned int GetCutRSEdgesCount() { return countCutEdges; };
		...
		unsigned int SimpleChainCount() const { return (unsigned int)simpleChain.size(); };
		const SimplifiedChain* SimpleChain( unsigned int idx) const { return simpleChain.at( idx); };
		
	protected:

		/** compute the reduced surface */
		void ComputeReducedSurfaceSimplified();
		...
		bool SphereSphereIntersection( vislib::math::Vector<float, 3> m1, float rad1, 
			vislib::math::Vector<float, 3> m2, float rad2);
		 ...
		 */
		float ComputeAngleBetweenProbes(
