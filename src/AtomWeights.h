...
static float getElementWeightByProtonCount(unsigned int protonCount) {
	switch (protonCount){
		case 1: return 1.0079f; // H
		case 6: return 12.011f; // C
		case 7: return 14.007f; // N
		case 8: return 15.999f; // O
		case 16: return 32.065f; // S
		default: return 0.0f;
	}
...
static float getElementWeightBySymbolString(const vislib::StringA& elementString) {
	...
	if (es.StartsWith("h")) return 1.0079f;
	else if (es.StartsWith("c")) return 12.011f;
	else if (es.StartsWith("n")) return 14.007f;
	else if (es.StartsWith("o")) return 15.999f;
	else if (es.StartsWith("s")) return 32.065f;
