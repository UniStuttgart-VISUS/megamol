...
bool SimpleMoleculeRenderer::create(void) {
    ...
    if (!this->GetCoreInstance()->ShaderSourceFactory().MakeShaderSource(
            "protein::std::sphereFragmentOR", fragSrc)) {
        Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR,
                "Unable to load vertex shader source for filter sphere shader (OR)");
        return false;
    }
    try {
    ...
    if (!this->GetCoreInstance()->ShaderSourceFactory().MakeShaderSource(
        ...
        Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR,
                "Unable to load geometry shader source for sphere shader");
        return false;
    }
    if (!this->GetCoreInstance()->ShaderSourceFactory().MakeShaderSource(
            "protein::std::sphereFragmentGeom", fragSrc)) {
        Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR,
                "Unable to load fragment shader source for sphere shader");
        return false;
    }
    ...
    if (!this->GetCoreInstance()->ShaderSourceFactory().MakeShaderSource(
        ...
        Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR,
                "Unable to load fragment shader source for cylinder shader");
        return false;
    }
