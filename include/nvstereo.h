...
namespace nv {
    namespace stereo {
        ...
        struct D3D11Type {
            ...
            static const NV_STEREO_REGISTRY_PROFILE_TYPE RegistryProfileType 
                = NVAPI_STEREO_DEFAULT_REGISTRY_PROFILE;

            // Note that the texture must be at least 20 bytes wide to handle the stereo header.
            static const int StereoTexWidth = 8;
            static const int StereoTexHeight = 1;
            static const DXGI_FORMAT StereoTexFormat 
                = DXGI_FORMAT_R32G32B32A32_FLOAT;
            static const int StereoBytesPerPixel = 16;

            static StagingResource *CreateStagingResource(Device *pDevice, 
                    float eyeSep, float sep, float conv) {
                ...
                sysData.SysMemPitch = StereoBytesPerPixel * stagingWidth;
                sysData.pSysMem = new unsigned char[sysData.SysMemPitch 
                    * stagingHeight];

                unsigned char *data = const_cast<unsigned char *>(
                    static_cast<const unsigned char *>(sysData.pSysMem));
                float *leftEyePtr = reinterpret_cast<float *>(data);
                float *rightEyePtr = leftEyePtr + StereoTexWidth
                    * StereoBytesPerPixel / sizeof(float);
                LPNVSTEREOIMAGEHEADER header 
                    = reinterpret_cast<LPNVSTEREOIMAGEHEADER>(data 
                    + sysData.SysMemPitch * StereoTexHeight);
                PopulateTextureData(leftEyePtr, rightEyePtr, header, 
                    stagingWidth, stagingHeight, StereoBytesPerPixel, 
                    eyeSep, sep, conv);
            ...
            static void UpdateTextureFromStaging(Device *pDevice, Texture *tex, 
                ...
                immediateContext->CopySubresourceRegion(tex, 0, 0, 0, 0, 
                    staging, 0, &stereoSrcBox);
