...
namespace protein {
	...
	public:
		...
		void setG(vislib::math::Vector<float, 3> v1, vislib::math::Vector<float, 3> v2, vislib::math::Vector<float, 3> v3, vislib::math::Vector<float, 3> v4);

		// set the number of segments to create --> this function also sets matrix S!
		void setN(unsigned int n);

		// set the backbone atom coordinates
		void setBackbone( std::vector<vislib::math::Vector<float, 3> > bb) { backbone.clear(); backbone = bb; };
