...
void writeSVG(const std::vector<int>& buckets, float rangeMin, float rangeMax) {
    ...
    svg <<"<?xml version=\"1.0\" standalone=\"no\"?>"<<std::endl
        ...
        <<"<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" viewBox=\"0 0 "<<width<<" "<<height<<"\">"<<std::endl;
    ...
    for (int i=0; i<=10; ++i) {
        svg<<"<text x=\""<<(0.09*width)<<"\" y=\""<<(0.1 + i*(0.8/10.0))*height<<"\" fill=\"black\" text-anchor=\"end\">"<<(10-i)/10.0*m<<"</text>"<<std::endl;
    }
...
bool stdplugin::datatools::DumpIColorHistogramModule::dump(::megamol::core::param::ParamSlot& param) {
    ...
    for (unsigned int pli = 0; pli < dat->GetParticleListCount(); pli++) {
        ...
        if (colRng <= 0.00001f) colRng = 1.0f;
