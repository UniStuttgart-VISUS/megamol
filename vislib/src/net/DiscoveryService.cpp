 ...
 * vislib::net::cluster::DiscoveryService::PeerNode::PeerNode
 */
vislib::net::cluster::DiscoveryService::PeerNode::PeerNode(const IPEndPoint& discoveryAddress,
    const IPEndPoint& responseAddress, const UINT cntResponseChances, DiscoveryConfigEx* discoverySource)
    ...
    this->discoveryAddress.SetPort(this->discoverySource->GetBcastAddress().GetPort());

    VLTRACE(Trace::LEVEL_VL_VERBOSE, "New peer node %s, discovered via %s.\n",
        this->responseAddress.ToStringA().PeekBuffer(), this->discoveryAddress.ToStringA().PeekBuffer());
}
 ...
 */
vislib::net::cluster::DiscoveryService::PeerNode& vislib::net::cluster::DiscoveryService::PeerNode::operator=(
    const PeerNode& rhs) {
    if (this != &rhs) {
...
bool vislib::net::cluster::DiscoveryService::PeerNode::decrementResponseChances(void) {
    if (this->cntResponseChances > 0) {
        ...
        VLTRACE(Trace::LEVEL_VL_VERBOSE,
            "Response chances for %s decremented "
            "to %d.\n",
            this->responseAddress.ToStringA().PeekBuffer(), this->cntResponseChances);
        return true;
 ...
 */
vislib::net::cluster::DiscoveryService::DiscoveryConfig::DiscoveryConfig(
    const vislib::net::IPEndPoint& responseAddress, const vislib::net::IPAddress& bcastAddress, const USHORT bindPort)
 ...
 */
vislib::net::cluster::DiscoveryService::DiscoveryConfig::DiscoveryConfig(
    const vislib::net::IPEndPoint& responseAddress, const vislib::net::IPAddress6& bcastAddress, const USHORT bindPort)
        : bcastAddress(bcastAddress, bindPort)
 ...
 */
vislib::net::cluster::DiscoveryService::DiscoveryConfig::DiscoveryConfig(
    const IPEndPoint& responseAddress, const IPAgnosticAddress& bcastAddress, const USHORT bindPort)
        : bcastAddress(bcastAddress, bindPort)
        , bindAddress(static_cast<IPEndPoint::AddressFamily>(bcastAddress.GetAddressFamily()), bindPort)
        , responseAddress(responseAddress) {}
 ...
 */
vislib::net::cluster::DiscoveryService::DiscoveryConfig::DiscoveryConfig(
    const IPEndPoint& responseAddress, const USHORT bindPort)
    ...
    NetworkInformation::GetAdaptersForUnicastAddress(candidates, responseAddress.GetIPAddress());

    for (SIZE_T i = 0; i < candidates.Count(); i++) {
        this->bcastAddress.SetIPAddress(candidates[i].GetBroadcastAddress(&confidence));
 ...
 * vislib::net::cluster::DiscoveryService::DiscoveryConfig::DiscoveryConfig
 */
vislib::net::cluster::DiscoveryService::DiscoveryConfig::DiscoveryConfig(const DiscoveryConfig& rhs) {
    *this = rhs;
}
...
vislib::net::cluster::DiscoveryService::DiscoveryConfig::operator=(const DiscoveryConfig& rhs) {
    if (this != &rhs) {
 ...
 * vislib::net::cluster::DiscoveryService::DiscoveryConfig::operator ==
 */
bool vislib::net::cluster::DiscoveryService::DiscoveryConfig::operator==(const DiscoveryConfig& rhs) const {
    return ((this->bcastAddress == rhs.bcastAddress) && (this->bindAddress == rhs.bindAddress) &&
            (this->responseAddress == rhs.responseAddress));
}
 ...
 */
const UINT vislib::net::cluster::DiscoveryService::DEFAULT_REQUEST_INTERVAL = 10 * 1000;
 ...
 */
void vislib::net::cluster::DiscoveryService::AddListener(DiscoveryListener* listener) {
 ...
 */
vislib::net::IPAddress vislib::net::cluster::DiscoveryService::GetDiscoveryAddress4(const PeerHandle& hPeer) const {
 ...
 */
vislib::net::IPAddress6 vislib::net::cluster::DiscoveryService::GetDiscoveryAddress6(const PeerHandle& hPeer) const {
    //IPAddress6 retval;
 ...
 */
bool vislib::net::cluster::DiscoveryService::IsSelf(const INT idx) const {
 ...
 */
void vislib::net::cluster::DiscoveryService::RemoveListener(DiscoveryListener* listener) {
    ASSERT(listener != NULL);
...
UINT vislib::net::cluster::DiscoveryService::SendUserMessage(
    ...
    for (SIZE_T i = 0; i < this->peerNodes.Count(); i++) {
        try {
            this->peerNodes[i]->discoverySource->SendMessageTo(msg, this->peerNodes[i]->discoveryAddress);
        } catch (SocketException e) {
            VLTRACE(Trace::LEVEL_VL_WARN,
                "DiscoveryService could not send "
                "user message (\"%s\").\n",
                e.GetMsgA());
            retval++;
        }
 ...
 */
UINT vislib::net::cluster::DiscoveryService::SendUserMessage(
    const PeerHandle& hPeer, const UINT32 msgType, const void* msgBody, const SIZE_T msgSize) {
    Message msg;     // The datagram we are going to send.
 ...
 */
void vislib::net::cluster::DiscoveryService::Start(const char* name, const DiscoveryConfig* configs,
    const SIZE_T cntConfigs, const UINT cntExpectedNodes, const UINT32 flags, const UINT requestInterval,
    const UINT requestIntervalIntensive, const UINT cntResponseChances) {
    ...
    this->requestIntervalIntensive = (requestInterval > requestIntervalIntensive)
...
bool vislib::net::cluster::DiscoveryService::Stop(const bool noWait) {
    ...
    try {
        ...
        VLTRACE(Trace::LEVEL_VL_ERROR,
            "Stopping the discovery sender thread "
 ...
 */
vislib::net::IPEndPoint vislib::net::cluster::DiscoveryService::operator[](const PeerHandle& hPeer) const {
    IPEndPoint retval;
...
DWORD vislib::net::cluster::DiscoveryService::Receiver::Run(void* dcfg) {
    ...
    ASSERT(reinterpret_cast<BYTE*>(&(msg.SenderBody)) == reinterpret_cast<BYTE*>(&msg) + 2 * sizeof(UINT32));
     ...
     */
    VLTRACE(Trace::LEVEL_VL_VERBOSE,
        "The discovery receiver thread is "
        "preparing its socket on %s ...\n",
        config->GetBindAddress().ToStringA().PeekBuffer());
    try {
        Socket::Startup();
        this->socket.Create(config->GetProtocolFamily(), Socket::TYPE_DGRAM, Socket::PROTOCOL_UDP);
        ...
        VLTRACE(Trace::LEVEL_VL_ERROR,
            "Discovery receiver thread could not "
    ...
    if (!config->GetDiscoveryService().IsObserver()) {
        ...
        config->GetDiscoveryService().addPeerNode(discoveryAddr, config, config->GetResponseAddress());
    }
    ...
    while (this->isRunning != 0) {
        try {
            ...
            if (this->socket.Receive(peerAddr, &msg, sizeof(Message)) != sizeof(Message)) {
                break;
            }

            if (msg.MagicNumber != MAGIC_NUMBER) {
                VLTRACE(Trace::LEVEL_VL_ERROR,
                    "Discovery service received "
                    "invalid magic number %d from %s.\n",
                    msg.MagicNumber, peerAddr.ToStringA().PeekBuffer());
                continue;
            }
            ...
            if (msg.MsgType >= MSG_TYPE_USER) {
                 ...
                 */
                config->GetDiscoveryService().fireUserMessage(peerAddr, config, msg.MsgType, msg.UserData);
                continue;
            }

            if (!config->GetDiscoveryService().GetName().Equals(msg.SenderBody.Name)) {
                VLTRACE(Trace::LEVEL_VL_ERROR,
                    "Discovery service received "
                    "message for different cluster \"%s\" from %s.\n",
                    msg.SenderBody.Name, peerAddr.ToStringA().PeekBuffer());
                continue;
            }
            ...
            case MSG_TYPE_IAMALIVE:
                ...
                config->GetDiscoveryService().addPeerNode(peerAddr, config, IPEndPoint(msg.SenderBody.ResponseAddress));
                break;
            ...
            case MSG_TYPE_IAMHERE:
                ...
                if (!config->GetDiscoveryService().IsObserver()) {
                    ...
                    msg.SenderBody.ResponseAddress = config->GetResponseAddress();
                    this->socket.Send(peerAddr, &msg, sizeof(Message));
                    VLTRACE(Trace::LEVEL_VL_INFO,
                        "Discovery service sent "
            ...
            case MSG_TYPE_SAYONARA:
                ...
                config->GetDiscoveryService().removePeerNode(IPEndPoint(msg.SenderBody.ResponseAddress));
                break;
            }

        } catch (SocketException e) {
            VLTRACE(Trace::LEVEL_VL_WARN,
                "A socket error occurred in the "
    ...
    try {
        ...
        VLTRACE(Trace::LEVEL_VL_ERROR,
            "Socket cleanup failed in the discovery "
 ...
 * vislib::net::cluster::DiscoveryService::DiscoveryConfigEx::DiscoveryConfigEx
 */
vislib::net::cluster::DiscoveryService::DiscoveryConfigEx::DiscoveryConfigEx(const DiscoveryConfigEx& rhs)
        : Super(rhs)
        , cds(rhs.cds) {}
 ...
 */
void vislib::net::cluster::DiscoveryService::DiscoveryConfigEx::SendCustomisedMessage(Message& message) {
 ...
 */
void vislib::net::cluster::DiscoveryService::DiscoveryConfigEx::SendMessageTo(Message& message, const IPEndPoint& to) {
    Socket* socket = NULL;
    ...
    if (message.MsgType < MSG_TYPE_USER) {
        ...
        VLTRACE(Trace::LEVEL_VL_VERBOSE,
            "Using broadcast socket for sending a "
            "message to %s ...\n",
        ...
        if (!this->socketUserMsg.IsValid()) {
            try {
                ...
                this->socketUserMsg.Create(this->GetProtocolFamily(), Socket::TYPE_DGRAM, Socket::PROTOCOL_UDP);
            } catch (SocketException e) {
                VLTRACE(Trace::LEVEL_VL_ERROR,
                    "The socket for sending user "
...
vislib::net::cluster::DiscoveryService::DiscoveryConfigEx::operator=(const DiscoveryConfigEx& rhs) {
    if (this != &rhs) {
        Super::operator=(rhs);
        this->cds = rhs.cds;
 ...
 * vislib::net::cluster::DiscoveryService::DiscoveryConfigEx::operator ==
 */
bool vislib::net::cluster::DiscoveryService::DiscoveryConfigEx::operator==(const DiscoveryConfigEx& rhs) const {
    if (Super::operator==(rhs)) {
        return ((this->cds == rhs.cds) && (this->socketSend == rhs.socketSend));
...
DWORD vislib::net::cluster::DiscoveryService::Sender::Run(void* cds) {
    ...
    ASSERT(reinterpret_cast<BYTE*>(&(request.SenderBody)) == reinterpret_cast<BYTE*>(&request) + 2 * sizeof(UINT32));
    ...
    try {
        ...
        VLTRACE(Trace::LEVEL_VL_ERROR,
            "Discovery sender thread could not "
...
#if (_MSC_VER >= 1400)
    ::strncpy_s(request.SenderBody.Name, MAX_NAME_LEN, ds->name.PeekBuffer(), MAX_NAME_LEN);
#else  /* (_MSC_VER >= 1400) */
    ...
    while (this->isRunning != 0) {
        ...
        try {
            if (this->isRunning != 0) {
                sys::Thread::Sleep(
                    (cntPeers < ds->cntExpectedNodes) ? ds->requestIntervalIntensive : ds->requestInterval);
            }
        } catch (sys::SystemException e) {
            VLTRACE(Trace::LEVEL_VL_WARN,
...
void vislib::net::cluster::DiscoveryService::addPeerNode(
    const IPEndPoint& discoveryAddr, DiscoveryConfigEx* discoverySource, const IPEndPoint& responseAddr) {
    ...
    if ((idx = this->peerFromResponseAddress(responseAddr)) >= 0) {
        ...
        hPeer = new PeerNode(discoveryAddr, responseAddr, this->cntResponseChances, discoverySource);
        this->peerNodes.Append(hPeer);

        this->listeners.Lock();
        ConstIterator<ListenerList::Iterator> it = this->listeners.GetConstIterator();
        while (it.HasNext()) {
...
void vislib::net::cluster::DiscoveryService::fireUserMessage(
    const IPEndPoint& sender, DiscoveryConfigEx* discoverySource, const UINT32 msgType, const BYTE* msgBody) {
    ...
    if ((idx = this->peerFromDiscoveryAddr(sender)) >= 0) {
        ...
        hPeer = new PeerNode(sender, IPEndPoint(IPAddress6::UNSPECIFIED, 0), this->cntResponseChances, discoverySource);
    }
    ...
    ConstIterator<ListenerList::Iterator> it = this->listeners.GetConstIterator();
    while (it.HasNext()) {
        it.Next()->OnUserMessage(*this, hPeer, (idx >= 0), msgType, msgBody);
    }
 ...
 */
INT_PTR vislib::net::cluster::DiscoveryService::peerFromResponseAddress(const IPEndPoint& addr) const {
 ...
 */
INT_PTR vislib::net::cluster::DiscoveryService::peerFromDiscoveryAddr(const IPEndPoint& addr) const {
    // TODO: Think of faster solution.
    INT_PTR retval = -1;

    for (SIZE_T i = 0; i < this->peerNodes.Count(); i++) {
        // TODO: Endpoint test for multiple discovery instances on one node
        if (this->peerNodes[i]->discoveryAddress.GetIPAddress() == addr.GetIPAddress()) {
            retval = i;
            break;
        }
    }
...
void vislib::net::cluster::DiscoveryService::prepareRequest(void) {
    ...
    for (SIZE_T i = 0; i < this->peerNodes.Count(); i++) {
        if (!this->peerNodes[i]->decrementResponseChances()) {
            ...
            ConstIterator<ListenerList::Iterator> it = this->listeners.GetConstIterator();
            while (it.HasNext()) {
                it.Next()->OnNodeLost(*this, this->peerNodes[i], DiscoveryListener::LOST_IMLICITLY);
            }
...
void vislib::net::cluster::DiscoveryService::prepareUserMessage(
    Message& outMsg, const UINT32 msgType, const void* msgBody, const SIZE_T msgSize) {
    ...
    ASSERT(reinterpret_cast<BYTE*>(&(outMsg.UserData)) == reinterpret_cast<BYTE*>(&outMsg) + 2 * sizeof(UINT32));
 ...
 */
void vislib::net::cluster::DiscoveryService::removePeerNode(const IPEndPoint& address) {
    INT_PTR idx = 0;
    ...
    if ((idx = this->peerFromResponseAddress(address)) >= 0) {
        ...
        ConstIterator<ListenerList::Iterator> it = this->listeners.GetConstIterator();
        while (it.HasNext()) {
            it.Next()->OnNodeLost(*this, hPeer, DiscoveryListener::LOST_EXPLICITLY);
        }
...
const UINT32 vislib::net::cluster::DiscoveryService::MAGIC_NUMBER =
    (static_cast<UINT32>('v') << 0) | (static_cast<UINT32>('c') << 8) | (static_cast<UINT32>('d') << 16) |
