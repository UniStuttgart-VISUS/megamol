 ...
 */
vislib::net::cluster::ReceiveMessagesCtx* vislib::net::cluster::AllocateRecvMsgCtx(
    AbstractClusterNode* receiver, Socket* socket) {
...
DWORD vislib::net::cluster::ReceiveMessages(void* receiveMessagesCtx) {
    ReceiveMessagesCtx* ctx = static_cast<ReceiveMessagesCtx*>(receiveMessagesCtx); // Receive context.
    ...
    try {
        ...
        VLTRACE(Trace::LEVEL_VL_ERROR,
            "Message receiver thread could not "
    ...
    try {
        ...
        while (true) {
            ...
            if (ctx->Socket->Receive(
                    static_cast<void*>(recvBuf), sizeof(MessageHeader), Socket::TIMEOUT_INFINITE, 0, true) == 0) {
            ...
            if (msgHdr->MagicNumber != MAGIC_NUMBER) {
                VLTRACE(Trace::LEVEL_WARN,
                    "Discarding data packet without "
            ...
            msgHdr = recvBuf.As<MessageHeader>();
            if (ctx->Socket->Receive(recvBuf.As<BYTE>() + sizeof(MessageHeader), msgHdr->Header.BlockLength,
                    Socket::TIMEOUT_INFINITE, 0, true) == 0) {
                VLTRACE(Trace::LEVEL_VL_INFO, "vislib::net::cluster::"
                                              "ReceiveMessages exits because of graceful disconnect.\n");
            }
            ...
            if (msgHdr->Header.BlockId == MSGID_MULTIPLE) {
                ...
                while (remBody > 0) {
                    ...
                    ctx->Receiver->onMessageReceived(
                        *ctx->Socket, blkHdr->BlockId, (blkHdr->BlockLength > 0) ? body : NULL, blkHdr->BlockLength);
