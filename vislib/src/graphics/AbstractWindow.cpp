...
#else  /* _WIN32 */
    throw MissingImplementationException("AbstractWindow::Close", __FILE__, __LINE__);
#endif /* _WIN32 */
}
 ...
 */
void vislib::graphics::AbstractWindow::Create(const vislib::StringA& title, int left, int top, int width, int height) {

#ifdef _WIN32
    this->Create(A2W(title), left, top, width, height);

#else  /* _WIN32 */
    throw MissingImplementationException("AbstractWindow::Create", __FILE__, __LINE__);
#endif /* _WIN32 */
}
 ...
 * vislib::graphics::AbstractWindow::Create(
 */
void vislib::graphics::AbstractWindow::Create(const vislib::StringW& title, int left, int top, int width, int height) {
    ...
    if ((this->hWnd = ::CreateWindowExW(dwExStyle, wndClass.PeekBuffer(), title.PeekBuffer(), dwStyle, wndRect.left,
             wndRect.top, wndRect.right - wndRect.left, wndRect.bottom - wndRect.top,
             NULL, // Parent window
             NULL, // Menu
             hInstance, NULL)) == NULL) {
        throw vislib::sys::SystemException(__FILE__, __LINE__);
 ...
 */
inline vislib::graphics::AbstractWindow::Point vislib::graphics::AbstractWindow::GetPosition(void) const {
#ifdef _WIN32
    RECT rect;
...
#else  /* _WIN32 */
    throw MissingImplementationException("AbstractWindow::GetPosition", __FILE__, __LINE__);
#endif /* _WIN32 */
}
 ...
 */
inline vislib::graphics::AbstractWindow::Dimension vislib::graphics::AbstractWindow::GetSize(void) const {
#ifdef _WIN32
    RECT rect;
    if (::GetWindowRect(this->hWnd, &rect) == FALSE) {
        throw vislib::sys::SystemException(__FILE__, __LINE__);
...
#else  /* _WIN32 */
    throw MissingImplementationException("AbstractWindow::GetSize", __FILE__, __LINE__);
#endif /* _WIN32 */
}
...
void vislib::graphics::AbstractWindow::Move(const int x, const int y) {
    ...
    if (::MoveWindow(this->hWnd, x, y, size.Width(), size.Height(), TRUE) == FALSE) {
        throw vislib::sys::SystemException(__FILE__, __LINE__);
    }
#else  /* _WIN32 */
    throw MissingImplementationException("AbstractWindow::Move", __FILE__, __LINE__);
#endif /* _WIN32 */
}
 ...
 */
void vislib::graphics::AbstractWindow::Resize(const int width, const int height) {
#ifdef _WIN32
    Point position = this->GetPosition();
    if (::MoveWindow(this->hWnd, position.X(), position.Y(), width, height, TRUE) == FALSE) {
        throw vislib::sys::SystemException(__FILE__, __LINE__);
    }
#else  /* _WIN32 */
    throw MissingImplementationException("AbstractWindow::Resize", __FILE__, __LINE__);
#endif /* _WIN32 */
}
 ...
 */
void vislib::graphics::AbstractWindow::onCreating(DWORD& inOutStyle, DWORD& inOutExStyle) throw() {
    // Nothing to do.
}
#endif /* _WIN32 */
 ...
 */
LRESULT vislib::graphics::AbstractWindow::onMessage(bool& outHandled, UINT msg, WPARAM wParam, LPARAM lParam) throw() {
    outHandled = false;
 ...
 */
void vislib::graphics::AbstractWindow::onResized(const int width, const int height) {
    // Nothing to do.
}
 ...
 */
void vislib::graphics::AbstractWindow::onWindowClassRegistering(WNDCLASSEXW& inOutWndClass) throw() {
    // Nothing to do.
}
#endif /* _WIN32 */
 ...
 */
void vislib::graphics::AbstractWindow::onWindowClassRegistered(const vislib::StringW className) throw() {
    // Nothing to do.
}
#endif /* _WIN32 */
 ...
 */
LRESULT CALLBACK vislib::graphics::AbstractWindow::wndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {

#pragma warning(disable : 4312)
    AbstractWindow* wnd =
        reinterpret_cast<AbstractWindow*>(static_cast<LONG_PTR>(::GetWindowLongPtrW(hWnd, GWLP_USERDATA)));
...
vislib::graphics::AbstractWindow::AbstractWindow(const AbstractWindow& rhs) {
    throw UnsupportedOperationException("AbstractWindow::AbstractWindow", __FILE__, __LINE__);
}
 ...
 */
vislib::StringW vislib::graphics::AbstractWindow::registerWindowClassW(HINSTANCE hInstance) {
 ...
 */
vislib::graphics::AbstractWindow& vislib::graphics::AbstractWindow::operator=(const AbstractWindow& rhs) {
    if (this != &rhs) {
