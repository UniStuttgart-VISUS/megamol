 ...
 */
int vislib::graphics::PpmBitmapCodec::AutoDetect(const void* mem, SIZE_T size) const {
    if (size < 3)
    ...
    if ((data[1] != '3') && (data[1] != '6') && (data[1] != 'F') && (data[1] != 'f'))
        return 0;                                     // wrong magic number
    return vislib::CharTraitsA::IsSpace(data[2]) ? 1  // correctly terminated magic number
...
#define _LOCAL_PPM_SIFT(variable, type, method)                               \
    ...
    variable = static_cast<type>(vislib::CharTraitsA::method(                 \
        vislib::StringA(&cd[p1], static_cast<vislib::CharTraitsA::Size>(p2 - p1)).PeekBuffer()));

    if ((cd[0] != 'p') && (cd[0] != 'P'))
        return false; // wrong magic number
    if ((cd[1] != '3') && (cd[1] != '6') && (cd[1] != 'F') && (cd[1] != 'f'))
        return false; // wrong magic number
    if (!vislib::CharTraitsA::IsSpace(cd[2]))
        return false;
    ...
    try {
        ...
        if (h < 0)
            return false; // width must be positive
        if (v <= 0)
            return false; // width must greater than zero

        if (cd[1] == '6') {
            // binary data

            if (v > 255)
                return false; // max value to large for binary files
            p2++;
            if (p2 + w * h * 3 > size)
                return false; // out of data

            img.CreateImage(w, h, 3, BitmapImage::CHANNELTYPE_BYTE, static_cast<const void*>(&cd[p2]));
            ...
            if (v < 255) {
                ...
                for (unsigned int i = 0; i < 3 * w * h; i++) {
                    img.PeekDataAs<BYTE>()[i] = static_cast<BYTE>(static_cast<float>(img.PeekDataAs<BYTE>()[i]) * f);
                }
            }
            ...
            sys::SystemInformation::Endianness machineEnd = sys::SystemInformation::SystemEndianness();
            ...
            if (machineEnd != sys::SystemInformation::ENDIANNESS_LITTLE_ENDIAN &&
                ...
                throw new vislib::MissingImplementationException(
                    "Your machine is too exotic for this implementation", __FILE__, __LINE__);
            }
...
bool vislib::graphics::PpmBitmapCodec::saveToMemory(vislib::RawStorage& outmem) const {
    ...
    if (img.GetChannelType() == BitmapImage::CHANNELTYPE_WORD) {
        ...
        if (img.GetChannelCount() != 1 && img.GetChannelCount() != 3) {
            throw new vislib::MissingImplementationException(
                "This implementation can only cope with RGB and greyscale images", __FILE__, __LINE__);
        }
    ...
    if (bin) {
        ...
        for (unsigned int i = 0; i < imgSize; i++) {
            outmem.AsAt<BYTE>(headLen)[i * 3] = (cr != UINT_MAX) ? bd[i * cc + cr] : 0;
            outmem.AsAt<BYTE>(headLen)[i * 3 + 1] = (cg != UINT_MAX) ? bd[i * cc + cg] : 0;
            outmem.AsAt<BYTE>(headLen)[i * 3 + 2] = (cb != UINT_MAX) ? bd[i * cc + cb] : 0;
        }
        ...
        if (bd != NULL) {
            for (unsigned int i = 0; i < imgSize; i++) {
                tmp.Format("%u %u %u%c", (cr != UINT_MAX) ? bd[i * cc + cr] : 0, (cg != UINT_MAX) ? bd[i * cc + cg] : 0,
                    (cb != UINT_MAX) ? bd[i * cc + cb] : 0, (((i + 1) % ppl) == 0) ? '\n' : ' ');
                data += tmp; // slow!
            }
        } else if (wd != NULL) {
            for (unsigned int i = 0; i < imgSize; i++) {
                tmp.Format("%u %u %u%c", (cr != UINT_MAX) ? wd[i * cc + cr] : 0, (cg != UINT_MAX) ? wd[i * cc + cg] : 0,
                    (cb != UINT_MAX) ? wd[i * cc + cb] : 0, (((i + 1) % ppl) == 0) ? '\n' : ' ');
                data += tmp; // slow!
            }
        } else
            return false; // internal format error
