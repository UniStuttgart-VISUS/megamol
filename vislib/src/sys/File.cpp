...
vislib::sys::File* vislib::sys::File::CreateTempFile(void) {
    ...
    if ((retval->handle = ::CreateFileA(CreateTempFileName(tempName), GENERIC_READ | GENERIC_WRITE, 0, NULL,
             CREATE_ALWAYS, FILE_ATTRIBUTE_TEMPORARY | FILE_FLAG_DELETE_ON_CLOSE, NULL)) == INVALID_HANDLE_VALUE) {
        SAFE_DELETE(retval);
...
vislib::sys::File::FileSize vislib::sys::File::GetSize(const char* filename) {
    ...
    return (static_cast<FileSize>(buf.nFileSizeHigh) << 32) + static_cast<FileSize>(buf.nFileSizeLow);
#else  /* _WIN32 */
    struct stat buf;
    int i = stat(filename, &buf);
    if (i != 0)
...
bool vislib::sys::File::Open(
    ...
    case SHARE_WRITE:
        ...
        break;
    case SHARE_READWRITE:
        share = FILE_SHARE_READ | FILE_SHARE_WRITE;
        break;
    default:
        throw IllegalParamException("shareMode", __FILE__, __LINE__);
    }
    ...
    case OPEN_CREATE:
        ...
        break;
    default:
        throw IllegalParamException("creationMode", __FILE__, __LINE__);
    }
    ...
    this->handle = ::open(filename, oflag, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
    return (this->handle != -1);
 ...
 */
bool vislib::sys::File::Open(
    const wchar_t* filename, const AccessMode accessMode, const ShareMode shareMode, const CreationMode creationMode) {
    this->Close();
...
#ifdef _WIN32
    DWORD readBytes;
    if (::ReadFile(this->handle, outBuf, static_cast<DWORD>(bufSize), &readBytes, NULL)) {
        return readBytes;
 ...
 */
vislib::sys::File::FileSize vislib::sys::File::Write(const void* buf, const FileSize bufSize) {
#ifdef _WIN32
    DWORD writtenBytes;
    if (::WriteFile(this->handle, buf, static_cast<DWORD>(bufSize), &writtenBytes, NULL)) {
        return writtenBytes;
