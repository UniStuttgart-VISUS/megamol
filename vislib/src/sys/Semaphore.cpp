 ...
 * vislib::sys::Semaphore::Semaphore
 */
vislib::sys::Semaphore::Semaphore(const char* name, long initialCount, long maxCount, bool* outIsNew) {
    Semaphore::enforceParamAssertions(initialCount, maxCount);
    ...
    if ((this->handle = ::OpenSemaphoreA(SYNCHRONIZE | SEMAPHORE_MODIFY_STATE, FALSE, name)) == NULL) {
    ...
    if (name != NULL) {
        ...
        if ((this->handle = ::sem_open(this->name.PeekBuffer(), 0, 0, 0)) == SEM_FAILED) {
            this->handle = ::sem_open(this->name.PeekBuffer(), O_CREAT, DFT_PERMS, initialCount);
            if (outIsNew != NULL) {
                *outIsNew = true;
            }
        }
 ...
 * vislib::sys::Semaphore::Semaphore
 */
vislib::sys::Semaphore::Semaphore(const wchar_t* name, long initialCount, long maxCount, bool* outIsNew) {
    Semaphore::enforceParamAssertions(initialCount, maxCount);
...
#ifdef _WIN32
    /* Try to open existing semaphore first. */
    if ((this->handle = ::OpenSemaphoreW(SYNCHRONIZE | SEMAPHORE_MODIFY_STATE, FALSE, name)) == NULL) {
...
vislib::sys::Semaphore::Semaphore(const Semaphore& rhs) {
    throw UnsupportedOperationException("vislib::sys::Semaphore::Semaphore", __FILE__, __LINE__);
}
 ...
 */
void vislib::sys::Semaphore::enforceParamAssertions(long& inOutInitialCount, long& inOutMaxCount) {
    if (inOutMaxCount <= 0) {
        inOutMaxCount = 1;
    }
 ...
 */
vislib::sys::Semaphore& vislib::sys::Semaphore::operator=(const Semaphore& rhs) {
    if (this != &rhs) {
