...
vislib::sys::MemmappedFile::MemmappedFile(void)
        ...
        : File()
        , viewStart(0)
        , viewSize(SystemInformation::AllocationGranularity())
        , mappedData(NULL)
        , viewDirty(false)
...
inline char* vislib::sys::MemmappedFile::SafeMapView() {
    ...
    ret = static_cast<char*>(MapViewOfFile(this->mapping, da, fp.HighPart, fp.LowPart, vs));
    if (ret == NULL) {
        throw IOException(::GetLastError(), __FILE__, __LINE__);
    }
 ...
 */
vislib::sys::File::FileSize vislib::sys::MemmappedFile::Read(void* outBuf, const vislib::sys::File::FileSize bufSize) {
    if (bufSize < 0) {
        // hate!
    ...
    if ((this->mappedData != NULL) && (filePos >= viewStart) && (filePos + bufSize <= viewStart + viewSize)) {
        ...
        if (this->mappedData == NULL || this->filePos < this->viewStart ||
            this->filePos >= this->viewStart + this->viewSize) {
            this->SafeUnmapView();
 ...
 */
vislib::sys::File::FileSize vislib::sys::MemmappedFile::Seek(
...
vislib::sys::File::FileSize vislib::sys::MemmappedFile::Write(const void* buf, const File::FileSize bufSize) {
    ...
    while (dataLeft > 0) {
        ...
        memcpy(static_cast<void*>(this->mappedData + this->filePos - this->viewStart), bufferPos,
