 ...
 */
#ifdef _WIN32
FARPROC vislib::sys::DynamicLinkLibrary::GetProcAddress(const CHAR* procName) const {
    return ::GetProcAddress(this->hModule, procName);
}
#else  /* _WIN32 */
void* vislib::sys::DynamicLinkLibrary::GetProcAddress(const CHAR* procName) const {
 ...
 */
bool vislib::sys::DynamicLinkLibrary::Load(
    const char* moduleName, bool dontResolveReferences, bool alternateSearchPath) {
 ...
 */
bool vislib::sys::DynamicLinkLibrary::Load(
    const wchar_t* moduleName, bool dontResolveReferences, bool alternateSearchPath) {
    this->loadErrorMsg.Clear();
    if (this->IsLoaded()) {
        this->loadErrorMsg = "Call to DynamicLinLibrary::Load() when"
                             "a library was already loaded.";
 ...
 * vislib::sys::DynamicLinkLibrary::DynamicLinkLibrary
 */
vislib::sys::DynamicLinkLibrary::DynamicLinkLibrary(const DynamicLinkLibrary& rhs) {
    throw UnsupportedOperationException("vislib::sys::DynamicLinkLibrary::DynamicLinkLibrary", __FILE__, __LINE__);
}
 ...
 */
vislib::sys::DynamicLinkLibrary& vislib::sys::DynamicLinkLibrary::operator=(const DynamicLinkLibrary& rhs) {
    if (this != &rhs) {
