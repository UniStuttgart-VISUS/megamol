 ...
 */
void vislib::SHA1HashProvider::TransformBlock(const BYTE* input, const SIZE_T cntInput) {
    // Must be initialised, as ctor does this.
    if ((input != NULL) && (cntInput > 0)) {
        SHA1HashProvider::input(&this->context, input, static_cast<UINT>(cntInput));
    }
}
 ...
 */
bool vislib::SHA1HashProvider::TransformFinalBlock(
 ...
 */
void vislib::SHA1HashProvider::input(SHA1Context* context, const BYTE* input, const UINT cntInput) {
    ASSERT(context != NULL);
    ASSERT(input != NULL);
    ...
    if (context->Computed) {
        ...
        throw IllegalStateException("vislib::SHA1HashProvider::input called "
                                    "after hash was already computed.",
            __FILE__, __LINE__);
    }

    if (context->Corrupted) {
        throw IllegalStateException("SHA1Context is corrupted.", __FILE__, __LINE__);
    }

    while (length-- && !context->Corrupted) {
        context->Message_Block[context->Message_Block_Index++] = (*input & 0xFF);
 ...
 */
void vislib::SHA1HashProvider::result(UINT8* messageDigest, SHA1Context* context) {
    ...
    for (UINT i = 0; i < SHA1HashProvider::HASH_SIZE; i++) {
        messageDigest[i] = context->Intermediate_Hash[i >> 2] >> 8 * (3 - (i & 0x03));
    }
}
 ...
 */
vislib::SHA1HashProvider& vislib::SHA1HashProvider::operator=(const SHA1HashProvider& rhs) {
    if (this != &rhs) {
