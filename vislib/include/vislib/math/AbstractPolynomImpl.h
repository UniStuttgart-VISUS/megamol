...
protected:
     ...
     */
    static unsigned int findRootsDeg1(const T& a0, const T& a1, T* outRoots, unsigned int size);
     ...
     * @return The found roots
     */
    static unsigned int findRootsDeg2(const T& a0, const T& a1, const T& a2, T* outRoots, unsigned int size);
     ...
     * @return The found roots
     */
    static unsigned int findRootsDeg3(
        const T& a0, const T& a1, const T& a2, const T& a3, T* outRoots, unsigned int size);
     ...
     * @return The found roots
     */
    static unsigned int findRootsDeg4(
        const T& a0, const T& a1, const T& a2, const T& a3, const T& a4, T* outRoots, unsigned int size);
    ...
    template<class Tf1, unsigned int Df1, class Sf1, template<class Tf2, unsigned int Df2, class Sf2> class Cf1>
...
C<T, D - 1, T[D]> AbstractPolynomImpl<T, D, S, C>::Derivative() const {
    ...
    for (unsigned int i = 0; i < D; i++) {
        rv.coefficients[i] = this->coefficients[i + 1] * static_cast<T>(i + 1);
    }
...
template<class T, unsigned int D, class S, template<class Tc, unsigned int Dc, class Sc> class C>
template<class Tp, unsigned int Dp, class Sp>
AbstractPolynomImpl<T, D, S, C>& AbstractPolynomImpl<T, D, S, C>::operator=(const C<Tp, Dp, Sp>& rhs) {
...
template<class T, unsigned int D, class S, template<class Tc, unsigned int Dc, class Sc> class C>
template<class Tp, unsigned int Dp, class Sp>
bool AbstractPolynomImpl<T, D, S, C>::operator==(const C<Tp, Dp, Sp>& rhs) const {

    for (unsigned int i = 0; i <= ((D < Dp) ? D : Dp); i++) {
        if (!vislib::math::IsEqual(this->coefficients[i], static_cast<T>(rhs.coefficients[i]))) {
            return false;
        }
    }
    ...
    for (unsigned int i = ((D < Dp) ? D : Dp) + 1; i <= Dp; i++) {
        if (!vislib::math::IsEqual(rhs.coefficients[i], static_cast<Tp>(0))) {
            return false;
        }
    }
 ...
 */
template<class T, unsigned int D, class S, template<class Tc, unsigned int Dc, class Sc> class C>
unsigned int AbstractPolynomImpl<T, D, S, C>::findRootsDeg1(const T& a0, const T& a1, T* outRoots, unsigned int size) {
...
unsigned int AbstractPolynomImpl<T, D, S, C>::findRootsDeg2(
    const T& a0, const T& a1, const T& a2, T* outRoots, unsigned int size) {
...
unsigned int AbstractPolynomImpl<T, D, S, C>::findRootsDeg3(
    ...
    T q = (static_cast<T>(27) * a3 * a3 * a0) - (static_cast<T>(9) * a3 * a2 * a1) + (static_cast<T>(2) * a2 * a2 * a2);
    ...
    if (dis < static_cast<T>(0)) {
        ...
        double cosphi = static_cast<double>(-q) / (2.0 * ::sqrt(-static_cast<double>(p * p * p)));
    ...
    T u = static_cast<T>(0.5) * static_cast<T>(::pow(-4.0 * static_cast<double>(q) + 4.0 * sqrtDis, 1.0 / 3.0));
    T v = static_cast<T>(0.5) * static_cast<T>(::pow(-4.0 * static_cast<double>(q) - 4.0 * sqrtDis, 1.0 / 3.0));
    T y1 = u + v;
    outRoots[0] = (y1 - a2) / (static_cast<T>(3) * a3);
...
unsigned int AbstractPolynomImpl<T, D, S, C>::findRootsDeg4(
    ...
    { // finds the smallest x for cubic polynom x^3 + px^2 + qx + r = 0;
        T cr[3];
        unsigned int crc = findRootsDeg3(r, q, p, static_cast<T>(1), cr, 3);
        ASSERT(crc > 0);
        if (crc == 3)
            y = Min(Min(cr[0], cr[1]), cr[2]);
        else if (crc == 2)
            y = Min(cr[0], cr[1]);
        else
            y = cr[0];
    ...
    if (IsEqual(esq, static_cast<T>(0)))
        return 0;

    T fsq = static_cast<T>(0.25) * y * y - d;
    if (IsEqual(fsq, static_cast<T>(0)))
        return 0;

    T ef = -(static_cast<T>(0.25) * a * y + static_cast<T>(0.5) * c);
    T e, f;

    if (((a > static_cast<T>(0)) && (y > static_cast<T>(0)) && (c > static_cast<T>(0))) ||
        ((a > static_cast<T>(0)) && (y < static_cast<T>(0)) && (c < static_cast<T>(0))) ||
        ((a < static_cast<T>(0)) && (y < static_cast<T>(0)) && (c > static_cast<T>(0))) ||
        ((a < static_cast<T>(0)) && (y > static_cast<T>(0)) && (c < static_cast<T>(0))) ||
        IsEqual(a, static_cast<T>(0)) || IsEqual(y, static_cast<T>(0)) || IsEqual(c, static_cast<T>(0))) {
        /* use ef - */

        if ((b < static_cast<T>(0)) && (y < static_cast<T>(0)) && (esq > static_cast<T>(0))) {
    ...
    if (((b > static_cast<T>(0)) && (y > static_cast<T>(0))) || ((b < static_cast<T>(0)) && (y < static_cast<T>(0)))) {
 ...
 */
template<class T, unsigned int D, class S, template<class Tc, unsigned int Dc, class Sc> class C>
unsigned int AbstractPolynomImpl<T, D, S, C>::uniqueRoots(T* outRoots, unsigned int size) {
    if (size <= 1)
        return size;
    if (size == 2)
        return (IsEqual(outRoots[0], outRoots[1])) ? 1 : 2;
