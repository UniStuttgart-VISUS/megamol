...
public:
     ...
     */
    inline ViewFrustum(const T left, const T right, const T bottom, const T top, const T zNear, const T zFar) {
     ...
     *                    plane.
     */
    inline ViewFrustum(const T fovy, const double aspectRatio, const T zNear, const T zFar) {
     ...
     */
    template<class Tp, class Sp>
    inline ViewFrustum& operator=(const AbstractViewFrustum<Tp, Sp>& rhs) {
        Super::operator=(rhs);
        return *this;
    }
...
ViewFrustum<T>::ViewFrustum(const AbstractViewFrustum<Tp, Sp>& rhs) {
    this->offsets[Super::IDX_BOTTOM] = static_cast<T>(rhs.GetBottomDistance());
    this->offsets[Super::IDX_TOP] = static_cast<T>(rhs.GetTopDistance());
    this->offsets[Super::IDX_LEFT] = static_cast<T>(rhs.GetLeftDistance());
    this->offsets[Super::IDX_RIGHT] = static_cast<T>(rhs.GetRightDistance());
