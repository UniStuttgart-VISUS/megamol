...
public:
    ...
    inline AbstractQuaternion<T, T[4]> Interpolate(const AbstractQuaternion<T, Sp>& rhs, Tp t) const {
     ...
     */
    void SetFromRotationMatrix(const T& m11, const T& m12, const T& m13, const T& m21, const T& m22, const T& m23,
    ...
    void Slerp(float alpha, const AbstractQuaternion<T, Sp1>& a, const AbstractQuaternion<T, Sp2>& b);
     ...
     */
    template<class Sp>
    AbstractQuaternion<T, T[4]> operator*(const AbstractQuaternion<T, Sp>& rhs) const;
...
protected:
     ...
     * @param w The new w-component.
     */
    inline AbstractQuaternion(const T& x, const T& y, const T& z, const T& w) {
 ...
 */
template<class T, class S>
void AbstractQuaternion<T, S>::AngleAndAxis(T& outAngle, Vector<T, 3>& outAxis) const {
    T d = Sqrt(Sqr(this->components[IDX_X]) + Sqr(this->components[IDX_Y]) + Sqr(this->components[IDX_Z]));
    ...
    if (!IsEqual<T>(d, static_cast<T>(0))) {
        ...
        // TODO: Not nice.
        outAngle = static_cast<T>(2.0 * ::acos(static_cast<double>(this->components[IDX_W])));
...
void AbstractQuaternion<T, S>::Invert() {
    ...
    if (!IsEqual<T>(norm, static_cast<T>(0))) {
        ...
        this->components[IDX_X] = this->components[IDX_Y] = this->components[IDX_Z] = static_cast<T>(0);
...
template<class T, class S>
template<class Tp, class Sp>
void AbstractQuaternion<T, S>::Set(const T& angle, const AbstractVector<Tp, 3, Sp>& axis) {
    Vector<Tp, 3> ax(axis);
    T len = ax.Normalise();
 ...
 */
template<class T, class S>
void AbstractQuaternion<T, S>::SetFromRotationMatrix(const T& m11, const T& m12, const T& m13, const T& m21,
    const T& m22, const T& m23, const T& m31, const T& m32, const T& m33) {
    ...
    Vector<T, 3> xo(static_cast<T>(1), static_cast<T>(0), static_cast<T>(0));
    Vector<T, 3> yo(static_cast<T>(0), static_cast<T>(1), static_cast<T>(0));
    Vector<T, 3> zo(static_cast<T>(0), static_cast<T>(0), static_cast<T>(1));

    AbstractQuaternion<T, T[4]> q1;
    if (xi == xo) {
        // rot 0°
        q1.Set(static_cast<T>(0), static_cast<T>(0), static_cast<T>(0), static_cast<T>(1));
        ...
        if (xb != xi) {
            throw IllegalParamException("Matrix is not rotation-only", __FILE__, __LINE__);
        }
    }
...
template<class T, class S>
template<class Sp1, class Sp2>
void AbstractQuaternion<T, S>::Slerp(
    float alpha, const AbstractQuaternion<T, Sp1>& a, const AbstractQuaternion<T, Sp2>& b) {
    ...
    if (alpha < FLOAT_EPSILON) {
        ...
        *this = b;
        ...
        for (unsigned int i = 0; i < 4; i++) {
            this->components[i] = slerpFrom * a.components[i] + slerpTo * b.components[i];
        }
    }
}
...
void AbstractQuaternion<T, S>::Square() {
    ...
    this->components[IDX_W] =
        Sqr(this->components[IDX_W]) -
        (Sqr(this->components[IDX_X] + Sqr(this->components[IDX_Y]) + Sqr(this->components[IDX_Z])));
 ...
 */
template<class T, class S>
AbstractQuaternion<T, S>& AbstractQuaternion<T, S>::operator=(const AbstractQuaternion& rhs) {
    if (this != &rhs) {
...
template<class T, class S>
template<class Tp, class Sp>
AbstractQuaternion<T, S>& AbstractQuaternion<T, S>::operator=(const AbstractQuaternion<Tp, Sp>& rhs) {
 ...
 */
template<class T, class S>
bool AbstractQuaternion<T, S>::operator==(const AbstractQuaternion& rhs) const {
...
template<class T, class S>
template<class Tp, class Sp>
bool AbstractQuaternion<T, S>::operator==(const AbstractQuaternion<Tp, Sp>& rhs) const {
    return (IsEqual<T>(this->components[IDX_X], rhs.X()) && IsEqual<T>(this->components[IDX_Y], rhs.Y()) &&
            IsEqual<T>(this->components[IDX_Z], rhs.Z()) && IsEqual<T>(this->components[IDX_W], rhs.W()));
}
...
template<class T, class S>
template<class Sp>
AbstractQuaternion<T, T[4]> AbstractQuaternion<T, S>::operator*(const AbstractQuaternion<T, Sp>& rhs) const {
    return AbstractQuaternion<T, T[4]>(this->components[IDX_W] * rhs.X() + rhs.W() * this->components[IDX_X] +
                                           this->components[IDX_Y] * rhs.Z() - this->components[IDX_Z] * rhs.Y(),

        this->components[IDX_W] * rhs.Y() + rhs.W() * this->components[IDX_Y] + this->components[IDX_Z] * rhs.X() -
            this->components[IDX_X] * rhs.Z(),

        this->components[IDX_W] * rhs.Z() + rhs.W() * this->components[IDX_Z] + this->components[IDX_X] * rhs.Y() -
            this->components[IDX_Y] * rhs.X(),

        this->components[IDX_W] * rhs.W() - (this->components[IDX_X] * rhs.X() + this->components[IDX_Y] * rhs.Y() +
...
template<class T, class S>
template<class Sp>
Vector<T, 3> AbstractQuaternion<T, S>::operator*(const AbstractVector<T, 3, Sp>& rhs) const {
    Vector<T, 3> u(this->components);
    return (
        (static_cast<T>(2.0) * ((u.Dot(rhs) * u) + (this->W() * u.Cross(rhs)))) + ((Sqr(this->W()) - u.Dot(u)) * rhs));
}
