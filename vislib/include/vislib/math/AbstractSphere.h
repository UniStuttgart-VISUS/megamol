...
public:
    ...
    enum RelativeLocation { OUTSIDE_SPHERE = -1, ON_SPHERE = 0, INSIDE_SPHERE = 1 };
     ...
     */
    template<class Sp>
    RelativeLocation CalcRelativeLocation(const AbstractPoint<T, 3, Sp>& point);
    ...
    inline T Volume(void) const {
        return (static_cast<T>(4.0 / 3.0 * M_PI) * this->xyzr[IDX_RADIUS] * this->xyzr[IDX_RADIUS] *
                this->xyzr[IDX_RADIUS]);
    }
 ...
 */
template<class T, class S>
AbstractSphere<T, S>::~AbstractSphere(void) {}
...
template<class T, class S>
template<class Sp>
typename AbstractSphere<T, S>::RelativeLocation AbstractSphere<T, S>::CalcRelativeLocation(
...
template<class T, class S>
template<class Sp>
void AbstractSphere<T, S>::SetCenter(const AbstractPoint<T, 3, Sp>& center) {
 ...
 */
template<class T, class S>
AbstractSphere<T, S>& AbstractSphere<T, S>::operator=(const AbstractSphere& rhs) {
    if (this != &rhs) {
...
template<class T, class S>
template<class Tp, class Sp>
AbstractSphere<T, S>& AbstractSphere<T, S>::operator=(const AbstractSphere<Tp, Sp>& rhs) {
...
bool AbstractSphere<T, S>::operator==(const AbstractSphere& rhs) const {
    return (IsEqual<T>(this->xyzr[IDX_X], rhs.xyzr[IDX_X]) && IsEqual<T>(this->xyzr[IDX_Y], rhs.xyzr[IDX_Y]) &&
            IsEqual<T>(this->xyzr[IDX_Z], rhs.xyzr[IDX_Z]) && IsEqual<T>(this->xyzr[IDX_RADIUS], rhs.xyzr[IDX_RADIUS]));
}
 ...
 */
template<class T, class S>
const UINT_PTR AbstractSphere<T, S>::IDX_X = 0;
 ...
 */
template<class T, class S>
const UINT_PTR AbstractSphere<T, S>::IDX_Y = 1;
 ...
 */
template<class T, class S>
const UINT_PTR AbstractSphere<T, S>::IDX_Z = 2;
