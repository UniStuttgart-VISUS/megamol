...
public:
    ...
    bool CalcIntersectionPoint(const AbstractPlane<T, Sp1>& plane2, const AbstractPlane<T, Sp2>& plane3,
    ...
    template<class Tp, class Sp1, class Sp2>
    IntersectionCount Intersect(AbstractPoint<T, 3, Sp1>& outWhere, const AbstractPoint<Tp, 3, Sp2>& p1,
     ...
     */
    void Points(
        vislib::math::Point<T, 3>& outP1, vislib::math::Point<T, 3>& outP2, vislib::math::Point<T, 3>& outP3) const;
    ...
    void Set(const AbstractPoint<Tp1, 3, Sp1>& point, const AbstractVector<Tp2, 3, Sp2>& normal);
...
template<class T, class S>
template<class Sp1, class Sp2, class Sp3>
bool AbstractPlane<T, S>::CalcIntersectionPoint(const AbstractPlane<T, Sp1>& plane2,
    const AbstractPlane<T, Sp2>& plane3, AbstractPoint<T, 3, Sp3>& outPoint) const {
    // planes: ax + by + cz + d = 0
    T a1, b1, c1, d1;
    ...
    if (IsEqual(denom, static_cast<T>(0)))
        return false;
...
template<class T, class S>
template<class Tp, class Sp>
T AbstractPlane<T, S>::Distance(const AbstractPoint<Tp, 3, Sp>& point) const {
    T a, b, c, d; // Normalised plane parameters.

    this->normalise(a, b, c, d);
    return (a * static_cast<T>(point.X()) + b * static_cast<T>(point.Y()) + c * static_cast<T>(point.Z()) + d);
}
...
template<class T, class S>
template<class Tp, class Sp>
typename AbstractPlane<T, S>::HalfSpace AbstractPlane<T, S>::Halfspace(const AbstractPoint<Tp, 3, Sp>& point) const {
...
template<class T, class S>
template<class Tp, class Sp1, class Sp2>
typename AbstractPlane<T, S>::IntersectionCount AbstractPlane<T, S>::Intersect(AbstractPoint<T, 3, Sp1>& outWhere,
    const AbstractPoint<Tp, 3, Sp2>& p1, const AbstractPoint<Tp, 3, Sp2>& p2) const {
    T dp1 = this->parameters[IDX_A] * p1.X() + this->parameters[IDX_B] * p1.Y() + this->parameters[IDX_C] * p1.Z();
    T dp2 = this->parameters[IDX_A] * p2.X() + this->parameters[IDX_B] * p2.Y() + this->parameters[IDX_C] * p2.Z();
 ...
 */
template<class T, class S>
void AbstractPlane<T, S>::Points(
    vislib::math::Point<T, 3>& outP1, vislib::math::Point<T, 3>& outP2, vislib::math::Point<T, 3>& outP3) const {
    T a, b, c, d;
    this->normalise(a, b, c, d);

    outP1 = outP3 = vislib::math::Point<T, 3>(-d * a, -d * b, -d * c);
    outP2 = vislib::math::Point<T, 3>(outP1.X() - b, outP1.Y() + a, outP1.Z());
...
template<class T, class S>
template<class Tp1, class Sp1, class Tp2, class Sp2>
void AbstractPlane<T, S>::Set(const AbstractPoint<Tp1, 3, Sp1>& point, const AbstractVector<Tp2, 3, Sp2>& normal) {
    this->parameters[IDX_A] = normal.X();
    this->parameters[IDX_B] = normal.Y();
    this->parameters[IDX_C] = normal.Z();
    this->parameters[IDX_D] = -1.0f * (this->parameters[IDX_A] * point.X() + this->parameters[IDX_B] * point.Y() +
 ...
 */
template<class T, class S>
AbstractPlane<T, S>& AbstractPlane<T, S>::operator=(const AbstractPlane& rhs) {
    if (this != &rhs) {
...
template<class T, class S>
template<class Tp, class Sp>
AbstractPlane<T, S>& AbstractPlane<T, S>::operator=(const AbstractPlane<Tp, Sp>& rhs) {
...
template<class T, class S>
template<class Tp, class Sp>
bool AbstractPlane<T, S>::operator==(const AbstractPlane<Tp, Sp>& rhs) const {
 ...
 */
template<class T, class S>
void AbstractPlane<T, S>::normalise(T& outA, T& outB, T& outC, T& outD) const {
    T len = Sqrt(Sqr(this->parameters[IDX_A]) + Sqr(this->parameters[IDX_B]) + Sqr(this->parameters[IDX_C]));
