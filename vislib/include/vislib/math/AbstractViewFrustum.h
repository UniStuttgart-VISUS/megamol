...
public:
     ...
     */
    inline void Set(const T left, const T right, const T bottom, const T top, const T zNear, const T zFar) {
     ...
     *                    plane.
     */
    void Set(const T fovy, const double aspectRatio, const T zNear, const T zFar);
    ...
    AbstractViewFrustum& operator=(const AbstractViewFrustum<Tp, Sp>& rhs);
     ...
     */
    template<class Tp, class Sp>
    inline bool operator!=(const AbstractViewFrustum<Tp, Sp>& rhs) const {
        return !(*this == rhs);
    }
 ...
 */
template<class T, class S>
void AbstractViewFrustum<T, S>::GetBottomBasePoints(vislib::Array<Point<T, 3>>& outPoints) const {
    // Compute bottom plane using intercept theorem.
    T scale = (this->offsets[IDX_NEAR] != static_cast<T>(0)) ? this->offsets[IDX_FAR] / this->offsets[IDX_NEAR]
                                                             : static_cast<T>(0);
    ...
    outPoints[Super::IDX_LEFT_BOTTOM_POINT].Set(l, b, this->offsets[IDX_FAR]);
    outPoints[Super::IDX_RIGHT_BOTTOM_POINT].Set(r, b, this->offsets[IDX_FAR]);
    outPoints[Super::IDX_RIGHT_TOP_POINT].Set(r, t, this->offsets[IDX_FAR]);
    outPoints[Super::IDX_LEFT_TOP_POINT].Set(l, t, this->offsets[IDX_FAR]);
}
 ...
 */
template<class T, class S>
void AbstractViewFrustum<T, S>::GetTopBasePoints(vislib::Array<Point<T, 3>>& outPoints) const {
    outPoints.Clear();
    outPoints.SetCount(4);
    outPoints[Super::IDX_LEFT_BOTTOM_POINT].Set(
        this->offsets[IDX_LEFT], this->offsets[IDX_BOTTOM], this->offsets[IDX_NEAR]);
    outPoints[Super::IDX_RIGHT_BOTTOM_POINT].Set(
        this->offsets[IDX_RIGHT], this->offsets[IDX_BOTTOM], this->offsets[IDX_NEAR]);
    outPoints[Super::IDX_RIGHT_TOP_POINT].Set(
        this->offsets[IDX_RIGHT], this->offsets[IDX_TOP], this->offsets[IDX_NEAR]);
    outPoints[Super::IDX_LEFT_TOP_POINT].Set(this->offsets[IDX_LEFT], this->offsets[IDX_TOP], this->offsets[IDX_NEAR]);
}
 ...
 */
template<class T, class S>
void AbstractViewFrustum<T, S>::Set(const T fovy, const double aspectRatio, const T zNear, const T zFar) {
 ...
 */
template<class T, class S>
AbstractViewFrustum<T, S>& AbstractViewFrustum<T, S>::operator=(const AbstractViewFrustum& rhs) {
    if (this != &rhs) {
...
template<class T, class S>
template<class Tp, class Sp>
AbstractViewFrustum<T, S>& AbstractViewFrustum<T, S>::operator=(const AbstractViewFrustum<Tp, Sp>& rhs) {
    if (static_cast<void*>(this) != static_cast<const void*>(&rhs)) {
        this->offsets[IDX_BOTTOM] = static_cast<T>(rhs.GetBottomDistance());
 ...
 */
template<class T, class S>
bool AbstractViewFrustum<T, S>::operator==(const AbstractViewFrustum& rhs) const {
...
template<class T, class S>
template<class Tp, class Sp>
bool AbstractViewFrustum<T, S>::operator==(const AbstractViewFrustum<Tp, Sp>& rhs) const {
