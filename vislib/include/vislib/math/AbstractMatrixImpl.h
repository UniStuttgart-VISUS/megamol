 ...
 */
enum MatrixLayout { COLUMN_MAJOR = 1, ROW_MAJOR };
...
public:
     ...
     */
    template<class Sp>
    inline AbstractMatrixImpl<T, D, L, S, C>& operator*=(const C<T, D, L, Sp>& rhs) {
     ...
     */
    template<class Sp>
    Vector<T, D - 1> operator*(const AbstractVector<T, D - 1, Sp>& rhs) const;
     ...
     */
    template<class Sp>
    inline Point<T, D - 1> operator*(const AbstractPoint<T, D - 1, Sp>& rhs) const {
        ShallowVector<T, D - 1> v(const_cast<double*>(rhs.PeekCoordinates()));
    ...
    inline AbstractMatrixImpl<T, D, L, S, C>& operator=(const C<Tp, Dp, Lp, Sp>& rhs) {
...
protected:
     ...
     */
    static VISLIB_FORCEINLINE T determinant2x2(const T& a00, const T& a10, const T& a01, const T& a11);
     ...
     * @return The determinant of the matrix
     */
    static VISLIB_FORCEINLINE T determinant3x3(const T& a00, const T& a10, const T& a20, const T& a01, const T& a11,
     ...
     * @return The determinant of the matrix
     */
    static VISLIB_FORCEINLINE T determinant4x4(const T& a00, const T& a10, const T& a20, const T& a30, const T& a01,
        const T& a11, const T& a21, const T& a31, const T& a02, const T& a12, const T& a22, const T& a32, const T& a03,
     ...
     */
    unsigned int findEigenvaluesSym(T* outEigenvalues, Vector<T, D>* outEigenvectors, unsigned int size) const;
...
void AbstractMatrixImpl<T, D, L, S, C>::Dump(std::ostream& out) const {
    out << std::setiosflags(std::ios::fixed) << std::setprecision(3) << std::setfill(' ');
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
Vector<T, D> AbstractMatrixImpl<T, D, L, S, C>::GetColumn(const int col) const {
    Vector<T, D> retval;
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
Vector<T, D> AbstractMatrixImpl<T, D, L, S, C>::GetRow(const int row) const {
    Vector<T, D> retval;
    ...
    for (unsigned int r = 0; r < D; r++) {
        for (unsigned int c = 0; c < D; c++) {
            ...
            for (unsigned int i = 0; i < D; i++) {
                t += this->components[indexOf(r, i)] * this->components[indexOf(i, c)];
            }
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
void AbstractMatrixImpl<T, D, L, S, C>::SetAt(const int row, const int col, const T value) {
    ...
    if ((idx >= 0) && (idx < static_cast<int>(CNT_COMPONENTS))) {
        ...
        throw OutOfRangeException(idx, 0, CNT_COMPONENTS - 1, __FILE__, __LINE__);
    }
}
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
template<class Sp>
AbstractMatrixImpl<T, D, L, S, C>& AbstractMatrixImpl<T, D, L, S, C>::operator+=(const C<T, D, L, Sp>& rhs) {
    for (unsigned int c = 0; c < CNT_COMPONENTS; c++) {
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
template<class Sp>
typename AbstractMatrixImpl<T, D, L, S, C>::DeepStorageMatrix AbstractMatrixImpl<T, D, L, S, C>::operator*(
    ...
    for (unsigned int r = 0; r < D; r++) {
        for (unsigned int c = 0; c < D; c++) {
            for (unsigned int i = 0; i < D; i++) {
                retvalComps[indexOf(r, c)] += this->components[indexOf(r, i)] * rhsComps[indexOf(i, c)];
            }
        }
    }
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
AbstractMatrixImpl<T, D, L, S, C>& AbstractMatrixImpl<T, D, L, S, C>::operator*=(const T rhs) {
    for (unsigned int c = 0; c < CNT_COMPONENTS; c++) {
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
template<class Sp>
Vector<T, D> AbstractMatrixImpl<T, D, L, S, C>::operator*(const AbstractVector<T, D, Sp>& rhs) const {
    Vector<T, D> retval;
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
template<class Sp>
Vector<T, D - 1> AbstractMatrixImpl<T, D, L, S, C>::operator*(const AbstractVector<T, D - 1, Sp>& rhs) const {
    Vector<T, D - 1> retval;
    ASSERT(retval.IsNull());
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
AbstractMatrixImpl<T, D, L, S, C>& AbstractMatrixImpl<T, D, L, S, C>::operator=(const C<T, D, L, S>& rhs) {

    if (this != &rhs) {
        ::memcpy(this->components, rhs.components, CNT_COMPONENTS * sizeof(T));
    }
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
bool AbstractMatrixImpl<T, D, L, S, C>::operator==(const C<T, D, L, S>& rhs) const {
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
template<class Tp, unsigned int Dp, MatrixLayout Lp, class Sp>
bool AbstractMatrixImpl<T, D, L, S, C>::operator==(const C<Tp, Dp, Lp, Sp>& rhs) const {
    ...
    for (unsigned int d = 0; d < CNT_COMPONENTS; d++) {
        if (!IsEqual<T>(this->components[d], (L == Lp) ? rhs.components[d] : rhs.components[transposeIndex(d)])) {
            return false;
        }
    }
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
T AbstractMatrixImpl<T, D, L, S, C>::operator()(const int row, const int col) const {
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
T& AbstractMatrixImpl<T, D, L, S, C>::operator()(const int row, const int col) {
    int idx = indexOf(row, col);
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
const unsigned int AbstractMatrixImpl<T, D, L, S, C>::CNT_COMPONENTS = D* D;
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
template<class Tp, unsigned int Dp, MatrixLayout Lp, class Sp>
void AbstractMatrixImpl<T, D, L, S, C>::assign(const C<Tp, Dp, Lp, Sp>& rhs) {

    for (unsigned int r = 0; r < D; r++) {
        for (unsigned int c = 0; c < D; c++) {
            if ((r < Dp) && (c < Dp)) {
                this->components[indexOf(r, c)] = static_cast<T>(rhs.GetAt(r, c));
            } else if (r == c) {
...
Polynom<T, D> AbstractMatrixImpl<T, D, L, S, C>::characteristicPolynom(void) const {
    ...
    if (!B[0].IsNull()) {
        throw Exception("Characteristic polynom calculation failed", __FILE__, __LINE__);
    }
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
T AbstractMatrixImpl<T, D, L, S, C>::determinant2x2(const T& a00, const T& a10, const T& a01, const T& a11) {
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
T AbstractMatrixImpl<T, D, L, S, C>::determinant3x3(const T& a00, const T& a10, const T& a20, const T& a01,
    const T& a11, const T& a21, const T& a02, const T& a12, const T& a22) {
    // rule of sarrus
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
T AbstractMatrixImpl<T, D, L, S, C>::determinant4x4(const T& a00, const T& a10, const T& a20, const T& a30,
    const T& a01, const T& a11, const T& a21, const T& a31, const T& a02, const T& a12, const T& a22, const T& a32,
    const T& a03, const T& a13, const T& a23, const T& a33) {
    // Method: 1 x laplace + sarrus
    return a02 * determinant3x3(a10, a20, a30, a11, a21, a31, a13, a23, a33) -
           a12 * determinant3x3(a00, a20, a30, a01, a21, a31, a03, a23, a33) +
           a22 * determinant3x3(a00, a10, a30, a01, a11, a31, a03, a13, a33) -
           a32 * determinant3x3(a00, a10, a20, a01, a11, a21, a03, a13, a23);
}
...
template<class T, unsigned int D, MatrixLayout L, class S,
    template<class Tc, unsigned int Dc, MatrixLayout Lc, class Sc> class C>
unsigned int AbstractMatrixImpl<T, D, L, S, C>::findEigenvaluesSym(
    ...
    for (l = 1; l <= static_cast<int>(D); l++) {
        ...
        do {
            for (m = l; m <= static_cast<int>(D) - 1; m++) {
                ...
                if (IsEqual(::fabs(e[m - 1]) + dd, dd))
                    break;
            }
            if (m != l) {
                if (iter++ == MAX_ITER) {
                    throw vislib::Exception("Too many iterations in FindEigenvalues", __FILE__, __LINE__);
                }
