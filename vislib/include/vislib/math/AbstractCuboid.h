...
public:
     ...
     */
    template<class Sp>
    bool Contains(const AbstractPoint<T, 3, Sp>& point, const UINT32 includeFace = 0) const;
     ...
     */
    template<class I>
    void GetClipPlanes(I oit) const;
     ...
     */
    inline void Set(const T& left, const T& bottom, const T& back, const T& right, const T& top, const T& front) {
     ...
     */
    template<class Sp>
    void Union(const AbstractCuboid<T, Sp>& cuboid);
...
const UINT32 AbstractCuboid<T, S>::FACE_ALL =
    AbstractCuboid<T, S>::FACE_LEFT | AbstractCuboid<T, S>::FACE_BOTTOM | AbstractCuboid<T, S>::FACE_BACK |
    AbstractCuboid<T, S>::FACE_RIGHT | AbstractCuboid<T, S>::FACE_TOP | AbstractCuboid<T, S>::FACE_FRONT;
 ...
 */
template<class T, class S>
const UINT32 AbstractCuboid<T, S>::FACE_BACK = 1 << AbstractCuboid<T, S>::IDX_BACK;
 ...
 */
template<class T, class S>
const UINT32 AbstractCuboid<T, S>::FACE_BOTTOM = 1 << AbstractCuboid<T, S>::IDX_BOTTOM;
 ...
 */
template<class T, class S>
const UINT32 AbstractCuboid<T, S>::FACE_FRONT = 1 << AbstractCuboid<T, S>::IDX_FRONT;
 ...
 */
template<class T, class S>
const UINT32 AbstractCuboid<T, S>::FACE_LEFT = 1 << AbstractCuboid<T, S>::IDX_LEFT;
 ...
 */
template<class T, class S>
const UINT32 AbstractCuboid<T, S>::FACE_RIGHT = 1 << AbstractCuboid<T, S>::IDX_RIGHT;
 ...
 */
template<class T, class S>
const UINT32 AbstractCuboid<T, S>::FACE_TOP = 1 << AbstractCuboid<T, S>::IDX_TOP;
...
template<class T, class S>
template<class Sp>
bool AbstractCuboid<T, S>::Contains(const AbstractPoint<T, 3, Sp>& point, const UINT32 includeFace) const {

    if ((point.X() < this->bounds[IDX_LEFT]) ||
        ((((includeFace & FACE_LEFT) == 0)) && (point.X() == this->bounds[IDX_LEFT]))) {
        /* Point is left of cuboid. */
        return false;
    }

    if ((point.Y() < this->bounds[IDX_BOTTOM]) ||
        ((((includeFace & FACE_BOTTOM) == 0)) && (point.Y() == this->bounds[IDX_BOTTOM]))) {
        /* Point is below cuboid. */
        return false;
    }

    if ((point.Z() < this->bounds[IDX_BACK]) ||
        ((((includeFace & FACE_BACK) == 0)) && (point.Z() == this->bounds[IDX_BACK]))) {
        /* Point is behind cuboid. */
        return false;
    }

    if ((point.X() > this->bounds[IDX_RIGHT]) ||
        ((((includeFace & FACE_RIGHT) == 0)) && (point.X() == this->bounds[IDX_RIGHT]))) {
        /* Point is right of cuboid. */
        return false;
    }

    if ((point.Y() > this->bounds[IDX_TOP]) ||
        ((((includeFace & FACE_TOP) == 0)) && (point.Y() == this->bounds[IDX_TOP]))) {
        /* Point is above cuboid. */
        return false;
    }

    if ((point.Z() > this->bounds[IDX_FRONT]) ||
        ((((includeFace & FACE_FRONT) == 0)) && (point.Z() == this->bounds[IDX_FRONT]))) {
...
template<class T, class S>
template<class I>
void AbstractCuboid<T, S>::GetClipPlanes(I oit) const {
...
template<class T, class S>
template<class Sp>
void vislib::math::AbstractCuboid<T, S>::Union(const AbstractCuboid<T, Sp>& cuboid) {
 ...
 */
template<class T, class S>
vislib::math::AbstractCuboid<T, S>& vislib::math::AbstractCuboid<T, S>::operator=(const AbstractCuboid& rhs) {
...
template<class T, class S>
template<class Tp, class Sp>
AbstractCuboid<T, S>& AbstractCuboid<T, S>::operator=(const AbstractCuboid<Tp, Sp>& rhs) {
 ...
 */
template<class T, class S>
bool vislib::math::AbstractCuboid<T, S>::operator==(const AbstractCuboid& rhs) const {
