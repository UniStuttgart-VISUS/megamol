 ...
 */
template<>
class WindowsService<CharTraitsA> {
...
public:
     ...
     */
    static void Install(const String& binaryPath, const String& svcName, const String& displayName,
        const DWORD svcType = SERVICE_WIN32_OWN_PROCESS, const DWORD startType = SERVICE_AUTO_START);
...
protected:
     ...
     */
    inline WindowsService(
        const String& name, const String& displayName, const DWORD svcType, const DWORD controlsAccepted)
...
private:
    ...
    inline WindowsService(const WindowsService& rhs) {
        throw UnsupportedOperationException("WindowsService", __FILE__, __LINE__);
    }
 ...
 */
template<>
class WindowsService<CharTraitsW> {
