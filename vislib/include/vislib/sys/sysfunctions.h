...
#else /* _WIN32 */
#define CONTAINING_RECORD(address, type, field) ((type*)((PCHAR)(address) - ((ULONG_PTR)(&((type*)4)->field) - 4)))
#endif /* _WIN32 */
#endif /* CONTAINING_RECORD */
#define CONTAINING_STRUCT(address, type, field) CONTAINING_RECORD(address, type, field)
...
bool FilenameGlobMatch(const T* filename, const T* pattern) {
    ...
    while ((fnp < fnl) && (pp < pl)) {
        ...
        case '[': {
            SIZE_T pps = pp;
            while ((pp < pl) && (pattern[pp] != ']'))
                pp++;
            if (pp == pl)
                return false;
            vislib::String<vislib::CharTraits<T>> matchGroup(
                pattern + pps + 1, static_cast<typename vislib::String<vislib::CharTraits<T>>::Size>(pp - (1 + pps)));
            if (!matchGroup.Contains(filename[fnp]))
                return false;
            fnp++;
            pp++;
        } break;
...
bool ReadTextFile(
    String<tp1>& outStr, const tp2* filename, TextFileFormat format = TEXTFF_UNSPECIFIC, bool forceFormat = false) {
    File file;
    bool retval = false;
 ...
 */
template<class tp1, class tp2>
bool ReadTextFile(String<tp1>& outStr, const String<tp2>& filename, TextFileFormat format = TEXTFF_UNSPECIFIC,
    bool forceFormat = false) {
    return ReadTextFile(outStr, filename.PeekBuffer(), format, forceFormat);
}
 ...
 * @throws IOException If reading from the file failed.
 */
bool ReadTextFile(StringA& outStr, File& file, TextFileFormat format = TEXTFF_UNSPECIFIC, bool forceFormat = false);
 ...
 * @throws IOException If reading from the file failed.
 */
bool ReadTextFile(StringW& outStr, File& file, TextFileFormat format = TEXTFF_UNSPECIFIC, bool forceFormat = false);
 ...
 *                         it has no DllGetVersion function.
 */
HRESULT GetDLLVersion(DLLVERSIONINFO& outVersion, const wchar_t* moduleName);
#endif /* _WIN32 */
 ...
 */
template<class T>
bool WriteFormattedLineToFile(File& out, const T* format, ...) {
...
bool WriteLineToFile(File& out, const T* text) {
    SIZE_T len = vislib::CharTraits<T>::SafeStringLength(text) * sizeof(T);
 ...
 */
template<class tp1, class tp2>
bool WriteTextFile(const String<tp1>& filename, const String<tp2>& text, bool force = false,
    TextFileFormat format = TEXTFF_UNSPECIFIC, TextFileFormatBOM bom = TEXTFF_BOM_UNSPECIFIC) {
 ...
 */
template<class tp1, class tp2>
bool WriteTextFile(const tp1* filename, const String<tp2>& text, bool force = false,
    TextFileFormat format = TEXTFF_UNSPECIFIC, TextFileFormatBOM bom = TEXTFF_BOM_UNSPECIFIC) {
    bool retval = false;
    File file;
    if (file.Open(
            filename, File::WRITE_ONLY, File::SHARE_EXCLUSIVE, force ? File::CREATE_OVERWRITE : File::CREATE_ONLY)) {
 ...
 * @throws SystemException in case of an error.
 */
bool WriteTextFile(File& file, const StringA& text, TextFileFormat format = TEXTFF_ASCII,
