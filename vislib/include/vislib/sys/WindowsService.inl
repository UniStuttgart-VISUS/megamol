...
#define IMPLEMENT_WINDOWS_SERVICE_INSTALL1(strType)                                                            \
    ...
    SC_HANDLE hSvc = NULL;                                                                                     \
                                                                                                               \
    if ((hSCMgr = ::OpenSCManager##strType(NULL, NULL, SC_MANAGER_ALL_ACCESS)) == NULL) {                      \
        throw SystemException(__FILE__, __LINE__);                                                             \
    }                                                                                                          \
                                                                                                               \
    if ((hSvc = ::CreateService##strType(hSCMgr, svcName, displayName, SERVICE_ALL_ACCESS, svcType, startType, \
             SERVICE_ERROR_NORMAL, binaryPath, NULL, NULL, NULL, NULL, NULL)) == NULL) {                       \
...
#define IMPLEMENT_WINDOWS_SERVICE_SETDESCRIPTION(strType)                                                 \
    ...
    bool retval = false;                                                                                  \
                                                                                                          \
    if (::RegOpenKey##strType(HKEY_LOCAL_MACHINE, HKSERVICES##strType, &hKeyServices) != ERROR_SUCCESS) { \
        return false;                                                                                     \
    }                                                                                                     \
                                                                                                          \
    if (::RegOpenKey##strType(hKeyServices, svcName, &hKeySvc) != ERROR_SUCCESS) {                        \
        ::RegCloseKey(hKeyServices);                                                                      \
        return false;                                                                                     \
    }                                                                                                     \
                                                                                                          \
    retval = (::RegSetValueEx##strType(hKeySvc, VALNAMEDESC##strType, 0, REG_SZ,                          \
                  reinterpret_cast<const BYTE*>(desc.PeekBuffer()),                                       \
...
#define IMPLEMENT_WINDOWS_SERVICE_INSTALL2(strType)                                                                \
    String##strType binaryPath;                                                                                    \
                                                                                                                   \
    if (::GetModuleFileName##strType(NULL, binaryPath.AllocateBuffer(MAX_PATH), MAX_PATH)) {                       \
        WindowsService::Install(binaryPath, this->name, this->displayName, this->status.dwServiceType, startType); \
    } else {                                                                                                       \
        throw SystemException(__FILE__, __LINE__);                                                                 \
    }
...
#define IMPLEMENT_WINDOWS_SERVICE_UNINSTALL(strType)                                       \
    ...
    if ((hSCMgr = ::OpenSCManager##strType(NULL, NULL, SC_MANAGER_ALL_ACCESS)) == NULL) {  \
        throw SystemException(__FILE__, __LINE__);                                         \
                                                                                           ...
                                                                                           \
    if ((hSvc = ::OpenService##strType(hSCMgr, this->name, SERVICE_ALL_ACCESS)) == NULL) { \
        ::CloseServiceHandle(hSCMgr);                                                      \
        throw SystemException(__FILE__, __LINE__);                                         \
    }                                                                                      \
                                                                                           \
...
#define IMPLEMENT_WINDOWS_SERVICE_RUN(strType)                                                 \
    ...
    SERVICE_TABLE_ENTRY##strType svcTable[] = {                                                \
        {const_cast<Char*>(this->name.PeekBuffer()),                                           \
            reinterpret_cast<LPSERVICE_MAIN_FUNCTION##strType>(&WindowsService::serviceMain)}, \
        {NULL, NULL}};                                                                         \
                                                                                               \
    ...
    if (::StartServiceCtrlDispatcher##strType(svcTable)) {                                     \
        ...
        WindowsService::instance = NULL;                                                       \
                                                                                               \
        if ((errorCode = ::GetLastError()) == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) {       \
...
#define IMPLEMENT_WINDOWS_SERVICE_HANDLER_EX                       \
    ...
    case SERVICE_CONTROL_CONTINUE:                                 \
        ...
        } catch (SystemException e) { retval = e.GetErrorCode(); } \
        break;                                                     \
                                                                   \
...
#define IMPLEMENT_WINDOWS_SERVICE_SERVICE_MAIN(strType)                                                           \
    ...
    SERVICE_STATUS& status = WindowsService::instance->status;                                                    \
                                                                                                                  \
    if ((hStatus = ::RegisterServiceCtrlHandlerEx##strType(                                                       \
             NULL, WindowsService::handlerEx, WindowsService::instance)) != NULL) {                               \
        status.dwCurrentState = SERVICE_RUNNING;                                                                  \
        ::SetServiceStatus(hStatus, &status);                                                                     \
                                                                                                                  \
        status.dwServiceSpecificExitCode = WindowsService::instance->OnRun(argc, const_cast<const Char**>(argv)); \
 ...
 */
void vislib::sys::WindowsService<vislib::CharTraitsA>::Install(const String& binaryPath, const String& svcName,
 ...
 */
bool vislib::sys::WindowsService<vislib::CharTraitsA>::SetDescription(const String& svcName, const String& desc) {
 ...
 * vislib::sys::WindowsService<vislib::CharTraitsA>::Install
 */
void vislib::sys::WindowsService<vislib::CharTraitsA>::Install(const DWORD startType) {
 ...
 */
DWORD vislib::sys::WindowsService<vislib::CharTraitsA>::OnDeviceEvent(const DWORD eventType, void* eventData) {
    return ERROR_CALL_NOT_IMPLEMENTED;
}
 ...
 */
DWORD
vislib::sys::WindowsService<vislib::CharTraitsA>::OnHardwareProfileChanged(const DWORD eventType) {
    return ERROR_CALL_NOT_IMPLEMENTED;
}
 ...
 */
DWORD vislib::sys::WindowsService<vislib::CharTraitsA>::OnPowerEvent(const DWORD eventType, void* eventData) {
    return ERROR_CALL_NOT_IMPLEMENTED;
}
 ...
 */
DWORD vislib::sys::WindowsService<vislib::CharTraitsA>::OnSessionChange(const DWORD eventType, void* eventData) {
    return ERROR_CALL_NOT_IMPLEMENTED;
}
 ...
 */
DWORD vislib::sys::WindowsService<vislib::CharTraitsA>::OnUserControl(const DWORD control) {
    return ERROR_CALL_NOT_IMPLEMENTED;
}
 ...
 */
void WINAPI vislib::sys::WindowsService<vislib::CharTraitsA>::serviceMain(DWORD argc, Char** argv) {
 ...
 */
vislib::sys::WindowsService<vislib::CharTraitsA>* vislib::sys::WindowsService<vislib::CharTraitsA>::instance = NULL;


/*
 * vislib::sys::WindowsService<vislib::CharTraitsA>::operator =
 */
vislib::sys::WindowsService<vislib::CharTraitsA>& vislib::sys::WindowsService<vislib::CharTraitsA>::operator=(
 ...
 */
void vislib::sys::WindowsService<vislib::CharTraitsW>::Install(const String& binaryPath, const String& svcName,
    const String& displayName, const DWORD svcType, const DWORD startType) {
 ...
 */
bool vislib::sys::WindowsService<vislib::CharTraitsW>::SetDescription(const String& svcName, const String& desc) {
 ...
 * vislib::sys::WindowsService<vislib::CharTraitsW>::Install
 */
void vislib::sys::WindowsService<vislib::CharTraitsW>::Install(const DWORD startType) {
 ...
 */
DWORD vislib::sys::WindowsService<vislib::CharTraitsW>::OnDeviceEvent(const DWORD eventType, void* eventData) {
    return ERROR_CALL_NOT_IMPLEMENTED;
}
 ...
 */
DWORD
vislib::sys::WindowsService<vislib::CharTraitsW>::OnHardwareProfileChanged(const DWORD eventType) {
    return ERROR_CALL_NOT_IMPLEMENTED;
}
 ...
 */
DWORD vislib::sys::WindowsService<vislib::CharTraitsW>::OnPowerEvent(const DWORD eventType, void* eventData) {
    return ERROR_CALL_NOT_IMPLEMENTED;
}
 ...
 */
DWORD vislib::sys::WindowsService<vislib::CharTraitsW>::OnSessionChange(const DWORD eventType, void* eventData) {
    return ERROR_CALL_NOT_IMPLEMENTED;
}
 ...
 */
DWORD vislib::sys::WindowsService<vislib::CharTraitsW>::OnUserControl(const DWORD control) {
    return ERROR_CALL_NOT_IMPLEMENTED;
}
 ...
 */
void WINAPI vislib::sys::WindowsService<vislib::CharTraitsW>::serviceMain(DWORD argc, Char** argv) {
 ...
 */
vislib::sys::WindowsService<vislib::CharTraitsW>* vislib::sys::WindowsService<vislib::CharTraitsW>::instance = NULL;


/*
 * vislib::sys::WindowsService<vislib::CharTraitsW>::operator =
 */
vislib::sys::WindowsService<vislib::CharTraitsW>& vislib::sys::WindowsService<vislib::CharTraitsW>::operator=(
    const WindowsService& rhs) {
    if (this != &rhs) {
        throw IllegalParamException("rhs", __FILE__, __LINE__);
    }
