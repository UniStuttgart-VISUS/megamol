 ...
 */
template<class T>
class RunnableThread : public Thread, public T {
 ...
 */
template<class T>
bool RunnableThread<T>::Terminate(const bool forceTerminate, const int exitCode) {
 ...
 */
template<class T>
RunnableThread<T>& RunnableThread<T>::operator=(const RunnableThread& rhs) {
    Thread::operator=(rhs);
    T::operator=(rhs);
    return *this;
}
