 ...
 */
template<class T>
DirectoryEntry<T>::DirectoryEntry(const vislib::String<T>& path, EntryType& type) : Path(path)
                                                                                  , Type(type) {
    // intentionally empty
}
 ...
 */
template<class T>
DirectoryEntry<T>::DirectoryEntry(const DirectoryEntry& src) : Path(src.Path)
                                                             , Type(src.Type) {
    // intentionally empty
}
 ...
 */
template<class T>
DirectoryEntry<T>& DirectoryEntry<T>::operator=(const DirectoryEntry<T>& rhs) {
    this->Path = rhs.Path;
    this->Type = rhs.Type;
    return *this;
}
 ...
 */
template<class T>
bool DirectoryEntry<T>::operator==(const DirectoryEntry<T>& rhs) const {
    return (this->Path == rhs.Path) && (this->Type == rhs.Type);
}
