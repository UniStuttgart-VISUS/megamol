 ...
 * that class.
 */
template<class T, class L = NullLockable, class LS = CriticalSectionLockable>
class PoolAllocator : public L {
public:
    /**
     ...
     */
    inline void _GetCounts(UINT& outPageCount, UINT& outElCount, UINT& outActElCount) const {
        ...
        for (Page* p = this->firstPage; p; p = p->next) {
            outPageCount++;
            outElCount += p->cnt;
            Element* e = reinterpret_cast<Element*>(reinterpret_cast<char*>(p) + sizeof(Page));
 ...
 */
template<class T, class L, class LS>
PoolAllocator<T, L, LS>::PoolAllocator(void) : allocSize(DefaultAllocationSize)
                                             , firstPage(NULL)
                                             , firstUnused(NULL) {
    // intentionally empty
}
...
T* PoolAllocator<T, L, LS>::AllocateObj(void) {
    ...
    if (this->firstUnused) {
        ...
        while (p) {
            SIZE_T offset = (reinterpret_cast<char*>(e) - reinterpret_cast<char*>(p)) - sizeof(Page);
        ...
        Page* newpage = static_cast<Page*>(::malloc(sizeof(Page) + this->allocSize * sizeof(Element)));
        e = reinterpret_cast<Element*>(reinterpret_cast<char*>(newpage) + sizeof(Page));
...
void PoolAllocator<T, L, LS>::Cleanup(void) {
    ...
    if (eelDirty) {
        ...
        for (pages = this->firstPage; pages; pages = pages->next) {
            Element* e = reinterpret_cast<Element*>(reinterpret_cast<char*>(pages) + sizeof(Page));
...
void PoolAllocator<T, L, LS>::deallocate(T*& inOutPtr) {
    ...
    ASSERT(((reinterpret_cast<char*>(e) - reinterpret_cast<char*>(p)) - sizeof(Page)) % sizeof(Element) == 0);
    ASSERT((((reinterpret_cast<char*>(e) - reinterpret_cast<char*>(p)) - sizeof(Page)) / sizeof(Element)) <= p->cnt);
 ...
 */
template<class T, class L, class LS>
bool PoolAllocator<T, L, LS>::isPageClean(const typename PoolAllocator<T, L, LS>::Page* page) {
    const Element* e = reinterpret_cast<const Element*>(reinterpret_cast<const char*>(page) + sizeof(Page));
