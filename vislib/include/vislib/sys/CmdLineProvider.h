...
public:
     ...
     */
    template<class Tf>
    friend class vislib::sys::CmdLineParser;
 ...
 */
template<class T>
String<T> CmdLineProvider<T>::GetModuleName(void) {
    String<T> str;
    DWORD len = 16;
    DWORD rlen;
    Char* buf = NULL;
 ...
 */
template<class T>
CmdLineProvider<T>::CmdLineProvider() : argCount(0)
                                      , arguments(NULL)
                                      , storeCount(0) /*, memoryAnchor(NULL)*/ {
 ...
 */
template<class T>
CmdLineProvider<T>::CmdLineProvider(const CmdLineProvider& rhs)
        : argCount(0)
        , arguments(NULL)
        , storeCount(0) /*, memoryAnchor(NULL)*/ {
    this->memoryAnchor[0] = NULL;
 ...
 */
template<class T>
CmdLineProvider<T>::CmdLineProvider(const Char* cmdLine)
        : argCount(0)
        , arguments(NULL)
        , storeCount(0) /*, memoryAnchor(NULL)*/ {
    this->memoryAnchor[0] = NULL;
 ...
 */
template<class T>
CmdLineProvider<T>::CmdLineProvider(const String<T>& cmdLine)
        : argCount(0)
        , arguments(NULL)
        , storeCount(0) /*, memoryAnchor(NULL)*/ {
    this->memoryAnchor[0] = NULL;
 ...
 */
template<class T>
CmdLineProvider<T>::CmdLineProvider(const Char* appName, const Char* cmdLine)
        : argCount(0)
        , arguments(NULL)
        , storeCount(0) /*, memoryAnchor(NULL)*/ {
    this->memoryAnchor[0] = NULL;
 ...
 */
template<class T>
CmdLineProvider<T>::CmdLineProvider(const Char* appName, const String<T>& cmdLine)
        : argCount(0)
        , arguments(NULL)
        , storeCount(0) /*, memoryAnchor(NULL)*/ {
    this->memoryAnchor[0] = NULL;
 ...
 */
template<class T>
CmdLineProvider<T>::CmdLineProvider(const String<T>& appName, const Char* cmdLine)
        : argCount(0)
        , arguments(NULL)
        , storeCount(0) /*, memoryAnchor(NULL)*/ {
    this->memoryAnchor[0] = NULL;
 ...
 */
template<class T>
CmdLineProvider<T>::CmdLineProvider(const String<T>& appName, const String<T>& cmdLine)
        : argCount(0)
        , arguments(NULL)
        , storeCount(0) /*, memoryAnchor(NULL)*/ {
    this->memoryAnchor[0] = NULL;
 ...
 */
template<class T>
CmdLineProvider<T>& CmdLineProvider<T>::operator=(const CmdLineProvider<T>& rhs) {
    if (&rhs != this) {
        this->clearArgumentList();
 ...
 */
template<class T>
void CmdLineProvider<T>::CreateCmdLine(const Char* appName, const Char* cmdLine) {
    Char* ci;
    Char* start = NULL;
    unsigned int state;
 ...
 */
template<class T>
typename CmdLineProvider<T>::Char* CmdLineProvider<T>::createArgument(Char* left, Char* right) {
    Char* buf = NULL;
 ...
 */
template<class T>
void CmdLineProvider<T>::Append(const Char* arg) {
 ...
 */
template<class T>
void CmdLineProvider<T>::Prepend(const Char* arg) {
    this->reflectCallerChanges();
 ...
 */
template<class T>
String<T> CmdLineProvider<T>::SingleStringCommandLine(bool includeFirst) {
    String<T> retval;
    ...
    if (this->argCount > startIndex) {
        ...
        for (int i = startIndex; i < this->argCount; i++) {
            ...
            for (Char* ci = this->arguments[i]; *ci != 0; ci++) {
                len++; // character
                if (T::IsSpace(*ci))
                    needQuots = true; // parameter with spaces
        ...
        for (int i = startIndex; i < this->argCount; i++) {
            if (i > startIndex)
                *(data++) = static_cast<Char>(' '); // seperating space
