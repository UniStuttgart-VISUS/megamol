...
public:
     ...
     */
    BitmapImage(
        unsigned int width, unsigned int height, unsigned int channels, ChannelType type, const void* data = NULL);
     ...
     *             (black/transparent) in all channels.
     */
    BitmapImage(unsigned int width, unsigned int height, const BitmapImage& tmpl, const void* data = NULL);
     ...
     */
    BitmapImage(ChannelType type, ChannelLabel label1, ChannelLabel label2);
     ...
     */
    BitmapImage(ChannelType type, ChannelLabel label1, ChannelLabel label2, ChannelLabel label3);
     ...
     *             (black/transparent) in all channels.
     */
    void CreateImage(unsigned int width, unsigned int height, const BitmapImage& tmpl, const void* data = NULL);
     ...
     */
    void ExtractFrom(
        const BitmapImage& src, unsigned int left, unsigned int top, unsigned int width, unsigned int height);
    ...
    inline ChannelLabel GetChannelLabel(unsigned int channel) const {
        return (channel >= this->numChans) ? CHANNEL_UNDEF : this->labels[channel];
    }
     ...
     */
    inline void LabelChannelsRGB(unsigned int r = 0, unsigned int g = 1, unsigned int b = 2) {
     ...
     */
    inline void LabelChannelsRGBA(unsigned int r = 0, unsigned int g = 1, unsigned int b = 2, unsigned int a = 3) {
        this->SetChannelLabel(r, CHANNEL_RED);
        this->SetChannelLabel(g, CHANNEL_GREEN);
        this->SetChannelLabel(b, CHANNEL_BLUE);
...
private:
    ...
    public:
         ...
         * @param src The source
         */
        static VISLIB_FORCEINLINE void CopyBit(unsigned char& dst, const unsigned short& src) {
         ...
         * @param src The source
         */
        static VISLIB_FORCEINLINE void CopyBit(unsigned char& dst, const float& src) {
            dst = static_cast<unsigned char>(((src < 0.0f) ? 0 : ((src > 1.0f) ? 1.0f : src)) * 255.0f);
        }
         ...
         * @param src The source
         */
        static VISLIB_FORCEINLINE void CopyBit(unsigned short& dst, const unsigned char& src) {
            dst = (static_cast<unsigned short>(src) << 8) + static_cast<unsigned short>(src);
        }
         ...
         * @param src The source
         */
        static VISLIB_FORCEINLINE void CopyBit(unsigned short& dst, const float& src) {
            dst = static_cast<unsigned char>(((src < 0.0f) ? 0 : ((src > 1.0f) ? 1.0f : src)) * 65535.0f);
        }
         ...
         * @param src The source
         */
        static VISLIB_FORCEINLINE void CopyBit(float& dst, const unsigned char& src) {
         ...
         * @param src The source
         */
        static VISLIB_FORCEINLINE void CopyBit(float& dst, const unsigned short& src) {
         ...
         */
        void ChannelMapping(SourceChannel* map, ChannelLabel* chan, unsigned int cnt);
    ...
    private:
        ...
        Conversion(const Conversion& src) {
            throw vislib::UnsupportedOperationException("Conversion::CopyCtor", __FILE__, __LINE__);
        }
        ...
        Conversion& operator=(const Conversion& rhs) {
            if (this != &rhs) {
                throw vislib::UnsupportedOperationException("Conversion::operator=", __FILE__, __LINE__);
            }
            return *this;
        }
     ...
     */
    void cropCopy(char* to, char* from, unsigned int fromWidth, unsigned int fromHeight, unsigned int cropX,
        unsigned int cropY, unsigned int cropWidth, unsigned int cropHeight, unsigned int bpp);
     ...
     */
    template<class DT, class ST>
    void fullConvert(unsigned int w, unsigned int h, DT* dst, ChannelLabel* dstChan, unsigned int dstChanCnt, ST* src,
        ChannelLabel* srcChan, unsigned int srcChanCnt);
     ...
     */
    template<class T>
    void invert(T maxval, unsigned int chan);
