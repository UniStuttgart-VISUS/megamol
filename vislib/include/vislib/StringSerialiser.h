 ...
 */
template<class T>
class StringSerialiser : public Serialiser {
public:
    /**
     * Ctor.
     *
    ...
    virtual void Serialise(const StringA& value, const char* name = NULL) {
        if (name != NULL) {
            ...
            this->data.Append(
                vislib::Pair<vislib::String<T>, vislib::String<T>>(vislib::String<T>(name), vislib::String<T>(value)));
        } else {
            this->data.Append(vislib::Pair<vislib::String<T>, vislib::String<T>>(NULL, vislib::String<T>(value)));
        }
    }
...
private:
    ...
    inline const vislib::String<T>& value(const char* name) {
        ...
        throw NoSuchElementException("deserialisation failed", __FILE__, __LINE__);
    }
    ...
    vislib::Array<vislib::Pair<vislib::String<T>, vislib::String<T>>> data;
...
StringSerialiser<T>::StringSerialiser(const typename T::Char* str)
        : Serialiser(SERIALISER_SUPPORTS_NAMES)
        , data()
        , nextDePos(0) {
 ...
 */
template<class T>
StringSerialiser<T>::~StringSerialiser(void) {
    // intentionally empty
}
...
void StringSerialiser<T>::SetInputString(const typename T::Char* str) {
    ...
    vislib::StringTokeniser<T> tokeniser(str, static_cast<typename T::Char>('\n'));
    ...
    while (tokeniser.HasNext()) {
        ...
        key.UnescapeCharacters(
            static_cast<typename T::Char>('\\'), vislib::String<T>("\n\r="), vislib::String<T>("nr="));
        value.UnescapeCharacters(
            static_cast<typename T::Char>('\\'), vislib::String<T>("\n\r="), vislib::String<T>("nr="));

        this->data.Append(vislib::Pair<vislib::String<T>, vislib::String<T>>(key, value));
    }
}
...
void StringSerialiser<T>::GetString(vislib::String<T>& outStr) const {
    ...
    for (unsigned int i = 0; i < this->data.Count(); i++) {
        str = this->data[i].Key();
        str.EscapeCharacters(static_cast<typename T::Char>('\\'), vislib::String<T>("\n\r="), vislib::String<T>("nr="));
