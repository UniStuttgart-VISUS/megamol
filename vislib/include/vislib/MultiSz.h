 ...
 */
template<class T>
SIZE_T MultiSz<T>::Count(const Char* multiSz) {
    SIZE_T retval = 0;
 ...
 */
template<class T>
SIZE_T MultiSz<T>::Length(const Char* multiSz) {
    const Char* cursor = multiSz;
 ...
 */
template<class T>
const typename MultiSz<T>::Char* MultiSz<T>::PeekAt(const SIZE_T idx, const Char* multiSz) {
    const Char* cursor = multiSz;
 ...
 */
template<class T>
MultiSz<T>::MultiSz(const Char** strings, const SIZE_T cntStrings) : data(NULL) {
 ...
 */
template<class T>
MultiSz<T>::MultiSz(const MultiSz& rhs) : data(NULL) {
    *this = rhs;
}
 ...
 */
template<class T>
void MultiSz<T>::Append(const Char* str) {
    if ((str != NULL) && (*str != 0)) {
...
void MultiSz<T>::Insert(const SIZE_T idx, const Char* str) {
    ...
    if (idx == oldCnt) {
        ...
        if ((str != NULL) && (*str != 0)) {
            ...
            ::memcpy(newData + offset + strLen, this->data + offset, (oldLen - offset) * sizeof(Char));
        ...
        throw OutOfRangeException(static_cast<int>(idx), 0, static_cast<int>(oldCnt), __FILE__, __LINE__);
    }
}
 ...
 */
template<class T>
void MultiSz<T>::Remove(const Char* str) {
 ...
 */
template<class T>
MultiSz<T>& MultiSz<T>::operator=(const MultiSz& rhs) {
    if (this != &rhs) {
        this->Clear();
 ...
 */
template<class T>
MultiSz<T>& MultiSz<T>::operator=(const Char* rhs) {
    this->Clear();
 ...
 */
template<class T>
bool MultiSz<T>::operator==(const MultiSz& rhs) const {
