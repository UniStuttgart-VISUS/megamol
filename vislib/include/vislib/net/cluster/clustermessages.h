 ...
 */
void InitialiseMessageHeader(MessageHeader& inOutHeader, const UINT32 blockId = 0, const UINT32 blockLength = 0);


/** The magic number which must be at the begin of all network messages. */
const UINT32 MAGIC_NUMBER =
    static_cast<UINT32>('v') | static_cast<UINT32>('l') << 8 | static_cast<UINT32>('c') << 16 | 1 << 24;
 ...
 */
#define DEFINE_VLC1_BASE_MSG(name, id) const UINT32 MSGID_##name = VLC1_BASE_MSG_ID(id)
 ...
 */
#define DEFINE_VLC1_CAM_MSG(name, id) DEFINE_VLC1_BASE_MSG(CAM_##name, 64 + (id))
