...
DWORD VoluMetricJob::Run(void* userData) {
    ...
    for (unsigned int frameI = 0; frameI < frameCnt; frameI++) {
        ...
        for (int x = 0; x < divX; x++) {
            for (int y = 0; y < divY; y++) {
                ...
                for (int z = 0; z < divZ; z++) {
                    ...
                    vislib::math::Cuboid<VoxelizerFloat> bx =
                        vislib::math::Cuboid<VoxelizerFloat>(left, bottom, back, right, top, front);
        ...
        this->debugLines[backBufferIndex][0].Set(static_cast<unsigned int>(idxNumOffset * 2),
            ...
            this->bboxVertData[backBufferIndex].As<VoxelizerFloat>(), vislib::graphics::NamedColours::BlanchedAlmond);
...
void VoluMetricJob::appendBox(vislib::RawStorage& data, vislib::math::Cuboid<VoxelizerFloat>& b, SIZE_T& offset) {
    vislib::math::ShallowPoint<VoxelizerFloat, 3>(data.AsAt<VoxelizerFloat>(offset + 0 * 3 * sizeof(VoxelizerFloat))) =
        b.GetLeftBottomFront();
    vislib::math::ShallowPoint<VoxelizerFloat, 3>(data.AsAt<VoxelizerFloat>(offset + 1 * 3 * sizeof(VoxelizerFloat))) =
        b.GetRightBottomFront();
    vislib::math::ShallowPoint<VoxelizerFloat, 3>(data.AsAt<VoxelizerFloat>(offset + 2 * 3 * sizeof(VoxelizerFloat))) =
        b.GetRightTopFront();
    vislib::math::ShallowPoint<VoxelizerFloat, 3>(data.AsAt<VoxelizerFloat>(offset + 3 * 3 * sizeof(VoxelizerFloat))) =
        b.GetLeftTopFront();
    vislib::math::ShallowPoint<VoxelizerFloat, 3>(data.AsAt<VoxelizerFloat>(offset + 4 * 3 * sizeof(VoxelizerFloat))) =
        b.GetLeftBottomBack();
    vislib::math::ShallowPoint<VoxelizerFloat, 3>(data.AsAt<VoxelizerFloat>(offset + 5 * 3 * sizeof(VoxelizerFloat))) =
        b.GetRightBottomBack();
    vislib::math::ShallowPoint<VoxelizerFloat, 3>(data.AsAt<VoxelizerFloat>(offset + 6 * 3 * sizeof(VoxelizerFloat))) =
        b.GetRightTopBack();
    vislib::math::ShallowPoint<VoxelizerFloat, 3>(data.AsAt<VoxelizerFloat>(offset + 7 * 3 * sizeof(VoxelizerFloat))) =
        b.GetLeftTopBack();
...
VISLIB_FORCEINLINE bool VoluMetricJob::isSurfaceJoinableWithSubvolume(
    SubJobData* surfJob, int surfIdx, SubJobData* volume) {
...
void VoluMetricJob::generateStatistics(vislib::Array<unsigned int>& uniqueIDs, vislib::Array<SIZE_T>& countPerID,
    vislib::Array<VoxelizerFloat>& surfPerID, vislib::Array<VoxelizerFloat>& volPerID,
    vislib::Array<VoxelizerFloat>& voidVolPerID) {
 ...
 */
bool hitTriangleMesh(const vislib::Array<VoxelizerFloat>& mesh,
    const vislib::math::ShallowPoint<VoxelizerFloat, 3>& seedPoint,
    ...
    for (unsigned int triIdx = 0; triIdx < triCount; triIdx++) {
        vislib::math::ShallowShallowTriangle<VoxelizerFloat, 3> triangle(
            const_cast<VoxelizerFloat*>(triPoints + triIdx * 3 * 3));
...
bool VoluMetricJob::testFullEnclosing(
    int enclosingIdx, int enclosedIdx, vislib::Array<vislib::Array<Surface*>>& globaIdSurfaces) {
    ...
    vislib::math::ShallowPoint<VoxelizerFloat, 3> seedPoint(
...
void VoluMetricJob::outputStatistics(unsigned int frameNumber, vislib::Array<unsigned int>& uniqueIDs,
    vislib::Array<SIZE_T>& countPerID, vislib::Array<VoxelizerFloat>& surfPerID,
    vislib::Array<VoxelizerFloat>& volPerID, vislib::Array<VoxelizerFloat>& voidVolPerID) {
