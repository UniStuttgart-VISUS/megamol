...
void access_data(uint idx, out vec4 objPos, out vec4 objColor, out vec3 objDir, out float rad) {
    objPos = vec4(pos_col_dir[idx].pos, 1.0f);

    rad = pos_col_dir[idx].rad;

    objColor = unpackUnorm4x8(pos_col_dir[idx].col);

    //objDir = normalize(pos_col_dir[idx].dir);
    objDir = vec3(pos_col_dir[idx].dir_x, pos_col_dir[idx].dir_y, pos_col_dir[idx].dir_z);
}


void touchplane(out vec4 outPos, out float outL, in vec4 inPos, in vec3 objDir, in out vec3 rad, out vec3 rotMatT0,
    out vec3 rotMatT1, out vec3 rotMatT2, out vec4 camPos, out vec4 rotLightDir) {
    vec4 tmp, tmp1;

    vec3 fromPos = inPos.xyz + (objDir * 0.5) * lengthScale;
    vec3 toPos = inPos.xyz - (objDir * 0.5) * lengthScale;
...
#define CYL_RAD rad.x
    ...
    if (any(notEqual(clipDat.xyz, vec3(0, 0, 0)))) {
        od = dot(inPos.xyz, clipDat.xyz) - rad.x;
    }
    ...
    if (l < rad.y) { // radius-square
        ...
        */
        ...
        float minCylX = -CYL_HALF_LEN + (((rad.x - p * sign(-CYL_HALF_LEN - camPos.x)) * (-CYL_HALF_LEN - camPos.x)) /
                                            (-d * sign(-CYL_HALF_LEN - camPos.x) + rad.x));
        float maxCylX = CYL_HALF_LEN + (((rad.x + p * sign(CYL_HALF_LEN - camPos.x)) * (CYL_HALF_LEN - camPos.x)) /
                                           (d * sign(CYL_HALF_LEN - camPos.x) + rad.x));
    ...
    vec2 minP = vec2(min(min(p1.x, p2.x), min(p3.x, p4.x)), min(min(p1.y, p2.y), min(p3.y, p4.y)));
    vec2 maxP = vec2(max(max(p1.x, p2.x), max(p3.x, p4.x)), max(max(p1.y, p2.y), max(p3.y, p4.y)));

    float w = (od > clipDat.w) ? 0.0 : 1.0;
    w = (CYL_HALF_LEN < lengthFilter * 0.5) ? 0.0 : w;
    outPos = vec4((minP + maxP) * 0.5, 0.5, w);
    maxP = (maxP - minP) * 0.5 * winHalf;
    outL = max(maxP.x, maxP.y) + 0.5;
}
...
void main(void) {
    ...
    if (g_idx < num_points) {
        ...
        //vec4 objColor;
        vec3 objDir;
        access_data(g_idx, pp[l_idx].objPos, pp[l_idx].objColor, objDir, pp[l_idx].rad.x);

        // remove the sphere radius from the w coordinates to the rad varyings
        vec4 inPos = pp[l_idx].objPos;
        //rad.x = (CONSTRAD < -0.5) ? inPos.w : CONSTRAD;
        //inPos.w = 1.0;
        /*float cid = MAX_COLV - MIN_COLV;
        if (cid < 0.000001) {
            gl_FrontColor = objColor;
        } else {
        ...
        pp[l_idx].discardFrag = uint(0);

        vec4 outPos;
        float l;
        touchplane(outPos, l, inPos, objDir, pp[l_idx].rad, pp[l_idx].rotMatT0, pp[l_idx].rotMatT1, pp[l_idx].rotMatT2,
            pp[l_idx].camPos, pp[l_idx].rotLightDir);
        ...
        /*float w = (od > clipDat.w) ? 0.0 : 1.0;
        w = (CYL_HALF_LEN < lengthFilter * 0.5) ? 0.0 : w;
        gl_MeshVerticesNV[l_idx].gl_Position = vec4((minP + maxP) * 0.5, 0.5, w);
        maxP = (maxP - minP) * 0.5 * winHalf;
        gl_MeshVerticesNV[l_idx].gl_PointSize = max(maxP.x, maxP.y) + 0.5;*/
        //} else {
        //    //if (bitflag_test(flags, FLAG_FILTERED, FLAG_FILTERED)) {
        //    gl_MeshVerticesNV[l_idx].gl_Position = vec4(0.0, 0.0, -1.0, 0.0);
        //    gl_MeshVerticesNV[l_idx].gl_PointSize = 1.0;
        //}

        gl_MeshVerticesNV[l_idx].gl_Position = outPos;
