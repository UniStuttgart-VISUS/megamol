    ...
    //outColor = vec4(LocalLighting(ray, normal, outlightDir.xyz, vertColor), 1.0);
    outColor = vec4(LocalLighting(ray, normal, outlightDir.xyz, colOut), vertColor.w);
    ...
    coord = gl_FragCoord 
        * vec4(viewAttr.z, viewAttr.w, 2.0, 0.0) 
        + vec4(-1.0, -1.0, -1.0, 1.0);
    if (min(abs(coord.x - sphere_frag_center.x), abs(coord.y - sphere_frag_center.y)) < 0.002) {
