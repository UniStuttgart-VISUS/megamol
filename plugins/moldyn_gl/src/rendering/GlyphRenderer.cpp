...
GlyphRenderer::GlyphRenderer(void)
    ...
    this->readFlagsSlot.SetCompatibleCall<mmstd_gl::FlagCallRead_GLDescription>();
...
bool GlyphRenderer::Render(mmstd_gl::CallRender3DGL& call) {
    ...
    auto* tfc = this->getTFSlot.CallAs<mmstd_gl::CallGetTransferFunctionGL>();
    auto* flagsc = this->readFlagsSlot.CallAs<mmstd_gl::FlagCallRead_GL>();
    ...
    if (use_flags) {
        (*flagsc)(mmstd_gl::FlagCallRead_GL::CallGetData);
