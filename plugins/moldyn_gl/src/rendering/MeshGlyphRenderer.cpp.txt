...
MeshGlyphRenderer::MeshGlyphRenderer(void)
...
bool MeshGlyphRenderer::create(void) {
    ...
    try {
        ...
        ellipsoid_shader_ = core::utility::make_glowl_shader("MeshEllipsoidShader", shdr_options,
            std::filesystem::path("glyph/ellipsoid_mesh.mesh.glsl"),
            std::filesystem::path("glyph/ellipsoid_mesh.frag.glsl"));
        box_shader_ = core::utility::make_glowl_shader("MeshBoxShader", shdr_options,
            std::filesystem::path("glyph/box_mesh.mesh.glsl"), std::filesystem::path("glyph/box_mesh.frag.glsl"));
    } catch (glowl::GLSLProgramException const& ex) {
        core::utility::log::Log::DefaultLog.WriteError("[MeshGlyphRenderer]: %s", ex.what());
        return false;
    } catch (...) {
        core::utility::log::Log::DefaultLog.WriteError("[MeshGlyphRenderer] Failed to create program");
        return false;
    }
...
bool MeshGlyphRenderer::makeShader(
    ...
    if (!ssf->MakeShaderSource(vertexName.c_str(), vertSrc)) {
        Log::DefaultLog.WriteMsg(
            Log::LEVEL_ERROR, "MeshGlyphRenderer: unable to load vertex shader source: %s", vertexName.c_str());
        return false;
    }
    if (!ssf->MakeShaderSource(fragmentName.c_str(), fragSrc)) {
        Log::DefaultLog.WriteMsg(
            Log::LEVEL_ERROR, "MeshGlyphRenderer: unable to load fragment shader source: %s", fragmentName.c_str());
        return false;
    }
    try {
        if (!shader.Create(vertSrc.Code(), vertSrc.Count(), fragSrc.Code(), fragSrc.Count())) {
            megamol::core::utility::log::Log::DefaultLog.WriteMsg(megamol::core::utility::log::Log::LEVEL_ERROR,
                "MeshGlyphRenderer: unable to compile shader: unknown error\n");
            return false;
        }
    } catch (AbstractOpenGLShader::CompileException& ce) {
        megamol::core::utility::log::Log::DefaultLog.WriteMsg(megamol::core::utility::log::Log::LEVEL_ERROR,
            "MeshGlyphRenderer: unable to compile shader (@%s): %s\n",
            vislib_gl::graphics::gl::AbstractOpenGLShader::CompileException::CompileActionName(ce.FailedAction()),
            ce.GetMsgA());
        return false;
    } catch (vislib::Exception& e) {
        megamol::core::utility::log::Log::DefaultLog.WriteMsg(megamol::core::utility::log::Log::LEVEL_ERROR,
            "MeshGlyphRenderer: unable to compile shader: %s\n", e.GetMsgA());
        return false;
    } catch (...) {
        megamol::core::utility::log::Log::DefaultLog.WriteMsg(megamol::core::utility::log::Log::LEVEL_ERROR,
            "MeshGlyphRenderer: unable to compile shader: unknown exception\n");
        return false;
    }
    return true;
}

bool MeshGlyphRenderer::GetExtents(core_gl::view::CallRender3DGL& call) {
...
void MeshGlyphRenderer::release(void) {
    this->boxShader.Release();
    this->ellipsoidShader.Release();
    glDeleteTextures(1, &this->greyTF);
}

bool megamol::moldyn_gl::rendering::MeshGlyphRenderer::validateData(geocalls::EllipsoidalParticleDataCall* edc) {
        ...
        for (uint32_t x = 0; x < edc->GetParticleListCount(); ++x) {
            ...
            default:
                megamol::core::utility::log::Log::DefaultLog.WriteError(
                    "MeshGlyphRenderer: no support for vertex data types SHORT_XYZ or NONE");
                return false;
            }
            ...
            case geocalls::SimpleSphericalParticles::COLDATA_UINT8_RGB:
                ...
                megamol::core::utility::log::Log::DefaultLog.WriteError(
                    "MeshGlyphRenderer: COLDATA_UINT8_RGB is deprecated and unsupported");
                return false;
            ...
            case geocalls::SimpleSphericalParticles::COLDATA_USHORT_RGBA:
                megamol::core::utility::log::Log::DefaultLog.WriteError(
                    "MeshGlyphRenderer: COLDATA_USHORT_RGBA is unsupported");
                return false;
...
bool MeshGlyphRenderer::Render(core_gl::view::CallRender3DGL& call) {
#ifdef PROFILING
    auto& pm = const_cast<frontend_resources::PerformanceManager&>(
        frontend_resources.get<frontend_resources::PerformanceManager>());
#endif
    ...
    for (unsigned int i = 0; i < epdc->GetParticleListCount(); i++) {
        ...
        case geocalls::SimpleSphericalParticles::COLDATA_FLOAT_I:
            ...
            if (tfc == nullptr) {
                ...
                megamol::core::utility::log::Log::DefaultLog.WriteError(
                    "MeshGlyphRenderer: could not retrieve transfer function!");
                return false;
            }
    ...
    for (unsigned int i = 0; i < epdc->GetParticleListCount(); i++) {
        ...
        for (GLuint x = 0; x < numChunks; ++x) {
            ...
            case Glyph::BOX:
                ...
                // but just drawing the front-facing triangles, that's better
                //glDrawArraysInstanced(GL_TRIANGLE_STRIP, 0, 4, static_cast<GLsizei>(actualItems) * 3);
                shader->setUniform("num_points", static_cast<unsigned int>(actualItems));
                glDrawMeshTasksNV(0, static_cast<GLsizei>(actualItems) / 16 + 1);
                break;
            case Glyph::ELLIPSOID:
                //glDrawArraysInstanced(GL_TRIANGLE_STRIP, 0, 4, static_cast<GLsizei>(actualItems) * 3);
                shader->setUniform("num_points", static_cast<unsigned int>(actualItems));
                //core::utility::log::Log::DefaultLog.WriteInfo("[MESH] NumPoints %d", actualItems);
                glDrawMeshTasksNV(0, static_cast<GLsizei>(actualItems) / 16 + 1);
                break;
            case Glyph::ARROW:
                glDrawArrays(GL_POINTS, 0, static_cast<GLsizei>(actualItems));
