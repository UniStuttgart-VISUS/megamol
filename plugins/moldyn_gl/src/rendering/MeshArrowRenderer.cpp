...
MeshArrowRenderer::MeshArrowRenderer(void)
...
bool MeshArrowRenderer::create(void) {
    ...
    try {
        ...
        arrowShader_ = core::utility::make_glowl_shader("ArrowShader", shdr_options,
            std::filesystem::path("arrow/arrow_mesh.mesh.glsl"), std::filesystem::path("arrow/arrow_mesh.frag.glsl"));
    } catch (glowl::GLSLProgramException const& ex) {
        core::utility::log::Log::DefaultLog.WriteError("[MeshArrowRenderer]: %s", ex.what());
        return false;
    } catch (...) {
        core::utility::log::Log::DefaultLog.WriteError("[MeshArrowRenderer] Failed to create program");
        return false;
    }
...
bool MeshArrowRenderer::GetExtents(core_gl::view::CallRender3DGL& call) {
...
void MeshArrowRenderer::release(void) {
...
bool MeshArrowRenderer::Render(core_gl::view::CallRender3DGL& call) {
#ifdef PROFILING
    auto& pm = const_cast<frontend_resources::PerformanceManager&>(
        frontend_resources.get<frontend_resources::PerformanceManager>());
#endif
    ...
    if (call_light != nullptr) {
        ...
        if (distant_lights.size() > 1) {
            megamol::core::utility::log::Log::DefaultLog.WriteWarn(
                "[MeshArrowRenderer] Only one single 'Distant Light' source is supported by this renderer");
        } else if (distant_lights.empty()) {
            megamol::core::utility::log::Log::DefaultLog.WriteWarn("[MeshArrowRenderer] No 'Distant Light' found");
        }
    ...
    if (c2 != nullptr) {
        ...
        if (cflags != nullptr) {
            if (c2->GetParticleListCount() > 1) {
                megamol::core::utility::log::Log::DefaultLog.WriteWarn(
                    "MeshArrowRenderer: Cannot use FlagStorage together with multiple particle lists!");
            } else {
                useFlags = true;
            }
        }
        ...
        for (unsigned int i = 0; i < c2->GetParticleListCount(); i++) {
            ...
            //default:
            //    megamol::core::utility::log::Log::DefaultLog.WriteWarn(
            //        "MeshArrowRenderer: cannot render arrows without directional data!");
            //    continue;
            //}
            ...
            arrowShader_->setUniform("num_points", static_cast<unsigned int>(parts.GetCount()));
            //glDrawArrays(GL_POINTS, 0, static_cast<GLsizei>(parts.GetCount()));
            glDrawMeshTasksNV(0, static_cast<unsigned int>(parts.GetCount()) / 32 + 1);
...
void megamol::moldyn_gl::rendering::MeshArrowRenderer::loadData(geocalls::MultiParticleDataCall& in_data) {
