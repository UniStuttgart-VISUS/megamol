...
bool megamol::gui::PickableCube::Draw(unsigned int picking_id, int& inout_face_id, int& inout_orientation_id, int& out_hovered_face_id,
    ...
    if (this->shader == nullptr) {
        ...
        std::string fragment_src =
            ...
            "} \n"
            "\n"
            "float smoothalpha(float val, float max_val) { \n"
            ...
            "        outFragColor = mix(vec4(vertex_color, 1.0), vec4(tex_color, 1.0), alpha); \n"
            "    } \n"
            "    \n"
            "    int face_id = current_face_id; \n"
            ...
            "        if (current_face_id == FACE_FRONT)       face_id = CORNER_BOTTOM_RIGHT_FRONT; \n"
            "        else if (current_face_id == FACE_BACK)   face_id = CORNER_BOTTOM_LEFT_BACK; \n"
            "        else if (current_face_id == FACE_RIGHT)  face_id = CORNER_BOTTOM_RIGHT_BACK; \n"
            "        else if (current_face_id == FACE_LEFT)   face_id = CORNER_BOTTOM_LEFT_FRONT; \n"
            "        else if (current_face_id == FACE_TOP)    face_id = CORNER_TOP_RIGHT_FRONT; \n"
            "        else if (current_face_id == FACE_BOTTOM) face_id = CORNER_BOTTOM_RIGHT_BACK; \n"
            ...
            "           ((current_face_id == FACE_BOTTOM)  && (face_hover_id == CORNER_BOTTOM_RIGHT_BACK))) { \n"
            "               float dist = sqrt(((1.0 - tex_coord.x) * (1.0 - tex_coord.x)) + ((1.0 - tex_coord.y) * (1.0 - tex_coord.y))); \n"
            "               corner_alpha = smoothalpha(dist , dc); \n"
            "        } \n"
            "    } else if ((tex_coord.x < dc) && (tex_coord.y < dc)) {           // ----- TOP LEFT ----- \n"
            "        if (current_face_id == FACE_FRONT)       face_id = CORNER_TOP_LEFT_FRONT; \n"
            "        else if (current_face_id == FACE_BACK)   face_id = CORNER_TOP_RIGHT_BACK; \n"
            "        else if (current_face_id == FACE_RIGHT)  face_id = CORNER_TOP_RIGHT_FRONT; \n"
            "        else if (current_face_id == FACE_LEFT)   face_id = CORNER_TOP_LEFT_BACK; \n"
            "        else if (current_face_id == FACE_TOP)    face_id = CORNER_TOP_LEFT_BACK; \n"
            "        else if (current_face_id == FACE_BOTTOM) face_id = CORNER_BOTTOM_LEFT_FRONT; \n"
            ...
            "           ((current_face_id == FACE_BOTTOM) && (face_hover_id == CORNER_BOTTOM_LEFT_FRONT))) { \n"
            "               float dist = sqrt((tex_coord.x * tex_coord.x) + (tex_coord.y * tex_coord.y)); \n"
            "               corner_alpha = smoothalpha(dist , dc); \n"
            "        } \n"
            "    } else if ((tex_coord.x < dc) && (tex_coord.y > (1.0 - dc))) {   // ----- BOTTOM LEFT ----- \n"
            "        if (current_face_id == FACE_FRONT)       face_id = CORNER_BOTTOM_LEFT_FRONT; \n"
            "        else if (current_face_id == FACE_BACK)   face_id = CORNER_BOTTOM_RIGHT_BACK; \n"
            "        else if (current_face_id == FACE_RIGHT)  face_id = CORNER_BOTTOM_RIGHT_FRONT; \n"
            "        else if (current_face_id == FACE_LEFT)   face_id = CORNER_BOTTOM_LEFT_BACK; \n"
            "        else if (current_face_id == FACE_TOP)    face_id = CORNER_TOP_LEFT_FRONT; \n"
            "        else if (current_face_id == FACE_BOTTOM) face_id = CORNER_BOTTOM_LEFT_BACK; \n"
            ...
            "           ((current_face_id == FACE_BOTTOM) && (face_hover_id == CORNER_BOTTOM_LEFT_BACK))) { \n"
            "               float dist = sqrt((tex_coord.x * tex_coord.x) + ((1.0 - tex_coord.y) * (1.0 - tex_coord.y))); \n"
            "               corner_alpha = smoothalpha(dist , dc); \n"
            "        } \n"
            "    } else if ((tex_coord.x > (1.0 - dc)) && (tex_coord.y < dc)) {   // ----- TOP RIGHT -----\n"
            "        if (current_face_id == FACE_FRONT)       face_id = CORNER_TOP_RIGHT_FRONT; \n"
            "        else if (current_face_id == FACE_BACK)   face_id = CORNER_TOP_LEFT_BACK; \n"
            "        else if (current_face_id == FACE_RIGHT)  face_id = CORNER_TOP_RIGHT_BACK; \n"
            "        else if (current_face_id == FACE_LEFT)   face_id = CORNER_TOP_LEFT_FRONT; \n"
            "        else if (current_face_id == FACE_TOP)    face_id = CORNER_TOP_RIGHT_BACK; \n"
            "        else if (current_face_id == FACE_BOTTOM) face_id = CORNER_BOTTOM_RIGHT_FRONT; \n"
            ...
            "           ((current_face_id == FACE_BOTTOM) && (face_hover_id == CORNER_BOTTOM_RIGHT_FRONT))) { \n"
            "               float dist = sqrt(((1.0 - tex_coord.x) * (1.0 - tex_coord.x)) + (tex_coord.y * tex_coord.y)); \n"
            "               corner_alpha = smoothalpha(dist , dc); \n"
            "        } \n"
            "    // ----- Edges ----- \n"
            "    } else if (tex_coord.x > (1.0 - de)) {                           // ----- RIGHT ----- \n"
            "        if (current_face_id == FACE_FRONT)       face_id = EDGE_FRONT_RIGHT; \n"
            "        else if (current_face_id == FACE_BACK)   face_id = EDGE_BACK_LEFT; \n"
            "        else if (current_face_id == FACE_RIGHT)  face_id = EDGE_BACK_RIGHT; \n"
            "        else if (current_face_id == FACE_LEFT)   face_id = EDGE_FRONT_LEFT; \n"
            "        else if (current_face_id == FACE_TOP)    face_id = EDGE_TOP_RIGHT; \n"
            "        else if (current_face_id == FACE_BOTTOM) face_id = EDGE_BOTTOM_RIGHT; \n"
            ...
            "           ((current_face_id == FACE_BOTTOM) && (face_hover_id == EDGE_BOTTOM_RIGHT))) { \n"
            "               edge_alpha = smoothalpha((1.0 - tex_coord.x) , de); \n"
            "        } \n"
            "    } else if (tex_coord.x < de) {                                   // ----- LEFT ----- \n"
            "        if (current_face_id == FACE_FRONT)       face_id = EDGE_FRONT_LEFT; \n"
            "        else if (current_face_id == FACE_BACK)   face_id = EDGE_BACK_RIGHT; \n"
            "        else if (current_face_id == FACE_RIGHT)  face_id = EDGE_FRONT_RIGHT; \n"
            "        else if (current_face_id == FACE_LEFT)   face_id = EDGE_BACK_LEFT; \n"
            "        else if (current_face_id == FACE_TOP)    face_id = EDGE_TOP_LEFT; \n"
            "        else if (current_face_id == FACE_BOTTOM) face_id = EDGE_BOTTOM_LEFT; \n"
            ...
            "           ((current_face_id == FACE_BOTTOM) && (face_hover_id == EDGE_BOTTOM_LEFT))) { \n"
            "               edge_alpha = smoothalpha(tex_coord.x , de); \n"
            "        } \n"
            "    } else if (tex_coord.y > (1.0 - de)) {                           // ----- BOTTOM ----- \n"
            "        if (current_face_id == FACE_FRONT)       face_id = EDGE_BOTTOM_FRONT; \n"
            "        else if (current_face_id == FACE_BACK)   face_id = EDGE_BOTTOM_BACK; \n"
            "        else if (current_face_id == FACE_RIGHT)  face_id = EDGE_BOTTOM_RIGHT; \n"
            "        else if (current_face_id == FACE_LEFT)   face_id = EDGE_BOTTOM_LEFT; \n"
            "        else if (current_face_id == FACE_TOP)    face_id = EDGE_TOP_FRONT; \n"
            "        else if (current_face_id == FACE_BOTTOM) face_id = EDGE_BOTTOM_BACK; \n"
            ...
            "           ((current_face_id == FACE_BOTTOM) && (face_hover_id == EDGE_BOTTOM_BACK))) { \n"
            "               edge_alpha = smoothalpha((1.0 - tex_coord.y) , de); \n"
            "        } \n"
            "    } else if (tex_coord.y < de) {                                   // ----- TOP ----- \n"
            "        if (current_face_id == FACE_FRONT)       face_id = EDGE_TOP_FRONT; \n"
            "        else if (current_face_id == FACE_BACK)   face_id = EDGE_TOP_BACK; \n"
            "        else if (current_face_id == FACE_RIGHT)  face_id = EDGE_TOP_RIGHT; \n"
            "        else if (current_face_id == FACE_LEFT)   face_id = EDGE_TOP_LEFT; \n"
            "        else if (current_face_id == FACE_TOP)    face_id = EDGE_TOP_BACK; \n"
            "        else if (current_face_id == FACE_BOTTOM) face_id = EDGE_BOTTOM_FRONT; \n"
            ...
            "           ((current_face_id == FACE_BOTTOM) && (face_hover_id == EDGE_BOTTOM_FRONT))) { \n"
            "               edge_alpha = smoothalpha(tex_coord.y , de); \n"
            "        } \n"
            "    } \n"
            "    \n"
            "    if (corner_alpha > 0.0) outFragColor = mix(outFragColor, vec4(1.0, 1.0, 1.0, 1.0), corner_alpha); \n"
            "    if (edge_alpha > 0.0)   outFragColor = mix(outFragColor, vec4(1.0, 1.0, 1.0, 1.0), edge_alpha); \n"
            "    // Ecoded ID \n"
            "    int encoded_id = int((picking_id             << BIT_OFFSET_ID)   | \n"
            "                         (face_id                << BIT_OFFSET_FACE) | \n"
