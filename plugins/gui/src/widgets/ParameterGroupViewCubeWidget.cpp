...
bool megamol::gui::PickableCube::Draw(unsigned int picking_id, int& inout_selected_face_id, int& inout_selected_orientation_id, int& out_hovered_face_id,
    ...
    if (this->shader == nullptr) {
        ...
        std::string vertex_src =
            ...
            "    \n"
            "    // Vertex Color \n"
            "    vertex_color = colors[face_index] * 0.3; \n"
            "    if (selected_face_id == face_id) { \n"
            "        vertex_color = colors[face_index] * (0.5 + (0.5 - 0.5 * (float(orientation_index) / 3.0))); \n"
            "    } \n"
            "    if ((hovered_face_id == face_id) && (hovered_orientation_id == orientation_id)) { \n"
            "        vertex_color = colors[face_index] + vec3(0.5, 0.5, 0.5); \n"
            "        vertex_color = clamp(vertex_color, vec3(0.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0)); \n"
            "        vertex_color *= (0.5 + (0.5 - 0.5 * (float(orientation_index) / 3.0))); \n"
            "    } \n"
            "    \n"
            "    // Up Arrow Texture \n"
        ...
        std::string fragment_src =
            ...
            "    hovered_alpha  = 0.0; \n"
            "    \n"
            "    if (id == FACE_FRONT)       new_id = FACE_FRONT_ID; \n"
            "    else if (id == FACE_BACK)   new_id = FACE_BACK_ID; \n"
            "    else if (id == FACE_RIGHT)  new_id = FACE_RIGHT_ID; \n"
            "    else if (id == FACE_LEFT)   new_id = FACE_LEFT_ID; \n"
            "    else if (id == FACE_TOP)    new_id = FACE_TOP_ID; \n"
            "    else if (id == FACE_BOTTOM) new_id = FACE_BOTTOM_ID; \n"
            "    \n"
            "    if (((id == FACE_FRONT)   && (selected_id == FACE_FRONT_ID))  || \n"
            "        ((id == FACE_BACK)    && (selected_id == FACE_BACK_ID))   || \n"
            "        ((id == FACE_RIGHT)   && (selected_id == FACE_RIGHT_ID))  || \n"
            "        ((id == FACE_LEFT)    && (selected_id == FACE_LEFT_ID))   || \n"
            "        ((id == FACE_TOP)     && (selected_id == FACE_TOP_ID))    || \n"
            "        ((id == FACE_BOTTOM)  && (selected_id == FACE_BOTTOM_ID))) { \n"
            "        float dist = sqrt((d_x * d_x) + (d_y * d_y)); \n"
            "        selected_alpha = smoothalpha(dist , d_ec); \n"
            "    } else if (((id == FACE_FRONT)   && (hovered_id == FACE_FRONT_ID))  || \n"
            "               ((id == FACE_BACK)    && (hovered_id == FACE_BACK_ID))   || \n"
            "               ((id == FACE_RIGHT)   && (hovered_id == FACE_RIGHT_ID))  || \n"
            "               ((id == FACE_LEFT)    && (hovered_id == FACE_LEFT_ID))   || \n"
            "               ((id == FACE_TOP)     && (hovered_id == FACE_TOP_ID))    || \n"
            "               ((id == FACE_BOTTOM)  && (hovered_id == FACE_BOTTOM_ID))) { \n"
            "        float dist = sqrt((d_x * d_x) + (d_y * d_y)); \n"
            ...
            "    float hovered_corner_alpha  = 0.0; \n"
            "    float hovered_edge_alpha    = 0.0; \n"
            "    float selected_corner_alpha = 0.0; \n"
            "    float selected_edge_alpha   = 0.0; \n"
            "    // ----- Corners ----- \n"
            ...
            "                                selected_corner_alpha, hovered_corner_alpha); \n"
            ...
            "    } \n"
            "    \n"
            "    if (hovered_corner_alpha > 0.0) outFragColor = mix(outFragColor, vec4(1.0, 1.0, 1.0, 1.0), hovered_corner_alpha); \n"
            "    if (hovered_edge_alpha > 0.0)   outFragColor = mix(outFragColor, vec4(1.0, 1.0, 1.0, 1.0), hovered_edge_alpha); \n"
            "    // Ecoded ID \n"
            "    int encoded_id = int((picking_id     << BIT_OFFSET_ID)   | \n"
            "                         (final_face_id  << BIT_OFFSET_FACE) | \n"
            "                         (orientation_id << BIT_OFFSET_ORIENTATION)); \n"
