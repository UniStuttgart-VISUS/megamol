...
Configurator::Configurator() : hotkeys(), utils(), state() {
...
bool megamol::gui::Configurator::Draw(
    ...
    int id = 0;
    const auto enum_modules_func = [&, this](core::Module* mod) {
        std::string module_name(mod->ClassName());
        if (this->utils.FindCaseInsensitiveSubstring(module_name, search_string)) {
            ImGui::PushID(id);
            if (ImGui::Selectable(module_name.c_str(), id == this->state.module_list_selected_id)) {
                this->state.module_list_selected_id = id;
            }
            ImGui::PopID();
        }
        id++;
    };
    core_instance->EnumModulesNoLock(nullptr, enum_modules_func);
    ...
    // ImGuiWindowFlags_NoMove); ImGui::PushItemWidth(120.0f);
    ...
    //    draw_list->AddBezierCurve(p1, p1 + ImVec2(+50, 0), p2 + ImVec2(-50, 0), p2, IM_COL32(200, 200, 100,
    //    255), 3.0f);
    //}
