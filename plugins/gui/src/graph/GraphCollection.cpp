...
bool megamol::gui::GraphCollection::LoadModuleStock(const megamol::core::CoreInstance* core_instance) {
    ...
    if (this->calls_stock.empty()) {
        megamol::core::utility::log::Log::DefaultLog.WriteWarn(
            "Load call stock before getting modules for having calls in place for "
...
ImGuiID megamol::gui::GraphCollection::LoadUpdateProjectFromCore(
    ImGuiID in_graph_uid, megamol::core::CoreInstance* core_instance) {
    ...
    if (in_graph_uid != GUI_INVALID_ID) return in_graph_uid;

    GraphPtr_t graph_ptr;
    ImGuiID current_graph_id = in_graph_uid;
    ...
    if (this->AddProjectFromCore(current_graph_id, core_instance, false)) {
        ...
        this->DeleteGraph(current_graph_id);
    }
...
ImGuiID megamol::gui::GraphCollection::LoadUpdateProjectFromCore(
    ImGuiID in_graph_uid, megamol::core::MegaMolGraph* core_graph) {
    ...
    if (this->AddProjectFromCore(current_graph_id, core_graph, false)) {
        return graph_ptr->uid;
    } else {
        this->DeleteGraph(current_graph_id);
    }
...
bool megamol::gui::GraphCollection::AddProjectFromCore(
    ImGuiID in_graph_uid, megamol::core::CoreInstance* core_instance, bool use_stock) {
    ...
    try {
        ...
        const auto module_func = [&, this](megamol::core::Module* mod) {
            ...
            if (!this->project_separate_name_and_namespace(full_name, module_namespace, module_name)) {
                megamol::core::utility::log::Log::DefaultLog.WriteError(
                    "Core Project: Invalid module name '%s'. [%s, %s, line %d]\n", full_name.c_str(), __FILE__,
    ...
    try {
        ...
        for (unsigned int i = 0; i < lines_count; i++) {
            ...
            if (lines[i].rfind(luacmd_view, 0) == 0) {
                ...
                if (!this->read_project_command_arguments(lines[i], arg_count, args)) {
                    megamol::core::utility::log::Log::DefaultLog.WriteError(
                        "Project File '%s' line %i: Error parsing lua command '%s' "
                ...
                if (!this->project_separate_name_and_namespace(view_full_name, view_namespace, view_name)) {
                    megamol::core::utility::log::Log::DefaultLog.WriteError(
                        "Project File '%s' line %i: Invalid view name argument "
        ...
        if (!found_main_view) {
            megamol::core::utility::log::Log::DefaultLog.WriteError(
                ...
                luacmd_view.c_str(), __FILE__, __FUNCTION__, __LINE__);
        }
        ...
        for (unsigned int i = 0; i < lines_count; i++) {
            ...
            if (lines[i].rfind(luacmd_module, 0) == 0) {
                ...
                if (!this->project_separate_name_and_namespace(module_full_name, module_namespace, module_name)) {
                    megamol::core::utility::log::Log::DefaultLog.WriteError(
                        "Project File '%s' line %i: Invalid module name argument "
        ...
        for (unsigned int i = 0; i < lines_count; i++) {
            ...
            if (lines[i].rfind(luacmd_call, 0) == 0) {
                ...
                if (!this->project_separate_name_and_namespace(
                    ...
                    megamol::core::utility::log::Log::DefaultLog.WriteError(
                        "Project File '%s' line %i: Invalid caller slot name "
                ...
                if (!this->project_separate_name_and_namespace(
                    ...
                    megamol::core::utility::log::Log::DefaultLog.WriteError(
                        "Project File '%s' line %i: Invalid callee slot name "
        ...
        for (unsigned int i = 0; i < lines_count; i++) {
            ...
            if (lines[i].rfind(luacmd_param, 0) == 0) {
                ...
                if ((param_slot_full_name.front() != '"') || (param_slot_full_name.back() != '"')) {
                    megamol::core::utility::log::Log::DefaultLog.WriteError(
                        "Project File '%s' line %i: Parameter name argument should "
                ...
                if (value_start_idx == std::string::npos) {
                    megamol::core::utility::log::Log::DefaultLog.WriteError(
                        "Project File '%s' line %i: Unable to find parameter value "
        ...
        megamol::core::utility::log::Log::DefaultLog.WriteInfo(
            "[Configurator] Successfully loaded project '%s' from file '%s'.\n", graph_ptr->name.c_str(),
            project_filename.c_str());
    ...
    try {
        ...
        if (found_graph_ptr != nullptr) {
            ...
            if (FileUtils::WriteFile(project_filename, projectstr)) {
                ...
                megamol::core::utility::log::Log::DefaultLog.WriteInfo(
                    "[Configurator] Successfully saved project '%s' to file '%s'.\n", found_graph_ptr->name.c_str(),
                    project_filename.c_str());
                return true;
            }
        } else {
...
bool megamol::gui::GraphCollection::get_module_stock_data(
    ...
    try {
        ...
        // megamol::core::utility::log::Log::DefaultLog.WriteInfo("[Configurator] Creating module '%s' ...",
        // mod_desc->ClassName());
...
ImVec2 megamol::gui::GraphCollection::project_read_confpos(const std::string& line) {
    ...
    if ((x_start_idx != std::string::npos) && (y_start_idx != std::string::npos) && (end_idx != std::string::npos)) {
        ...
        try {
            ...
            megamol::core::utility::log::Log::DefaultLog.WriteError(
                " Error while reading x value of confPos: %s [%s, %s, line %d]\n", e.what(), __FILE__, __FUNCTION__,
                __LINE__);
            return conf_pos;
        }
        try {
            ...
            megamol::core::utility::log::Log::DefaultLog.WriteError(
                " Error while reading y value of confPos: %s [%s, %s, line %d]\n", e.what(), __FILE__, __FUNCTION__,
                __LINE__);
            return conf_pos;
        }
        conf_pos = ImVec2(x, y);
