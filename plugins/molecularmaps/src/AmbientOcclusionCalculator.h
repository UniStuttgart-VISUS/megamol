...
namespace molecularmaps {
	...
	public:
		...
		struct AOSettings {
			 ...
			 * Constructor
			 */
			AOSettings(void) : angleFactor(1.0f), evalFactor(1.0f), falloffFactor(1.0f), genFac(1.0f), maxDist(0.5f), minDist(0.5f), 
				numSampleDirections(8), scaling(1.0f), volSizeX(1), volSizeY(1), volSizeZ(1) {
			}
			...
			bool isDirty(const AOSettings& aos) {
				if (std::abs(this->angleFactor - aos.angleFactor) > FLT_EPSILON) return true;
				if (std::abs(this->evalFactor - aos.evalFactor) > FLT_EPSILON) return true;
				if (std::abs(this->falloffFactor - aos.falloffFactor) > FLT_EPSILON) return true;
				if (std::abs(this->genFac - aos.genFac) > FLT_EPSILON) return true;
				if (std::abs(this->maxDist - aos.maxDist) > FLT_EPSILON) return true;
				if (std::abs(this->minDist - aos.minDist) > FLT_EPSILON) return true;
				if (std::abs(this->scaling - aos.scaling) > FLT_EPSILON) return true;
				if (this->numSampleDirections != aos.numSampleDirections) return true;
				if (this->volSizeX != aos.volSizeX) return true;
				if (this->volSizeY != aos.volSizeY) return true;
				if (this->volSizeZ != aos.volSizeZ) return true;
				return false;
			}
		};
		 ...
		 * @return Pointer to the vector containing the resulting brightness values per vertex three times in a row. If the method was not successful, it returns a nullptr.
		 */
		 ...
		 */
		bool initilialize(core::CoreInstance * instance, const std::vector<float> * vertices, const std::vector<float> * vertex_normals, 
