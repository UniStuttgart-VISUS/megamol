...
struct CudaRay {
	 ...
	 * Initialises an empty instance.
	 */
	__host__ __device__ CudaRay(void) : dir(make_float3(0.0f, 0.0f, 0.0f)), 
...
struct CudaBoundingBox {
	 ...
	 * Initialises an empty instance.
	 */
	__host__ __device__ CudaBoundingBox(void) : max(make_float3(0.0f)), 
	 ...
	 * Initialises an empty instance.
	 */
	__host__ __device__  CudaBoundingBox(const float3& p_max, const float3& p_min) : 
			max(p_max), min(p_min) { 
	...
	__host__ __device__ CudaBoundingBox(const float3* p_vertices, const uint p_vertices_cnt) {
		...
		for (uint i = 0; i < p_vertices_cnt; i++) {
			...
			if (p_vertices[i].x < min.x) min.x = p_vertices[i].x;
			if (p_vertices[i].y < min.y) min.y = p_vertices[i].y;
			if (p_vertices[i].z < min.z) min.z = p_vertices[i].z;

			// Update the right, back, top point.
			if (p_vertices[i].x > max.x) max.x = p_vertices[i].x;
			if (p_vertices[i].y > max.y) max.y = p_vertices[i].y;
			if (p_vertices[i].z > max.z) max.z = p_vertices[i].z;
		}
	}
...
struct CudaOctreeNode {
	 ...
	 * Initialises an empty instance.
	 */
	__host__ __device__ CudaOctreeNode(const CudaBoundingBox& p_bounding_box,
			const uint p_child_cnt, const uint p_face_cnt) : bounding_box(p_bounding_box) {
...
struct Cut {
	 ...
	 * Initialises an empty instance.
	 */
	Cut(const std::vector<float>& p_colours, const std::vector<uint>& p_faces,
		const std::vector<float>& p_normals, const uint p_tunnel_id,
		const std::vector<float>& p_vertices) : tunnel_id(p_tunnel_id) {
...
struct Ray {
	 ...
	 * Initialises an empty instance.
	 */
	Ray(void) : dir(vec3f(0.0f, 0.0f, 0.0f)), origin(vec3f(0.0f, 0.0f, 0.0f)),
		inv_dir(vec3f(0.0f, 0.0f, 0.0f))
	{ }
...
struct BoundingBox {
	...
	bool SphereIntersection(const vec4d& p_sphere) const {
		...
		double distsquared = (x - p_sphere.X()) * (x - p_sphere.X()) + (y - p_sphere.Y()) * (y - p_sphere.Y()) + (z - p_sphere.Z()) * (z - p_sphere.Z());
 ...
 */
	 ...
	 */
	EndVertexParams(
		const std::pair<vec4d, std::array<uint, 5>>& p_gate,
		const std::array<vec4d, 2>& p_gate_center,
		std::array<vec4d, 4>& p_gate_vector, 
		const vec3d& p_pivot) :
		gate(p_gate), 
...
struct OctreeNode {
	 ...
	 * Initialises an empty instance.
	 */
	OctreeNode(const BoundingBox& p_bounding_box, const uint p_cuda_idx,
			const std::vector<uint>& p_faces) : bounding_box(p_bounding_box),
			cuda_idx(p_cuda_idx) {
		this->children = std::vector<OctreeNode>(0);
		this->children.reserve(8);
		this->children.shrink_to_fit();
	 ...
	 */
	OctreeNode(const BoundingBox& p_bounding_box, const uint p_cuda_idx, 
			const size_t p_face_cnt) : bounding_box(p_bounding_box),
			cuda_idx(p_cuda_idx) {
		this->children = std::vector<OctreeNode>(0);
		this->children.reserve(8);
		this->children.shrink_to_fit();
...
struct VoronoiEdge {
	 ...
	 * Initialises an empty instance.
	 */
	VoronoiEdge(void) : end_vertex(0), gate_sphere(vec4d()),
		start_vertex(0)
	{ }
	 ...
	 * Initialises an empty instance.
	 */
	VoronoiEdge(const uint p_end_vertex, 
		const vec4d& p_gate_sphere, const uint p_start_vertex) :
		end_vertex(p_end_vertex), gate_sphere(p_gate_sphere),
...
struct VoronoiVertex {
	...
	static inline uint64_t ComputeHash(const vec4ui& p_atom_ids) {
		uint64_t r = 11;
		uint64_t hash =
			static_cast<uint64_t>(r + 2 * p_atom_ids.GetX()) *
			static_cast<uint64_t>(r + 2 * p_atom_ids.GetY()) *
			static_cast<uint64_t>(r + 2 * p_atom_ids.GetZ()) *
			static_cast<uint64_t>(r + 2 * p_atom_ids.GetW());
		return hash / 2;
	}
	 ...
	 * Initialises an empty instance.
	 */
	VoronoiVertex(const vec4ui& p_atoms, const uint p_id, const uint p_infinity_count = 0, const vec4d& p_vertex = vec4d()) {
