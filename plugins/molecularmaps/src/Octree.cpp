 ...
 */
size_t Octree::ConvertToCUDAOctree(std::vector<CudaOctreeNode>& p_octree_nodes,
		std::vector<std::vector<uint>>& p_node_faces) {
	...
	while (queue_idx != 0) {
		...
		p_octree_nodes[curr->cuda_idx] = CudaOctreeNode(
			CudaBoundingBox(curr->bounding_box.max, curr->bounding_box.min),
 ...
 * Octree::createFaceBoundingBoxes
 */
void Octree::createFaceBoundingBoxes(const std::vector<uint>& p_faces, const 
	...
	for (size_t i = 0; i < size; i++) {
		...
		vertices[0] = make_float3(p_vertices[face_idxs.x * 3], p_vertices[face_idxs.x * 3 + 1],
			p_vertices[face_idxs.x * 3 + 2]);
		vertices[1] = make_float3(p_vertices[face_idxs.y * 3], p_vertices[face_idxs.y * 3 + 1],
			p_vertices[face_idxs.y * 3 + 2]);
		vertices[2] = make_float3(p_vertices[face_idxs.z * 3], p_vertices[face_idxs.z * 3 + 1],
			p_vertices[face_idxs.z * 3 + 2]);
 ...
 */
void Octree::CreateOctreeRootNode(const std::vector<uint>& p_faces, const float3& p_max,
		const float3& p_min, const float3& p_min_dim, const std::vector<float>& p_vertices) {
 ...
 */
int Octree::IntersectOctree(const std::vector<uint>& p_faces, const Ray& p_ray,
 ...
 */
int Octree::IntersectOctree(const std::vector<uint>& p_faces, const Ray &p_ray, 
		const std::vector<float>& p_vertices, uint& p_face_id) {
	// Initialise the queue for the intersection tests.
	vec3ui face_idxs;
	std::vector<const OctreeNode*> queue = std::vector<const OctreeNode*>(this->node_cnt);
	std::vector<vec3f> vertices;
...
bool Octree::sphereTriangleIntersection(const vec4d& p_sphere, const std::vector<vec3f>& p_vertices) {
	...
	for(const auto& vert : p_vertices) {
		...
		if (squaredDist <= radiusSquared) return true;
	}
	return false;
}