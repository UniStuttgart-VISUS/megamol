 ...
 */
double Computations::AngularDistance(const vec3d& p_start_pivot, 
		const vec3d& p_pivot, const vec4d& p_vertToTest) {
 ...
 */
size_t Computations::ComputeGateCenter(const std::array<vec4d, 4>& p_gate_vector, 
		std::array<vec4d, 2>& p_gate_center, std::array<vec4d, 2>& p_bary_solutions, 
		std::array<vec3d, 2>& p_circles) {
 ...
 */
size_t Computations::ComputeVoronoiCircle(const std::array<vec3d, 3>& p_circles, 
		std::array<vec3d, 2>& p_result_circles) {
	...
	// Compute the a, b and c values for each of the aX� + bx + c terms. For each dependent variable we have one of these terms.
	...
	// Set the correct order for the computation of the polynom X� + Y� - R� = 0. The independent variable has the value of 1 the
	// the others are a and d.
	double m_a;
 ...
 */
uint Computations::ComputeVoronoiSphere(const std::array<vec4d, 4>& p_spheres, 
		std::array<vec4d, 2>& p_result_spheres) {
	// Look for the smallest radius.
	...
	// Compute the a, b and c values for each of the aX� + bx + c terms. For each dependent variable we have one of these terms.
	double a = m_1_sq * r_1_sq + m_4_sq * r_2_sq + m_7_sq * r_3_sq + 2.0 * (m_1_m_4 * v_1_v_2 + m_1_m_7 * v_1_v_3 + m_4_m_7 * v_2_v_3);
	double b = (-m_1_sq * v_1_b_0 - m_1_m_4 * v_1_b_1 - m_1_m_4 * v_2_b_0 - m_1_m_7 * v_1_b_2 - m_1_m_7 * v_3_b_0 - m_4_sq * v_2_b_1 - m_4_m_7 *
		v_2_b_2 - m_4_m_7 * v_3_b_1 - m_7_sq * v_3_b_2) * 2.0;
	double c = m_1_sq * b_0_sq + m_4_sq * b_1_sq + m_7_sq * b_2_sq + 2.0 * (m_1_m_7 * b_0_b_2 + m_4_m_7 * b_2_b_1 + m_1_m_4 * b_0_b_1);

	double d = m_2_sq * r_1_sq + m_5_sq * r_2_sq + m_8_sq * r_3_sq + 2.0 * (m_2_m_5 * v_1_v_2 + m_2_m_8 * v_1_v_3 + m_5_m_8 * v_2_v_3);
	double e = (-m_2_sq * v_1_b_0 - m_2_m_5 * v_1_b_1 - m_2_m_5 * v_2_b_0 - m_2_m_8 * v_1_b_2 - m_2_m_8 * v_3_b_0 - m_5_sq * v_2_b_1 - m_5_m_8 *
		v_2_b_2 - m_5_m_8 * v_3_b_1 - m_8_sq * v_3_b_2) * 2.0;
	double f = m_2_sq * b_0_sq + m_5_sq * b_1_sq + m_8_sq * b_2_sq + 2.0 * (m_2_m_8 * b_0_b_2 + m_5_m_8 * b_2_b_1 + m_2_m_5 * b_0_b_1);

	double g = m_3_sq * r_1_sq + m_6_sq * r_2_sq + m_9_sq * r_3_sq + 2.0 * (m_3_m_6 * v_1_v_2 + m_3_m_9 * v_1_v_3 + m_6_m_9 * v_2_v_3);
	double h = (-m_3_sq * v_1_b_0 - m_3_m_6 * v_1_b_1 - m_3_m_6 * v_2_b_0 - m_3_m_9 * v_1_b_2 - m_3_m_9 * v_3_b_0 - m_6_sq * v_2_b_1 - m_6_m_9 *
		v_2_b_2 - m_6_m_9 * v_3_b_1 - m_9_sq * v_3_b_2) * 2.0;
	double i = m_3_sq * b_0_sq + m_6_sq * b_1_sq + m_9_sq * b_2_sq + 2.0 * (m_3_m_9 * b_0_b_2 + m_6_m_9 * b_2_b_1 + m_3_m_6 * b_0_b_1);

	// Set the correct order for the computation of the polynom X� + Y� + Z� - R� = 0. The independent variable has the value of 1 the
	// the others are a, d and g.
	double m_a;
	switch (free_variable) {
	case 0:
		m_a = a + d + g - 1.0;
 ...
 */
uint Computations::ComputeVoronoiSphereR(const std::array<vec4d, 4>& p_spheres, 
		std::array<vec4d, 2>& p_result_spheres) {
	// Look for the smallest radius.
	double radius = p_spheres[0].GetW();
	size_t sphere_id = 0;
 ...
 */
vislib::math::Plane<double> Computations::CreatePlane(const vec4d& p_first_sphere,
		const vec4d& p_second_sphere, const vec4d& p_third_sphere) {
	vislib::math::Plane<double> retval(
		vislib::math::Point<double, 3>(p_first_sphere.PeekComponents()),
 ...
 */
size_t Computations::incircleBaryCoords(const std::array<vec4d, 4>& p_spheres, 
		std::array<vec4d, 2>& p_solutions, std::array<vec3d, 2>& p_circles) {
 ...
 */
bool Computations::LiesInsideConvexHull(const std::vector<vec3f>& p_atoms,
		const vec4d point_to_test, std::vector<vec3f>& p_directions) {
	// Intialise the convex hull test.
	vec3f testPoint;
	testPoint.Set(static_cast<float>(point_to_test.GetX()),
		static_cast<float>(point_to_test.GetY()),
