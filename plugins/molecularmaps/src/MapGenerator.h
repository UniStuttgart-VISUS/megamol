...
namespace molecularmaps {
	...
	class MapGenerator : public core::view::Renderer3DModule {
	public:
	...
	private:
		 ...
		 */
		vec3f computeGeodesicPoint(const float p_a, const float p_b, const float p_c,
			const float p_lat_0, const float p_lon_0, const float p_lat_1,
			const float p_lon_1);
		 ...
		 * @return Always true.
		 */
		bool createBoundingSphere(const float p_offset, float& p_radius, 
		 ...
		 */
		Cut createCut(const bool p_second_rebuild, const uint p_tunnel_id, 
			uint& p_vertex_id, const std::vector<uint>& p_vertex_ids,
		 ...
		 */
		bool findBoundaryMeridian(const Poles& p_poles, std::vector<int>& p_types,
		 ...
		 */
		bool findPoles(const vislib::math::Vector<float, 3>& p_eye_dir,
			const vislib::math::Vector<float, 3>& p_up_dir,
			const vislib::math::Vector<float, 3>& p_center,
			Poles& p_poles);
		 ...
		 */
		void identifyBorderEdges(const std::vector<bool>& p_face_shadowed, 
		 ...
		 */
		void identifyBorderEdges(const std::vector<uint>& p_face_group,
			std::vector<FaceGroup>& p_groups);
		 ...
		 * @return Always true.
		 */
		bool initialiseZvalues(const Poles& p_poles, std::vector<float>& p_zvalues,
		 ...
		 * otherwise
		 */
		bool invertEdge(const uint p_start_id, const uint p_end_id,
			const std::vector<bool>& p_tunnels);
		 ...
		 */
		uint processAOOutput(const std::vector<float>* p_ao_vals, std::vector<Cut>& p_cuts, 
		 ...
		 * Octree of the faces
		 */
		void processTopologyOutput(std::vector<Cut>& p_cuts, uint& p_tunnel_id, 
			std::vector<bool>& p_tunnels, 
			const std::vector<VoronoiVertex>& p_voronoi_vertices, 
			const std::vector<VoronoiEdge>& p_voronoi_edges,
		 ...
		 */
		void rebuildSurface(const std::vector<Cut>& p_cuts, const bool p_second_rebuild,
			const std::vector<bool>& p_tunnels);
		 ...
		 */
		void renderLatLonLines(const uint p_num_lat, const uint p_num_lon, const uint p_tess_lat,
		 ...
		 * @return Always true.
		 */
		bool setDvalues(const size_t p_vertex_cnt, float& p_theta,
			const Poles& p_poles);
		 ...
		 */
		std::vector<std::string> splitString(const std::string& p_string,
			const char p_delim, std::vector<std::string>& p_elements);
