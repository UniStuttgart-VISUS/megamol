...
MapGenerator::MapGenerator(void)
    ...
    , render_equator_length(
          "renderEquatorLength", "Enables or disables the rendering of the information text about the equator length")
...
bool MapGenerator::capColouring(
    ...
    for (size_t i = 0; i < re_vertex_cnt; i++) {
        ...
        vec3f pos = vec3f(
            this->vertices_rebuild[i * 3 + 0], this->vertices_rebuild[i * 3 + 1], this->vertices_rebuild[i * 3 + 2]);
        bool found = false;

        // Loop over the mesh with the cap and get the vertex positions.
        for (size_t j = 0; j < cap_vertex_cnt; j++) {
            vec3f cap_pos = vec3f(cap_vertices[j * 3 + 0], cap_vertices[j * 3 + 1], cap_vertices[j * 3 + 2]);
        ...
        if (!found) {
            this->vertexColors_rebuild[i * 3 + 0] =
                0.75f * this->vertexColors_rebuild[i * 3 + 0] + 0.25f * cap_colour.GetX();
            this->vertexColors_rebuild[i * 3 + 1] =
                0.75f * this->vertexColors_rebuild[i * 3 + 1] + 0.25f * cap_colour.GetY();
            this->vertexColors_rebuild[i * 3 + 2] =
                0.75f * this->vertexColors_rebuild[i * 3 + 2] + 0.25f * cap_colour.GetZ();
        }
    }
...
bool MapGenerator::colourBindingSite(protein_calls::BindingSiteCall* p_bs, const vec3f& p_colour,
    ...
    if (p_bs->GetBindingSiteCount() >= 3) {
        ...
        for (uint i = 0; i < res_cnt; i++) {
            ...
            if (serin_res_idx != res_cnt && histidin_res_idx != res_cnt && first_oxyanion_res_idx != res_cnt &&
        ...
        if (serin_res_idx != res_cnt) {
            ...
            if (serin_res->Identifier() == MolecularDataCall::Residue::AMINOACID) {
                ...
                serin_pos.Set(p_mdc->AtomPositions()[serin_idx * 3 + 0], p_mdc->AtomPositions()[serin_idx * 3 + 1],
        ...
        if (histidin_res_idx != res_cnt) {
            ...
            if (histidin_res->Identifier() == MolecularDataCall::Residue::AMINOACID) {
                ...
                histidin_pos.Set(p_mdc->AtomPositions()[histidin_idx * 3 + 0],
                    p_mdc->AtomPositions()[histidin_idx * 3 + 1], p_mdc->AtomPositions()[histidin_idx * 3 + 2]);
        ...
        if (first_oxyanion_res_idx != res_cnt) {
            ...
            if (first_oxyanion_res->Identifier() == MolecularDataCall::Residue::AMINOACID) {
                uint first_oxyanion_idx =
                    dynamic_cast<const MolecularDataCall::AminoAcid*>(first_oxyanion_res)->CAlphaIndex();
        ...
        if (second_oxyanion_res_idx != res_cnt) {
            ...
            if (second_oxyanion_res->Identifier() == MolecularDataCall::Residue::AMINOACID) {
                uint second_oxyanion_idx =
                    dynamic_cast<const MolecularDataCall::AminoAcid*>(second_oxyanion_res)->CAlphaIndex();
        ...
        vec4d sphere = vec4d(static_cast<double>(center.GetX()), static_cast<double>(center.GetY()),
            static_cast<double>(center.GetZ()), static_cast<double>(p_radius + p_radiusOffset));
 ...
 */
vec3f MapGenerator::computeGeodesicPoint(const float p_a, const float p_b, const float p_c, const float p_lat_0,
    ...
    retval.SetZ(
        this->sphere_data.GetZ() + (p_c + this->sphere_data.GetW()) * (p_a * sin(p_lat_0) + p_b * sin(p_lat_1)));
...
void MapGenerator::convertToLatLon(const vec3f& p_point, float& p_lat, float& p_lon) {
    p_lat = asinf(p_point.GetZ() / sqrtf(p_point.GetX() * p_point.GetX() + p_point.GetY() * p_point.GetY() +
 ...
 */
bool MapGenerator::createBoundingSphere(
    ...
    for (uint i = 0; i < 2; i++) {
        for (uint j = 0; j < vertex_cnt; j++) {
            idx = j * 3;
            pos.Set(p_vector[idx], p_vector[idx + 1], p_vector[idx + 2]);
            diff = pos - p_center;
            len = diff.Length();
            if (len > p_radius) {
                alpha = len / p_radius;
                ...
                p_center = 0.5f * ((1.0f + 1.0f / alphaSq) * p_center + (1.0f - 1.0f / alphaSq) * pos);
            }
        }
    }
 ...
 */
Cut MapGenerator::createCut(const bool p_second_rebuild, const uint p_tunnel_id, uint& p_vertex_id,
...
void MapGenerator::createGeodesicLines(const GeodesicMode p_mode) {
    ...
    std::vector<size_t> tunnel_offset =
        std::vector<size_t>(this->vertices_added_tunnel_id.back() + 2, this->vertices_added.size());
...
bool MapGenerator::createSphere(const vec3f& p_eye_dir, const vec3f& p_up_dir) {
    ...
    look_at_point_sphere.SetX(
        center.GetX() + radius * sin(theta_values[this->look_at_id]) * cos(phi_values[this->look_at_id]));
    look_at_point_sphere.SetY(center.GetY() + radius * cos(theta_values[this->look_at_id]));
    look_at_point_sphere.SetZ(
        center.GetZ() + radius * sin(theta_values[this->look_at_id]) * sin(phi_values[this->look_at_id]));
    ...
    look_at_dir_pre_rot.Set(this->vertices_rebuild[this->look_at_id * 3],
        this->vertices_rebuild[this->look_at_id * 3 + 1], this->vertices_rebuild[this->look_at_id * 3 + 2]);
...
void MapGenerator::depthFirstSearch(const size_t p_cur, const std::vector<VoronoiEdge>& p_edges,
    ...
    for (size_t i = begin; i < end; i++) {
        if (!p_visited[p_edges[i].end_vertex]) {
            this->depthFirstSearch(
                p_edges[i].end_vertex, p_edges, p_reversed_edges, p_start_offset, p_end_offset, p_visited, p_group);
        }
    }
    ...
    for (size_t i = begin; i < end; i++) {
        if (!p_visited[p_reversed_edges[i].start_vertex]) {
            this->depthFirstSearch(p_reversed_edges[i].start_vertex, p_edges, p_reversed_edges, p_start_offset,
                p_end_offset, p_visited, p_group);
        }
    }
}
...
bool MapGenerator::GetMeshExtents(Call& call) {
    ...
    if (this->store_new_mesh) {
        ...
        if (selected == MeshMode::MESH_ORIGINAL) {
            ...
            themesh.SetVertexData(static_cast<uint>(this->vertices_rebuild.size() / 3), this->vertices_rebuild.data(),
                this->normals_rebuild.data(), this->vertexColors_rebuild.data(), nullptr, false);
            themesh.SetTriangleData(
                static_cast<uint>(this->faces_rebuild.size() / 3), this->faces_rebuild.data(), false);
            this->meshBoundingBox = this->computeBoundingBox(this->vertices_rebuild);
        } else if (selected == MeshMode::MESH_SPHERE) {
            themesh.SetVertexData(static_cast<uint>(this->vertices_sphere.size() / 3), this->vertices_sphere.data(),
                this->vertices_sphere.data(), this->vertexColors_rebuild.data(), nullptr, false);
            themesh.SetTriangleData(
                static_cast<uint>(this->faces_rebuild.size() / 3), this->faces_rebuild.data(), false);
            ...
            themesh.SetVertexData(static_cast<uint>(this->vertices_map.size() / 3), this->vertices_map.data(),
                this->vertices_map.data(), this->vertexColors_rebuild.data(), nullptr, false);
...
bool MapGenerator::findBoundaryMeridian(const Poles& p_poles, std::vector<int>& p_types,
    std::vector<bool>& p_valid_vertices, const std::vector<float>& p_zvalues, const vec3f& p_eye_dir) {
    // Initialise poles.
    ...
    vec3f origin = vec3f(
        this->vertices_rebuild[cur * 3 + 0], this->vertices_rebuild[cur * 3 + 1], this->vertices_rebuild[cur * 3 + 2]);
    ...
    for (const auto& edge : this->vertex_edge_offset[cur]) {
        ...
        vec3f target = vec3f(this->vertices_rebuild[nxt * 3 + 0], this->vertices_rebuild[nxt * 3 + 1],
...
bool MapGenerator::findCircles(std::vector<FaceGroup>& p_groups) {
    for (auto& group : p_groups) {
        ...
        while (!allElementsTrue(marked_edges)) {
            ...
            auto id_0_begin = std::lower_bound(sorted_edges_id0.begin(), sorted_edges_id0.end(), cur_vertex_id,
                [](const Edge& lhs, const uint rhs) { return lhs.vertex_id_0 < rhs; });
            auto id_0_end = std::upper_bound(sorted_edges_id0.begin(), sorted_edges_id0.end(), cur_vertex_id,
            ...
            auto id_1_begin = std::lower_bound(sorted_edges_id1.begin(), sorted_edges_id1.end(), cur_vertex_id,
                [](const Edge& lhs, const uint rhs) { return lhs.vertex_id_1 < rhs; });
            auto id_1_end = std::upper_bound(sorted_edges_id1.begin(), sorted_edges_id1.end(), cur_vertex_id,
...
bool MapGenerator::findPoles(const vislib::math::Vector<float, 3>& p_eye_dir,
    const vislib::math::Vector<float, 3>& p_up_dir, const vislib::math::Vector<float, 3>& p_center, Poles& p_poles) {
    ...
    while (!finished) {
        ...
        if (p_poles.north == p_poles.south)
            step += 1.5f;
        else
            finished = true;
    }
 ...
 * MapGenerator::identifyBorderEdges
 */
void MapGenerator::identifyBorderEdges(const std::vector<uint>& p_face_group, std::vector<FaceGroup>& p_groups) {
...
bool MapGenerator::initialiseMapShader(bool shaderReload) {
    ...
    if (!shaderReload) {
        ...
        for (size_t i = 0; i < this->geodesic_lines_vbos.size(); i++) {
            ...
            glBufferData(GL_ARRAY_BUFFER,
                sizeof(float) * static_cast<unsigned long long>(this->geodesic_lines[i].size()),
    ...
    if (!this->map_shader_init || shaderReload) {
        ...
        try {
            ...
            core::utility::log::Log::DefaultLog.WriteMsg(core::utility::log::Log::LEVEL_ERROR, "Unable to %s map shader (@%s): %s\n",
                buildState,
        ...
        if (!instance()->ShaderSourceFactory().MakeShaderSource("geolinesShader::geometry", geom)) {
            core::utility::log::Log::DefaultLog.WriteMsg(
                core::utility::log::Log::LEVEL_ERROR, "Unable to load geometry shader source for geodesic lines shader");
            return false;
        }
        ...
        try {
            ...
            core::utility::log::Log::DefaultLog.WriteMsg(
                core::utility::log::Log::LEVEL_ERROR, "Unable to %s geodesic lines shader: %s\n", buildState, e.GetMsgA());
            return false;
        } catch (...) {
            core::utility::log::Log::DefaultLog.WriteMsg(
                core::utility::log::Log::LEVEL_ERROR, "Unable to %s geodesic lines shader: Unknown exception\n", buildState);
            return false;
        }
    }
 ...
 */
bool MapGenerator::initialiseZvalues(
 ...
 */
bool MapGenerator::invertEdge(const uint p_start_id, const uint p_end_id, const std::vector<bool>& p_tunnels) {
...
void MapGenerator::processTopologyOutput(std::vector<Cut>& p_cuts, uint& p_tunnel_id, std::vector<bool>& p_tunnels,
    ...
    core::utility::log::Log::DefaultLog.WriteMsg(core::utility::log::Log::LEVEL_INFO, "Creating the Voronoi Cut Octree...");
    ...
    this->cuda_kernels->ComputeVoronoiAO(cuda_octree_nodes, cuda_node_faces, this->faces_rebuild, ray_dirs,
        this->vertices_rebuild, p_voronoi_vertices, static_cast<uint>(node_cnt), voronoi_tunnel, voro_faces,
        potential_vertices);
#else
    // Compute all rays on a very coarse sampled sphere.
...
#    pragma omp parallel for
    ...
    std::sort(tmp_voro_edges_reverse.begin(), tmp_voro_edges_reverse.end(),
        [](const VoronoiEdge& a, const VoronoiEdge& b) { return a.end_vertex < b.end_vertex; });
    ...
    std::vector<size_t> end_vertex_offset =
        std::vector<size_t>(p_voronoi_vertices.size() + 1, tmp_voro_edges_reverse.size());
...
#pragma omp parallel for
    ...
    for (size_t group_id = 0; group_id < voronoi_groups.size(); group_id++) {
        ...
        for (auto face_it = voronoi_groups[group_id].face_map.begin();
             face_it != voronoi_groups[group_id].face_map.end(); face_it++) {
            ...
            if (!marked_faces[face_it->second]) {
                ...
                auto retval = this->findEnclosures(face_it->second, static_cast<uint>(group_id), full_enclosure,
                    face_group_ids, voronoi_groups.size(), marked_faces, 10);
    ...
    for (auto face_it = voronoi_groups[0].face_map.begin(); face_it != voronoi_groups[0].face_map.end(); face_it++) {
        ...
        if (!marked_faces[face_it->second]) {
            // Get the group of faces and check what other group surrounds it.
            std::map<uint, uint> full_enclosure;
            auto retval = this->findEnclosures(
                face_it->second, 0, full_enclosure, face_group_ids, voronoi_groups.size(), marked_faces, 50);
 ...
 */
void MapGenerator::rebuildSurface(
    ...
    if (this->lastDataHash != ctmd->DataHash() || this->computeButton.IsDirty() || shaderReloaded ||
        ...
        if (ctmd->Count() > 0 && ctmd->Objects()[0].GetVertexCount() > 0) {
            ...
            if (!fillLocalMesh(ctmd->Objects()[0])) {
                core::utility::log::Log::DefaultLog.WriteMsg(core::utility::log::Log::LEVEL_ERROR, "The mesh data is malformed!");
                return false;
            }

            // Create mesh topology information.
            core::utility::log::Log::DefaultLog.WriteMsg(core::utility::log::Log::LEVEL_INFO, "Creating the topology information...");
            ...
            if (isGenusN(
                ...
                core::utility::log::Log::DefaultLog.WriteMsg(core::utility::log::Log::LEVEL_INFO, "The mesh has genus %d.", genus);
                ...
                if (isGenusN(vertex_cnt, face_cnt, genus)) {
                    ...
                    // Rebuild the local copy with the new cuts and the tunnels removed.
                    core::utility::log::Log::DefaultLog.WriteMsg(core::utility::log::Log::LEVEL_INFO, "Rebuilding the surface...");
                ...
                core::utility::log::Log::DefaultLog.WriteMsg(core::utility::log::Log::LEVEL_INFO, "The mesh has genus %d.", 0);
                this->faces_rebuild = this->faces;
                this->normals_rebuild = this->normals;
                this->vertexColors_rebuild = this->vertexColors;
                this->vertices_rebuild = this->vertices;
            ...
            core::utility::log::Log::DefaultLog.WriteMsg(core::utility::log::Log::LEVEL_INFO, "Initialising the map shader...");
            if (!initialiseMapShader()) return false;
            this->computed_map = true;

        } else {
            core::utility::log::Log::DefaultLog.WriteMsg(core::utility::log::Log::LEVEL_WARN, "The mesh input data is empty!");
            return false;
        }
...
#define HARDCODED_SIZE
    ...
    if (this->display_param.Param<param::EnumParam>()->Value() == DisplayMode::PROTEIN) {
        ...
        this->triMeshRenderer.update(&this->faces, &this->vertices, &this->vertexColors_voronoi, &this->normals);
        ...
        this->triMeshRenderer.update(
            &this->faces_rebuild, &this->vertices_rebuild, &this->vertexColors_rebuild, &this->normals_rebuild);

    } else if (this->display_param.Param<param::EnumParam>()->Value() == DisplayMode::SPHERE) {
        if (this->computed_sphere) {
            this->triMeshRenderer.update(
                &this->faces_rebuild, &this->vertices_sphere, &this->vertexColors_rebuild, &this->normals_rebuild);
 ...
 */
void MapGenerator::renderLatLonLines(
    ...
    if (!glIsBuffer(this->lat_lon_lines_vbo)) {
        ...
        this->lat_lon_lines_colours.reserve(
            ((p_num_lat + 1) * 2 * (p_tess_lat + 1) + (p_num_lon + 1) * 2 * (p_tess_lon + 1)) * 3);
...
void MapGenerator::renderTunnels() {
    ...
    glDrawElements(
        GL_TRIANGLES, static_cast<unsigned int>(this->tunnel_faces.size()), GL_UNSIGNED_INT, this->tunnel_faces.data());
...
bool MapGenerator::setDvalues(const size_t p_vertex_cnt, float& p_theta, const Poles& p_poles) {
    ...
    for (const auto& edge : this->vertex_edge_offset[p_poles.north]) {
        if (edge.vertex_id_0 == p_poles.north) {
            ...
            for (const auto& n_edge : this->vertex_edge_offset[edge.vertex_id_1]) {
                if (n_edge.vertex_id_0 == edge.vertex_id_1 && n_edge.vertex_id_1 != p_poles.north &&
                    d_values[n_edge.vertex_id_1] != 1) {
                    to_process.push_back(n_edge.vertex_id_1);
                    d_values[n_edge.vertex_id_1] = 2;
                }

                if (n_edge.vertex_id_1 == edge.vertex_id_1 && n_edge.vertex_id_0 != p_poles.north &&
            ...
            // Intialise the neighbours of the neighbours of the north pole.
            for (const auto& n_edge : this->vertex_edge_offset[edge.vertex_id_0]) {
                if (n_edge.vertex_id_0 == edge.vertex_id_0 && n_edge.vertex_id_1 != p_poles.north &&
                    d_values[n_edge.vertex_id_1] != 1) {
                    to_process.push_back(n_edge.vertex_id_1);
                    d_values[n_edge.vertex_id_1] = 2;
                }

                if (n_edge.vertex_id_1 == edge.vertex_id_0 && n_edge.vertex_id_0 != p_poles.north &&
                    d_values[n_edge.vertex_id_0] != 1) {
                    to_process.push_back(n_edge.vertex_id_0);
                    d_values[n_edge.vertex_id_0] = 2;
                }
 ...
 * MapGenerator::splitString
 */
std::vector<std::string> MapGenerator::splitString(
