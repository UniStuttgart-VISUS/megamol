 ...
 */
__device__ bool RayTriangleIntersection(const CudaRay p_ray, const float3 p_vertex_0,
		const float3 p_vertex_1, const float3 p_vertex_2) {
	...
	if (denom > 1e-5) {
		...
		if (v < 0.0f || (u + v) > denom) return false;
 ...
 * found
 */
__global__ void ComputeOctreeIntersection(const CudaOctreeNode* p_octree_nodes,
		const uint* p_node_faces, const uint* p_node_faces_offset, 
		const uint3* p_faces, const float3* p_rays, const float3* p_vertices, 
		const float3* p_origins, uint* p_queues, int* p_intersections, 
		float* p_ao_values, uint p_max_nodes, uint p_round, uint p_max_rounds, 
		uint p_vertex_cnt) {
...
__global__ void GetNeighbourIds(uint* p_neighbour_ids, bool* p_valid_z_values,
		const Edge* p_vertex_edge_offset, const uint* p_vertex_edge_offset_depth,
		uint p_vertex_cnt) {
	const uint idx = GetThreadIndex();
	if (idx >= p_vertex_cnt) return;
 ...
 */
__global__ void GroupFacesKernel(uint* p_face_group_ids, const bool* p_face_shadowed,
		const Edge* p_face_edge_offset, const uint* p_face_edge_offset_depth,
		const uint p_face_cnt) {
	const uint idx = GetThreadIndex();
 ...
 */
__global__ void MatchEdges(Edge* p_edges, Edge* p_sorted_edges,
 ...
 * @param p_face_cnt The number of faces in the mesh.
 */
__global__ void SetFaceEdgeOffset(Edge* p_face_edge_offset, Edge* p_face_id_0_offset, 
		Edge* p_face_id_1_offset, uint depth, uint p_face_cnt) {
	const uint idx = GetThreadIndex();
	if (idx >= p_face_cnt) return;
 ...
 * @param p_vertex_cnt The number of vertices in the mesh.
 */
__global__ void SetVertexEdgeOffset(Edge* p_vertex_edge_offset, Edge* p_vertex_id_0_sorted,
		Edge* p_vertex_id_1_sorted, uint* p_vertex_id_0_offset, uint* p_vertex_id_1_offset,
		uint* depth, uint p_vertex_cnt) {
	const uint idx = GetThreadIndex();
	if (idx >= p_vertex_cnt) return;
 ...
 * @param p_vertex_cnt The number of vertices in the mesh.
 */
__global__ void SetTheataValues(float* p_theta_values, bool* p_valid_theta_values,
		const float* p_zvalues, const float p_theta, const float p_theta_const, 
		uint p_vertex_cnt) {
	const uint idx = GetThreadIndex();
	if (idx >= p_vertex_cnt) return;
	if (!p_valid_theta_values[idx]) return;
	...
	while (dtheta > (C_PI / 200000.0f)) {
		...
		if (p_zvalues[idx] > tval) tlow = theta;
		else tup = theta;
		dtheta = tup - tlow;
	}
...
__global__ void SortNeighbourIds(uint* p_neighbour_ids, uint* p_neighbour_ids_offset,
		...
		const int* p_vertex_type, uint p_vertex_cnt) {
	const uint idx = GetThreadIndex();
	if (idx >= p_vertex_cnt) return;
	if (!p_valid_z_values[idx]) return;
...
bool CUDAKernels::ComputeVoronoiAO(const std::vector<CudaOctreeNode>& p_cuda_octree_nodes, 
		const std::vector<std::vector<uint>>& p_cuda_node_faces,
		const std::vector<uint>& p_faces, 
	...
	for (size_t i = 0; i < p_voronoi_vertices.size(); i++) {
		cuda_voronoi_vertices[i] = make_float3(p_voronoi_vertices[i].vertex.GetX(),
			p_voronoi_vertices[i].vertex.GetY(),
	...
	for (uint i = 0; i < max_rounds; i++) {
		ComputeOctreeIntersection <<< Grid(vertex_cnt, 256), 256 >>> (
			...
			thrust::raw_pointer_cast(cuda_faces_d.data().get()),
			thrust::raw_pointer_cast(cuda_rays_d.data().get()),
			...
			thrust::raw_pointer_cast(ao_vals_d.data().get()),
			queue_size, i, max_rounds, vertex_cnt);
...
bool CUDAKernels::CreateMeshTopology(const std::vector<uint>& p_faces,
	...
	// Create edges.
	InitEdges <<< Grid(face_cnt, 256), 256 >>> (thrust::raw_pointer_cast(faces_d.data().get()), 
		thrust::raw_pointer_cast(edges_d.data().get()), face_cnt);
	checkCudaErrors(cudaDeviceSynchronize());
	...
	thrust::sort(sorted_edges_face_id_0_d.begin(), sorted_edges_face_id_0_d.end(),
		FaceID0Cmp());

	// Sort the edges ascending to face_id_1.
	thrust::device_vector<Edge> sorted_edges_face_id_1_d = edges;
	thrust::sort(sorted_edges_face_id_1_d.begin(), sorted_edges_face_id_1_d.end(),
		FaceID1Cmp());

	// Set the face edge offset.
	SetFaceEdgeOffset <<< Grid(face_cnt, 256), 256 >>> (
		thrust::raw_pointer_cast(cuda_face_offset_d.data().get()),
		thrust::raw_pointer_cast(sorted_edges_face_id_0_d.data().get()),
		thrust::raw_pointer_cast(sorted_edges_face_id_1_d.data().get()),
		face_offset_depth, face_cnt);
	checkCudaErrors(cudaDeviceSynchronize());
	...
	resizeOffsetFaces(cuda_face_offset, p_face_edge_offset, p_face_edge_offset_depth,
		cuda_face_offset_depth);
	...
	thrust::sort(sorted_edges_vertex_id_0_d.begin(), sorted_edges_vertex_id_0_d.end(),
		VertexID0Cmp());
	thrust::copy(sorted_edges_vertex_id_0_d.begin(), sorted_edges_vertex_id_0_d.end(),
		sorted_edges_vertex_id_0.begin());
	...
	thrust::sort(sorted_edges_vertex_id_1_d.begin(), sorted_edges_vertex_id_1_d.end(),
		VertexID1Cmp());
	thrust::copy(sorted_edges_vertex_id_1_d.begin(), sorted_edges_vertex_id_1_d.end(),
		sorted_edges_vertex_id_1.begin());
	...
	SetVertexEdgeOffset <<< Grid(face_cnt, 256), 256 >>> (
		...
		thrust::raw_pointer_cast(cuda_vertex_offset_depth_d.data().get()),
		vertex_cnt);
	checkCudaErrors(cudaDeviceSynchronize());
	...
	resizeOffsetVertices(cuda_vertex_offset, p_vertex_edge_offset, p_vertex_edge_offset_depth,
		cuda_vertex_offset_depth, Edge());
...
bool CUDAKernels::CreateThetaValues(const float p_theta, std::vector<float>& p_theta_values, 
	...
	SetTheataValues <<< Grid(vertex_cnt, 256), 256 >>> (
		thrust::raw_pointer_cast(p_theta_values_d.data().get()),
		thrust::raw_pointer_cast(p_valid_theta_vertices_d.data().get()),
		thrust::raw_pointer_cast(p_zvalues_d.data().get()),
		p_theta, tconst, vertex_cnt);
	checkCudaErrors(cudaDeviceSynchronize());
...
std::vector<FaceGroup> CUDAKernels::GroupFaces(const std::vector<bool>& p_face_shadowed,
		const std::vector<std::vector<Edge>>& p_face_edge_offset,
		const std::vector<uint>& p_face_edge_offset_depth,
		const vislib::Array<vec3f>& p_group_colour_table,
		const std::vector<uint>& p_faces,
	...
	while (changes) {
		...
		// Run the kernel.
		GroupFacesKernel <<< Grid(face_cnt, 256), 256 >>> (
			thrust::raw_pointer_cast(face_group_ids_d.data().get()),
			thrust::raw_pointer_cast(face_shadowed_d.data().get()),
			thrust::raw_pointer_cast(cuda_face_offset_d.data().get()),
			thrust::raw_pointer_cast(face_edge_offset_depth_d.data().get()),
			face_cnt);
		checkCudaErrors(cudaDeviceSynchronize());
 ...
 */
void CUDAKernels::resizeOffsetVertices(std::vector<Edge>& p_offset_in,
 ...
 */
void CUDAKernels::resizeOffsetFaces(std::vector<Edge>& p_offset_in, 
		std::vector<std::vector<Edge>>& p_offset_out, 
