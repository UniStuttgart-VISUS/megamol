...
namespace molecularmaps {
	...
	public:
		 ...
		 */
		static size_t ComputeVoronoiCircle(const std::array<vec3d, 3>& p_circles, 
		 ...
		 * @return the number of solutions found
		 */
		static uint ComputeVoronoiSphere(const std::array<vec4d, 4>& p_spheres, 
		 ...
		 * @return the number of solutions found
		 */
		static uint ComputeVoronoiSphereR(const std::array<vec4d, 4>& p_spheres,
			std::array<vec4d, 2>& p_result_spheres);
		 ...
		 */
		static vislib::math::Plane<double> CreatePlane(const vec4d& p_first_sphere, 
		 ...
		 */
		static bool LiesInsideConvexHull(const std::vector<vec3f>& p_atoms, 
		...
		static inline void NormaliseVector(vec3d& p_vec) {
			...
			__m128 sse_vector = _mm_setr_ps(
				static_cast<float>(p_vec.GetX()), 
				static_cast<float>(p_vec.GetY()), 
				static_cast<float>(p_vec.GetZ()), 
				0.0f);
	...
	private:
		 ...
		 */
		static size_t incircleBaryCoords(const std::array<vec4d, 4>& p_spheres, 
