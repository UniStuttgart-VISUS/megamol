...
namespace molecularmaps {
	...
	public:
		 ...
		 */
		size_t ConvertToCUDAOctree(std::vector<CudaOctreeNode>& p_octree_nodes,
		 ...
		 */
		void CreateOctreeRootNode(const std::vector<uint>& p_faces, const float3& p_max, 
		 ...
		 * found
		 */
		int IntersectOctree(const std::vector<uint>& p_faces, const Ray& p_ray,
	...
	private:
		 ...
		 * @param p_vertices the vertices of the surface
		 */
		void createFaceBoundingBoxes(const std::vector<uint>& p_faces, 
			const std::vector<float>& p_vertices);
