...
void VoronoiChannelCalculator::checkVertexAndGateValidity(MolecularDataCall* mdc) {
	...
	for (auto it = this->voronoi_vertices.begin(); it != this->voronoi_vertices.end(); it++) {
		...
		this->vertexValidFlags[i] = ((this->vertices[i].GetW() >= this->probeRadius) && !(it->second.infinity_count > 2));
	...
	//vislib::sys::Log::DefaultLog.WriteMsg(vislib::sys::Log::LEVEL_INFO, "Filtered %d voronoi vertices, because their radius was lower than %f", filtered_radius_vertices, this->probeRadius);
	//vislib::sys::Log::DefaultLog.WriteMsg(vislib::sys::Log::LEVEL_INFO, "Filtered %d voronoi infinity vertices", filtered_inf_vertices);
	...
	// If the last thread has less than items_per_thread vertices to process, we need more than core_num threads in total.
	...
	for (size_t i = 0; i < this->voronoi_threads.size(); i++) {
		...
		this->voronoi_threads[i] = std::thread([&valid_vertices, this, &atomData, begin_index, end_index] {
			...
			for (size_t a = begin_index; a < end_index; a++) {
				...
				this->vertexValidFlags[idx] = Computations::LiesInsideConvexHull(atomData,
					valid_vertices[a].second, directions);
			}
		});
	}
	...
	for (size_t i = 0; i < this->voronoi_threads.size(); i++) {
		...
		this->voronoi_threads[i] = std::thread([&valid_gates, this, &atomData, begin_index, end_index] {
			...
			for (size_t a = begin_index; a < end_index; a++) {
				...
				this->gateValidFlags[idx] = Computations::LiesInsideConvexHull(atomData,
					valid_gates[a].second, directions);
			}
		});
	}
	this->stopThreads();
...
bool VoronoiChannelCalculator::constructVoronoiDiagram(MolecularDataCall* mdc) {
	...
	for (uint i = 0; i < mdc->AtomCount(); i++) {
		atomData[i].Set(ptr[i * 3 + 0], ptr[i * 3 + 1], ptr[i * 3 + 2], 
			mdc->AtomTypes()[mdc->AtomTypeIndices()[i]].Radius());
	}
	...
	while (stop != 2) {
		...
		stop = 0;
		for (size_t i = 0; i < this->voronoi_threads.size(); i++) {
			this->voronoi_threads[i] = std::thread(std::bind(
				&VoronoiChannelCalculator::nextVoronoiVertexInit, this));
		}
	...
	while (stop != 2) {
		...
		for (size_t i = 0; i < this->voronoi_threads.size(); i++) {
			this->voronoi_threads[i] = std::thread(std::bind(
				&VoronoiChannelCalculator::nextVoronoiVertex, this));
		}
	...
	std::thread cleanup([&]() {
		this->searchGrid.ClearSearchGrid();
	});
	cleanup.detach();
...
void VoronoiChannelCalculator::nextVoronoiVertex() {
	...
	while (!stop) {
		...
		std::array<vec4d, 4> gateVector{
			this->searchGrid.GetAtoms()[gate.second[0]],
			this->searchGrid.GetAtoms()[gate.second[1]],
			this->searchGrid.GetAtoms()[gate.second[2]],
			vec4d()
		};
		...
		if (minIdx >= 0) {
			...
			this->voronoi_mutex.lock();
			VoronoiVertex possible_vertex = VoronoiVertex(vec4ui(gate.second[0], gate.second[1], gate.second[2], minIdx), 0);
			...
			if (it == this->voronoi_vertices.end()) {
				...
				this->voronoi_vertices.insert(std::pair<uint64_t, VoronoiVertex>(possible_vertex.vertex_hash, possible_vertex));
			...
			this->voronoi_mutex.lock();
			auto vertex_hash = VoronoiVertex::ComputeHash(vec4ui(gate.second[0], gate.second[1], gate.second[2], gate.second[3]));
...
void VoronoiChannelCalculator::nextVoronoiVertexInit() {
	...
	while (!stop && !this->initVertexFound) {
		...
		if (minIdx >= 0) {
			...
			if (gate.second[0] < thresh && gate.second[1] < thresh && gate.second[2] < thresh && static_cast<uint>(minIdx) < thresh) {
				...
				std::pair<vec4d, std::array<uint, 5>> new1(edgeEndResult, { static_cast<uint>(minIdx), gate.second[0], gate.second[1], gate.second[2], 0 });
				std::pair<vec4d, std::array<uint, 5>> new2(edgeEndResult, { static_cast<uint>(minIdx), gate.second[1], gate.second[2], gate.second[0], 0 });
				std::pair<vec4d, std::array<uint, 5>> new3(edgeEndResult, { static_cast<uint>(minIdx), gate.second[0], gate.second[2], gate.second[1], 0 });
				if (this->active_gates == 0) {
					this->gatesToTest_two.push_back(new3);
					this->gatesToTest_two.push_back(new2);
					this->gatesToTest_two.push_back(new1);
...
bool VoronoiChannelCalculator::Render(view::CallRender3D& call) {
	...
	for (auto e : this->voronoi_edges) {	
		if (this->vertexValidFlags[e.start_vertex] && this->vertexValidFlags[e.end_vertex] && gateValidFlags[i]) { // the whole edge is valid
			glVertex3f(this->vertices[e.start_vertex].GetX(), this->vertices[e.start_vertex].GetY(), this->vertices[e.start_vertex].GetZ());
			glVertex3f(static_cast<float>(e.gate_sphere.GetX()), static_cast<float>(e.gate_sphere.GetY()), static_cast<float>(e.gate_sphere.GetZ()));
			glVertex3f(static_cast<float>(e.gate_sphere.GetX()), static_cast<float>(e.gate_sphere.GetY()), static_cast<float>(e.gate_sphere.GetZ()));
			glVertex3f(this->vertices[e.end_vertex].GetX(), this->vertices[e.end_vertex].GetY(), this->vertices[e.end_vertex].GetZ());
 ...
 */
bool VoronoiChannelCalculator::Update(MolecularDataCall* mdc, 
		std::vector<VoronoiVertex>& p_voronoi_vertices,
