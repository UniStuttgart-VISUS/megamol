 ...
 */
AmbientOcclusionCalculator::AmbientOcclusionCalculator(void) : aoSampleMax(0), colourSSBOHandle(0), dirTexture(0), lvlTexture(0), mdc(nullptr), normalSSBOHandle(0), 
	resultVector(std::vector<float>(0)), shaderChanged(false), vertex_normals(nullptr), vertexSSBOHandle(0), vertices(nullptr), 
	volTexture(0), volumeInitialized(false) {
}
...
void AmbientOcclusionCalculator::createEmptyVolume(AmbientOcclusionCalculator::AOSettings settings) {
	...
	glActiveTexture(GL_TEXTURE7);
	glBindTexture(GL_TEXTURE_3D, this->volTexture);
	glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, settings.volSizeX, settings.volSizeY, settings.volSizeZ, GL_RED, GL_FLOAT, vol.data());
...
void AmbientOcclusionCalculator::createVolumeCPU(AmbientOcclusionCalculator::AOSettings settings) {
	...
	float voxelVol = (rangeOSx / static_cast<float>(sx)) * (rangeOSy / static_cast<float>(sy)) * (rangeOSz / static_cast<float>(sz));
...
#pragma omp parallel for
	for (j = 0; j < static_cast<int>(mdc->AtomCount()); j++) {
		...
		if (x < 0) x = 0; else if (x >= static_cast<int>(sx)) x = sx - 1;

		int y = static_cast<int>(((ppos[1] - minOSy) / rangeOSy) * static_cast<float>(sy));
		if (y < 0) y = 0; else if (y >= static_cast<int>(sy)) y = sy - 1;

		int z = static_cast<int>(((ppos[2] - minOSz) / rangeOSz) * static_cast<float>(sz));
		if (z < 0) z = 0; else if (z >= static_cast<int>(sz)) z = sz - 1;
...
void AmbientOcclusionCalculator::readColourData(void) {
	...
	float *p = static_cast<float*>(glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, this->resultVector.size() * sizeof(float), GL_MAP_READ_BIT));
...
void AmbientOcclusionCalculator::resizeVolume(AmbientOcclusionCalculator::AOSettings settings) {
	...
	glActiveTexture(GL_TEXTURE7);
	glBindTexture(GL_TEXTURE_3D, this->volTexture);
	glTexImage3D(GL_TEXTURE_3D, 0, GL_R32F, settings.volSizeX, settings.volSizeY, settings.volSizeZ,
		0, GL_RED, GL_FLOAT, vol.data());
...
void AmbientOcclusionCalculator::uploadVertexData(void) {
	glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, this->vertexSSBOHandle);
	glBufferData(GL_SHADER_STORAGE_BUFFER, this->vertices->size() * sizeof(float), this->vertices->data(), GL_DYNAMIC_DRAW);
	glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, this->normalSSBOHandle);
	glBufferData(GL_SHADER_STORAGE_BUFFER, this->vertex_normals->size() * sizeof(float), this->vertex_normals->data(), GL_DYNAMIC_DRAW);
