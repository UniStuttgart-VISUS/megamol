    ...
    public:
        ...
        inline bool GetSafeVoxel(int x, int y, int z) const {
            return ((x < 0) || (y < 0) || (z < 0)) ? false
                : this->GetSafeVoxel(static_cast<unsigned int>(x), static_cast<unsigned int>(y), static_cast<unsigned int>(z));
        }
         ...
         * @param volume Pointer to the bool volume
         */
        inline void SetVolume(unsigned int sizeX, unsigned int sizeY, unsigned int sizeZ, float voxSizeX, float voxSizeY, float voxSizeZ, bool *volume) {
            this->sizeX = sizeX;
            this->sizeY = sizeY;
            this->sizeZ = sizeZ;
