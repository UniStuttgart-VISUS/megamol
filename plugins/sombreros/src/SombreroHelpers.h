#ifndef MMSOMBREROSPLUGIN_SOMBREROHELPERS_H_INCLUDED
#define MMSOMBREROSPLUGIN_SOMBREROHELPERS_H_INCLUDED
...
namespace sombreros {
     ...
     * 4. The backbone nitrogen of the second oxyanion hole residue or, in the case of non-gx-types, the first sidechain oxygen

     * This computation is only possible if the incoming BindingSiteCall has at least three binding sites of which the third consists of at least two amino acids.
    ...
    inline vislib::math::Vector<float, 3> EstimateBindingSitePosition(protein_calls::MolecularDataCall * mdc, protein_calls::BindingSiteCall * bsc) {
        ...
        for (uint cCnt = 0; cCnt < mdc->ChainCount(); cCnt++) {
            firstMol = mdc->Chains()[cCnt].FirstMoleculeIndex();
            for (uint mCnt = firstMol; mCnt < firstMol + mdc->Chains()[cCnt].MoleculeCount(); mCnt++) {
                firstRes = mdc->Molecules()[mCnt].FirstResidueIndex();
                for (uint rCnt = firstRes; rCnt < firstRes + mdc->Molecules()[mCnt].ResidueCount(); rCnt++) {
                    firstAtom = mdc->Residues()[firstRes + rCnt]->FirstAtomIndex();
                    auto bla = firstAtom;
                    for (uint bsidx = 0; bsidx < bscount; bsidx++) {
                        auto bsRes = bsc->GetBindingSite(bsidx);
                        for (uint bsridx = 0; bsridx < bsRes->Count(); bsridx++) {
                            ...
                            if (mdc->Chains()[cCnt].Name() == bsRes->operator[](bsridx).First() &&
                                ...
                                mdc->ResidueTypeNames()[mdc->Residues()[firstRes + rCnt]->Type()] == bsc->GetBindingSiteResNames(bsidx)->operator[](bsridx)) {
                                for (uint aCnt = firstAtom; aCnt < firstAtom + mdc->Residues()[firstRes + rCnt]->AtomCount(); aCnt++) {
                                    auto atomName = mdc->AtomTypes()[mdc->AtomTypeIndices()[aCnt]].Name();
                                    if (fallbackMode || // TODO further assumptions
                                        (atomName.Equals("OG") && bsidx == 0) ||
                                        (atomName.Equals("ND2") && bsidx == 1) ||
