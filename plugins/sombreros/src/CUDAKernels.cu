 ...
 */
__host__ __device__ bool operator<(const CUDAKernels::Edge& lhs, const CUDAKernels::Edge& rhs) {
 ...
 */
__global__ void InitEdges(const uint3* p_faces, CUDAKernels::Edge* p_edges, uint p_face_cnt) {
	const uint idx = GetThreadIndex();
	if (idx >= p_face_cnt) return;

	// First edge.
	{
		CUDAKernels::Edge cur = p_edges[idx * 3];
		cur.face_id_0 = idx;
		cur.face_id_1 = -1;
	...
	// Second edge.
	{
		CUDAKernels::Edge cur = p_edges[idx * 3 + 1];
		cur.face_id_0 = idx;
		cur.face_id_1 = -1;
	...
	// Third edge.
	{
		CUDAKernels::Edge cur = p_edges[idx * 3 + 2];
		cur.face_id_0 = idx;
		cur.face_id_1 = -1;
...
__global__ void MatchEdges(CUDAKernels::Edge* p_edges, CUDAKernels::Edge* p_sorted_edges,
	...
	CUDAKernels::Edge cur = p_edges[idx];
