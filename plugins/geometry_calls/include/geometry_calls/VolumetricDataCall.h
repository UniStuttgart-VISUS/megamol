 ...
 */
class VolumetricDataCall : public core::AbstractGetData3DCall {

public:
    /** Possible type of grids. */
    typedef enum GridType_t GridType;

    /** Possible types of scalars. */
    typedef enum ScalarType_t ScalarType;

    /** Structure containing all required metadata about a data set. */
    typedef struct VolumetricMetadata_t Metadata;
     ...
     */
    static bool GetMetadata(VolumetricDataCall& call);
    ...
    inline const void* GetData(void) const {
        if (metadata != nullptr && metadata->MemLoc == RAM) {
            ...
            megamol::core::utility::log::Log::DefaultLog.WriteError(
                "Trying to get volume data from VRAM: Volume data location set to RAM.");
            return nullptr;
        }
    }
    ...
    inline uint32_t GetVRAMData(void) const {
        if (metadata != nullptr && metadata->MemLoc == VRAM) {
            ...
            megamol::core::utility::log::Log::DefaultLog.WriteError(
                "Trying to get volume data from RAM: Volume data location set to VRAM.");
            return 0;
        }
    }
     ...
     * Gets the voxel value relative to [min, max] from channel c.
     */
    const float GetAbsoluteVoxelValue(const uint32_t x, const uint32_t y, const uint32_t z, const uint32_t c = 0) const;
...
/** Call Descriptor.  */
typedef core::factories::CallAutoDescription<VolumetricDataCall> VolumetricDataCallDescription;
