...
bool megamol::archvis::FEMMeshDataSource::getDataCallback(core::Call & caller)
{
	...
	if (fem_call->getUpdateFlag())
	{
		...
		for (auto& element : fem_data->getElements())
		{
			...
			case FEMDataStorage::CUBE:
				...
				indices.insert(indices.end(),{
					// front
					static_cast<uint32_t>(node_indices[0] - 1), static_cast<uint32_t>(node_indices[1] - 1), static_cast<uint32_t>(node_indices[2] - 1),
					static_cast<uint32_t>(node_indices[2] - 1), static_cast<uint32_t>(node_indices[3] - 1), static_cast<uint32_t>(node_indices[0] - 1),
					// right
					static_cast<uint32_t>(node_indices[1] - 1), static_cast<uint32_t>(node_indices[5] - 1), static_cast<uint32_t>(node_indices[6] - 1),
					static_cast<uint32_t>(node_indices[6] - 1), static_cast<uint32_t>(node_indices[2] - 1), static_cast<uint32_t>(node_indices[1] - 1),
					// back
					static_cast<uint32_t>(node_indices[7] - 1), static_cast<uint32_t>(node_indices[6] - 1), static_cast<uint32_t>(node_indices[5] - 1),
					static_cast<uint32_t>(node_indices[5] - 1), static_cast<uint32_t>(node_indices[4] - 1), static_cast<uint32_t>(node_indices[7] - 1),
					// left
					static_cast<uint32_t>(node_indices[4] - 1), static_cast<uint32_t>(node_indices[0] - 1), static_cast<uint32_t>(node_indices[3] - 1),
					static_cast<uint32_t>(node_indices[3] - 1), static_cast<uint32_t>(node_indices[7] - 1), static_cast<uint32_t>(node_indices[4] - 1),
					// bottom
					static_cast<uint32_t>(node_indices[4] - 1), static_cast<uint32_t>(node_indices[5] - 1), static_cast<uint32_t>(node_indices[1] - 1),
					static_cast<uint32_t>(node_indices[1] - 1), static_cast<uint32_t>(node_indices[0] - 1), static_cast<uint32_t>(node_indices[4] - 1),
					// top
					static_cast<uint32_t>(node_indices[3] - 1), static_cast<uint32_t>(node_indices[2] - 1), static_cast<uint32_t>(node_indices[6] - 1),
					static_cast<uint32_t>(node_indices[6] - 1), static_cast<uint32_t>(node_indices[7] - 1), static_cast<uint32_t>(node_indices[3] - 1)
				});
		...
		std::vector<std::pair< std::vector<float>::iterator, std::vector<float>::iterator>> vb_iterators = { {vbs[0].begin(),vbs[0].end()} };
