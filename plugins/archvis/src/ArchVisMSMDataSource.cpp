...
ArchVisMSMDataSource::ArchVisMSMDataSource()
    ...
    try {
        ...
        this->m_rcv_socket.Create(vislib::net::Socket::ProtocolFamily::FAMILY_INET,
            vislib::net::Socket::Type::TYPE_DGRAM, vislib::net::Socket::Protocol::PROTOCOL_UDP);
...
bool ArchVisMSMDataSource::getDataCallback(megamol::core::Call& caller) {
    ...
    if (this->m_partsList_slot.IsDirty() || this->m_nodeElement_table_slot.IsDirty() || this->m_nodes_slot.IsDirty() ||
    ...
    if (this->m_rcv_IPAddr_slot.IsDirty() || this->m_rcv_port_slot.IsDirty()) {
        ...
        try {
            ...
            megamol::core::utility::log::Log::DefaultLog.WriteError( ("Socket Exception during connection: %s", e.GetMsgA()));
            return false;
        }
    }
...
void ArchVisMSMDataSource::parseNodeElementTable(std::string const& filename, std::vector<Node>& nodes,
            ...
            if (std::strcmp("//", linestart.c_str()) != 0 && line.length() > 0) {
                if (lines_read < node_cnt) {
                    ...
                    if (std::stoi(type) == 2) {
                        ...
                        beam_elements.push_back(std::make_tuple<int, int>(std::stoi(idx0), std::stoi(idx1)));
                    } else if (std::stoi(type) == 1) {
                        std::string idx0, idx1;

                        ss >> idx0 >> idx1;

                        diagonal_elements.push_back(std::make_tuple<int, int>(std::stoi(idx0), std::stoi(idx1)));
                    }
                }
...
void ArchVisMSMDataSource::parseNodeList(std::string const& filename, std::vector<Vec3>& node_positions) {
    std::ifstream file;
    file.open(filename, std::ifstream::in);
...
void ArchVisMSMDataSource::parseElementList(
    std::string const& filename, std::vector<std::tuple<int, int, int, int, int>>& element_data) {
    std::ifstream file;
    file.open(filename, std::ifstream::in);
    ...
    if (file.is_open()) {
        ...
        while (!file.eof()) {
            ...
            element_data.push_back(std::make_tuple<int, int, int, int, int>(
                std::move(type), std::stoi(idx0) - 1, std::stoi(idx1) - 1, std::stoi(idx2) - 1, std::stoi(idx3) - 1));
...
void ArchVisMSMDataSource::parseInputElementList(std::string const& filename, std::vector<int>& input_elements) {
    std::ifstream file;
    file.open(filename, std::ifstream::in);
...
void ArchVisMSMDataSource::spawnAndUpdateTextLabels() {
    double elapsed_spawn_time =
        std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - m_last_spawn_time)
            .count();
    double elapsed_update_time =
        std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - m_last_update_time)
            .count();
    m_last_update_time = std::chrono::steady_clock::now();
    ...
    if (m_text_particles.size() > 0) {
        for (int i = 0; i < m_scale_model.getElementCount(); ++i) {
            if (m_scale_model.getElementType(i) != ScaleModel::DIAGONAL) {
                ...
                (*particle_itr).position =
                    m_scale_model.getElementCenter(i) + (m_scale_model.getElementCenter(i) * Vec3(0.25, 0.0, 0.25));
    ...
    if (elapsed_spawn_time > 33) {
        ...
        for (int i = 0; i < m_scale_model.getElementCount(); ++i) {
            if (m_scale_model.getElementType(i) != ScaleModel::DIAGONAL) {
                ...
                new_label.color = (force < 0.0f) ? blue * (-force / 100.0f) + white * (1.0f - (-force / 100.0f))
                                                 : red * (force / 100.0f) + white * (1.0f - (force / 100.0f));
