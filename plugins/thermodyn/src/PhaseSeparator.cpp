...
bool megamol::thermodyn::PhaseSeparator::getDataCallback(core::Call& c) {
    ...
    if (inCall->DataHash() != inDataHash_ || inCall->FrameID() != frameID_) {
        ...
        auto smooth = tmp;
        smooth.erase(std::remove_if(smooth.begin(), smooth.end(), [&max, &perc](auto const& a){return !(a > max-perc);}), smooth.end());
        ...
        smooth = tmp;
        smooth.erase(std::remove_if(smooth.begin(), smooth.end(), [&min, &perc](auto const& a){return !(a < min+perc);}), smooth.end());
        //std::nth_element(smooth.begin(), smooth.begin()+(smooth.size()/2), smooth.end());
