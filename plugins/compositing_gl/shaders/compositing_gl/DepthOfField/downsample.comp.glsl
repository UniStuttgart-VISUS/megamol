...
layout(binding = 2, rg8ui) writeonly uniform image2D coc_4_tx2D;
...
void main() {
    ...
    vec3 color = texelFetch(color_tx2D, pixel_coords, 0); // TODO: sampleLinear (bilinear)
    vec2 coc = texelFetch(coc_tx2D, pixel_coord_00, 0); // TODO: samplePoint, TODO: does this need to be pixel_coord_01 (top left) because of coordinate system differences between opengl/directx

    float coc_far_00 = texelFetch(coc_tx2D, pixel_coord_00, 0).y; // TODO: samplePoint
    float coc_far_10 = texelFetch(coc_tx2D, pixel_coord_10, 0).y; // TODO: samplePoint
    float coc_far_01 = texelFetch(coc_tx2D, pixel_coord_01, 0).y; // TODO: samplePoint
    float coc_far_11 = texelFetch(coc_tx2D, pixel_coord_11, 0).y; // TODO: samplePoint

    float weight_00 = 1000.0;
    vec3 color_mul_coc_far = weight_00 * texelFetch(color_tx2D, pixel_coord_00, 0); // TODO: samplePoint, TODO: is vec3 correct?
    float weight_sum = weight_00;

    float weight_10 = 1.0 / (abs(coc_far_00 - coc_far_10) + 0.001);
    color_mul_coc_far += weight_10 * texelFetch(color_tx2D, pixel_coord_10, 0); // TODO: samplePoint
    weight_sum += weight_00;

    float weight_01 = 1.0 / (abs(coc_far_00 - coc_far_01) + 0.001);
    color_mul_coc_far += weight_01 * texelFetch(color_tx2D, pixel_coord_01, 0); // TODO: samplePoint
    weight_sum += weight_01;

    float weight_11 = 1.0 / (abs(coc_far_00 - coc_far_11) + 0.001);
    color_mul_coc_far += weight_11 * texelFetch(color_tx2D, pixel_coord_11, 0); // TODO: samplePoint
