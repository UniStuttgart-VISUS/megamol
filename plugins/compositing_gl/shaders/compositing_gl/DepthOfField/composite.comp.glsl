...
layout(OUTFORMAT) writeonly uniform image2D depth_of_field_tx2D;
...
void main() {
    ...
    vec2 pixel_size_4 = 2.0 * pixel_size;
    ...
    float coc_far_top_left = textureLod(coc_point_tx2D, tex_coord_01, 0).y;

    // vec4(top_left, top_right, bottom_left, bottom_right)
    // TODO: does this gather the proper texels? might need to offset tex_coord_00 with  + 0.5 * pixel_size_4 or smth like that
    vec4 coc_far_x4 = textureGather(coc_4_point_tx2D, tex_coord_00, 1).xywz;
    vec4 coc_far_diffs = abs(vec4(coc_far_top_left) - coc_far_x4);
    ...
    vec2 image_coord = tex_coord_01 / pixel_size;
    ...
    result = mix(result, dof_far, blend * coc_far_top_left);
