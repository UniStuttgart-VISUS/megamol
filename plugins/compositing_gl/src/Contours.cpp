...
bool megamol::compositing_gl::Contours::create() {
    ...
    try {
        ...
        blurShader_ = core::utility::make_glowl_shader(
            "contours_blur", shdr_options, std::filesystem::path("compositing_gl/contours_gauss_blur.comp.glsl"));
    ...
    intermediateTex_ = std::make_shared<glowl::Texture2D>("contours_intermediate", tx_layout, nullptr);
    intermediateTex2_ = std::make_shared<glowl::Texture2D>("contours_intermediate2", tx_layout, nullptr);
...
void megamol::compositing_gl::Contours::recalcKernel() {
