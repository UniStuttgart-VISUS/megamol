...
void main()
{
    ...
    float a = texelFetchOffset(g_DepthSource, baseCoord, ivec2( 0, 0 ) ).x;
    //float a = g_DepthSource.Load( baseCoord, ivec2( 0, 0 ) ).x;
    float d = texelFetchOffset(g_DepthSource, baseCoord, ivec2( 1, 1 ) ).x;
    ...
    imageStore(g_HalfDepths0, pixPos.xy, vec4(out0, 0.f, 0.f, 0.f));
    imageStore(g_HalfDepths3, pixPos.xy, vec4(out3, 0.f, 0.f, 0.f));
}
