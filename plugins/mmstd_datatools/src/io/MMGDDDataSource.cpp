...
bool io::MMGDDDataSource::filenameChanged(core::param::ParamSlot& slot) {
    ...
    if (!file->Open(filename.Param<core::param::FilePathParam>()->Value().c_str(), File::READ_ONLY, File::SHARE_READ, File::OPEN_ONLY)) {
        Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR, "Unable to open MMGDD-File \"%s\".", filename.Param<core::param::FilePathParam>()->Value().c_str());
        SAFE_DELETE(file);
        this->setFrameCount(1);
        this->initFrameCache(1);
        return true;
