		...
		namespace datatools
		{
			namespace io
			{
				...
				public:
					...
					AbstractSTLWriter(const char* call_name) : core::Module(), last_callee(nullptr)
						...
						, write_now("Write now", "Write an STL file immediately")
					{
						...
						this->output_slot.SetCallback(call_name, "GetExtent", &AbstractSTLWriter<InputCallDescription>::get_extent_callback);
						this->output_slot.SetCallback(call_name, "GetData", &AbstractSTLWriter<InputCallDescription>::get_data_callback);
