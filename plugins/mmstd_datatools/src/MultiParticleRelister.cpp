...
bool MultiParticleRelister::manipulateData(megamol::core::moldyn::MultiParticleDataCall& outData, megamol::core::moldyn::MultiParticleDataCall& inData) {
    ...
    if ((inData.DataHash() != inDataHash)
            || (inDataHash == 0)
            || (inData.FrameID() != inFrameId)
            || (prc->DataHash() != inRelistHash)
            || (inRelistHash == 0)) {
        ...
        uint64_t inPC = (inPLC >= 0) ? inData.AccessParticles(0).GetCount() : 0; // HAZARD: I want all particles in the first list
        if (inPC == prc->SourceParticleCount()) {
            copyData(inData.AccessParticles(0), *prc);

        } else {
            megamol::core::utility::log::Log::DefaultLog.WriteError("(MultiParticleRelister) Cannot combine data from MultiParticleDataCall and ParticleRelistCall: Particle counts not equal");
        }
