...
template<typename T, int DIM>
inline void expand_cluster_with_similarity(std::shared_ptr<kd_tree_t<T, DIM>> const& D, index_t P, search_res_t<T> Nvec, index_t C,
    T eps, index_t minPts, cluster_result_t& clusters, std::vector<char>& visited,
...
template<typename T, int DIM>
inline cluster_result_t DBSCAN_with_similarity(std::shared_ptr<kd_tree_t<T, DIM>> const& D, T eps, index_t minPts, std::function<bool(index_t, index_t)> const& similarity) {
    auto const& data = D->dataset;
    auto const num_points = data.kdtree_get_point_count();
    cluster_result_t clusters(num_points, static_cast<cluster_type_ut>(cluster_type::UNDEFINED));
    std::vector<char> visited(num_points, 0);
    ...
    for (std::remove_const_t<decltype(num_points)> idx = 0; idx < num_points; ++idx) {
        ...
        if (N < minPts) {
            ...
            ++cluster_idx;
            expand_cluster_with_similarity(D, idx, tmp_res, cluster_idx, eps, minPts, clusters, visited, params, similarity);
        }
    }
...
template<typename T, int DIM>
inline void expand_cluster_with_similarity_and_score(std::shared_ptr<kd_tree_t<T, DIM>> const& D, index_t P, T P_score, search_res_t<T> Nvec,
    index_t C, T eps, index_t minPts, cluster_result_t& clusters, std::vector<char>& visited,
    nanoflann::SearchParams const& params, std::function<bool(index_t, index_t)> const& similarity,
    std::function<index_t(index_t,std::vector<index_t> const&)> const& score, std::unordered_map<index_t, std::vector<index_t>>& cluster_identity) {
    auto const& data = D->dataset;
    ...
    for (search_res_t<T>::size_type vec_idx = 0; vec_idx < Nvec.size(); ++vec_idx) {
        ...
        if (visited[idx] == 0) {
            ...
            tmp_res.erase(std::remove_if(tmp_res.begin(), tmp_res.end(),
                [pivot, &similarity](auto const& el) { return !similarity(pivot, el.first); }), tmp_res.end());
            N = tmp_res.size();
            ...
            if (N >= minPts) {
                ...
                std::transform(tmp_res.cbegin(), tmp_res.cend(), scores.begin(), [&score, pivot_score, pivot_size, &tmp_res](auto const& el) {
                    auto const score_val = score(el.first, tmp_res);
                    auto const change = (score_val - pivot_score) / static_cast<T>(pivot_size + 1);
                    return std::make_pair(el.first, change);
                });
...
template<typename T, int DIM>
inline cluster_result_t DBSCAN_with_similarity_and_score(std::shared_ptr<kd_tree_t<T, DIM>> const& D, T eps, index_t minPts, std::function<bool(index_t, index_t)> const& similarity,
    ...
    for (std::remove_const_t<decltype(num_points)> idx = 0; idx < num_points; ++idx) {
        ...
        auto N = D->radiusSearch(query, eps, tmp_res, params);
        //auto old_N = N;
        tmp_res.erase(std::remove_if(
            tmp_res.begin(), tmp_res.end(), [idx, &similarity](auto const& el) { return !similarity(idx, el.first); }), tmp_res.end());
        N = tmp_res.size();
        //core::utility::log::Log::DefaultLog.WriteInfo("Size of Neighborhood %d was %d", N, old_N);
        if (N < minPts) {
            clusters[idx] = static_cast<cluster_type_ut>(cluster_type::NOISE);
            ...
            T const idx_score = 0;
            //= score(idx, tmp_res);
            expand_cluster_with_similarity_and_score(
                D, idx, idx_score, tmp_res, cluster_idx, eps, minPts, clusters, visited, params, similarity, score, cluster_identity);
        }
        cpb.Set(idx);
    }
