    ...
    namespace stdplugin
    {
        namespace datatools
        {
            namespace io
            {
                ...
                template <typename InputCall, typename InputCallDescription>
                class AbstractSTLWriter : public core::Module
                {
                public:
                    AbstractSTLWriter(const char* call_name)
                        : AbstractSTLWriter(call_name, "GetData", "GetExtent") {}
                    ...
                    AbstractSTLWriter(const char* call_name, const char* data_cb_name, const char* extent_cb_name) : core::Module(), last_callee(nullptr)
                        ...
                        , write_now("Write now", "Write an STL file immediately")
                    {
                        ...
                        this->output_slot.SetCallback(call_name, extent_cb_name, &AbstractSTLWriter<InputCall, InputCallDescription>::get_extent_callback<InputCall>);
                        this->output_slot.SetCallback(call_name, data_cb_name, &AbstractSTLWriter<InputCall, InputCallDescription>::get_data_callback<InputCall>);
                ...
                protected:
                    ...
                    /// <returns>True on success; false otherwise</returns>
                    virtual bool copy_info_upstream(core::Call& caller, core::Call& callee) = 0;
                    ...
                    /// <param name="callee">Outgoing call</param>
                    /// <returns>True on success; false otherwise</returns>
                    virtual bool copy_info_downstream(core::Call& caller, core::Call& callee) = 0;

                    virtual bool check_update(core::Call& caller, core::Call& callee) {
                        return false;
                    };
                    ...
                    /// <param name="callee">Outgoing call</param>
                    /// <returns>True on success; false otherwise</returns>
                    virtual bool copy_data(core::Call& caller, core::Call& callee) = 0;
                    ...
                    /// <param name="callee">Outgoing call</param>
                    /// <returns>True on success; false otherwise</returns>
                    virtual bool write_data(core::Call& callee) = 0;
                    ...
                    /// <returns>True on success; false otherwise</returns>
                    template <typename T = InputCall>
                    std::enable_if_t<std::is_base_of_v<core::AbstractGetData3DCall, T>,
                        bool>
                    get_extent_callback(core::Call& caller)
                    {
                    ...
                    {
                        // Create outgoing call
                        auto& incoming_call = dynamic_cast<T&>(caller);
                        auto& outgoing_call = *this->input_slot.template CallAs<T>();
                    ...
                    std::enable_if_t<std::is_base_of_v<core::AbstractGetData3DCall, T>,
                        bool>
                    get_data_callback(core::Call& caller)
                    {
                        // Create outgoing call
                        auto& incoming_call = dynamic_cast<core::AbstractGetData3DCall&>(caller);
                        auto& outgoing_call = *this->input_slot.template CallAs<core::AbstractGetData3DCall>();
                    ...
                    get_data_callback(core::Call& caller)
                    {
                        ...
                        if (check_update(incoming_call, outgoing_call))
                        {
                            // Copy data from outgoing to incoming call
                            if (!copy_data(incoming_call, outgoing_call))
                            {
                    ...
                    std::string get_filename(const std::size_t frame_id) {
                        // Get filename
                        const auto& vislib_filename =
                            this->filename_slot.template Param<core::param::FilePathParam>()->Value();
                            ...
                            if (this->filename_slot.IsDirty()) {
                                ...
                                while (directory.HasNext()) {
                                    ...
                                    if (extracted_suffix.length() >= leading_zeroes.length() &&
                                        ...
                                        this->increment = std::max(
                                            this->increment, static_cast<std::size_t>(std::stoull(extracted_suffix)));
                                        found = true;
                                    }
                                }
                    ...
                    void write(const std::size_t num_triangles, const std::size_t frame_id, const VFT* vertices, const NFT* normals, const IT* indices = nullptr)
                    {
                        ...
                        //    std::stringstream ss;
                        //    ss << std::put_time(std::localtime(&now_c), "%Y-%m-%d_%H-%M-%S") << "_";
                        //    ss << std::chrono::duration_cast<std::chrono::milliseconds>(now - std::chrono::system_clock::from_time_t(now_c)).count();
                        ...
                        if (this->ascii_binary_slot.template Param<core::param::EnumParam>()->Value() == 0)
                        {
                            megamol::core::utility::log::Log::DefaultLog.WriteInfo(
                                "Writing binary STL file '%s'", current_filename.c_str());
                            write_binary(current_filename, static_cast<uint32_t>(num_triangles), vertices, normals,
                                pointer_or_identity<IT>(indices));
                        }
                        else
                        {
                            megamol::core::utility::log::Log::DefaultLog.WriteInfo(
                                "Writing ASCII STL file '%s'", current_filename.c_str());
                            write_ascii(
                                current_filename, num_triangles, vertices, normals, pointer_or_identity<IT>(indices));
                        }
                    }
                private:
                    /// <summary>
