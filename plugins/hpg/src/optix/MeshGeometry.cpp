...
megamol::hpg::optix::MeshGeometry::MeshGeometry()
...
bool megamol::hpg::optix::MeshGeometry::create() {
    return true;
}


void megamol::hpg::optix::MeshGeometry::release() {}


void megamol::hpg::optix::MeshGeometry::init(CallContext& ctx) {
    ...
    OPTIX_CHECK_ERROR(optixBuiltinISModuleGet(
        ctx.get_ctx(), ctx.get_module_options(), ctx.get_pipeline_options(), &opts, &triangle_intersector_));

    mesh_module_ =
        MMOptixModule(embedded_mesh_programs, ctx.get_ctx(), ctx.get_module_options(), ctx.get_pipeline_options(),
            OPTIX_PROGRAM_GROUP_KIND_HITGROUP, triangle_intersector_, {"mesh_intersect", "mesh_closesthit"});
    mesh_occlusion_module_ =
        MMOptixModule(embedded_mesh_programs, ctx.get_ctx(), ctx.get_module_options(), ctx.get_pipeline_options(),
            OPTIX_PROGRAM_GROUP_KIND_HITGROUP, triangle_intersector_, {"mesh_intersect", "mesh_closesthit_occlusion"});
}


bool megamol::hpg::optix::MeshGeometry::assertData(mesh::CallMesh& call, CallContext& ctx) {
    ...
    for (auto const& mesh : meshes) {
        ...
        CUDA_CHECK_ERROR(cuMemcpyHtoDAsync(
            mesh_idx_data_.back(), mesh_data.indices.data, mesh_data.indices.byte_size, ctx.get_exec_stream()));
        ...
        CUDA_CHECK_ERROR(cuMemcpyHtoDAsync(
            mesh_pos_data_.back(), tmp_data.data(), tmp_data.size() * sizeof(float), ctx.get_exec_stream()));
...
bool megamol::hpg::optix::MeshGeometry::get_data_cb(core::Call& c) {
...
bool megamol::hpg::optix::MeshGeometry::get_extents_cb(core::Call& c) {
    auto out_geo = dynamic_cast<CallGeometry*>(&c);
    if (out_geo == nullptr)
        return false;
    auto in_data = _in_data_slot.CallAs<mesh::CallMesh>();
