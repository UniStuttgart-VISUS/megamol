...
#include "hpg/optix/utils_host.h"
...
megamol::hpg::optix::TransitionCalculator::TransitionCalculator()
    ...
    in_ctx_slot_.SetCompatibleCall<CallContextDescription>();
    MakeSlotAvailable(&in_ctx_slot_);
}
...
bool megamol::hpg::optix::TransitionCalculator::create() {
    return true;
}


void megamol::hpg::optix::TransitionCalculator::release() {}


bool megamol::hpg::optix::TransitionCalculator::init(CallContext& ctx) {
    OptixBuiltinISOptions opts = {};
    opts.builtinISModuleType = OPTIX_PRIMITIVE_TYPE_TRIANGLE;
    opts.usesMotionBlur = false;
    OPTIX_CHECK_ERROR(optixBuiltinISModuleGet(
        ctx.get_ctx(), ctx.get_module_options(), ctx.get_pipeline_options(), &opts, &builtin_triangle_intersector_));

    mesh_module_ = MMOptixModule(embedded_transitioncalculator_programs, ctx.get_ctx(), ctx.get_module_options(),
        ctx.get_pipeline_options(), OPTIX_PROGRAM_GROUP_KIND_HITGROUP, builtin_triangle_intersector_,
        {"tc_intersect", "tc_closesthit"});

    raygen_module_ = MMOptixModule(embedded_transitioncalculator_programs, ctx.get_ctx(), ctx.get_module_options(),
        ctx.get_pipeline_options(), OPTIX_PROGRAM_GROUP_KIND_RAYGEN, {"tc_raygen_program"});

    miss_module_ = MMOptixModule(embedded_transitioncalculator_programs, ctx.get_ctx(), ctx.get_module_options(),
        ctx.get_pipeline_options(), OPTIX_PROGRAM_GROUP_KIND_MISS, {"tc_miss_program"});
    ...
    OPTIX_CHECK_ERROR(optixPipelineCreate(
        ctx.get_ctx(), ctx.get_pipeline_options(), ctx.get_pipeline_link_options(), groups.data(), 3, log, &log_size, &pipeline_));

    return true;
}


bool megamol::hpg::optix::TransitionCalculator::assertData(
    ...
    CUDA_CHECK_ERROR(
        cuMemcpyHtoDAsync(mesh_idx_data, mesh_data.indices.data, mesh_data.indices.byte_size, ctx.get_exec_stream()));
    ...
    CUDA_CHECK_ERROR(
        cuMemcpyHtoDAsync(mesh_pos_data, tmp_data.data(), tmp_data.size() * sizeof(float), ctx.get_exec_stream()));
    ...
    OPTIX_CHECK_ERROR(optixAccelComputeMemoryUsage(ctx.get_ctx(), &accelOptions, &build_input, 1, &bufferSizes));
    ...
    OPTIX_CHECK_ERROR(optixAccelBuild(ctx.get_ctx(), ctx.get_exec_stream(), &accelOptions, &build_input, 1, geo_temp,
        bufferSizes.tempSizeInBytes, geo_buffer, bufferSizes.outputSizeInBytes, &geo_handle, nullptr, 0));
    ...
    for (unsigned int plIdx = 0; plIdx < plCount0; ++plIdx) {
        ...
        CUDA_CHECK_ERROR(cuMemcpyHtoDAsync(
            ray_buffer_.back(), ray_vec.data(), ray_vec.size() * sizeof(RayH), ctx.get_exec_stream()));
        ...
        CUDA_CHECK_ERROR(cuMemsetD32Async(mesh_inbound_ctr, 0, (num_vertices / 3), ctx.get_exec_stream()));
        CUDA_CHECK_ERROR(cuMemsetD32Async(mesh_outbound_ctr, 0, (num_vertices / 3), ctx.get_exec_stream()));
        CUDA_CHECK_ERROR(cuMemsetD8Async(ray_state, 0, ray_vec.size(), ctx.get_exec_stream()));
        ...
        sbt_.SetSBT(&raygen_record, sizeof(raygen_record), nullptr, 0, &miss_record, sizeof(miss_record), 1,
            &mesh_sbt_record, sizeof(mesh_sbt_record), 1, nullptr, 0, 0, ctx.get_exec_stream());

        glm::uvec2 launch_dim = glm::uvec2(std::ceilf(std::sqrtf(static_cast<float>(ray_vec.size()))));
        OPTIX_CHECK_ERROR(optixLaunch(pipeline_, ctx.get_exec_stream(), 0, 0, sbt_, launch_dim.x, launch_dim.y, 1));
        ...
        CUDA_CHECK_ERROR(cuMemcpyDtoHAsync(mesh_inbound_vec.data(), mesh_inbound_ctr, (num_vertices / 3) * sizeof(std::uint32_t),
            ctx.get_exec_stream()));
        CUDA_CHECK_ERROR(cuMemcpyDtoHAsync(mesh_outbound_vec.data(), mesh_outbound_ctr, (num_vertices / 3) * sizeof(std::uint32_t),
            ctx.get_exec_stream()));
        CUDA_CHECK_ERROR(cuMemcpyDtoHAsync(ray_state_vec.data(), ray_state, ray_vec.size(), ctx.get_exec_stream()));
...
bool megamol::hpg::optix::TransitionCalculator::get_data_cb(core::Call& c) {
    ...
    if (in_paths == nullptr)
        return false;
    auto in_ctx = in_ctx_slot_.CallAs<CallContext>();
    if (in_ctx == nullptr)
        return false;
    if (!(*in_ctx)(0))
        return false;
...
bool megamol::hpg::optix::TransitionCalculator::get_extent_cb(core::Call& c) {
    auto out_geo = dynamic_cast<mesh::CallMesh*>(&c);
    if (out_geo == nullptr)
        return false;
