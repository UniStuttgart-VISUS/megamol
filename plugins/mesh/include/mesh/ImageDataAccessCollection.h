...
public:
    ...
    /**
     * Add an accessor for an image. Caution! Takes no ownership of image data, simply provides acces via raw
     * pointer.
     */
...
inline void ImageDataAccessCollection::addImage(
    TextureFormat format, int width, int height, uint8_t* data, size_t byte_size) {
    Image img;
    img.format = format;
    img.width = width;
    img.height = height;
