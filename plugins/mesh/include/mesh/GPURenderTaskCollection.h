...
template <typename PerDrawDataType>
inline void GPURenderTaskCollection::addRenderTask(
...
inline void GPURenderTaskCollection::addRenderTasks(
    ...
    auto query = std::find_if(m_gpu_render_tasks.begin(), m_gpu_render_tasks.end(), [&shader_prgm, &mesh](std::shared_ptr<GPURenderTask> const& rt) {
