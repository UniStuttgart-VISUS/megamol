...
megamol::datraw_plugin::VolumetricDataSource::VolumetricDataSource(void)
    ...
    this->paramFileName.SetUpdateCallback(
        &VolumetricDataSource::onFileNameChanged);
    ...
    enumParam = new core::param::EnumParam(-1);
    enumParam->SetTypePair(-1, _T("Auto"));
    enumParam->SetTypePair(VolumetricDataCall::ScalarType::SIGNED_INTEGER,
        _T("int"));
    enumParam->SetTypePair(VolumetricDataCall::ScalarType::UNSIGNED_INTEGER,
        _T("uint"));
    enumParam->SetTypePair(VolumetricDataCall::ScalarType::FLOATING_POINT,
        _T("float"));
...
DatRawDataFormat megamol::datraw_plugin::VolumetricDataSource::getOutputDataFormat(
    ...
    size_t scalarLength = (sizeParam->Value() >= 0)
        ? sizeParam->Value()
...
bool megamol::datraw_plugin::VolumetricDataSource::onFileNameChanged(
    ...
    bool isAsync = this->paramLoadAsync.Param<core::param::BoolParam>(
        )->Value();
    if (isAsync) {
    ...
    /* Read the header. */
    vislib::StringA fileName(this->paramFileName.Param<
        core::param::FilePathParam>()->Value());
    if (::datRaw_readHeader(fileName.PeekBuffer(), this->fileInfo, nullptr)
            != FALSE) {
        Log::DefaultLog.WriteInfo(_T("Successfully loaded dat file %hs."),
            fileName.PeekBuffer());

        static_assert(STATIC_ARRAY_COUNT(this->metadata.SliceDists)
            == STATIC_ARRAY_COUNT(this->metadata.Resolution), "Dimension of "
            "slice distance and resolution arrays match.");
        static_assert(STATIC_ARRAY_COUNT(this->metadata.SliceDists)
            == STATIC_ARRAY_COUNT(this->metadata.IsUniform), "Dimension of "
            "slice distance and uniformity flag arrays match.");
        int dimensions = vislib::math::Min(
            static_cast<int>(STATIC_ARRAY_COUNT(this->metadata.SliceDists)),
            this->fileInfo->dimensions);

        // Erase *all* old slice distances from heap.
        ::ZeroMemory(this->metadata.SliceDists,
            sizeof(this->metadata.SliceDists));

        switch (this->fileInfo->gridType) {
            case DR_GRID_CARTESIAN:
                this->metadata.GridType
                    = VolumetricDataCall::GridType::CARTESIAN;
                Log::DefaultLog.WriteInfo(_T("The grid is cartesian."));
                for (int i = 0; i < dimensions; ++i) {
                    this->metadata.SliceDists[i]
                        = this->fileInfo->sliceDist + i;
                    this->metadata.IsUniform[i] = true;
                    Log::DefaultLog.WriteInfo(_T("The grid is uniform in ")
                        _T("dimension %d and has a slice distance of %f."), i,
                        this->metadata.SliceDists[i][0]);
                }
                break;

            case DR_GRID_RECTILINEAR:
                this->metadata.GridType
                    = VolumetricDataCall::GridType::RECTILINEAR;
                Log::DefaultLog.WriteInfo(_T("The grid is rectilinear."));
                for (int i = 0, j = 0; i < dimensions; ++i) {
                    this->metadata.SliceDists[i]
                        = this->fileInfo->sliceDist + j;
            ...
            case DR_GRID_TETRAHEDRAL:
                this->metadata.GridType
                    = VolumetricDataCall::GridType::TETRAHEDRAL;
                Log::DefaultLog.WriteWarn(1, _T("The grid is tetrahedral. ")
                    _T("Tetrahedral grids are currently not supported."));
                break;

            default:
                Log::DefaultLog.WriteError(1, _T("The data set uses the ")
                    _T("unexpected grid type %d."), this->fileInfo->gridType);
            case DR_GRID_NONE:
                this->metadata.GridType
                    = VolumetricDataCall::GridType::NONE;
                break;
        }

        ::ZeroMemory(&this->metadata.Resolution,
            sizeof(this->metadata.Resolution));
        for (int i = 0; i < dimensions; ++i) {
            ...
            Log::DefaultLog.WriteInfo(_T("Resolution in dimension %d ")
                _T("is %u."), i, this->metadata.Resolution[i]);
        }
		::ZeroMemory(&this->metadata.Origin,
			sizeof(this->metadata.Origin));
		for (int i = 0; i < dimensions; ++i) {
			this->metadata.Origin[i] = this->fileInfo->origin[i];
			Log::DefaultLog.WriteInfo(_T("Origin in dimension %d ")
				_T("is %f."), i, this->metadata.Origin[i]);
		}
        ...
        switch (this->fileInfo->dataFormat) {
            ...
            case DR_FORMAT_LONG:
                this->metadata.ScalarType
                    = VolumetricDataCall::ScalarType::SIGNED_INTEGER;
            ...
            case DR_FORMAT_ULONG:
                this->metadata.ScalarType
                    = VolumetricDataCall::ScalarType::UNSIGNED_INTEGER;
            ...
            case DR_FORMAT_DOUBLE:
                this->metadata.ScalarType
                    = VolumetricDataCall::ScalarType::FLOATING_POINT;
            ...
            case DR_FORMAT_RAW:
                this->metadata.ScalarType
                    = VolumetricDataCall::ScalarType::BITS;
            ...
            default:
                this->metadata.ScalarType
                    = VolumetricDataCall::ScalarType::UNKNOWN;
                Log::DefaultLog.WriteWarn(_T("The type of the scalar ")
                    _T("values is unkown."));
                break;
        }

        this->metadata.ScalarLength = ::datRaw_getFormatSize(
            this->fileInfo->dataFormat);
        Log::DefaultLog.WriteInfo(_T("Scalar values comprise %u bytes."),
            this->metadata.ScalarLength);

        this->metadata.Components = this->fileInfo->numComponents;
        Log::DefaultLog.WriteInfo(_T("Each voxel comprises %u scalars."),
            this->metadata.Components);

        this->metadata.NumberOfFrames = this->fileInfo->timeSteps;
        Log::DefaultLog.WriteInfo(_T("The data set comprises %u frames."),
            this->metadata.NumberOfFrames);

        /* Compute extents. */
        ::ZeroMemory(this->metadata.Extents, sizeof(this->metadata.Extents));
        for (int d = 0; (d < STATIC_ARRAY_COUNT(this->metadata.Extents))
                && (d < this->fileInfo->dimensions); ++d) {
            switch (this->fileInfo->gridType) {
                case DatRawGridType::DR_GRID_CARTESIAN:
                    this->metadata.Extents[d] = this->fileInfo->sliceDist[d]
                        * static_cast<float>(this->fileInfo->resolution[d]);
                    break;
                ...
                case DatRawGridType::DR_GRID_RECTILINEAR:
                    if (this->metadata.IsUniform[d]) {
                        ...
                        this->metadata.Extents[d] = this->fileInfo->sliceDist[
                            r - 1] * static_cast<float>(r);
                    }
                ...
                default:
                    throw vislib::IllegalStateException(_T("The available ")
                        _T("grid type is not supported."), __FILE__, __LINE__);
            }
        }

    } else {
        Log::DefaultLog.WriteError(1, _T("Failed to read and parse dat file ")
            _T("%hs."), fileName.PeekBuffer());
    ...
    try {
        ...
        if (this->fileInfo == nullptr) {
            throw vislib::IllegalStateException(_T("A valid dat file must be ")
                _T("loaded before the data can be read."), __FILE__, __LINE__);
        }
        ...
        if (isAsync) {
            ...
            if (c.GetData() != nullptr) {
                throw vislib::IllegalStateException(_T("A user-defined ")
                    ...
                    _T("loading has been enabled."), __FILE__, __LINE__);
            }
            ...
            for (size_t i = 0; i < this->buffers.Count(); ++i) {
                ...
                if (b != buffer) {
                    ...
                    if (b->Status.compare_exchange_strong(expected,
                            BUFFER_STATUS_UNUSED)) {
                        unusedBuffers.Add(b);
                    }
                } /* end if (b != buffer) */
            ...
            if (buffer != nullptr) {
                ...
                buffer->Status.compare_exchange_strong(expected,
                ...
                for (size_t i = 1; (i < this->metadata.NumberOfFrames)
                        && (i < this->buffers.Count() - 1); ++i) {
                    unsigned int frameID = (c.FrameID() + i)
                        % (unsigned int)this->metadata.NumberOfFrames;
                    if (this->bufferForFrameIDUnsafe(frameID) < 0) {
                        ...
                        if (bs != nullptr) {
                            ...
                            Log::DefaultLog.WriteInfo(_T("Queueing frame %u ")
                                _T("to be loaded for future use."), frameID);
                ...
                this->spinExchange(buffer->Status, BUFFER_STATUS_USED,
                    expecteds, STATIC_ARRAY_COUNT(expecteds), false);
#endif
                ...
                Log::DefaultLog.WriteError(1, _T("There are insufficient ")
            ...
            if (c.GetData() == nullptr) {
                 ...
                 */
                dst.SetCount(vislib::math::Min(this->assertBuffersUnsafe(
                    cnt, true), cnt));

                if ((bufferIdx >= 0) && (this->buffers[bufferIdx]->Status
                        == BUFFER_STATUS_READY)) {
                    loadStart = 1;
            ...
            for (size_t i = loadStart; i < cnt; ++i) {
                ...
                Log::DefaultLog.WriteInfo(_T("Loading frame %u in format ")
                    _T("%hs to 0x%p"), frameID, ::datRaw_getDataFormatName(
                    format), dst);
#endif /* (defined(DEBUG) || defined(_DEBUG)) */
                retval = (::datRaw_loadStep(this->fileInfo,
                        static_cast<int>(frameID),
                        &buffer,
                        format) != 0);
                ::datRaw_close(this->fileInfo);
                if (!retval) {
                    Log::DefaultLog.WriteError(_T("Loading frame %u failed."),
                        frameID);
                    break;
                }
...
bool megamol::datraw_plugin::VolumetricDataSource::onGetExtents(core::Call& call) {
    ...
    try {
        ...
        if (this->fileInfo == nullptr) {
            throw vislib::IllegalStateException(_T("A valid dat file must be ")
                _T("loaded before the extents can be retrieved."), __FILE__,
                __LINE__);
        }
        /*
        ...
        /* Complete request. */
        c.SetExtent((unsigned int)this->metadata.NumberOfFrames, 
			this->metadata.Origin[0], this->metadata.Origin[1], this->metadata.Origin[2], 
			this->metadata.Extents[0] + this->metadata.Origin[0], 
			this->metadata.Extents[1] + this->metadata.Origin[1],
            this->metadata.Extents[2] + this->metadata.Origin[2]);
...
bool megamol::datraw_plugin::VolumetricDataSource::onGetMetadata(core::Call& call) {
    ...
    try {
        ...
        if (this->fileInfo == nullptr) {
            throw vislib::IllegalStateException(_T("A valid dat file must be ")
                _T("loaded before the meta data can be retrieved."), __FILE__,
                __LINE__);
        }
        /*
...
bool megamol::datraw_plugin::VolumetricDataSource::onTryGetData(core::Call& call) {
    ...
    try {
        ...
        if (bufferIdx >= 0) {
            ...
            if (buffer->Status.compare_exchange_strong(expected,
                    BUFFER_STATUS_USED)) {
...
bool megamol::datraw_plugin::VolumetricDataSource::resumeAsyncLoad(void) {
    ...
    int expected[] = { LOADER_STATUS_PAUSING, LOADER_STATUS_PAUSED };
        this->spinExchange(this->loaderStatus, LOADER_STATUS_RUNNING, expected,
        STATIC_ARRAY_COUNT(expected), true);
    this->evtStartLoading.Set();
...
bool megamol::datraw_plugin::VolumetricDataSource::suspendAsyncLoad(
    ...
    if (this->loaderStatus.compare_exchange_strong(expected,
            LOADER_STATUS_PAUSING)) {
        ASSERT(this->loaderThread.IsRunning());
        this->evtStartLoading.Set();
        if (isWait) {
...
DWORD megamol::datraw_plugin::VolumetricDataSource::loadAsync(void *userData) {
    ...
    try {
        ...
        while (!that->loaderStatus.compare_exchange_strong(expected,
                LOADER_STATUS_STOPPED)) {

            /* Check all buffers whether there is something to do. */
            for (size_t i = 0; (i < that->buffers.Count())
                    && (that->loaderStatus.load() == LOADER_STATUS_RUNNING);
                    ++i) {
                expected = BUFFER_STATUS_PENDING;
                if (that->buffers[i]->Status.compare_exchange_strong(expected,
...
#if (defined(DEBUG) || defined(_DEBUG))
                    Log::DefaultLog.WriteInfo(_T("Loading frame %u in format ")
                        _T("%hs to 0x%p (async)."), that->buffers[i]->FrameID,
                        ::datRaw_getDataFormatName(format), dst);
#endif /* (defined(DEBUG) || defined(_DEBUG)) */
                    if (::datRaw_loadStep(that->fileInfo,
                            static_cast<int>(that->buffers[i]->FrameID),
                            &dst,
                            format)) {
                        that->buffers[i]->Status.store(BUFFER_STATUS_READY);
                    } else {
                        Log::DefaultLog.WriteError(_T("Loading frame %u ")
                            _T("failed."), that->buffers[i]->FrameID);
                        that->buffers[i]->Status.store(BUFFER_STATUS_UNUSED);
                    }
                    ::datRaw_close(that->fileInfo);

                    /* Sleep if requested by user. */
                    auto asyncSleep = that->paramAsyncSleep.Param<IntParam>(
                        )->Value();
                    if (asyncSleep > 0) {
            ...
            that->loaderStatus.compare_exchange_strong(expected,
                LOADER_STATUS_PAUSED);
...
size_t megamol::datraw_plugin::VolumetricDataSource::assertBuffersUnsafe(
    ...
    try {
        ...
        if (cntBuffers < cntFrames) {
            ...
            int deletable[] = { BUFFER_STATUS_READY, BUFFER_STATUS_UNUSED,
                BUFFER_STATUS_PENDING };
            for (size_t i = 0; (i < this->buffers.Count())
                    && (this->buffers.Count() > cntFrames); ++i) {
                if (VolumetricDataSource::spinExchange(
                        this->buffers[i]->Status, BUFFER_STATUS_DELETING,
                        deletable, STATIC_ARRAY_COUNT(deletable), true)) {
