    ...
    for (auto& element : elements) {
        ...
        case 0:
            m_elements.push_back(
                Element(ElementType::STRUT, std::tuple<int, int>(std::get<1>(element), std::get<2>(element)), 0.0f));
            break;
        case 1:
            m_elements.push_back(
                Element(ElementType::DIAGONAL, std::tuple<int, int>(std::get<1>(element), std::get<2>(element)), 0.0f));
            break;
        case 2:
            m_elements.push_back(Element(ElementType::FLOOR,
                std::tuple<int, int, int, int>(
                    std::get<1>(element), std::get<2>(element), std::get<3>(element), std::get<4>(element)),
                0.0f));
            break;
        default:
            break;
...
ScaleModel::Mat4x4 ScaleModel::computeElementTransform(std::tuple<int, int, int, int> node_indices,
    ...
    Vec3 corner_xz_displaced =
        node_positions[std::get<2>(node_indices)] + node_displacements[std::get<2>(node_indices)];
