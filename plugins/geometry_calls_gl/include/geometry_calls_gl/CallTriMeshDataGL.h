 ...
 */
class CallTriMeshDataGL : public core::AbstractGetData3DCall {
public:
    /**
    ...
    public:
        ...
        inline unsigned int AddVertexAttribPointer(uint8_t* ptr) {
            if (this->vattCount == MAX_PARAMETER_NUMBER)
                return UINT_MAX;
        ...
        inline void SetVertexData(
            unsigned int cnt, Tp1 vertices, Tp2 normals, Tp3 colours, Tp4 textureCoordinates, bool takeOwnership) {
            this->clearVrtData();
            this->vrtCnt = cnt;
            if (cnt > 0) {
