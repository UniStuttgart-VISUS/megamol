...
/*
 * MeshTranslateRotateScale::InterfaceIsDirty
 */
bool MeshTranslateRotateScale::InterfaceIsDirty() const {
    return (translateSlot.IsDirty() || quaternionSlot.IsDirty() || scaleSlot.IsDirty());
}


/*
 * MeshTranslateRotateScale::InterfaceResetDirty
 */
void MeshTranslateRotateScale::InterfaceResetDirty() {
...
/*
 * MeshTranslateRotateScale::manipulateData
 */
bool MeshTranslateRotateScale::manipulateData(
    geocalls_gl::CallTriMeshDataGL& outData, geocalls_gl::CallTriMeshDataGL& inData) {
    ...
    */
    ...
    this->mesh = new geocalls_gl::CallTriMeshDataGL::Mesh[mlc];
    for (unsigned int i = 0; i < mlc; i++) {
        ...
        const float* vd;
        if (vdt == geocalls_gl::CallTriMeshDataGL::Mesh::DT_FLOAT) {
        ...
        if (m.HasColourPointer()) {
            if (m.GetColourDataType() == geocalls_gl::CallTriMeshDataGL::Mesh::DT_BYTE) {
                cd = const_cast<unsigned char*>(m.GetColourPointerByte());
            }
        }
        if (m.HasTriIndexPointer()) {
            if (m.GetTriDataType() == geocalls_gl::CallTriMeshDataGL::Mesh::DT_UINT32) {
...
#if 0
/*
 * MeshTranslateRotateScale::manipulateExtent
 */
bool MeshTranslateRotateScale::manipulateExtent(
