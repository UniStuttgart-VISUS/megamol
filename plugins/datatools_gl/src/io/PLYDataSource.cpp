...
io::PLYDataSource::PLYDataSource()
    ...
    this->getSphereData.SetCallback(geocalls::MultiParticleDataCall::ClassName(),
        geocalls::MultiParticleDataCall::FunctionName(0), &PLYDataSource::getSphereDataCallback);
    this->getSphereData.SetCallback(geocalls::MultiParticleDataCall::ClassName(),
        geocalls::MultiParticleDataCall::FunctionName(1), &PLYDataSource::getSphereExtentCallback);
    this->MakeSlotAvailable(&this->getSphereData);

    this->getMeshData.SetCallback(
        CallTriMeshDataGL::ClassName(), CallTriMeshDataGL::FunctionName(0), &PLYDataSource::getMeshDataCallback);
    this->getMeshData.SetCallback(
        CallTriMeshDataGL::ClassName(), CallTriMeshDataGL::FunctionName(1), &PLYDataSource::getMeshExtentCallback);
...
bool io::PLYDataSource::getSphereDataCallback(core::Call& caller) {
    auto c2 = dynamic_cast<geocalls::MultiParticleDataCall*>(&caller);
    if (c2 == nullptr)
        return false;

    c2->SetParticleListCount(1);
    geocalls::MultiParticleDataCall::Particles& p = c2->AccessParticles(0);
    ...
    if (p.GetCount() > 0) {
        if (this->posPointers.pos_float != nullptr) {
            p.SetVertexData(
                geocalls::SimpleSphericalParticles::VertexDataType::VERTDATA_FLOAT_XYZ, this->posPointers.pos_float);
        } else if (this->posPointers.pos_double != nullptr) {
            p.SetVertexData(
                geocalls::SimpleSphericalParticles::VertexDataType::VERTDATA_DOUBLE_XYZ, this->posPointers.pos_double);
        } else {
            p.SetCount(0);
        }
        if (colorPointers.col_uchar != nullptr) {
            p.SetColourData(
                geocalls::SimpleSphericalParticles::ColourDataType::COLDATA_UINT8_RGB, this->colorPointers.col_uchar);
        } else if (colorPointers.col_float != nullptr) {
            p.SetColourData(
                geocalls::SimpleSphericalParticles::ColourDataType::COLDATA_FLOAT_RGB, this->colorPointers.col_float);
        } else {
            p.SetColourData(geocalls::SimpleSphericalParticles::ColourDataType::COLDATA_NONE, nullptr);
        }
    }
...
bool io::PLYDataSource::getMeshDataCallback(core::Call& caller) {
    auto c2 = dynamic_cast<CallTriMeshDataGL*>(&caller);
    if (c2 == nullptr)
        return false;
