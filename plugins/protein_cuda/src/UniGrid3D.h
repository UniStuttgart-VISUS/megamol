...
public:
     ...
     */
    UniGrid3D(
        vislib::math::Vector<unsigned int, 3> gridDim, vislib::math::Vector<float, 3> gridOrg, float gridStepSize);

    /** Dtor. */
    ~UniGrid3D();

    cudaError MemCpyFromDevice(const T* dvPt) {
        cudaMemcpy(this->data, dvPt, this->gridDim.X() * this->gridDim.Y() * this->gridDim.Z() * sizeof(T),
 ...
 */
template<class T>
UniGrid3D<T>::UniGrid3D() : gridDim(0, 0, 0)
                          , gridOrg(0.0f, 0.0f, 0.0f)
                          , gridStepSize(0.0f)
                          , data(NULL) {}
