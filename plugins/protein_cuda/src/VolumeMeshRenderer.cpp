...
VolumeMeshRenderer::VolumeMeshRenderer(void)
        ...
        , isoValue(0.5f)
        , maxDistance(2.0f)
        , maxDeltaDistance(1.0f)
        , centroidColorsIndex(-1)
        , vertexCount(0)
        , cubeCount(0)
        , cubeCountAllocated(0)
        , cubeCountOld(0)
        , activeCubeCountOld(0)
        , activeCubeCountAllocted(0)
        , vertexCountAllocted(0)
        , centroidCountAllocated(0)
        , centroidCountLast(0)
        , centroidsLast(0)
        , centroidColorsLast(0)
        , centroidLabelsLast(0)
        , centroidAreasLast(0)
        , featureCounter(0)
        , featureListIdx(0)
        , modified(0)
    ...
    this->diagramCalleeSlot.SetCallback(DiagramCall::ClassName(),
        DiagramCall::FunctionName(DiagramCall::CallForGetData), &VolumeMeshRenderer::GetDiagramData);
    this->MakeSlotAvailable(&this->diagramCalleeSlot);
    this->splitMergeCalleeSlot.SetCallback(SplitMergeCall::ClassName(),
        SplitMergeCall::FunctionName(SplitMergeCall::CallForGetData), &VolumeMeshRenderer::GetSplitMergeData);
    this->MakeSlotAvailable(&this->splitMergeCalleeSlot);
    this->centerLineDiagramCalleeSlot.SetCallback(DiagramCall::ClassName(),
        DiagramCall::FunctionName(DiagramCall::CallForGetData), &VolumeMeshRenderer::GetCenterLineDiagramData);
    ...
    this->dataBBox.Set(36.990002f - 20.0f, 69.709999f - 20.0f, 83.580002f - 20.0f, 36.990002f + 20.0f,
        69.709999f + 20.0f, 83.580002f + 20.0f);
    ...
    if (resSelectionCall != nullptr) {
        ...
        if (resSelPtr) {
            for (unsigned int i = 0; i < resSelPtr->Count(); i++) {
                ...
                unsigned int firstResIdx =
                    mol->Molecules()[mol->Chains()[chainIdx].FirstMoleculeIndex()].FirstResidueIndex();
                unsigned int lastResIdx = mol->Molecules()[mol->Chains()[chainIdx].FirstMoleculeIndex() +
                                                           mol->Chains()[chainIdx].MoleculeCount() - 1]
                                              .FirstResidueIndex() +
                                          mol->Molecules()[mol->Chains()[chainIdx].FirstMoleculeIndex() +
                                                           mol->Chains()[chainIdx].MoleculeCount() - 1]
                                              .ResidueCount();
...
#pragma omp parallel for
    ...
    */
    ...
    this->calcMap(mol, posInter, this->qualityParam.Param<param::IntParam>()->Value(),
        this->radscaleParam.Param<param::FloatParam>()->Value(),
        this->gridspacingParam.Param<param::FloatParam>()->Value(),
        this->isoValueParam.Param<param::FloatParam>()->Value(), false);
    ...
    try {
        ...
        if (time != lastTime) {
            ...
            vislib::math::Vector<float, 3> scale(
    ...
    // Mesh placement fix
    vislib::math::Vector<float, 3> tmpOrig(
        mol->AccessBoundingBoxes().ObjectSpaceBBox().GetLeftBottomBack().PeekCoordinates());
...
//#define DRAW_FEATURE_TRIANGLES
    ...
    for (unsigned int tCnt = 0; tCnt < this->featureTrianglesCount; tCnt++) {
        glVertex3f(this->featureTriangleVerticesHost[3 * tCnt].x, this->featureTriangleVerticesHost[3 * tCnt].y,
            this->featureTriangleVerticesHost[3 * tCnt].z);
        glVertex3f(this->featureTriangleVerticesHost[3 * tCnt + 1].x, this->featureTriangleVerticesHost[3 * tCnt + 1].y,
            this->featureTriangleVerticesHost[3 * tCnt + 1].z);
        glVertex3f(this->featureTriangleVerticesHost[3 * tCnt + 2].x, this->featureTriangleVerticesHost[3 * tCnt + 2].y,
            this->featureTriangleVerticesHost[3 * tCnt + 2].z);
    }
    glEnd();
    // ... TEST feature triangle drawing
...
//#define DRAW_FEATURE_VERTICES
    ...
    for (unsigned int tCnt = 0; tCnt < this->featureVertexCntNew; tCnt++) {
        glVertex3f(this->featureTriangleVerticesHost[tCnt].x, this->featureTriangleVerticesHost[tCnt].y,
            this->featureTriangleVerticesHost[tCnt].z);
    }
    glEnd();
    // draw edges
    glLineWidth(1.0f);
        ...
        for (unsigned int fCnt = 0; fCnt < this->clNodes.Count(); fCnt++) {
            ...
            for (auto nodes : this->clNodes[fCnt]) {
                ...
                if (nodes->isStartNode)
                    ...
                    continue; // DRAW ONLY START AND END NODES OF THE CENTERLINE
...
void VolumeMeshRenderer::SortTriangleMesh() {
    ...
    CUDA_VERIFY(
        cudaGraphicsResourceGetMappedPointer(reinterpret_cast<void**>(&vertices), &resourceSize, positionResource));
    float4* normals;
    CUDA_VERIFY(cudaGraphicsMapResources(1, &normalResource, 0));
    CUDA_VERIFY(
        cudaGraphicsResourceGetMappedPointer(reinterpret_cast<void**>(&normals), &resourceSize, normalResource));
    float4* colors;
...
bool VolumeMeshRenderer::UpdateMesh(float* densityMap, vislib::math::Vector<float, 3> translation,
    ...
    if (this->cubeCountAllocated > 0) {
        ...
        CUDA_VERIFY(cudaMemcpy(verticesPerTetrahedronOld, verticesPerTetrahedron,
            sizeof(uint) * this->activeCubeCount * 6, cudaMemcpyDeviceToDevice));
    ...
    cudaExtent aoVolumeHostExtent = make_cudaExtent(
        this->aoShader.getVolumeSizeX() - 2, this->aoShader.getVolumeSizeY() - 2, this->aoShader.getVolumeSizeZ() - 2);
    ...
    CUDA_VERIFY(ClassifyTetrahedronsInACube(verticesPerTetrahedron, cubeMap, this->isoValue, activeCubeCount));
    ...
    // ========================================================================
    CUDA_VERIFY(GenerateTriangles(vertices, neighborAtomOfVertex_d, normals, translation.X(), translation.Y(),
        translation.Z(), scale.X(), scale.Y(), scale.Z(), vertexOffsets, cubeMap, this->isoValue, tetrahedronCount));
    cudaDeviceSynchronize(); // Paranoia
    // TODO use cudaMemcpyAsync here?
    CUDA_VERIFY(cudaMemcpy(this->neighborAtomOfVertex, this->neighborAtomOfVertex_d, this->vertexCount * sizeof(int),
    ...
    while (true) {
        ...
        CUDA_VERIFY(MeshScan(vertices, normals, vertexOffsets, triangleAO, this->aoThreshold, eqList, refList, modified,
            cubeStates, cubeOffsets, cubeMap, this->isoValue, tetrahedronCount));
    ...
    CUDA_VERIFY(CentroidReduce(&centroidCount, centroidLabels, centroidSums, centroidCounts, vertexLabelsCopy,
        verticesCopy, this->vertexCount));
    ...
    //cudaEventCreate(&stop);
    //cudaEventRecord( start, 0);
    CUDA_VERIFY(ComputeCentroidArea(
        centroidAreas, featureStartEnd, triangleAreas, vertexLabelsCopy, centroidLabels, triangleCount, centroidCount));
    cudaDeviceSynchronize(); // Paranoia
    //cudaDeviceSynchronize();
    //cudaEventRecord( stop, 0);
    //cudaEventSynchronize( stop);
    ...
    */
    ...
    CUDA_VERIFY(RemoveSmallSegments(centroidAreas, triangleAO, vertexLabels, centroidLabels, areaThreshold,
        this->aoThreshold, triangleCount, centroidCount, segmentsRemoved));
    cudaDeviceSynchronize(); // Paranoia
    ...
    if (segmentsRemovedHost) {
        ...
        while (true) {
            CUDA_VERIFY(cudaMemset(modified, false, sizeof(bool)));
            cudaDeviceSynchronize(); // Paranoia
            CUDA_VERIFY(MeshScan(vertices, normals, vertexOffsets, triangleAO, this->aoThreshold, eqList, refList,
                modified, cubeStates, cubeOffsets, cubeMap, this->isoValue, tetrahedronCount));
            CUDA_VERIFY(cudaDeviceSynchronize()); // Paranoia
            CUDA_VERIFY(cudaMemcpy(&hostModified, modified, sizeof(bool), cudaMemcpyDeviceToHost));
            if (!hostModified) {
                break;
        ...
        CUDA_VERIFY(cudaMemcpy(verticesCopy, vertices, this->vertexCount * sizeof(float4), cudaMemcpyDeviceToDevice));
        CUDA_VERIFY(
            cudaMemcpy(vertexLabelsCopy, vertexLabels, this->vertexCount * sizeof(uint), cudaMemcpyDeviceToDevice));
        CUDA_VERIFY(CentroidReduce(&centroidCount, centroidLabels, centroidSums, centroidCounts, vertexLabelsCopy,
            verticesCopy, this->vertexCount));
        ValidateCentroidMemory(centroidCount);
        CUDA_VERIFY(CentroidFinalize(centroids, centroidSums, centroidCounts, centroidCount));
        ...
        CUDA_VERIFY(ComputeSurfaceArea(verticesCopy, triangleAreas, triangleCount));
        cudaDeviceSynchronize(); // Paranoia
        CUDA_VERIFY(ComputeCentroidArea(centroidAreas, featureStartEnd, triangleAreas, vertexLabelsCopy, centroidLabels,
            triangleCount, centroidCount));
        cudaDeviceSynchronize(); // Paranoia
    }
    ...
    if (centroidsLast != 0 && centroidCount > 0 && this->cubeCountOld > 0) {
        ...
        CUDA_VERIFY(
            cudaMemcpy(tetrahedronLabelPairHost, tetrahedronLabelPair, sizeof(int2) * length, cudaMemcpyDeviceToHost));
        cudaDeviceSynchronize();
        ...
        for (unsigned int fcnt = 0; fcnt < centroidCount; fcnt++) {
            ...
            for (unsigned int i = 0; static_cast<int>(i) < length; i++) {
                if (centroidLabelsHost[fcnt] == tetrahedronLabelPairHost[i].x) {
                    ...
                    if (tetrahedronLabelPairHost[i].y == largestFeatureLabelOld ||
                        tetrahedronLabelPairHost[i].y == -1) {
                        continue;
                    }
        ...
        for (unsigned int fcnt = 0; fcnt < centroidCountLast; fcnt++) {
            ...
            prevFeatureDataLastIdx[fcnt] =
                this->featureList[this->featureListIdx[fcnt]]->GetMappable()->GetDataCount() - 1;
        }
        ...
        for (unsigned int i = 0; i < centroidCount; i++) {
            if (cur2prevMatching[i].Count() == 0) {
                ...
                if (prev2curMatching[j].Count() > 1) {
                    ...
                    if (bestK == i) {
                        // Diagram
                        ms = static_cast<MolecularSurfaceFeature*>(
                            this->featureList[this->featureListIdx[j]]->GetMappable());
                        ...
                        ds->AddMarker(
                            new DiagramCall::DiagramMarker(ms->GetDataCount() - 1, DiagramCall::DIAGRAM_MARKER_SPLIT));
                        // SplitMerge
                        ...
                        ds = new DiagramCall::DiagramSeries(
                            featureName, new MolecularSurfaceFeature(static_cast<float>(mol->FrameCount()),
                                             vislib::math::Vector<float, 3>(
                                                 centroidsHost[i].x, centroidsHost[i].y, centroidsHost[i].z)));
                        ds->SetColor(centroidColorsHost[i].x, centroidColorsHost[i].y, centroidColorsHost[i].z,
                            centroidColorsHost[i].w);
                        ms = static_cast<MolecularSurfaceFeature*>(ds->GetMappable());
                        ms->AppendValue(mol->Calltime(), centroidAreasHost[i]);
                        ms->SetPosition(centroidsHost[i].x, centroidsHost[i].y, centroidsHost[i].z);
                        ...
                        ds->AddMarker(new DiagramCall::DiagramMarker(
                            ms->GetDataCount() - 1, DiagramCall::DIAGRAM_MARKER_SPLIT, partners));
                        // SplitMerge
                        sms = new SplitMergeCall::SplitMergeSeries(
                            featureName, new SplitMergeFeature(static_cast<float>(mol->FrameCount()),
                                             vislib::math::Vector<float, 3>(
                                                 centroidsHost[i].x, centroidsHost[i].y, centroidsHost[i].z)));
                        sms->SetColor(centroidColorsHost[i].x, centroidColorsHost[i].y, centroidColorsHost[i].z,
                            centroidColorsHost[i].w);
                        smf = static_cast<SplitMergeFeature*>(sms->GetMappable());
                        smf->AppendValue(mol->Calltime(), centroidAreasHost[i]);
                        smf->SetPosition(centroidsHost[i].x, centroidsHost[i].y, centroidsHost[i].z);
                        ...
                        if (tmpFeatureListIdx[i] == this->featureListIdx[j]) {
                            ...
                            this->transitionList.Add(
                                new SplitMergeCall::SplitMergeTransition(tmpFeatureListIdx[i], 0, centroidAreasHost[i],
                                    this->featureListIdx[j], prevFeatureDataLastIdx[j], centroidAreasLast[j]));
                        }
                    }
                } else {
                    // --> continuity
                ...
                for (uint k = 0; k < cur2prevMatching[i].Count(); k++) {
                    ...
                    if (tmpDiff < diff) {
                        ...
                        if (bestJ >= 0) {
                            ...
                            MolecularSurfaceFeature* ms = static_cast<MolecularSurfaceFeature*>(ds->GetMappable());
                            ds->AddMarker(new DiagramCall::DiagramMarker(
                                ms->GetDataCount() - 1, DiagramCall::DIAGRAM_MARKER_MERGE));
                        ...
                        MolecularSurfaceFeature* ms = static_cast<MolecularSurfaceFeature*>(ds->GetMappable());
                        ds->AddMarker(
                            new DiagramCall::DiagramMarker(ms->GetDataCount() - 1, DiagramCall::DIAGRAM_MARKER_MERGE));
                        partners->Append(this->featureListIdx[j]);
                    }
                }
                ...
                ds->AddMarker(new DiagramCall::DiagramMarker(
                    ms->GetDataCount() - 1, DiagramCall::DIAGRAM_MARKER_MERGE, partners));
                // SplitMerge
                ...
                for (unsigned int mpIdx = 0; mpIdx < (*partners).Count(); mpIdx++) {
                    ...
                    this->transitionList.Add(
                        new SplitMergeCall::SplitMergeTransition(tmpFeatureListIdx[i], ms->GetDataCount() - 1,
                            centroidAreasHost[i], (*partners)[mpIdx], lastDataIdx, smf->GetOrdinateValue(lastDataIdx)));
                }
            }
        }
        ...
        for (unsigned int i = 0; i < unmatchedFeatures.Count(); i++) {
            ...
            for (unsigned int j = 0; j < extinctFeatureIdx.Count(); j++) {
                ...
                featureDistMatrix[i * extinctFeatureIdx.Count() + j] =
                    (vislib::math::Vector<float, 3>(
                         centroidsHost[featureIdx].x, centroidsHost[featureIdx].y, centroidsHost[featureIdx].z) -
                        ms->GetPosition())
                        .Length();
        ...
        thrust::sort_by_key(featureDistMatrix,
            featureDistMatrix + (unmatchedFeatures.Count() * extinctFeatureIdx.Count()), featureIdxMatrix);
        // match features
        int centroidIdx;
        int extinctIdx;
        for (unsigned int i = 0; i < (unmatchedFeatures.Count() * extinctFeatureIdx.Count()); i++) {
            ...
            if (centroidIdx >= 0 && extinctIdx >= 0 && featureDistMatrix[i] < this->maxDistance) {
                ...
                ms->SetPosition(
                    centroidsHost[centroidIdx].x, centroidsHost[centroidIdx].y, centroidsHost[centroidIdx].z);
                tmpFeatureListIdx[centroidIdx] = extinctIdx;
                centroidColorsHost[centroidIdx] =
                    make_float4(ds->GetColor().X(), ds->GetColor().Y(), ds->GetColor().Z(), ds->GetColor().W());
        ...
        for (unsigned int j = 0; j < unmatchedFeatures.Count(); j++) {
            ...
            ds = new DiagramCall::DiagramSeries(featureName,
                new MolecularSurfaceFeature(static_cast<float>(mol->FrameCount()),
                    vislib::math::Vector<float, 3>(centroidsHost[i].x, centroidsHost[i].y, centroidsHost[i].z)));
            ds->SetColor(
                centroidColorsHost[i].x, centroidColorsHost[i].y, centroidColorsHost[i].z, centroidColorsHost[i].w);
            ms = static_cast<MolecularSurfaceFeature*>(ds->GetMappable());
            ms->AppendValue(mol->Calltime(), centroidAreasHost[i]);
            ms->SetPosition(centroidsHost[i].x, centroidsHost[i].y, centroidsHost[i].z);
            this->featureList.Append(ds);
            ...
            sms = new SplitMergeCall::SplitMergeSeries(featureName,
                new SplitMergeFeature(static_cast<float>(mol->FrameCount()),
                    vislib::math::Vector<float, 3>(centroidsHost[i].x, centroidsHost[i].y, centroidsHost[i].z)));
            sms->SetColor(
                centroidColorsHost[i].x, centroidColorsHost[i].y, centroidColorsHost[i].z, centroidColorsHost[i].w);
            smf = static_cast<SplitMergeFeature*>(sms->GetMappable());
    ...
    */
    ...
    for (unsigned int i = 0; i < centroidCount; i++) {
        ...
        if (!this->featureSelection[this->featureListIdx[i]]) {
            ...
            centroidColorsHost[i] = make_float4(this->featureList[this->featureListIdx[i]]->GetColor().X(),
                this->featureList[this->featureListIdx[i]]->GetColor().Y(),
                this->featureList[this->featureListIdx[i]]->GetColor().Z(), w);
        }
    }
    ...
    CUDA_VERIFY(cudaMemcpy(
        this->featureStartEndHost, this->featureStartEnd, centroidCount * sizeof(uint2), cudaMemcpyDeviceToHost));
    cudaDeviceSynchronize();
    ...
    for (unsigned int fCnt = 1; fCnt < centroidCount; fCnt++) {
        ...
        CUDA_VERIFY(cudaMemcpy(this->featureVertices, &(verticesCopy[3 * fStart]), fLength * 3 * sizeof(float4),
            cudaMemcpyDeviceToDevice));
        this->featureTrianglesCount = fLength;
        cudaDeviceSynchronize();
        // reset feature vertex count
        this->featureVertexCntNew = 0;
        CUDA_VERIFY(TriangleVerticesToIndexList(this->featureVertices, this->featureVerticesOut, this->featureVertexIdx,
            this->featureVertexCnt, this->featureVertexCntOut, this->featureVertexStartIdx, this->featureVertexIdxOut,
            fLength * 3, this->featureVertexCntNew));
        cudaDeviceSynchronize();

        // reset feature edge count
        this->edgeCount = 0;
        TriangleEdgeList(this->featureVertexIdxOut, this->featureEdgeCnt, this->featureEdgeCntOut, fLength,
            this->featureEdges, this->featureEdgesOut, this->edgeCount);

        // download compacted feature vertices
        CUDA_VERIFY(cudaMemcpy(this->featureTriangleVerticesHost, this->featureVerticesOut,
            this->featureVertexCntNew * sizeof(float4), cudaMemcpyDeviceToHost));
        ...
        CUDA_VERIFY(cudaMemcpy(this->featureTriangleEdgesHost, this->featureEdgesOut, this->edgeCount * sizeof(uint2),
            cudaMemcpyDeviceToHost));
        // download triangle edge count
        CUDA_VERIFY(cudaMemcpy(this->featureTriangleEdgeCountHost, this->featureEdgeCntOut,
            this->edgeCount * sizeof(uint), cudaMemcpyDeviceToHost));
        ...
        if (!clg[fCnt - 1]->freeEdgeRing.empty() && !clg[fCnt - 1]->freeEdgeRing[0].empty()) {
            ...
            clg[fCnt - 1]->CenterLine(clg[fCnt - 1]->freeEdgeRing[0], clEdges[fCnt - 1], clNodes[fCnt - 1],
                this->minDistCenterLineParam.Param<param::FloatParam>()->Value());
        ...
        if (this->blendIt) {
            if (this->resSelectionCall && this->atomSelection[atomIdx]) {
                ...
                this->vertexColors[4 * i + 3] = vislib::math::Min(
                    this->alphaParam.Param<param::FloatParam>()->Value(), this->vertexColors[4 * i + 3]);
            }
        }
    }
...
float4 VolumeMeshRenderer::GetNextColor() {
    ...
    while (repick) {
        ...
        if (centroidsLast != 0) {
            for (uint j = 0; j < centroidCountLast; ++j) {
                if (nextColor.x == centroidColorsLast[j].x && nextColor.y == centroidColorsLast[j].y &&
...
void VolumeMeshRenderer::ValidateCubeMemory() {
    ...
    if (cubeCount > 0) {
        ...
        cudaExtent aoVolumeExtent = make_cudaExtent(
            this->aoShader.getVolumeSizeX(), this->aoShader.getVolumeSizeY(), this->aoShader.getVolumeSizeZ());
...
uint VolumeMeshRenderer::CudaGetLast(uint* buffer, uint* scanBuffer, size_t size) {
    ...
    CUDA_VERIFY(cudaMemcpy(&lastElement, (buffer + size - 1), sizeof(uint), cudaMemcpyDeviceToHost));
    CUDA_VERIFY(cudaMemcpy(&lastScanElement, (scanBuffer + size - 1), sizeof(uint), cudaMemcpyDeviceToHost));
...
int VolumeMeshRenderer::calcMap(MolecularDataCall* mol, float* posInter, int quality, float radscale, float gridspacing,
...
bool VolumeMeshRenderer::GetDiagramData(core::Call& call) {
    ...
    if (dc->GetSeriesCount() == 0) {
        ...
        ds->AddMarker(new DiagramCall::DiagramMarker(
            4, DiagramCall::DIAGRAM_MARKER_MERGE, "universe joining something\n or other"));
        dc->AddSeries(ds);
...
bool VolumeMeshRenderer::GetCenterLineDiagramData(core::Call& call) {
    ...
    for (SIZE_T fIdx = this->featureCenterLines.Count(); fIdx < (this->clg.Count() + 1); fIdx++) {
        ...
        ds = new DiagramCall::DiagramSeries(featureName, new MolecularSurfaceFeature(1.0f));
        ...
        */
        ...
        while (node2 != nullptr && !node2->isStartNode) {
            ...
            if (node2 != nullptr && node2->isStartNode) {
                ds->AddMarker(new DiagramCall::DiagramMarker(
                    ms->GetDataCount() - 1, DiagramCall::DIAGRAM_MARKER_EXIT, const_cast<char*>("Channel exit!")));
            }
        }
