...
public:
    ...
    enum FeatureType { CAVITY = 0, CHANNEL = 1, POCKET = 2 };
    ...
    public:
        ...
        inline Node* getNode1() {
            return node1;
        }
        inline Node* getNode2() {
            return node2;
        }
        inline const Node* Node1() const {
            return node1;
        }
        inline const Node* Node2() const {
            return node2;
        }
    ...
    struct CenterLineNode {
        CenterLineNode() : p(), minimumDistance(0.0f), isRing(true), isStartNode(false) {}
    ...
    struct CenterLineEdge {
        CenterLineEdge() : node1(nullptr), node2(nullptr), visited(false) {}
    ...
    void SetTriangleMesh(
        unsigned int vertexCnt, float* vertices, unsigned int edgeCnt, unsigned int* edges, unsigned int* freeEdge);

    void CenterLine(Edges& selection, CenterLineEdges& centerLineEdges, CenterLineNodes& centerLineNodes,
        float minCenterLineNodeDist = 0.0f);
