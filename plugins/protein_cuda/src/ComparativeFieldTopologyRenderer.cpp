...
ComparativeFieldTopologyRenderer::ComparativeFieldTopologyRenderer(void)
        ...
        /* Boolean flags */
        recalcEfield(true)
        , recalcStreamlines(true)
        , recalcCritPoints(true)
        , recalcStreamlinesManualSeed(true)
        , recalcNeighbours(true)
...
bool ComparativeFieldTopologyRenderer::Render(core_gl::view::CallRender3DGL& call) {
    ...
    if (!(*cmd0)(protein_calls::VTIDataCall::CallForGetData))
        return false;
    if (!(*cmd1)(protein_calls::VTIDataCall::CallForGetData))
        return false;
    ...
    // Get bounding box of first data set
    bbox0.Set(cmd0->GetOrigin().X(), cmd0->GetOrigin().Y(), cmd0->GetOrigin().Z(),
        cmd0->GetOrigin().X() + (cmd0->GetGridsize().X() - 1) * cmd0->GetSpacing().X(),
        cmd0->GetOrigin().Y() + (cmd0->GetGridsize().Y() - 1) * cmd0->GetSpacing().Y(),
        cmd0->GetOrigin().Z() + (cmd0->GetGridsize().Z() - 1) * cmd0->GetSpacing().Z());

    // Get bounding box of first data set
    bbox1.Set(cmd0->GetOrigin().X(), cmd1->GetOrigin().Y(), cmd1->GetOrigin().Z(),
        cmd1->GetOrigin().X() + (cmd1->GetGridsize().X() - 1) * cmd1->GetSpacing().X(),
        cmd1->GetOrigin().Y() + (cmd1->GetGridsize().Y() - 1) * cmd1->GetSpacing().Y(),
        cmd1->GetOrigin().Z() + (cmd1->GetGridsize().Z() - 1) * cmd1->GetSpacing().Z());
    ...
    if ((cmd1->GetGridsize().GetX() == 0) || (cmd1->GetGridsize().GetY() == 0) || (cmd1->GetGridsize().GetZ() == 0)) {
        return true;
    }
    ...
    if (this->recalcEfield) {
        ...
        Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: ... #0 done (%f s)", this->ClassName(),
        ...
        Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: ... #1 done (%f s)", this->ClassName(),
            this->efield0.GetCritPointCount(),
            (double(clock() - t) / double(CLOCKS_PER_SEC))); // DEBUG
    ...
    if (!glIsTexture(this->potentialTex0)) {
        ...
        glTexImage3DEXT(GL_TEXTURE_3D, 0, GL_RGBA32F, cmd0->GetGridsize().X(), cmd0->GetGridsize().Y(),
            cmd0->GetGridsize().Z(), 0, GL_ALPHA, GL_FLOAT, (float*)(cmd0->GetPointDataByIdx(0, 0)));
    ...
    if (!glIsTexture(this->potentialTex1)) {
        ...
        glTexImage3DEXT(GL_TEXTURE_3D, 0, GL_RGBA32F, cmd1->GetGridsize().X(), cmd1->GetGridsize().Y(),
            cmd1->GetGridsize().Z(), 0, GL_ALPHA, GL_FLOAT, (float*)(cmd0->GetPointDataByIdx(0, 0)));
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
 ...
 */
void ComparativeFieldTopologyRenderer::calcElectrostaticField(
    protein_calls::VTIDataCall* cmd, VecField3f& efield, VecField3f& egradfield) {

    float* efieldBuff = new float[cmd->GetGridsize().X() * cmd->GetGridsize().Y() * cmd->GetGridsize().Z() * 3];
#pragma omp parallel for
        ...
        for (int y = 0; y < cmd->GetGridsize().Y(); y++) {
            for (int x = 0; x < cmd->GetGridsize().X(); x++) {
                ...
                if (x == 0) { // Forward differences
                    efieldBuff[3 * cnt + 0] =
                        (this->GetPotentialAt(cmd, x + 1, y, z) - this->GetPotentialAt(cmd, x, y, z)) /
                        cmd->GetSpacing().X();
                } else if (x == cmd->GetGridsize().X() - 1) { // Backward differences
                    efieldBuff[3 * cnt + 0] =
                        (this->GetPotentialAt(cmd, x, y, z) - this->GetPotentialAt(cmd, x - 1, y, z)) /
                        cmd->GetSpacing().X();
                } else { // Central differences
                    efieldBuff[3 * cnt + 0] =
                        (this->GetPotentialAt(cmd, x + 1, y, z) - this->GetPotentialAt(cmd, x - 1, y, z)) /
                        (2.0f * cmd->GetSpacing().X());
                }

                // y component
                if (y == 0) { // Forward differences
                    efieldBuff[3 * cnt + 1] =
                        (this->GetPotentialAt(cmd, x, y + 1, z) - this->GetPotentialAt(cmd, x, y, z)) /
                        cmd->GetSpacing().Y();
                } else if (y == cmd->GetGridsize().Y() - 1) { // Backward differences
                    efieldBuff[3 * cnt + 1] =
                        (this->GetPotentialAt(cmd, x, y, z) - this->GetPotentialAt(cmd, x, y - 1, z)) /
                        cmd->GetSpacing().Y();
                } else { // Central differences
                    efieldBuff[3 * cnt + 1] =
                        (this->GetPotentialAt(cmd, x, y + 1, z) - this->GetPotentialAt(cmd, x, y - 1, z)) /
                        (2.0f * cmd->GetSpacing().Y());
                }

                // z component
                if (z == 0) { // Forward differences
                    efieldBuff[3 * cnt + 2] =
                        (this->GetPotentialAt(cmd, x, y, z + 1) - this->GetPotentialAt(cmd, x, y, z)) /
                        cmd->GetSpacing().Z();
                } else if (z == cmd->GetGridsize().Z() - 1) { // Backward differences
                    efieldBuff[3 * cnt + 2] =
                        (this->GetPotentialAt(cmd, x, y, z) - this->GetPotentialAt(cmd, x, y, z - 1)) /
                        cmd->GetSpacing().Z();
                } else { // Central differences
                    efieldBuff[3 * cnt + 2] =
                        (this->GetPotentialAt(cmd, x, y, z + 1) - this->GetPotentialAt(cmd, x, y, z - 1)) /
                        (2.0f * cmd->GetSpacing().Z());
                }
    ...
    float* efieldBuffTmp = new float[cmd->GetGridsize().X() * cmd->GetGridsize().Y() * cmd->GetGridsize().Z() * 3];

    memcpy(efieldBuffTmp, efieldBuff,
        cmd->GetGridsize().X() * cmd->GetGridsize().Y() * cmd->GetGridsize().Z() * 3 * sizeof(float));
...
#pragma omp parallel for
            ...
            for (int x = 0; x < cmd->GetGridsize().X(); x++) {
                ...
                if (myisnan(efieldBuff[3 * cnt + 0]) || myisinf(efieldBuff[3 * cnt + 0])) {
                    ...
                    if (!(myisinf(cmd->GetPointDataByIdx(0, 0)[left]))) {
                        efieldBuffTmp[3 * middle + 0] = ((float*)(cmd->GetPointDataByIdx(0, 0))[middle] -
                                                            (float*)(cmd->GetPointDataByIdx(0, 0))[left]) /
                                                        cmd->GetSpacing().X();
                    } else if (!(myisinf(cmd->GetPointDataByIdx(0, 0)[right]))) {
                        efieldBuffTmp[3 * middle + 0] =
                            ((float*)(cmd->GetPointDataByIdx(0, 0))[right] - (float*)(cmd->GetPointDataByIdx(0, 0))) /
                            cmd->GetSpacing().X();
                    } else {
                ...
                if (myisnan(efieldBuff[3 * cnt + 1]) || myisinf(efieldBuff[3 * cnt + 1])) {
                    ...
                    if (!(myisinf(cmd->GetPointDataByIdx(0, 0)[down]))) {
                        efieldBuffTmp[3 * middle + 1] =
                            (cmd->GetPointDataByIdx(0, 0)[middle] - cmd->GetPointDataByIdx(0, 0)[down]) /
                            cmd->GetSpacing().Y();
                    } else if (!(myisinf(cmd->GetPointDataByIdx(0, 0)[up]))) {
                        efieldBuffTmp[3 * middle + 1] =
                            (cmd->GetPointDataByIdx(0, 0)[up] - cmd->GetPointDataByIdx(0, 0)[middle]) /
                            cmd->GetSpacing().Y();
                    } else {
                ...
                if (myisnan(efieldBuff[3 * cnt + 2]) || myisinf(efieldBuff[3 * cnt + 2])) {
                    ...
                    if (!(myisinf(cmd->GetPointDataByIdx(0, 0)[back]))) {
                        efieldBuffTmp[3 * middle + 2] =
                            (cmd->GetPointDataByIdx(0, 0)[middle] - cmd->GetPointDataByIdx(0, 0)[back]) /
                            cmd->GetSpacing().Z();
                    } else if (!(myisinf(cmd->GetPointDataByIdx(0, 0)[front]))) {
                        efieldBuffTmp[3 * middle + 2] =
                            (cmd->GetPointDataByIdx(0, 0)[front] - cmd->GetPointDataByIdx(0, 0)[middle]) /
                            cmd->GetSpacing().Z();
                    } else {
    ...
    efield.SetData(efieldBuffTmp, cmd->GetGridsize().X(), cmd->GetGridsize().Y(), cmd->GetGridsize().Z(),
        cmd->GetSpacing().X(), cmd->GetSpacing().Y(), cmd->GetSpacing().Z(), cmd->GetOrigin().X(), cmd->GetOrigin().Y(),
        cmd->GetOrigin().Z());
    ...
    float* efieldGradBuff = new float[cmd->GetGridsize().X() * cmd->GetGridsize().Y() * cmd->GetGridsize().Z() * 3];
#pragma omp parallel for
    for (int z = 0; z < cmd->GetGridsize().Z(); z++) {
        for (int y = 0; y < cmd->GetGridsize().Y(); y++) {
            for (int x = 0; x < cmd->GetGridsize().X(); x++) {
                ...
                // x component
                if (x == 0) { // Forward differences
                    efieldGradBuff[3 * cnt + 0] =
                        (efield.GetAt(x + 1, y, z).Norm() - efield.GetAt(x, y, z).Norm()) / cmd->GetSpacing().X();
                } else if (x == cmd->GetGridsize().X() - 1) { // Backward differences
                    efieldGradBuff[3 * cnt + 0] =
                        (efield.GetAt(x, y, z).Norm() - efield.GetAt(x - 1, y, z).Norm()) / cmd->GetSpacing().X();
                } else { // Central differences
                    efieldGradBuff[3 * cnt + 0] =
                        (efield.GetAt(x + 1, y, z).Norm() - efield.GetAt(x - 1, y, z).Norm()) /
                        (2.0f * cmd->GetSpacing().X());
                }

                // y component
                if (y == 0) { // Forward differences
                    efieldGradBuff[3 * cnt + 1] =
                        (efield.GetAt(x, y + 1, z).Norm() - efield.GetAt(x, y, z).Norm()) / cmd->GetSpacing().Y();
                } else if (y == cmd->GetGridsize().Y() - 1) { // Backward differences
                    efieldGradBuff[3 * cnt + 1] =
                        (efield.GetAt(x, y, z).Norm() - efield.GetAt(x, y - 1, z).Norm()) / cmd->GetSpacing().Y();
                } else { // Central differences
                    efieldGradBuff[3 * cnt + 1] =
                        (efield.GetAt(x, y + 1, z).Norm() - efield.GetAt(x, y - 1, z).Norm()) /
                        (2.0f * cmd->GetSpacing().Y());
                }

                // z component
                if (z == 0) { // Forward differences
                    efieldGradBuff[3 * cnt + 2] =
                        (efield.GetAt(x, y, z + 1).Norm() - efield.GetAt(x, y, z).Norm()) / cmd->GetSpacing().Z();
                } else if (z == cmd->GetGridsize().Z() - 1) { // Backward differences
                    efieldGradBuff[3 * cnt + 2] =
                        (efield.GetAt(x, y, z).Norm() - efield.GetAt(x, y, z - 1).Norm()) / cmd->GetSpacing().Z();
                } else { // Central differences
                    efieldGradBuff[3 * cnt + 2] =
                        (efield.GetAt(x, y, z + 1).Norm() - efield.GetAt(x, y, z - 1).Norm()) /
                        (2.0f * cmd->GetSpacing().Z());
                }
    ...
    // Init vector field (makes deep copy of the data)
    egradfield.SetData(efieldGradBuff, cmd->GetGridsize().X(), cmd->GetGridsize().Y(), cmd->GetGridsize().Z(),
        cmd->GetSpacing().X(), cmd->GetSpacing().Y(), cmd->GetSpacing().Z(), cmd->GetOrigin().X(), cmd->GetOrigin().Y(),
        cmd->GetOrigin().Z());
...
#pragma omp parallel for
        for (int cnt = 0; cnt < arrowCnt; cnt++) {
            ...
            this->gridPos[cnt][1] = cmd->GetOrigin().Y() +
                                    ((cnt / cmd->GetGridsize().X()) % cmd->GetGridsize().Y()) * cmd->GetSpacing().Y();
            this->gridPos[cnt][2] = cmd->GetOrigin().Z() +
                                    ((cnt / cmd->GetGridsize().X()) / cmd->GetGridsize().Y()) * cmd->GetSpacing().Z();
        }

#pragma omp parallel for
        for (int cnt = 0; cnt < arrowCnt; cnt++) {

            Vector<float, 3> e(this->efield0.PeekBuff()[3 * cnt + 0], this->efield0.PeekBuff()[3 * cnt + 1],
        ...
        for (int cnt = 0; cnt < arrowCnt; cnt++) {
            ...
            if ((this->gridPos[cnt][0] > this->arrowFilterXMax) || (this->gridPos[cnt][0] < this->arrowFilterXMin))
                continue;

            if ((this->gridPos[cnt][1] > this->arrowFilterYMax) || (this->gridPos[cnt][1] < this->arrowFilterYMin))
                continue;

            if ((this->gridPos[cnt][2] > this->arrowFilterZMax) || (this->gridPos[cnt][2] < this->arrowFilterZMin))
                continue;
    ...
    // Draw points
    glDrawElements(GL_POINTS, static_cast<GLsizei>(arrowVisIdx.Count()), GL_UNSIGNED_INT, arrowVisIdx.PeekElements());
...
bool ComparativeFieldTopologyRenderer::renderStreamlineBundleManual() {
    ...
    if (this->recalcStreamlinesManualSeed) {
        ...
        Vector<float, 3> seedPt =
            Vector<float, 3>(this->streamBundleSeedX, this->streamBundleSeedY, this->streamBundleSeedZ);
        ...
        if (this->efield0.IsValidGridpos(seedPt)) {
            ...
            for (float s = step; s <= rad; s += step) {
                for (float a = 0; a < 2 * M_PI; a += alpha) {
                    ...
                    this->streamlinesManualSeed0.Last()->IntegrateRK4(seedPt + normal * s, // Starting point
            ...
            // Center
            this->streamlinesManualSeed0.Add(new Streamline());
            this->streamlinesManualSeed0.Last()->IntegrateRK4(seedPt, // Starting point
                this->efield0,                                        // Vector field
                this->streamlineMaxStepsManual,                       // Maximum length
                this->streamlineStepsizeManual / 10.0f,               // Step size
                this->streamlineEpsManual,                            // Epsilon
            ...
            Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: ... #0 done, number of streamlines %u (%f s)",
                this->ClassName(), this->streamlinesManualSeed0.Count(),
                (double(clock() - t) / double(CLOCKS_PER_SEC))); // DEBUG
            t = clock();                                         // DEBUG
            ...
            for (float s = step; s <= rad; s += step) {
                for (float a = 0; a < 2 * M_PI; a += alpha) {
                    ...
                    this->streamlinesManualSeed1.Last()->IntegrateRK4(seedPt + normal * s, // Starting point
            ...
            // Center
            this->streamlinesManualSeed1.Add(new Streamline());
            this->streamlinesManualSeed1.Last()->IntegrateRK4(seedPt, // Starting point
                this->efield1,                                        // Vector field
                this->streamlineMaxStepsManual,                       // Maximum length
                this->streamlineStepsizeManual / 10.0f,               // Step size
                this->streamlineEpsManual,                            // Epsilon
            ...
            Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: ... #1 done, number of streamlines %u (%f s)",
                this->ClassName(), this->streamlinesManualSeed1.Count(),
                (double(clock() - t) / double(CLOCKS_PER_SEC))); // DEBUG
    ...
    if (this->toggleStreamlinesManual) {
        ...
        glUniform1iARB(
            this->streamlineShader.ParameterLocation("shading"), static_cast<int>(this->streamlineShadingManual));
        ...
        for (unsigned int cnt = 0; cnt < this->streamlinesManualSeed0.Count(); cnt++) {
            ...
            glVertexAttribPointerARB(
                vertexPos, 3, GL_FLOAT, GL_FALSE, 0, this->streamlinesManualSeed0[cnt]->PeekVertexArr());
            glVertexAttribPointerARB(
                vertexTangent, 3, GL_FLOAT, GL_FALSE, 0, this->streamlinesManualSeed0[cnt]->PeekTangentArr());
            glVertexAttribPointerARB(
                vertexTC, 3, GL_FLOAT, GL_FALSE, 0, this->streamlinesManualSeed0[cnt]->PeekTexCoordArr());
            // Draw points
        ...
        for (unsigned int cnt = 0; cnt < this->streamlinesManualSeed1.Count(); cnt++) {
            // Set attribute pointers
            glVertexAttribPointerARB(
                vertexPos, 3, GL_FLOAT, GL_FALSE, 0, this->streamlinesManualSeed1[cnt]->PeekVertexArr());
            glVertexAttribPointerARB(
                vertexTangent, 3, GL_FLOAT, GL_FALSE, 0, this->streamlinesManualSeed1[cnt]->PeekTangentArr());
            glVertexAttribPointerARB(
                vertexTC, 3, GL_FLOAT, GL_FALSE, 0, this->streamlinesManualSeed1[cnt]->PeekTexCoordArr());
            // Draw points
        ...
        GLenum err = glGetError();
        if (err != GL_NO_ERROR) {
            Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR, "%s: glError in 'renderStreamlineBundleManual' %s \n",
                this->ClassName(), gluErrorString(err));
            return false;
        }
    }
...
bool ComparativeFieldTopologyRenderer::renderStreamlinesRoi(
    const protein_calls::VTIDataCall* cmd0, const protein_calls::VTIDataCall* cmd1) {
    using namespace vislib;
    using namespace vislib::math;
    ...
    if (this->recalcStreamlines) {
        ...
        SetGridParams(make_uint3(this->efield0.GetDim().X(), this->efield0.GetDim().Y(), this->efield0.GetDim().Z()),
            ...
            make_float3(
                this->efield0.GetSpacing().X(), this->efield0.GetSpacing().Y(), this->efield0.GetSpacing().Z()));
        ...
        UpdatePositionRK4(this->efield0.PeekBuff(),
            make_uint3(this->efield0.GetDim().X(), this->efield0.GetDim().Y(), this->efield0.GetDim().Z()),
            seedPointEnd0, static_cast<uint>(this->seedPosStart0.Count()) / 3, this->streamlineMaxSteps);

        Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: ... CUDA #0 (forward) done (%f s)", this->ClassName(),
            (double(clock() - t) / double(CLOCKS_PER_SEC))); // DEBUG
        t = clock();                                         // DEBUG
...
#pragma omp parallel for
        for (int cnt = 0; cnt < static_cast<int>(this->seedPosStart0.Count() / 3); cnt++) {
            this->seedCellEnd0[3 * cnt + 0] = static_cast<unsigned int>(
                (seedPointEnd0[3 * cnt + 0] - this->efield0.GetOrg().X()) / this->efield0.GetSpacing().X());
            this->seedCellEnd0[3 * cnt + 1] = static_cast<unsigned int>(
                (seedPointEnd0[3 * cnt + 1] - this->efield0.GetOrg().Y()) / this->efield0.GetSpacing().Y());
            this->seedCellEnd0[3 * cnt + 2] = static_cast<unsigned int>(
                (seedPointEnd0[3 * cnt + 2] - this->efield0.GetOrg().Z()) / this->efield0.GetSpacing().Z());
        }
        ...
        UpdatePositionRK4(this->efield0.PeekBuff(),
            make_uint3(this->efield0.GetDim().X(), this->efield0.GetDim().Y(), this->efield0.GetDim().Z()),
            seedPointEnd0, static_cast<uint>(this->seedPosStart0.Count()) / 3, this->streamlineMaxSteps, true);

        Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: ... CUDA #0 (backward) done (%f s)", this->ClassName(),
            (double(clock() - t) / double(CLOCKS_PER_SEC))); // DEBUG
        t = clock();                                         // DEBUG
...
#pragma omp parallel for
        for (int cnt = 0; cnt < static_cast<int>(this->seedPosStart0.Count() / 3); cnt++) {
            this->seedCellEndBackward0[3 * cnt + 0] = static_cast<unsigned int>(
                (seedPointEnd0[3 * cnt + 0] - this->efield0.GetOrg().X()) / this->efield0.GetSpacing().X());
            this->seedCellEndBackward0[3 * cnt + 1] = static_cast<unsigned int>(
                (seedPointEnd0[3 * cnt + 1] - this->efield0.GetOrg().Y()) / this->efield0.GetSpacing().Y());
            this->seedCellEndBackward0[3 * cnt + 2] = static_cast<unsigned int>(
                (seedPointEnd0[3 * cnt + 2] - this->efield0.GetOrg().Z()) / this->efield0.GetSpacing().Z());
        }
        ...
        SetGridParams(make_uint3(this->efield1.GetDim().X(), this->efield1.GetDim().Y(), this->efield1.GetDim().Z()),
            ...
            make_float3(this->bbox.GetRight(), this->bbox.GetTop(), this->bbox.GetFront()),
            make_float3(
                this->efield1.GetSpacing().X(), this->efield1.GetSpacing().Y(), this->efield1.GetSpacing().Z()));
        SetStreamlineParams(this->streamlineStepsize / 10.0f, this->streamlineMaxSteps);
        ...
        UpdatePositionRK4(this->efield1.PeekBuff(),
            make_uint3(this->efield1.GetDim().X(), this->efield1.GetDim().Y(), this->efield1.GetDim().Z()),
            seedPointEnd1, static_cast<uint>(this->seedPosStart1.Count()) / 3, this->streamlineMaxSteps);

        Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: ... CUDA #1 (forward) done (%f s)", this->ClassName(),
            (double(clock() - t) / double(CLOCKS_PER_SEC))); // DEBUG
        t = clock();                                         // DEBUG
...
#pragma omp parallel for
        for (int cnt = 0; cnt < static_cast<int>(this->seedPosStart1.Count() / 3); cnt++) {
            this->seedCellEnd1[3 * cnt + 0] = static_cast<unsigned int>(
                (seedPointEnd1[3 * cnt + 0] - this->efield1.GetOrg().X()) / this->efield1.GetSpacing().X());
            this->seedCellEnd1[3 * cnt + 1] = static_cast<unsigned int>(
                (seedPointEnd1[3 * cnt + 1] - this->efield1.GetOrg().Y()) / this->efield1.GetSpacing().Y());
            this->seedCellEnd1[3 * cnt + 2] = static_cast<unsigned int>(
                (seedPointEnd1[3 * cnt + 2] - this->efield1.GetOrg().Z()) / this->efield1.GetSpacing().Z());
        ...
        UpdatePositionRK4(this->efield1.PeekBuff(),
            make_uint3(this->efield1.GetDim().X(), this->efield1.GetDim().Y(), this->efield1.GetDim().Z()),
            seedPointEnd1, static_cast<uint>(this->seedPosStart1.Count()) / 3, this->streamlineMaxSteps, true);

        Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: ... CUDA #1 (backward) done (%f s)", this->ClassName(),
            (double(clock() - t) / double(CLOCKS_PER_SEC))); // DEBUG
        t = clock();                                         // DEBUG
...
#pragma omp parallel for
        for (int cnt = 0; cnt < static_cast<int>(this->seedPosStart1.Count() / 3); cnt++) {
            this->seedCellEndBackward1[3 * cnt + 0] = static_cast<unsigned int>(
                (seedPointEnd1[3 * cnt + 0] - this->efield1.GetOrg().X()) / this->efield1.GetSpacing().X());
            this->seedCellEndBackward1[3 * cnt + 1] = static_cast<unsigned int>(
                (seedPointEnd1[3 * cnt + 1] - this->efield1.GetOrg().Y()) / this->efield1.GetSpacing().Y());
            this->seedCellEndBackward1[3 * cnt + 2] = static_cast<unsigned int>(
                (seedPointEnd1[3 * cnt + 2] - this->efield1.GetOrg().Z()) / this->efield1.GetSpacing().Z());
        ...
        for (unsigned int cnt = 0; cnt < this->seedCellEnd0.Count() / 3; cnt++) {
            this->cellEndpoints0[cmd0->GetGridsize().X() * (cmd0->GetGridsize().Y() * this->seedCellEnd0[3 * cnt + 2] +
                                                               this->seedCellEnd0[3 * cnt + 1]) +
                                 this->seedCellEnd0[3 * cnt + 0]]
                .Add(cnt);
        }
        for (unsigned int cnt = 0; cnt < this->seedCellEndBackward0.Count() / 3; cnt++) {
            this->cellEndpoints0[cmd0->GetGridsize().X() *
                                     (cmd0->GetGridsize().Y() * this->seedCellEndBackward0[3 * cnt + 2] +
                                         this->seedCellEndBackward0[3 * cnt + 1]) +
                                 this->seedCellEndBackward0[3 * cnt + 0]]
                .Add(cnt);
        }
        ...
        for (unsigned int cnt = 0; cnt < this->seedCellEnd1.Count() / 3; cnt++) {
            this->cellEndpoints1[cmd1->GetGridsize().X() * (cmd1->GetGridsize().Y() * this->seedCellEnd1[3 * cnt + 2] +
                                                               this->seedCellEnd1[3 * cnt + 1]) +
                                 this->seedCellEnd1[3 * cnt + 0]]
                .Add(cnt);
        }
        for (unsigned int cnt = 0; cnt < this->seedCellEndBackward1.Count() / 3; cnt++) {

            this->cellEndpoints1[cmd1->GetGridsize().X() *
                                     (cmd1->GetGridsize().Y() * this->seedCellEndBackward1[3 * cnt + 2] +
                                         this->seedCellEndBackward1[3 * cnt + 1]) +
                                 this->seedCellEndBackward1[3 * cnt + 0]]
                .Add(cnt);
        }
            ...
            if ((this->efield0.GetCritPoint(cnt).GetType() == VecField3f::CritPoint::SOURCE) ||
                ...
                if ((this->neighbours0[cnt].Count() == 0) || (this->streamlinesShowAll)) {
                    ...
                    unsigned int cellIdx =
                        cmd0->GetGridsize().X() * (cmd0->GetGridsize().Y() * cellId.Z() + cellId.Y()) + cellId.X();
                    ...
                    for (unsigned int e = 0; e < this->cellEndpoints0[cellIdx].Count(); e++) {
                        ...
                        if (!usedSeedPoints[this->cellEndpoints0[cellIdx][e]]) {
                            ...
                            this->streamlines0.Last()->IntegrateRK4(posStart, // Starting point
                                this->efield0,                                // Vector field
        ...
        Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: ... done, %u streamlines in #0 (%f s)", this->ClassName(),
        ...
        for (int cnt = 0; cnt < static_cast<int>(this->efield1.GetCritPointCount()); cnt++) {
            ...
            if ((this->efield1.GetCritPoint(cnt).GetType() == VecField3f::CritPoint::SOURCE) ||
                ...
                if ((this->neighbours1[cnt].Count() == 0) || (this->streamlinesShowAll)) {
                    ...
                    Vector<unsigned int, 3> cellId = this->efield1.GetCritPoint(cnt).GetCellId();
                    unsigned int cellIdx =
                        cmd1->GetGridsize().X() * (cmd1->GetGridsize().Y() * cellId.Z() + cellId.Y()) + cellId.X();
                    ...
                    for (unsigned int e = 0; e < this->cellEndpoints1[cellIdx].Count(); e++) {
                        ...
                        if (!usedSeedPoints[this->cellEndpoints1[cellIdx][e]]) {
                            ...
                            this->streamlines1.Last()->IntegrateRK4(posStart, // Starting point
                                this->efield1,                                // Vector field
                                this->streamlineMaxSteps,                     // Maximum length
                                this->streamlineStepsize / 10.0f,             // Step size
                                this->streamlineEps,                          // Epsilon
        ...
        Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: ... done, %u streamlines in #1 (%f s)", this->ClassName(),
            (double(clock() - t) / double(CLOCKS_PER_SEC)),
    ...
    if (toggleStreamlines) {
        ...
        for (unsigned int cnt = 0; cnt < this->streamlines0.Count(); cnt++) {
            ...
            glVertexAttribPointerARB(
                vertexTangent, 3, GL_FLOAT, GL_FALSE, 0, this->streamlines0[cnt]->PeekTangentArr());
        ...
        for (unsigned int cnt = 0; cnt < this->streamlines1.Count(); cnt++) {
            ...
            glVertexAttribPointerARB(
                vertexTangent, 3, GL_FLOAT, GL_FALSE, 0, this->streamlines1[cnt]->PeekTangentArr());
        ...
        GLenum err = glGetError();
        if (err != GL_NO_ERROR) {
            Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR, "%s: glError in 'renderStreamlinesRoi' %s \n", this->ClassName(),
                gluErrorString(err));
            return false;
        }
    }
...
void ComparativeFieldTopologyRenderer::updateParams() {
    ...
    if (this->streamlineShadingSlot.IsDirty()) {
        this->streamlineShading =
            static_cast<StreamlineShading>(this->streamlineShadingSlot.Param<core::param::EnumParam>()->Value());
    ...
    if (this->streamlineShadingManualSlot.IsDirty()) {
        this->streamlineShadingManual =
            static_cast<StreamlineShading>(this->streamlineShadingManualSlot.Param<core::param::EnumParam>()->Value());
