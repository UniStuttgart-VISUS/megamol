...
MoleculeCudaSESRenderer::MoleculeCudaSESRenderer(void)
        ...
        , triangleVBO(0)
        , triangleVertex(0)
        , visibleTriangleFBO(0)
        , visibleTriangleColor(0)
        , visibleTriangleDepth(0)
        , visibilityTexVBO(0)
        , query(0)
        , adjacentTriangleFBO(0)
        , adjacentTriangleColor(0)
        , adjacentTriangleDepth(0)
        , adjacentTriangleVertex(0)
        , adjacentTriangleVBO(0)
        , adjacentTriangleTexVBO(0)
        , singTexData(0)
        , singTexCoords(0)
        , singTex(0)
        , probeVoxelMap(0)
        ...
        , cudaTexResource(0)
        , visTriaTestVerts(0)
        , cudaVisTriaVboResource(0)
        , torusVbo(0)
        , m_dProbePos(0)
        , m_dSortedProbePos(0)
        , m_dProbeNeighbors(0)
        , m_dProbeNeighborCount(0)
        , cudaSTriaResource(0)
        , m_dGridProbeHash(0)
        , m_dGridProbeIndex(0) {
...
void protein_cuda::MoleculeCudaSESRenderer::release(void) {
    ...
    if (this->vicinityTable)
        delete[] this->vicinityTable;
    if (this->voxelMap)
        delete[] this->voxelMap;
    if (this->atomPosRSGS)
        delete[] this->atomPosRSGS;
    if (this->atomColRSGS)
        delete[] this->atomColRSGS;
    ...
    if (this->adjacentTriangleVertex)
        delete[] this->adjacentTriangleVertex;
...
bool MoleculeCudaSESRenderer::create(void) {
    ...
    try {
        ...
        Log::DefaultLog.WriteMsg(
            Log::LEVEL_ERROR, "%s: Unable to create sphere Id writing shader: %s\n", this->ClassName(), e.GetMsgA());
        return false;
    }
    ...
    try {
        ...
        Log::DefaultLog.WriteMsg(
            Log::LEVEL_ERROR, "%s: Unable to create point drawing shader: %s\n", this->ClassName(), e.GetMsgA());
        return false;
    }
    ...
    try {
        ...
        Log::DefaultLog.WriteMsg(
            Log::LEVEL_ERROR, "%s: Unable to create RS triangle drawing shader: %s\n", this->ClassName(), e.GetMsgA());
        return false;
    }
    ...
    try {
        if (!this->drawVisibleTriangleShader.Compile(
        ...
        Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR,
            "%s: Unable to create visible triangle drawing geometry shader: %s\n", this->ClassName(), e.GetMsgA());
        return false;
    }
    ...
    try {
        if (!this->visibleTriangleIdxShader.Compile(
                vertSrc.Code(), vertSrc.Count(), geomSrc.Code(), geomSrc.Count(), fragSrc.Code(), fragSrc.Count())) {
            throw vislib::Exception("Generic creation failure", __FILE__, __LINE__);
        } else {
            // set GL_POINTS primitives as INPUT
        ...
        Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR,
            "%s: Unable to create visible triangle index emission geometry shader: %s\n", this->ClassName(),
            e.GetMsgA());
        return false;
    }
...
bool MoleculeCudaSESRenderer::initCuda(MolecularDataCall* protein, uint gridDim, core_gl::view::CallRender3DGL* cr3d) {
    ...
    float3 worldSize = make_float3(protein->AccessBoundingBoxes().ObjectSpaceBBox().Width(),
    ...
    this->params.worldOrigin = make_float3(protein->AccessBoundingBoxes().ObjectSpaceBBox().GetLeftBottomBack().GetX(),
...
void MoleculeCudaSESRenderer::CreateVisibilityFBO(unsigned int maxSize) {
    ...
    glTexImage2D(
        GL_TEXTURE_2D, 0, GL_RGBA32F, this->visibilityTexWidth, this->visibilityTexHeight, 0, GL_RGBA, GL_FLOAT, NULL);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32, this->visibilityTexWidth, this->visibilityTexHeight, 0,
    ...
    glBufferData(GL_PIXEL_PACK_BUFFER_EXT, this->visibilityTexWidth * this->visibilityTexHeight * 4 * sizeof(float),
...
void MoleculeCudaSESRenderer::CreateTriangleFBO(unsigned int atomCount, unsigned int vicinityCount) {
    ...
    if (!this->triangleFBO) {
        ...
        glBindTexture(GL_TEXTURE_2D, this->triangleColor0);
        glTexImage2D(
            GL_TEXTURE_2D, 0, GL_RGBA32F, vicinityCount * vicinityCount, atomCount, 0, GL_RGBA, GL_FLOAT, NULL);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32, vicinityCount * vicinityCount, atomCount, 0,
            GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
...
void MoleculeCudaSESRenderer::CreateVisibleTriangleFBO(unsigned int atomCount, unsigned int vicinityCount) {
    ...
    if (!glIsFramebufferEXT(this->visibleTriangleFBO)) {
        ...
        glBindTexture(GL_TEXTURE_2D, this->visibleTriangleColor);
        glTexImage2D(
            GL_TEXTURE_2D, 0, GL_RGBA16F, vicinityCount * vicinityCount, atomCount, 0, GL_RGBA, GL_FLOAT, NULL);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
        ...
        glFramebufferTexture2DEXT(
            GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, this->visibleTriangleColor, 0);
        // init depth texture
        glBindTexture(GL_TEXTURE_2D, this->visibleTriangleDepth);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, vicinityCount * vicinityCount, atomCount, 0,
            GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
        ...
        glTexParameterf(GL_TEXTURE_2D, GL_DEPTH_TEXTURE_MODE, GL_LUMINANCE);
        glFramebufferTexture2DEXT(
            GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_TEXTURE_2D, this->visibleTriangleDepth, 0);
        glBindTexture(GL_TEXTURE_2D, 0);
        glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0);
    }
}
    ...
    */
    ...
    for (cnt = 0; cnt < protein->AtomCount(); ++cnt) {
        ...
        if (this->visibleAtomMask[cnt] > 0.5f) {
            ...
            this->visibleAtomsList[this->visibleAtomCount * 4 + 3] =
...
void MoleculeCudaSESRenderer::ComputeVisibleAtomsVicinityTable(MolecularDataCall* protein) {
    ...
    for (cnt = 0; cnt < protein->AtomCount(); ++cnt) {
        ...
        xCoord = (unsigned int)std::min(
            maxXId, (unsigned int)std::max(0, (int)floorf((tmpVec.GetX() - bBox.Left()) / float(voxelLength))));
        yCoord = (unsigned int)std::min(
            maxYId, (unsigned int)std::max(0, (int)floorf((tmpVec.GetY() - bBox.Bottom()) / float(voxelLength))));
        zCoord = (unsigned int)std::min(
            maxZId, (unsigned int)std::max(0, (int)floorf((tmpVec.GetZ() - bBox.Back()) / float(voxelLength))));
    ...
    for (cnt1 = 0; cnt1 < protein->AtomCount(); ++cnt1) {
        ...
        if (this->visibleAtomMask[cnt1] < 0.5)
            continue;
                ...
                for (cntZ = ((zId > 0) ? (-1) : 0); cntZ < ((zId < (int)maxZId) ? 2 : 1); ++cntZ) {
                    ...
                    for (cnt = 0; cnt < this->voxelMap[firstIdx]; ++cnt) {
                        ...
                        if (distance < threshold && vicinityCounter <= MAX_ATOM_VICINITY) {
                            this->vicinityTable[cnt1 * (MAX_ATOM_VICINITY + 1) * 4 + 4 * vicinityCounter + 0] =
                                pos.GetX();
                            this->vicinityTable[cnt1 * (MAX_ATOM_VICINITY + 1) * 4 + 4 * vicinityCounter + 1] =
                                pos.GetY();
                            this->vicinityTable[cnt1 * (MAX_ATOM_VICINITY + 1) * 4 + 4 * vicinityCounter + 2] =
                                pos.GetZ();
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, (MAX_ATOM_VICINITY + 1), protein->AtomCount(), 0, GL_RGBA, GL_FLOAT,
        this->vicinityTable);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
...
void MoleculeCudaSESRenderer::ComputeVicinityTableCUDA(MolecularDataCall* protein) {
    ...
    if (this->cudaInitalized) {
        ...
        // calculate grid hash
        calcHash(m_dGridParticleHash, m_dGridParticleIndex, m_dPos, this->numAtoms);
        ...
        reorderDataAndFindCellStart(m_dCellStart, m_dCellEnd, m_dSortedPos, m_dGridParticleHash, m_dGridParticleIndex,
            m_dPos, this->numAtoms, this->numGridCells);

        // count neighbors of all atoms
        countNeighbors2(m_dNeighborCount, m_dNeighbors, m_dSortedPos, m_dGridParticleIndex, m_dCellStart, m_dCellEnd,
            this->numAtoms, MAX_ATOM_VICINITY, this->numGridCells);
...
void MoleculeCudaSESRenderer::CreateAdjacentTriangleFBO(unsigned int atomCount, unsigned int vicinityCount) {
    ...
    if (!this->adjacentTriangleFBO) {
        ...
        glBindTexture(GL_TEXTURE_2D, 0);
        glFramebufferTexture2DEXT(
            GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, this->adjacentTriangleColor, 0);
        // init depth texture
        glBindTexture(GL_TEXTURE_2D, this->adjacentTriangleDepth);
        glTexImage2D(
            GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32, vicinityCount, atomCount, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
        glTexParameterf(GL_TEXTURE_2D, GL_DEPTH_TEXTURE_MODE, GL_LUMINANCE);
        glFramebufferTexture2DEXT(
            GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_TEXTURE_2D, this->adjacentTriangleDepth, 0);
        glBindTexture(GL_TEXTURE_2D, 0);
        glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0);
        ...
        glBufferData(
            GL_PIXEL_PACK_BUFFER_EXT, atomCount * vicinityCount * 4 * sizeof(float), NULL, GL_DYNAMIC_DRAW_ARB);
        glBindBuffer(GL_PIXEL_PACK_BUFFER_EXT, 0);
    }
}
...
void MoleculeCudaSESRenderer::FindAdjacentTrianglesCUDA(MolecularDataCall* mol) {
    ...
    if (!glIsBuffer(this->visibilityPbo)) {
        ...
        glBufferData(GL_PIXEL_UNPACK_BUFFER, mol->AtomCount() * MAX_ATOM_VICINITY * MAX_ATOM_VICINITY * sizeof(float),
            0, GL_DYNAMIC_DRAW);
    ...
    if (!cudaTexResource) {
        checkCudaErrors(cudaGraphicsGLRegisterImage(
            &cudaTexResource, this->visibleTriangleColor, GL_TEXTURE_2D, cudaGraphicsMapFlagsReadOnly));
    }
    ...
    findAdjacentTrianglesCuda(ptr, in_array, m_dPoint1, m_dProbePosTable, m_dNeighborCount, m_dNeighbors, m_dSortedPos,
        m_dVisibleAtoms, m_dVisibleAtomsId, mol->AtomCount(), this->visibleAtomCount, MAX_ATOM_VICINITY);
    ...
    glBindTexture(GL_TEXTURE_2D, this->visibleTriangleColor);
    glTexSubImage2D(
        GL_TEXTURE_2D, 0, 0, 0, MAX_ATOM_VICINITY * MAX_ATOM_VICINITY, mol->AtomCount(), GL_RED, GL_FLOAT, NULL);
    glBindTexture(GL_TEXTURE_2D, 0);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
    CHECK_FOR_OGL_ERROR();
}
...
void MoleculeCudaSESRenderer::CreateTransformFeedbackVBO(MolecularDataCall* mol) {
    ...
    if (glIsBuffer(this->sphericalTriaVBO))
        return;
...
void MoleculeCudaSESRenderer::CreateSingularityTextureCuda(MolecularDataCall* mol, unsigned int numProbes) {
    ...
    if (!this->cudaSTriaResource)
        checkCudaErrors(
            cudaGraphicsGLRegisterBuffer(&this->cudaSTriaResource, this->sTriaVbo, cudaGraphicsMapFlagsReadOnly));
    float4* outSTriaPtr;
    size_t num_bytes;
    ...
    // calculate grid hash
    calcHash(m_dGridProbeHash, m_dGridProbeIndex, m_dProbePos, numProbes);
    ...
    reorderDataAndFindCellStart(m_dCellStart, m_dCellEnd, m_dSortedProbePos, m_dGridProbeHash, m_dGridProbeIndex,
        m_dProbePos, numProbes, this->numGridCells);
    ...
    countProbeNeighbors(
        ...
        coordsPtr,
        //m_dProbeNeighbors,
        ptr, m_dSortedProbePos, m_dGridProbeIndex, m_dCellStart, m_dCellEnd, numProbes, MAX_PROBE_VICINITY,
        this->numGridCells);
...
void MoleculeCudaSESRenderer::RenderVisibleAtomsGPU(MolecularDataCall* protein) {
    ...
    for (unsigned int cnt = 0; cnt < this->visibleAtomCount; ++cnt) {
        ...
        glVertex4f(positions[atomId * 3 + 0], positions[atomId * 3 + 1], positions[atomId * 3 + 2],
...
void MoleculeCudaSESRenderer::RenderSESCuda(MolecularDataCall* mol, unsigned int primitiveCount) {
    ...
    for (unsigned int cnt = 0; cnt < mol->AtomCount(); ++cnt) {
        glVertex4f(positions[cnt * 3 + 0], positions[cnt * 3 + 1], positions[cnt * 3 + 2],
...
void MoleculeCudaSESRenderer::ComputeRSCuda(MolecularDataCall* mol) {
    ...
    if (!m_dPoint1) {
        ...
        allocateArray(
            (void**)&m_dProbePosTable, mol->AtomCount() * MAX_ATOM_VICINITY * MAX_ATOM_VICINITY * 4 * sizeof(float));
    ...
    // count neighbors of all atoms
    computeReducedSurfaceCuda(m_dPoint1,
        //m_dPoint2,
        //m_dPoint3,
        m_dProbePosTable, m_dNeighborCount, m_dNeighbors, m_dSortedPos, m_dGridParticleIndex, m_dVisibleAtoms,
        m_dVisibleAtomsId, mol->AtomCount(), this->visibleAtomCount, MAX_ATOM_VICINITY);
...
void MoleculeCudaSESRenderer::RenderTrianglesCuda(MolecularDataCall* mol) {
    ...
    if (!glIsBuffer(this->triangleVBO)) {
        ...
        checkCudaErrors(
            cudaGraphicsGLRegisterBuffer(&this->cudaVboResource, this->triangleVBO, cudaGraphicsMapFlagsWriteDiscard));
    }
    ...
    computeTriangleVBOCuda(dptr, this->m_dPoint1,
        ...
        m_dSortedPos, m_dVisibleAtoms, mol->AtomCount(), this->visibleAtomCount, MAX_ATOM_VICINITY, 0);
...
void MoleculeCudaSESRenderer::RenderTrianglesCuda2(MolecularDataCall* mol) {
    // element count (3 vertices + 3 colors ) * 3 floats per triangle
    unsigned int count = MAX_ATOM_VICINITY * MAX_ATOM_VICINITY * std::min(mol->AtomCount(), 512U) * 6 * 3;
    // write VBO for fast drawing, if necessary
    if (!glIsBuffer(this->triangleVBO)) {
        // generate triangle VBO
        glGenBuffers(1, &this->triangleVBO);
    ...
    for (unsigned int cnt = 0; cnt < numRuns; ++cnt) {
        ...
        computeTriangleVBOCuda(dptr, this->m_dPoint1,
            //this->m_dPoint2, this->m_dPoint3,
            m_dSortedPos, m_dVisibleAtoms, mol->AtomCount(), std::min(this->visibleAtomCount, 512U), MAX_ATOM_VICINITY,
            cnt * 512U);
    ...
    */
    ...
    if (!this->cudaVisTriaVboResource)
        checkCudaErrors(cudaGraphicsGLRegisterBuffer(
            &this->cudaVisTriaVboResource, this->sphericalTriaVBO, cudaGraphicsMapFlagsReadOnly));
    ...
    if (!glIsBuffer(this->torusVbo)) {
        ...
        // register the vertex buffer object with CUDA
        checkCudaErrors(cudaGraphicsGLRegisterBuffer(
            &this->cudaTorusVboResource, this->torusVbo, cudaGraphicsMapFlagsWriteDiscard));
    }
    ...
    if (!glIsBuffer(this->sTriaVbo)) {
        ...
        // register the vertex buffer object with CUDA
        checkCudaErrors(cudaGraphicsGLRegisterBuffer(
            &this->cudaSTriaVboResource, this->sTriaVbo, cudaGraphicsMapFlagsWriteDiscard));
    }
    ...
    checkCudaErrors(
        cudaGraphicsResourceGetMappedPointer((void**)&outTorusVboPtr, &num_bytes, this->cudaTorusVboResource));
    // map OpenGL buffer object for writing from CUDA
    checkCudaErrors(cudaGraphicsMapResources(1, &this->cudaSTriaVboResource, 0));
    checkCudaErrors(
        cudaGraphicsResourceGetMappedPointer((void**)&outSTriaVboPtr, &num_bytes, this->cudaSTriaVboResource));
    // map OpenGL buffer object for reading from CUDA
    ...
    computeSESPrimiticesVBOCuda(outTorusVboPtr, outSTriaVboPtr, inVboPtr, this->m_dSortedPos, this->m_dVisibleAtoms,
        this->m_dPoint1, this->m_dProbePosTable, mol->AtomCount(), this->visibleAtomCount, MAX_ATOM_VICINITY,
        primitiveCount);
