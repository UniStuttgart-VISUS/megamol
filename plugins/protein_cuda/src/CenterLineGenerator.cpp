...
void CenterLineGenerator::SetTriangleMesh(unsigned int meshVertexCnt, float* meshVertices, unsigned int meshEdgeCnt,
    unsigned int* meshEdges, unsigned int* meshFreeEdges) {
    // temp variables
...
CenterLineGenerator::Edge* CenterLineGenerator::findEdgeNeighborInSet(Edge* edge, Edges& set, bool removeFromSet) {
    ...
    for (auto it = set.begin(); it != set.end(); ++it) {
        ...
        if (retEdge->getNode1() == node1 || retEdge->getNode1() == node2 || retEdge->getNode2() == node1 ||
            retEdge->getNode2() == node2) {
            if (removeFromSet) {
                set.erase(it);
            }
            return retEdge;
...
void CenterLineGenerator::CenterLine(
    Edges& selection, CenterLineEdges& centerLineEdges, CenterLineNodes& centerLineNodes, float minCenterLineNodeDist) {
    /*
    1. Input:
            Mesh M
            Initial Section S0
