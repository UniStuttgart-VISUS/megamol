...
void Filter::release(void) {
    ...
    if (this->atomVisibility != NULL)
        delete[] this->atomVisibility;
    if (this->atmPosProt != NULL)
        delete[] this->atmPosProt;
    if (this->isSolventAtom != NULL)
        delete[] this->isSolventAtom;

    if (this->atomPosD != NULL)
        checkCudaErrors(cudaFree(this->atomPosD));
    if (this->atomPosProtD != NULL)
        checkCudaErrors(cudaFree(this->atomPosProtD));
    if (this->atomPosProtSortedD != NULL)
        checkCudaErrors(cudaFree(this->atomPosProtSortedD));

    if (this->gridAtomHashD != NULL)
        checkCudaErrors(cudaFree(this->gridAtomHashD));
    if (this->gridAtomIndexD != NULL)
        checkCudaErrors(cudaFree(this->gridAtomIndexD));

    if (this->isSolventAtomD != NULL)
        checkCudaErrors(cudaFree(this->isSolventAtomD));

    if (this->cellStartD != NULL)
        checkCudaErrors(cudaFree(this->cellStartD));
    if (this->cellEndD != NULL)
        checkCudaErrors(cudaFree(this->cellEndD));
    ;

    if (this->atomVisibilityD != NULL)
        checkCudaErrors(cudaFree(this->atomVisibilityD));

    if (this->neighbourCellPosD != NULL)
        checkCudaErrors(cudaFree(this->neighbourCellPosD));
...
bool Filter::getData(megamol::core::Call& call) {
    ...
    megamol::protein_calls::MolecularDataCall* molOut =
    ...
    // Interpolate and filter if calltime has changed
        ...
        if ((molIn->FrameID() + 1 < molOut->FrameCount()) && (this->interpolParam.Param<param::BoolParam>()->Value())) {
        ...
        float threshold = vislib::math::Min(molOut->AccessBoundingBoxes().ObjectSpaceBBox().Width(),
                              vislib::math::Min(molOut->AccessBoundingBoxes().ObjectSpaceBBox().Height(),
                                  molOut->AccessBoundingBoxes().ObjectSpaceBBox().Depth())) *
                          0.75f;
...
#pragma omp parallel for
        ...
        case SOLVENT:
            ...
            break;
        case SOLVENTALT:
            this->filterSolventAtomsAlt(posInter);
            break;
        }
...
bool Filter::getExtent(core::Call& call) {
    ...
    // Get extend.
    if (!(*molOut)(megamol::protein_calls::MolecularDataCall::CallForGetExtent))
        return false;
...
void Filter::updateParams(megamol::protein_calls::MolecularDataCall* mol) {
    ...
    if ((mol->AtomCount() != this->atmCnt) || this->solvRadiusParam.IsDirty() || this->gridSizeParam.IsDirty()) {
        ...
        this->params.numNeighbours = (this->params.discRange.x * 2 + 1) * (this->params.discRange.y * 2 + 1) *
                                     (this->params.discRange.z * 2 + 1);

        //Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "numNeighbours: %u",this->params.numNeighbours );

        float cellBodyDiagonal =
            sqrt(this->params.cellSize.x * this->params.cellSize.x + this->params.cellSize.y * this->params.cellSize.y +
        ...
        checkCudaErrors(
            cudaMalloc((void**)&this->atomPosProtSortedD, sizeof(float) * (this->atmCnt - this->solvAtmCnt) * 3));

        checkCudaErrors(
            cudaMalloc((void**)&this->gridAtomHashD, sizeof(unsigned int) * (this->atmCnt - this->solvAtmCnt)));
        checkCudaErrors(
            cudaMalloc((void**)&this->gridAtomIndexD, sizeof(unsigned int) * (this->atmCnt - this->solvAtmCnt)));
        ...
        for (unsigned int i = 0; i < this->params.numNeighbours; i++) {
            ...
            neighbourCellPos[i * 3 + 1] =
                ((i / this->params.discRangeWide.x) % this->params.discRangeWide.y) - this->params.discRange.y;
            neighbourCellPos[i * 3 + 2] =
                ((i / this->params.discRangeWide.x) / this->params.discRangeWide.y) - this->params.discRange.z;
        }
        ...
        checkCudaErrors(
            cudaMemcpy(this->isSolventAtomD, this->isSolventAtom, sizeof(bool) * this->atmCnt, cudaMemcpyHostToDevice));
    }
}
        ...
        for (ch = 0; ch < mol->ChainCount(); ch++) {
            ...
            for (m = mol->Chains()[ch].FirstMoleculeIndex();
                ...
                for (res = mol->Molecules()[m].FirstResidueIndex();
                    ...
                    const_cast<megamol::protein_calls::MolecularDataCall::Residue**>(mol->Residues())[res]->SetFilter(
                        0);
                    ...
                    for (at = mol->Residues()[res]->FirstAtomIndex();
                        ...
                        if (this->atomVisibility[at] == 1) {
                            const_cast<megamol::protein_calls::MolecularDataCall::Residue**>(mol->Residues())[res]
                                ->SetFilter(1);
                            break;
                        }
                    }
                    ...
                    if (mol->Residues()[res]->Filter() == 1) {
                        const_cast<megamol::protein_calls::MolecularDataCall::Molecule*>(mol->Molecules())[m].SetFilter(
                            1);
...
void Filter::filterSolventAtomsAlt(float* atomPos) {
    ...
    checkCudaErrors(cudaMemcpy(this->atomPosD, atomPos, sizeof(float) * this->atmCnt * 3, cudaMemcpyHostToDevice));

    checkCudaErrors(cudaMemcpy(this->atomPosProtD, this->atmPosProt,
        sizeof(float) * (this->atmCnt - this->solvAtmCnt) * 3, cudaMemcpyHostToDevice));
    ...
    // Calculate hash grid
    calcFilterHashGrid(
        this->gridAtomHashD, this->gridAtomIndexD, this->atomPosProtD, (this->atmCnt - this->solvAtmCnt));
    ...
    reorderFilterData(this->cellStartD, this->cellEndD, this->gridAtomHashD, this->gridAtomIndexD, this->atomPosProtD,
        this->atomPosProtSortedD, (this->atmCnt - this->solvAtmCnt));
    ...
    calcSolventVisibilityAlt(this->cellStartD, this->cellEndD, this->atomPosD, this->atomPosProtSortedD,
        this->isSolventAtomD, this->atomVisibilityD, this->neighbourCellPosD, this->atmCnt, this->params.numNeighbours);


    // Copy visibility information from device to host
    checkCudaErrors(
        cudaMemcpy(this->atomVisibility, this->atomVisibilityD, sizeof(int) * this->atmCnt, cudaMemcpyDeviceToHost));
}
...
void Filter::filterSolventAtoms(float* atomPos) {
    ...
    // Calculate visibility of solvent atoms
    calcSolventVisibility(this->cellStartD, this->cellEndD, this->atomPosD, this->atomPosProtSortedD,
        this->isSolventAtomD, this->atomVisibilityD, this->atmCnt);
