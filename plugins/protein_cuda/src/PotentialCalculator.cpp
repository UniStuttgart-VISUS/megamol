...
PotentialCalculator::PotentialCalculator()
    ...
    this->dataCalleeSlot.SetCallback(VTIDataCall::ClassName(), VTIDataCall::FunctionName(VTIDataCall::CallForGetData),
        &PotentialCalculator::getData);
    this->dataCalleeSlot.SetCallback(VTIDataCall::ClassName(), VTIDataCall::FunctionName(VTIDataCall::CallForGetExtent),
...
bool PotentialCalculator::getExtent(core::Call& call) {
    ...
    this->particleBBox.Set(dcOut->AccessBoundingBoxes().ObjectSpaceBBox().Left(),
        dcOut->AccessBoundingBoxes().ObjectSpaceBBox().Bottom(), dcOut->AccessBoundingBoxes().ObjectSpaceBBox().Back(),
        dcOut->AccessBoundingBoxes().ObjectSpaceBBox().Right(), dcOut->AccessBoundingBoxes().ObjectSpaceBBox().Top(),
    ...
    this->potentialBBox.Set(this->potentialGrid.minC[0], this->potentialGrid.minC[1], this->potentialGrid.minC[2],
        this->potentialGrid.maxC[0], this->potentialGrid.maxC[1], this->potentialGrid.maxC[2]);
...
bool PotentialCalculator::computeChargeDistribution(const MolecularDataCall* mol) {
    ...
    volSize = this->chargesGrid.size[0] * this->chargesGrid.size[1] * this->chargesGrid.size[2];
...
bool PotentialCalculator::computePotentialMap(const MolecularDataCall* mol) {
    ...
    size_t volSize = this->potentialGrid.size[0] * this->potentialGrid.size[1] * this->potentialGrid.size[2];
    ...
    case CONTINUUM_SOLVATION_POISSON_BOLTZMAN:
        break; // TODO
    case GPU_POISSON_SOLVER:
        ...
        CudaSafeCall(SolvePoissonEq(this->potentialGridSpacing,
            make_uint3(this->potentialGrid.size[0], this->potentialGrid.size[1], this->potentialGrid.size[2]),
            this->charges.Peek(), this->potential_D.Peek(), this->potential.Peek()));
 ...
 */
bool PotentialCalculator::computePotentialMapDCS(
    const MolecularDataCall* mol, float sphericalCutOff, bool usePeriodicImages) {
...
#pragma omp parallel for
    ...
    if (!CudaSafeCall(DirectCoulombSummation(this->atomData.Peek(), mol->AtomCount(), this->potential.Peek(),
            make_uint3(this->potentialGrid.size[0], this->potentialGrid.size[1], this->potentialGrid.size[2]),
 ...
 * TODO
 */
bool PotentialCalculator::computePotentialMapEwaldSum(const MolecularDataCall* mol, float beta) {

    // Volume of one unit cell of the periodic lattice
    size_t v = this->potentialGrid.size[0] * this->potentialGrid.size[1] * this->potentialGrid.size[2];
    ...
    for (int x = 0; x < static_cast<int>(this->potentialGrid.size[0]); ++x) {
        for (int y = 0; y < static_cast<int>(this->potentialGrid.size[1]); ++y) {
            for (int z = 0; z < static_cast<int>(this->potentialGrid.size[2]); ++z) {
                ...
                uint posIdx = this->potentialGrid.size[0] * (this->potentialGrid.size[1] * z + y) + x;
                ...
                printf("Grid pos %f %f %f\n", latticePos.X(), latticePos.Y(), latticePos.Z());
...
float PotentialCalculator::computeChargeWeightedStructureFactor(uint maxWaveLength, MolecularDataCall* mol) {
 ...
 */
void PotentialCalculator::getPeriodicImages(Vec3f atomPos, vislib::Array<Vec3f>& imgArr) {

    if (this->bboxType == BBOX_CUBIC) {
        imgArr.SetCount(6 * 3); // 6 images per atom

        // x+1, y, z
        imgArr.Add(Vec3f(atomPos.X() + this->particleBBox.Width(), atomPos.Y(), atomPos.Z()));
        // x-1, y, z
        imgArr.Add(Vec3f(atomPos.X() - this->particleBBox.Width(), atomPos.Y(), atomPos.Z()));
        // x, y+1, z
        imgArr.Add(Vec3f(atomPos.X(), atomPos.Y() + this->particleBBox.Height(), atomPos.Z()));
        // x, y-1, z
        imgArr.Add(Vec3f(atomPos.X(), atomPos.Y() - this->particleBBox.Height(), atomPos.Z()));
        // x, y, z+1
        imgArr.Add(Vec3f(atomPos.X(), atomPos.Y(), atomPos.Z() + this->particleBBox.Depth()));
        // x, y, z-1
        imgArr.Add(Vec3f(atomPos.X(), atomPos.Y(), atomPos.Z() - this->particleBBox.Depth()));
...
void PotentialCalculator::updateParams() {
    ...
    if (this->computationalMethodSlot.IsDirty()) {
        ...
        this->computationalMethod =
            static_cast<ComputationalMethod>(this->computationalMethodSlot.Param<core::param::EnumParam>()->Value());
    }
