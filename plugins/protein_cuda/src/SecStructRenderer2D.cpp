...
SecStructRenderer2D::SecStructRenderer2D(void)
        ...
        , showDirectConnectionsParam("Show::showDirectConnections",
              "Show the direct atom-atom connections between all atoms of the main chain.")
...
bool SecStructRenderer2D::GetExtents(mmstd_gl::CallRender2DGL& call) {
    ...
    if (!(*mdc)(MolecularDataCall::CallForGetExtent))
        return false;
    if (!(*pdc)(PlaneDataCall::CallForGetExtent))
        return false;

    if (!(*mdc)(MolecularDataCall::CallForGetData))
        return false;
    if (!(*pdc)(PlaneDataCall::CallForGetData))
        return false;
    ...
    if (mdc->DataHash() != this->lastDataHash) {
        ...
        this->bbRect.Set(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
        ...
        for (unsigned int i = 0; i < this->cAlphas.size(); i++) {
            this->cAlphas[i].pos[0] = (2.0f * static_cast<float>(this->bbRect.AspectRatio()) *
                                          (this->cAlphas[i].pos[0] - this->bbRect.Left()) / this->bbRect.Width()) -
                                      1.0f * static_cast<float>(this->bbRect.AspectRatio());
            this->cAlphas[i].pos[1] =
                (2.0f * (this->cAlphas[i].pos[1] - this->bbRect.Bottom()) / this->bbRect.Height()) - 1.0f;
        }
        ...
        glBufferData(
            GL_SHADER_STORAGE_BUFFER, this->cAlphas.size() * sizeof(CAlpha), this->cAlphas.data(), GL_DYNAMIC_COPY);
