...
void MoleculeCBCudaRenderer::ContourBuildupCPU(MolecularDataCall* mol) {
    ...
    if (this->cudaInitalized) {
        ...
        countNeighbors(m_dNeighborCount, m_dNeighbors, m_dSmallCircles, m_dSortedPos, m_dGridParticleIndex,
            m_dCellStart, m_dCellEnd, this->numAtoms, this->atomNeighborCount, this->numGridCells);
    }
        ...
        for (unsigned int jCnt = 0; jCnt < m_hNeighborCount[iCnt]; ++jCnt) {
            ...
            for (unsigned int kCnt = 0; kCnt < m_hNeighborCount[iCnt]; ++kCnt) {
                ...
                vislib::math::Vector<float, 3> h =
                    vj * (vj.Dot(vj - vk) * vk.Dot(vk)) / denom + vk * ((vk - vj).Dot(vk) * vj.Dot(vj)) / denom;
                // compute cases
...
#define DRAW_SMALL_ALL_CIRCLES 1
    ...
    for (cnt1 = 0; cnt1 < mol->AtomCount(); ++cnt1) {
        tmpVec1.Set(
            mol->AtomPositions()[cnt1 * 3 + 0], mol->AtomPositions()[cnt1 * 3 + 1], mol->AtomPositions()[cnt1 * 3 + 2]);
        for (cnt2 = 0; cnt2 < this->neighbors[cnt1].Count(); ++cnt2) {
            ...
            tmpVec3 = tmpVec2.Cross(ey);
            tmpVec3.Normalise();
            tmpVec3 *= sqrt((mol->AtomTypes()[mol->AtomTypeIndices()[cnt1]].Radius() + this->probeRadius) *
                                (mol->AtomTypes()[mol->AtomTypeIndices()[cnt1]].Radius() + this->probeRadius) -
                            tmpVec2.SquareLength());
            tmpQuat.Set(float(vislib::math::PI_DOUBLE / 50.0), tmpVec2 / tmpVec2.Length());
            for (cnt3 = 0; cnt3 < 100; ++cnt3) {
                tmpVec3 = tmpQuat * tmpVec3;
                glVertex4f(tmpVec1.X() + tmpVec2.X() + tmpVec3.X(), tmpVec1.Y() + tmpVec2.Y() + tmpVec3.Y(),
                    tmpVec1.Z() + tmpVec2.Z() + tmpVec3.Z(), 0.1f);
            }
        }
    }
    glEnd();
    ...
    for (int iCnt = 0; iCnt < static_cast<int>(mol->AtomCount()); ++iCnt) {
        ...
        for (int jCnt = 0; jCnt < this->neighbors[iCnt].Count(); ++jCnt) {
            ...
            for (int kCnt = 0; kCnt < this->neighbors[iCnt].Count(); ++kCnt) {
                ...
                vislib::math::Vector<float, 3> h =
                    vk * (vk.Dot(vk - vj) * vj.Dot(vj)) / denom + vj * ((vj - vk).Dot(vj) * vk.Dot(vk)) / denom;
                // do nothing if h is outside of the extended sphere of atom i
                if (h.Length() > R)
                    continue;
                // compute the root
            ...
            // debug rendering
            tmpVec1.Set(
                mol->AtomPositions()[iCnt * 3], mol->AtomPositions()[iCnt * 3 + 1], mol->AtomPositions()[iCnt * 3 + 2]);
            ...
            for (unsigned int aCnt = 0; aCnt < arcAngles.Count(); ++aCnt) {
                ...
                rotQuat.Set(-(arcAngles[aCnt].Second() - arcAngles[aCnt].First()) /
                                this->stepsParam.Param<param::IntParam>()->Value(),
                    vj / vj.Length());
                for (cnt3 = 0; cnt3 < this->stepsParam.Param<param::IntParam>()->Value(); ++cnt3) {
                    ...
                    glVertex4f(tmpVec1.X() + tmpVec2.X() + vj.X(), tmpVec1.Y() + tmpVec2.Y() + vj.Y(),
                        tmpVec1.Z() + tmpVec2.Z() + vj.Z(), 0.15f);
                }
            }
#endif
        } // for( int jCnt = 0; jCnt < this->neighbors[iCnt].Count(); ++jCnt )
    ...
    for (cnt1 = 0; cnt1 < mol->AtomCount(); ++cnt1) {
        if (cnt1 == 0)
            ...
            glColor4f(1.0, 0.0, 0.0, this->opacityParam.Param<param::FloatParam>()->Value());
        glVertex4f(mol->AtomPositions()[cnt1 * 3 + 0], mol->AtomPositions()[cnt1 * 3 + 1],
            mol->AtomPositions()[cnt1 * 3 + 2],
            //mol->AtomTypes()[mol->AtomTypeIndices()[cnt1]].Radius());
            //mol->AtomTypes()[mol->AtomTypeIndices()[cnt1]].Radius() * 0.1f);
            mol->AtomTypes()[mol->AtomTypeIndices()[cnt1]].Radius() + this->probeRadius);
