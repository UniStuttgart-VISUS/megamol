...
QuickSurfRaycaster::QuickSurfRaycaster(void)
        ...
        , selectedIsovals("render::selectedIsovals",
              "Semicolon seperated list of normalized isovalues we want to ray cast the isoplanes from")
        ...
        , triggerConvertButtonParam(
              "render::triggerConversion", "Button starting the conversion from volume to mesh data")
...
bool QuickSurfRaycaster::calcVolume(float3 bbMin, float3 bbMax, float* positions, int quality, float radscale,
 ...
 */
bool QuickSurfRaycaster::GetExtents(core_gl::view::CallRender3DGL& call) {
    ...
    if (mpdc != NULL) {
        ...
        call.AccessBoundingBoxes() = mdc->AccessBoundingBoxes();
        call.SetTimeFramesCount(mdc->FrameCount());
    } else if (vdc != NULL) {
        if (!(*vdc)(vdc->IDX_GET_EXTENTS))
            return false;

        call.AccessBoundingBoxes() = vdc->AccessBoundingBoxes();
        call.SetTimeFramesCount(vdc->FrameCount());
    }
 ...
 */
bool QuickSurfRaycaster::initCuda(core_gl::view::CallRender3DGL& cr3d) {
 ...
 */
bool QuickSurfRaycaster::initPixelBuffer(core_gl::view::CallRender3DGL& cr3d) {

    glm::ivec2 viewport = cr3d.GetViewResolution();
 ...
 */
bool QuickSurfRaycaster::Render(core_gl::view::CallRender3DGL& call) {
    ...
    if (!this->volume_fbo.IsValid() || this->volume_fbo.GetWidth() != call.GetFramebuffer()->getWidth() ||
        this->volume_fbo.GetHeight() != call.GetFramebuffer()->getHeight()) {

        unsigned int width = call.GetFramebuffer()->getWidth();
        unsigned int height = call.GetFramebuffer()->getHeight();
            ...
            for (unsigned int i = 0; i < mpdc->GetParticleListCount(); i++) {
                ...
                if (numColors > 0) {
                    ...
                    for (UINT64 j = 0; j < parts.GetCount();
                        ...
                        if (colorPos[numColors - 1] < concMin)
                            concMin = colorPos[numColors - 1];
                        if (colorPos[numColors - 1] > concMax)
                            concMax = colorPos[numColors - 1];
                    }
                }
            }
#endif
...
#define ALWAYS4COLORS
                    ...
                    for (int k = 0; k < 4; k++) {
                        ...
                        this->colorTable[particleCnt * 4 + 3] =
                            ((colorPos[numColors - 1] - concMin) / (concMax - concMin)) *
                            concFactorParam.Param<param::FloatParam>()->Value();
#else
            ...
            for (int i = 0; i < particleCnt; i++) {
                this->colorTable[i * 4 + 3] = ((this->colorTable[i * 4 + 3] - concMin) / (concMax - concMin)) *
                                              concFactorParam.Param<param::FloatParam>()->Value();
            }
#endif
    ...
    initCuda(call);
    initPixelBuffer(call);

    float factor = scalingFactor.Param<param::FloatParam>()->Value();

    glm::ivec2 viewport = call.GetViewResolution();
    ...
    float fovy = (float)(cam_intrinsics.fovy * M_PI / 180.0f);
    ...
    dim3 gridSize = dim3(iDivUp(viewport.x, blockSize.x), iDivUp(viewport.y, blockSize.y));
    ...
    if (recomputeVolume && !onlyVolumetric) {
        ...
        bool suc = this->calcVolume(bbMin, bbMax, particles, this->qualityParam.Param<param::IntParam>()->Value(),
            ...
            (concMax - concMin) * isoVals[0] + concMin, (concMax - concMin) * isoVals[isoVals.size() - 1] + concMin,
            true, myTime);
#else
        bool suc = this->calcVolume(bbMin, bbMax, particles, this->qualityParam.Param<param::IntParam>()->Value(),
            this->radscaleParam.Param<param::FloatParam>()->Value(),
            this->gridspacingParam.Param<param::FloatParam>()->Value(),
            1.0f, // necessary to switch off velocity scaling
            0.0f, this->maxRadius.Param<param::FloatParam>()->Value(), true, myTime);
#endif
    ...
    if (!onlyVolumetric) { // quicksurfed data
        ...
        if (this->triggerConvertButtonParam.IsDirty()) {
            ...
            convertToMesh(static_cast<float*>(volPtr), volumeExtentSmall, bbMin, bbMax,
                this->convertedIsoValueParam.Param<param::FloatParam>()->Value(), concMin, concMax);
    ...
    glUniform1i(
        textureShader.ParameterLocation("showDepth"), this->showDepthTextureParam.Param<param::BoolParam>()->Value());
    ...
    if (selectedIsovals.IsDirty() || firstTransfer) {
        isoVals.clear();
        vislib::StringA vala = selectedIsovals.Param<param::StringParam>()->Value().c_str();
