 ...
 */
bool protein_cuda::LIC::CalcLicX(UniGrid3D<float3>& grid, UniGrid3D<float>& randBuff, unsigned int streamLen,
    ...
    int gridCoordX = static_cast<int>(licBuffX.GetGridOrg().X() - grid.GetGridOrg().X() /
        ...
        for (int y = 0; y < static_cast<int>(licBuffX.GetGridDim().Y()); y++) {
            for (int z = 0; z < static_cast<int>(licBuffX.GetGridDim().Z()); z++) {
                ...
                if (licBuffXAlpha.GetAt(0, y, z) < minStreamCnt) {
                    ...
                    licBuffX.SetAt(
                        0, y, z, licBuffX.GetAt(0, y, z) + licCol / (2.0f * static_cast<float>(length) + 1.0f));
                    ...
                    for (int str = 1; str <= length; str++) {
                        ...
                        licBuffX.SetAt(0, static_cast<unsigned int>(vPosForward[str - 1].Y()),
                            static_cast<unsigned int>(vPosForward[str - 1].Z()),
                            licBuffX.GetAt(0, static_cast<unsigned int>(vPosForward[str - 1].Y()),
                                static_cast<unsigned int>(vPosForward[str - 1].Z())) +
                                forwardVal);

                        licBuffXAlpha.SetAt(0, static_cast<unsigned int>(vPosForward[str - 1].Y()),
                            static_cast<unsigned int>(vPosForward[str - 1].Z()),
                            licBuffXAlpha.GetAt(0, static_cast<unsigned int>(vPosForward[str - 1].Y()),
                                static_cast<unsigned int>(vPosForward[str - 1].Z())) +
                                1.0f);
                    ...
                    for (int str = 1; str <= length; str++) {
                        ...
                        // Compute convolution by adding/subtracting one value
                        licBuffX.SetAt(0, static_cast<unsigned int>(vPosBackward[str - 1].Y()),
                            static_cast<unsigned int>(vPosBackward[str - 1].Z()),
                            licBuffX.GetAt(0, static_cast<unsigned int>(vPosBackward[str - 1].Y()),
                                static_cast<unsigned int>(vPosBackward[str - 1].Z())) +
                                backwardVal);

                        licBuffXAlpha.SetAt(0, static_cast<unsigned int>(vPosBackward[str - 1].Y()),
                            static_cast<unsigned int>(vPosBackward[str - 1].Z()),
                            licBuffXAlpha.GetAt(0, static_cast<unsigned int>(vPosBackward[str - 1].Y()),
                                static_cast<unsigned int>(vPosBackward[str - 1].Z())) +
                                1.0f);
 ...
 */
bool protein_cuda::LIC::CalcLicY(UniGrid3D<float3>& grid, UniGrid3D<float>& randBuff, unsigned int streamLen,
    float minStreamCnt, float vecScale, vislib::math::Vector<unsigned int, 3> licDim, bool projectVec2D,
        ...
        for (int x = 0; x < static_cast<int>(licBuffY.GetGridDim().X()); x++) {
            for (int z = 0; z < static_cast<int>(licBuffY.GetGridDim().Z()); z++) {
                ...
                if (licBuffYAlpha.GetAt(x, 0, z) < minStreamCnt) {
                    ...
                    licBuffY.SetAt(
                        x, 0, z, licBuffY.GetAt(x, 0, z) + licCol / (2.0f * static_cast<float>(length) + 1.0f));
                    ...
                    for (int str = 1; str <= length; str++) {
                        ...
                        // Compute convolution by adding/subtracting one value
                        licBuffY.SetAt(static_cast<unsigned int>(vPosForward[str - 1].X()), 0,
                            static_cast<unsigned int>(vPosForward[str - 1].Z()),
                            licBuffY.GetAt(static_cast<unsigned int>(vPosForward[str - 1].X()), 0,
                                static_cast<unsigned int>(vPosForward[str - 1].Z())) +
                                forwardVal);

                        licBuffYAlpha.SetAt(static_cast<unsigned int>(vPosForward[str - 1].X()), 0,
                            static_cast<unsigned int>(vPosForward[str - 1].Z()),
                            licBuffYAlpha.GetAt(static_cast<unsigned int>(vPosForward[str - 1].X()), 0,
                                static_cast<unsigned int>(vPosForward[str - 1].Z())) +
                                1.0f);
                    ...
                    for (int str = 1; str <= length; str++) {
                        ...
                        // Compute convolution by adding/subtracting one value
                        licBuffY.SetAt(static_cast<unsigned int>(vPosBackward[str - 1].X()), 0,
                            static_cast<unsigned int>(vPosBackward[str - 1].Z()),
                            licBuffY.GetAt(static_cast<unsigned int>(vPosBackward[str - 1].X()), 0,
                                static_cast<unsigned int>(vPosBackward[str - 1].Z())) +
                                backwardVal);

                        licBuffYAlpha.SetAt(static_cast<unsigned int>(vPosBackward[str - 1].X()), 0,
                            static_cast<unsigned int>(vPosBackward[str - 1].Z()),
                            licBuffYAlpha.GetAt(static_cast<unsigned int>(vPosBackward[str - 1].X()), 0,
                                static_cast<unsigned int>(vPosBackward[str - 1].Z())) +
                                1.0f);
 ...
 */
bool protein_cuda::LIC::CalcLicZ(UniGrid3D<float3>& grid, UniGrid3D<float>& randBuff, unsigned int streamLen,
    float minStreamCnt, float vecScale, vislib::math::Vector<unsigned int, 3> licDim, bool projectVec2D,
        ...
        for (int x = 0; x < static_cast<int>(licBuffZ.GetGridDim().X()); x++) {
            for (int y = 0; y < static_cast<int>(licBuffZ.GetGridDim().Y()); y++) {
                ...
                if (licBuffZAlpha.GetAt(x, y, 0) < minStreamCnt) {
                    ...
                    licBuffZ.SetAt(
                        x, y, 0, licBuffZ.GetAt(x, y, 0) + licCol / (2.0f * static_cast<float>(length) + 1.0f));
                    ...
                    for (int str = 1; str <= length; str++) {
                        ...
                        // Compute convolution by adding/subtracting one value
                        licBuffZ.SetAt(static_cast<unsigned int>(vPosForward[str - 1].X()),
                            static_cast<unsigned int>(vPosForward[str - 1].Y()), 0,
                            licBuffZ.GetAt(static_cast<unsigned int>(vPosForward[str - 1].X()),
                                static_cast<unsigned int>(vPosForward[str - 1].Y()), 0) +
                                forwardVal);

                        licBuffZAlpha.SetAt(static_cast<unsigned int>(vPosForward[str - 1].X()),
                            static_cast<unsigned int>(vPosForward[str - 1].Y()), 0,
                            licBuffZAlpha.GetAt(static_cast<unsigned int>(vPosForward[str - 1].X()),
                                static_cast<unsigned int>(vPosForward[str - 1].Y()), 0) +
                                1.0f);
                    ...
                    for (int str = 1; str <= length; str++) {
                        ...
                        // Compute convolution by adding/subtracting one value
                        licBuffZ.SetAt(static_cast<unsigned int>(vPosBackward[str - 1].X()),
                            static_cast<unsigned int>(vPosBackward[str - 1].Y()), 0,
                            licBuffZ.GetAt(static_cast<unsigned int>(vPosBackward[str - 1].X()),
                                static_cast<unsigned int>(vPosBackward[str - 1].Y()), 0) +
                                backwardVal);

                        licBuffZAlpha.SetAt(static_cast<unsigned int>(vPosBackward[str - 1].X()),
                            static_cast<unsigned int>(vPosBackward[str - 1].Y()), 0,
                            licBuffZAlpha.GetAt(static_cast<unsigned int>(vPosBackward[str - 1].X()),
                                static_cast<unsigned int>(vPosBackward[str - 1].Y()), 0) +
                                1.0f);
 ...
 */
float protein_cuda::LIC::sampleRandBuffWrap(UniGrid3D<float>& randBuff, vislib::math::Vector<float, 3> pos) {

    vislib::math::Vector<unsigned int, 3> p(static_cast<unsigned int>(pos.X()) % randBuff.GetGridDim().X(),
 ...
 */
void protein_cuda::LIC::clampVec(
