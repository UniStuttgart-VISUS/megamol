...
SecStructFlattener::SecStructFlattener(void)
        ...
        , oxygenOffsetParam(
              "plane::preserveDirections", "Should the directions between c alpha and oxygen atoms be preserved?")
        , timestepParam("simulation::timestepSize", "The length of single time step of the force directed simulation")
        , timestepsPerFrameParam(
              "simulation::timestepsPerFrame", "The number of simulation timesteps that are performed per frame")
        , maxTimestepParam("simulation::maxTimestep",
              "The index of the last performed timestep. A negative number means infinite running.")
        , connectionSpringConstantParam(
              "simulation::springs::conSpringConstant", "The spring constant for the atom connection springs.")
        , connectionFrictionParam(
              "simulation::springs::conFriction", "The friction parameter for the atom connection springs.")
        , hbondSpringConstantParam(
              "simulation::springs::hbondSpringConstant", "The spring constant for the h bond springs.")
        , hbondFrictionParam("simulation::springs::hbondFriction", "The friction parameter for the h bond springs.")
        , repellingForceCutoffDistanceParam(
              "simulation::repelling::cutoffDistance", "Cutoff distance after which no repelling forces happen.")
        , repellingForceStrengthFactor(
              "simulation::repelling::strengthFactor", "Factor controlling the strength of the repelling forces.")
        , forceToCenterParam("simulation::forceToCenter", "Activate a force towards the center of the bounding box.")
        , forceToCenterStrengthParam(
              "simulation::forceToCenterStrength", "The strength of the force towards the center.")
    ...
    this->dataOutSlot.SetCallback(
        MolecularDataCall::ClassName(), MolecularDataCall::FunctionName(0), &SecStructFlattener::getData);
    this->dataOutSlot.SetCallback(
        MolecularDataCall::ClassName(), MolecularDataCall::FunctionName(1), &SecStructFlattener::getExtent);
    this->MakeSlotAvailable(&this->dataOutSlot);

    this->planeOutSlot.SetCallback(
        PlaneDataCall::ClassName(), PlaneDataCall::FunctionName(0), &SecStructFlattener::getPlaneData);
    this->planeOutSlot.SetCallback(
        PlaneDataCall::ClassName(), PlaneDataCall::FunctionName(1), &SecStructFlattener::getPlaneExtent);
...
void SecStructFlattener::flatten(bool transferPositions) {
    ...
    if (this->flatPlaneMode.IsDirty() || this->arbPlaneCenterParam.IsDirty() || this->arbPlaneNormalParam.IsDirty() ||
        this->oxygenOffsetParam.IsDirty() || transferPositions) {
    ...
    if (somethingDirty || transferPositions) {
        transferAtomData(this->atomPositions, this->atomPositionsSize / 3, this->cAlphaIndices.data(),
            static_cast<unsigned int>(this->cAlphaIndices.size()));
    }
}
...
SecStructFlattener::FlatPlane SecStructFlattener::getFlatPlaneByIndex(unsigned int idx) {
    switch (idx) {
    case 0:
        return FlatPlane::XY_PLANE;
    case 1:
        return FlatPlane::XZ_PLANE;
    case 2:
        return FlatPlane::YZ_PLANE;
    case 3:
        return FlatPlane::LEAST_COMMON;
    case 4:
        return FlatPlane::ARBITRARY;
    default:
        return FlatPlane::XY_PLANE;
    }
}
...
bool SecStructFlattener::getExtent(core::Call& call) {
    ...
    if (this->hbondFrictionParam.IsDirty() || this->hbondSpringConstantParam.IsDirty() ||
        this->connectionFrictionParam.IsDirty() || this->connectionSpringConstantParam.IsDirty() ||
        this->repellingForceCutoffDistanceParam.IsDirty() || this->repellingForceStrengthFactor.IsDirty() ||
        ...
        for (unsigned int i = 0; i < mdc->AtomCount(); i++) {
            ...
            elName.ToLowerCase();
            elName.TrimSpaces();
            if (elName.StartsWith("ca"))
                cAlphaIndices.push_back(i);
            if (elName.StartsWith("o") && elName.Length() == 1)
                oIndices.push_back(i); // cut out all o atoms besides the first per aminoacid
        }

        transferSpringData(mdc->AtomPositions(), mdc->AtomCount(), mdc->GetHydrogenBonds(), mdc->HydrogenBondCount(),
            this->cAlphaIndices.data(), static_cast<unsigned int>(this->cAlphaIndices.size()), this->oIndices.data(),
            static_cast<unsigned int>(this->oIndices.size()),
            this->connectionFrictionParam.Param<param::FloatParam>()->Value(),
            this->connectionSpringConstantParam.Param<param::FloatParam>()->Value(),
            this->hbondFrictionParam.Param<param::FloatParam>()->Value(),
            this->hbondSpringConstantParam.Param<param::FloatParam>()->Value(), moleculeStarts.data(),
            static_cast<unsigned int>(moleculeStarts.size()),
            this->repellingForceCutoffDistanceParam.Param<param::FloatParam>()->Value(),
            this->repellingForceStrengthFactor.Param<param::FloatParam>()->Value());
    }

    if (lastHash != mdc->DataHash() || this->flatPlaneMode.IsDirty() || this->arbPlaneCenterParam.IsDirty() ||
        this->arbPlaneNormalParam.IsDirty() || forceReset) {
...
void SecStructFlattener::runSimulation(void) {
    ...
    unsigned int numTimesteps =
        static_cast<unsigned int>(this->timestepsPerFrameParam.Param<param::IntParam>()->Value());
