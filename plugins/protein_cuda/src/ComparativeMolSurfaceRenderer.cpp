...
ComparativeMolSurfaceRenderer::ComparativeMolSurfaceRenderer(void)
        ...
        , surfaceMappingExternalForcesWeightSclSlot(
              "surfMap::externalForcesScl", "Scale factor for the external forces weighting")
        ...
        , subStepLevelSlot("subdiv::initStepSize", "Initial stepsize")
        , triggerComputeSubdiv(true)
        , cudaqsurf1(NULL)
        , cudaqsurf2(NULL)
        , triggerComputeVolume(true)
        , triggerInitPotentialTex(true)
        , triggerComputeSurfacePoints1(true)
        , triggerComputeSurfacePoints2(true)
        , triggerSurfaceMapping(true)
        , triggerRMSFit(true)
        , triggerComputeLines(true)
        , oldCalltime(-1.0)
        , qsGridDelta(2.0f) {

    /* Make data caller/callee slots available */

    this->vboSlaveSlot1.SetCallback(VBODataCall::ClassName(), VBODataCall::FunctionName(VBODataCall::CallForGetExtent),
        &ComparativeMolSurfaceRenderer::getVBOExtent);
    this->vboSlaveSlot1.SetCallback(VBODataCall::ClassName(), VBODataCall::FunctionName(VBODataCall::CallForGetData),
        &ComparativeMolSurfaceRenderer::getVBOData1);
    this->MakeSlotAvailable(&this->vboSlaveSlot1);

    this->vboSlaveSlot2.SetCallback(VBODataCall::ClassName(), VBODataCall::FunctionName(VBODataCall::CallForGetExtent),
        &ComparativeMolSurfaceRenderer::getVBOExtent);
    this->vboSlaveSlot2.SetCallback(VBODataCall::ClassName(), VBODataCall::FunctionName(VBODataCall::CallForGetData),
    ...
    this->surfMappedSpringStiffnessSlot.SetParameter(
        new core::param::FloatParam(this->surfMappedSpringStiffness, 0.01f));
    ...
    this->regExternalForcesWeightSlot.SetParameter(
        new core::param::FloatParam(this->regExternalForcesWeight, 0.0f, 1.0f));
    ...
    this->showSubdivSlot.SetParameter(new core::param::BoolParam(this->showSubdiv));
    ...
    this->subStepLevel = 1;
    this->subStepLevelSlot.SetParameter(new core::param::IntParam(this->subStepLevel));
...
bool ComparativeMolSurfaceRenderer::computeDensityMap(
    const float* atomPos, const MolecularDataCall* mol, CUDAQuickSurf* cqs) {
...
bool ComparativeMolSurfaceRenderer::create(void) {
    ...
    try {
        ...
        Log::DefaultLog.WriteError( "%s: Unable to create the ppl shader (vertex flag): %s\n",
            this->ClassName(), e.GetMsgA());
        return false;
    }
 ...
 */
bool ComparativeMolSurfaceRenderer::fitMoleculeRMS(MolecularDataCall* mol1, MolecularDataCall* mol2) {
...
#pragma omp parallel for
    ...
    for (int cnt = 0; cnt < static_cast<int>(posCnt); ++cnt) {
        this->centroid += Vec3f(this->atomPosFitted.Peek()[cnt * 3 + 0], this->atomPosFitted.Peek()[cnt * 3 + 1],
...
bool ComparativeMolSurfaceRenderer::GetExtents(core_gl::view::CallRender3DGL& call) {
    ...
    protein_calls::VTIDataCall* cmd0 = this->volDataSlot1.CallAs<protein_calls::VTIDataCall>();
    if (cmd0 == NULL) {
        return false;
    }
    ...
    protein_calls::VTIDataCall* cmd1 = this->volDataSlot2.CallAs<protein_calls::VTIDataCall>();
    if (cmd1 == NULL) {
        return false;
    }
    ...
    if (this->cmpMode == COMPARE_1_1) {
        ...
        cr3d->SetTimeFramesCount(std::min(
            std::min(cmd0->FrameCount(), mol0->FrameCount()), std::min(cmd1->FrameCount(), mol1->FrameCount())));
 ...
 */
bool ComparativeMolSurfaceRenderer::initPotentialMap(VTIDataCall* cmd, gridParams& gridPotentialMap,
    GLuint& potentialTex, CudaDevArr<float>& tex_D, int3& dim, float3& org, float3& delta) {
    ...
    if (!CudaSafeCall(cudaMemcpy(
            tex_D.Peek(), cmd->GetPointDataByIdx(0, 0), sizeof(float) * tex_D.GetCount(), cudaMemcpyHostToDevice))) {
        return false;
    }
    ...
    glTexImage3DEXT(GL_TEXTURE_3D, 0, GL_RGBA32F, cmd->GetGridsize().X(), cmd->GetGridsize().Y(),
        cmd->GetGridsize().Z(), 0, GL_ALPHA, GL_FLOAT, cmd->GetPointDataByIdx(0, 0));
...
bool ComparativeMolSurfaceRenderer::Render(core_gl::view::CallRender3DGL& call) {
    ...
    if ((this->triggerRMSFit) || (mol1->DataHash() != this->datahashParticles1) ||
        (mol2->DataHash() != this->datahashParticles2) || (calltime != this->calltimeOld)) {
...
#ifdef USE_TIMER
        Log::DefaultLog.WriteInfo( "%s: Time for RMS fitting: %.6f s", this->ClassName(),
            (double(clock() - t) / double(CLOCKS_PER_SEC)));
#endif

        // (Re)-compute bounding box
        this->computeDensityBBox(
            mol1->AtomPositions(), this->atomPosFitted.Peek(), mol1->AtomCount(), mol2->AtomCount());
    ...
    if ((this->triggerInitPotentialTex) || (vti1->DataHash() != this->datahashPotential1) ||
        (vti2->DataHash() != this->datahashPotential2) || (calltime != this->calltimeOld)) {
    ...
    if (this->triggerComputeSurfacePoints1) {
        ...
        if (!this->deformSurf1.MorphToVolumeGradient(
                ...
                this->procField1D.Peek(),
#endif //  USE_PROCEDURAL_DATA
                this->volDim, this->volOrg, this->volDelta, this->qsIsoVal, this->interpolMode, this->regMaxIt,
    ...
    if (this->triggerComputeSurfacePoints2) {
        ...
        if (!this->deformSurf2.ComputeTexCoordsOfRMSDFittedPositions(this->gridPotential2.minC,
    ...
    if (this->triggerSurfaceMapping) {
            ...
            for (int i = 0; i < this->maxSubdivLevel; ++i) {
                ...
                if (this->maxSubdivLevel > 0) {
                    ...
                    if (!this->deformSurfMapped.MorphToVolumeTwoWayGVFSubdiv(
                            ...
                            this->procField1D.Peek(),
#endif //  USE_PROCEDURAL_DATA
                            this->deformSurf2.PeekCubeStates(), this->deformSurf1.PeekCubeStates(), this->volDim,
                            this->volOrg, this->volDelta, this->qsIsoVal, this->interpolMode, this->maxSubdivSteps,
                            this->surfMappedMinDisplScl, this->surfMappedSpringStiffness, this->surfaceMappingForcesScl,
                            this->surfaceMappingExternalForcesWeightScl, this->surfMappedGVFScl, this->surfMappedGVFIt,
                            false, false)) {
        ...
        if (this->maxSubdivLevel > 0) {
            if (!CudaSafeCall(
                    cudaMemcpy(this->mappedSurfVertexFlags.Peek(), this->deformSurfMapped.PeekVertexFlagDevPt(),
                        sizeof(float) * this->deformSurfMapped.GetVertexCnt(), cudaMemcpyDeviceToHost))) {
                return false;
            }
        }
        ...
        if (this->surfaceMappedColorMode == SURFACE_POTENTIAL_SIGN) {
            if (!this->deformSurfMapped.ComputeSurfAttribSignDiff(this->deformSurf2,
                    this->rmsCentroid.PeekComponents(), // In case the start surface has been fitted using RMSD
                    rmsRotInv.PeekComponents(), this->rmsTranslation.PeekComponents(), this->surfAttribTex1_D.Peek(),
                    this->texDim1, this->texOrg1, this->texDelta1, this->surfAttribTex2_D.Peek(), this->texDim2,
    ...
    if (this->cmpMode == COMPARE_1_ROT) {
        glTranslatef(this->centroid.X(), this->centroid.Y(), this->centroid.Z());
    ...
    if (this->surface1RM != SURFACE_NONE) {
        ...
        // Render surface #1
        if (!this->renderSurface(this->deformSurf1.GetVtxDataVBO(), static_cast<uint>(this->deformSurf1.GetVertexCnt()),
    ...
    if (this->surface2RM != SURFACE_NONE) {
        ...
        // Render surface #2
        if (!this->renderSurface(this->deformSurf2.GetVtxDataVBO(), static_cast<uint>(this->deformSurf2.GetVertexCnt()),
    ...
    if (this->surfaceMappedRM != SURFACE_NONE) {
        ...
        // Render mapped surface
        if (!this->renderMappedSurface(this->deformSurf2.GetVtxDataVBO(), this->deformSurfMapped.GetVtxDataVBO(),
...
bool ComparativeMolSurfaceRenderer::renderEdges() {
    ...
    glVertexPointer(3, GL_FLOAT, static_cast<GLsizei>(this->deformSurfMapped.vertexDataStride * sizeof(float)),
        reinterpret_cast<void*>(0));

    glDrawElements(GL_LINES, edgeCnt * 2, GL_UNSIGNED_INT, this->deformSurfMapped.PeekEdges());
...
bool ComparativeMolSurfaceRenderer::renderExternalForces() {
    ...
    if (this->triggerComputeLines) {
        ...
        if (!CudaSafeCall(cudaMemcpy(gvf.Peek(), this->deformSurfMapped.PeekExternalForces(),
                sizeof(float) * gridSize * 4, cudaMemcpyDeviceToHost))) {
            return false;
        }
        ...
        for (int x = 0; x < this->volDim.x; ++x) {
            for (int y = 0; y < this->volDim.y; ++y) {
                for (int z = 0; z < this->volDim.z; ++z) {
                    ...
                    Vec3f pos(this->volOrg.x + x * this->volDelta.x, this->volOrg.y + y * this->volDelta.y,
                    ...
                    if ((pos.X() <= this->posXMax) && (pos.X() >= this->posXMin) && (pos.Y() <= this->posYMax) &&
                        (pos.Y() >= this->posYMin) && (pos.Z() <= this->posZMax) && (pos.Z() >= this->posZMin)) {
    ...
    if (this->triggerComputeLines) {
        ...
        for (int x = 0; x < this->volDim.x; ++x) {
            for (int y = 0; y < this->volDim.y; ++y) {
                ...
                Vec3f pos0(
                    this->volOrg.x + x * this->volDelta.x, this->volOrg.y + y * this->volDelta.y, this->volOrg.z);
                Vec3f pos1(
                    this->volOrg.x + x * this->volDelta.x, this->volOrg.y + y * this->volDelta.y, this->volMaxC.z);
        ...
        for (int z = 0; z < this->volDim.z; ++z) {
            for (int y = 0; y < this->volDim.y; ++y) {
                ...
                Vec3f pos0(
                    this->volOrg.x, this->volOrg.y + y * this->volDelta.y, this->volOrg.z + z * this->volDelta.z);
                Vec3f pos1(
                    this->volMaxC.x, this->volOrg.y + y * this->volDelta.y, this->volOrg.z + z * this->volDelta.z);
        ...
        for (int z = 0; z < this->volDim.z; ++z) {
            for (int x = 0; x < this->volDim.x; ++x) {
                ...
                Vec3f pos0(
                    this->volOrg.x + x * this->volDelta.x, this->volOrg.y, this->volOrg.z + z * this->volDelta.z);
                Vec3f pos1(
                    this->volOrg.x + x * this->volDelta.x, this->volMaxC.y, this->volOrg.z + z * this->volDelta.z);
 ...
 */
bool ComparativeMolSurfaceRenderer::renderSurface(GLuint vbo, uint vertexCnt, GLuint vboTriangleIdx,
    uint triangleVertexCnt, SurfaceRenderMode renderMode, SurfaceColorMode colorMode, GLuint potentialTex,
    Vec3f uniformColor, float alphaScl) {
 ...
 */
bool ComparativeMolSurfaceRenderer::renderSurfaceWithSubdivFlag(DeformableGPUSurfaceMT& surf) {
    ...
    glDrawElements(
        GL_TRIANGLES, static_cast<GLsizei>(surf.GetTriangleCnt() * 3), GL_UNSIGNED_INT, reinterpret_cast<void*>(0));
 ...
 */
bool ComparativeMolSurfaceRenderer::renderSurfaceWithUncertainty(DeformableGPUSurfaceMT& surf) {
    if (!surf.GetVtxDataVBO()) {
        printf("No vertex data VBO!!!\n");
        return false;
    }
 ...
 */
bool ComparativeMolSurfaceRenderer::renderMappedSurface(GLuint vboOld, GLuint vboNew, uint vertexCnt,
    GLuint vboTriangleIdx, uint triangleVertexCnt, SurfaceRenderMode renderMode, SurfaceColorMode colorMode) {
...
void ComparativeMolSurfaceRenderer::updateParams() {
    ...
    if (this->cmpModeSlot.IsDirty()) {
        this->cmpMode = static_cast<CompareMode>(this->cmpModeSlot.Param<core::param::EnumParam>()->Value());
    ...
    if (this->surfMappedExtForceSlot.IsDirty()) {
        this->surfMappedExtForce =
            static_cast<ExternalForces>(this->surfMappedExtForceSlot.Param<core::param::EnumParam>()->Value());
    ...
    if (this->uncertaintyCriterionSlot.IsDirty()) {
        this->uncertaintyCriterion =
            static_cast<UncertaintyCriterion>(this->uncertaintyCriterionSlot.Param<core::param::EnumParam>()->Value());
    ...
    if (this->surfMappedSpringStiffnessSlot.IsDirty()) {
        this->surfMappedSpringStiffness = this->surfMappedSpringStiffnessSlot.Param<core::param::FloatParam>()->Value();
    ...
    if (this->surfMappedGVFSclSlot.IsDirty()) {
        this->surfMappedGVFScl = this->surfMappedGVFSclSlot.Param<core::param::FloatParam>()->Value();
    ...
    if (this->surfMappedGVFItSlot.IsDirty()) {
        this->surfMappedGVFIt = this->surfMappedGVFItSlot.Param<core::param::IntParam>()->Value();
    ...
    if (this->distFieldThreshSlot.IsDirty()) {
        this->distFieldThresh = this->distFieldThreshSlot.Param<core::param::FloatParam>()->Value();
    ...
    if (this->regExternalForcesWeightSlot.IsDirty()) {
        this->regExternalForcesWeight = this->regExternalForcesWeightSlot.Param<core::param::FloatParam>()->Value();
    ...
    if (this->surfregMinDisplSclSlot.IsDirty()) {
        this->surfregMinDisplScl = 1.0f / this->surfregMinDisplSclSlot.Param<core::param::FloatParam>()->Value();
    ...
    if (this->surface1RMSlot.IsDirty()) {
        this->surface1RM =
            static_cast<SurfaceRenderMode>(this->surface1RMSlot.Param<core::param::EnumParam>()->Value());
    ...
    if (this->surface1ColorModeSlot.IsDirty()) {
        this->surface1ColorMode =
            static_cast<SurfaceColorMode>(this->surface1ColorModeSlot.Param<core::param::EnumParam>()->Value());
    ...
    if (this->surface2RMSlot.IsDirty()) {
        this->surface2RM =
            static_cast<SurfaceRenderMode>(this->surface2RMSlot.Param<core::param::EnumParam>()->Value());
    ...
    if (this->surface2ColorModeSlot.IsDirty()) {
        this->surface2ColorMode =
            static_cast<SurfaceColorMode>(this->surface2ColorModeSlot.Param<core::param::EnumParam>()->Value());
    ...
    if (this->surfaceMappedRMSlot.IsDirty()) {
        this->surfaceMappedRM =
            static_cast<SurfaceRenderMode>(this->surfaceMappedRMSlot.Param<core::param::EnumParam>()->Value());
    ...
    if (this->surfaceMappedColorModeSlot.IsDirty()) {
        this->surfaceMappedColorMode =
            static_cast<SurfaceColorMode>(this->surfaceMappedColorModeSlot.Param<core::param::EnumParam>()->Value());
    ...
    if (this->unmappedTrisColorSlot.IsDirty()) {
        this->theUnmappedTrisColor =
            static_cast<UnmappedTrisColor>(this->unmappedTrisColorSlot.Param<core::param::EnumParam>()->Value());
...
bool ComparativeMolSurfaceRenderer::initProcFieldData() {
    ...
    Vec3f center(this->volOrg.x + (this->volMaxC.x - this->volOrg.x) * 0.5,
        this->volOrg.y + (this->volMaxC.y - this->volOrg.y) * 0.5,
        this->volOrg.z + (this->volMaxC.z - this->volOrg.z) * 0.5);
    // Loop through all grid points
    for (int x = 0; x < static_cast<int>(this->volDim.x); ++x) {
        for (int y = 0; y < static_cast<int>(this->volDim.y); ++y) {
            for (int z = 0; z < static_cast<int>(this->volDim.z); ++z) {
                ...
                float R = 13.0f;
                float rad = 5.5;
                len = pow(distVec.X() * distVec.X() + distVec.Y() * distVec.Y() + distVec.Z() * distVec.Z() + R * R -
                              rad * rad,
                          2.0f) -
                      4.0f * R * R * (distVec.X() * distVec.X() + distVec.Y() * distVec.Y());
                lenSqrt = len * len;
    ...
    // Copy to device array
    if (!CudaSafeCall(cudaMemcpy(
            this->procField1D.Peek(), this->procField1.Peek(), sizeof(float) * fieldSize, cudaMemcpyHostToDevice))) {
        return false;
    }
    if (!CudaSafeCall(cudaMemcpy(
            this->procField2D.Peek(), this->procField2.Peek(), sizeof(float) * fieldSize, cudaMemcpyHostToDevice))) {
        return false;
    }
