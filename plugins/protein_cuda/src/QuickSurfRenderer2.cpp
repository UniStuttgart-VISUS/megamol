...
QuickSurfRenderer2::QuickSurfRenderer2(void)
        ...
        , m_hPosSize(0)
        , numParticles(0)
        , currentSurfaceArea(0.0f)
        , recomputeAreaDiagram(true)
        , areaDiagramData(0)
        , callTime(0.0f)
    ...
    this->areaDiagramCalleeSlot.SetCallback(DiagramCall::ClassName(),
        DiagramCall::FunctionName(DiagramCall::CallForGetData), &QuickSurfRenderer2::GetAreaDiagramData);
...
bool QuickSurfRenderer2::Render(megamol::core_gl::view::CallRender3DGL& call) {
    ...
    */
    ...
    glUniform1i(this->lightShader.ParameterLocation("twoSidedLight"),
        this->twoSidedLightParam.Param<param::BoolParam>()->Value());
    ...
    this->calcSurf(c2, m_hPos, this->qualityParam.Param<param::IntParam>()->Value(),
        this->radscaleParam.Param<param::FloatParam>()->Value(),
        this->gridspacingParam.Param<param::FloatParam>()->Value(),
        this->isovalParam.Param<param::FloatParam>()->Value(), true);
    ...
    if (this->recomputeAreaDiagram) {
        ...
        for (unsigned int i = 1; i < parts->FrameCount(); i++) {
            ...
            for (unsigned int k = 0; k < parts->GetParticleListCount(); k++) {
                ...
                for (UINT64 j = 0; j < particles.GetCount();
                     j++, pos = reinterpret_cast<const float*>(reinterpret_cast<const char*>(pos) + posStride)) {
            ...
            this->calcSurf(parts, m_hPos, this->qualityParam.Param<param::IntParam>()->Value(),
                this->radscaleParam.Param<param::FloatParam>()->Value(),
                this->gridspacingParam.Param<param::FloatParam>()->Value(),
                this->isovalParam.Param<param::FloatParam>()->Value(), true);
...
bool QuickSurfRenderer2::GetAreaDiagramData(core::Call& call) {
    ...
    if (this->recomputeAreaDiagram) {
        ...
        for (unsigned int i = 1; i < parts->FrameCount(); i++) {
            ...
            static_cast<MolecularSurfaceFeature*>(this->areaDiagramData->GetMappable())
                ->AppendValue(log(static_cast<float>(i)), this->currentSurfaceArea);
        }
...
int QuickSurfRenderer2::calcSurf(MultiParticleDataCall* mol, float* posInter, int quality, float radscale,
...
#define USE_BOUNDING_BOX
    ...
    for (unsigned int i = 0; i < numParticles; ++i) {
        if (minrad > posInter[i * 4 + 3])
            minrad = posInter[i * 4 + 3];
        if (maxrad < posInter[i * 4 + 3])
            maxrad = posInter[i * 4 + 3];
    }
    ...
    for (unsigned int i = 0; i < mol->GetParticleListCount(); i++) {
        megamol::core::moldyn::MultiParticleDataCall::Particles& parts = mol->AccessParticles(i);
        if (minrad > parts.GetGlobalRadius())
            minrad = parts.GetGlobalRadius();
        if (maxrad < parts.GetGlobalRadius())
            maxrad = parts.GetGlobalRadius();
    }
    ...
    float gridDim[3] = {
        ceilf(bBoxDim[0] / gridspacing) + 1, ceilf(bBoxDim[1] / gridspacing) + 1, ceilf(bBoxDim[2] / gridspacing) + 1};
