...
ProteinVariantMatch::ProteinVariantMatch(void)
    ...
    this->diagDataOutCalleeSlot.SetCallback(DiagramCall::ClassName(),
        DiagramCall::FunctionName(DiagramCall::CallForGetData), &ProteinVariantMatch::getDiagData);
    ...
    this->matrixDataOutCalleeSlot.SetCallback(VariantMatchDataCall::ClassName(),
        VariantMatchDataCall::FunctionName(VariantMatchDataCall::CallForGetData), &ProteinVariantMatch::getMatrixData);
    ...
    this->surfMapSpringStiffnessSlot.SetParameter(
        new core::param::FloatParam(this->surfMapSpringStiffness, 0.0f, 1.0f));
    ...
    this->surfMapExternalForcesWeightSlot.SetParameter(
        new core::param::FloatParam(this->surfMapExternalForcesWeight, 0.0f, 1.0f));
    ...
    this->surfRegSpringStiffnessSlot.SetParameter(
        new core::param::FloatParam(this->surfRegSpringStiffness, 0.0f, 1.0f));
    ...
    this->surfRegExternalForcesWeightSlot.SetParameter(
        new core::param::FloatParam(this->surfRegExternalForcesWeight, 0.0f, 1.0f));
    ...
    this->qsRadScl = 1.0f;
    this->qsRadSclSlot.SetParameter(new core::param::FloatParam(this->qsRadScl));
    this->MakeSlotAvailable(&this->qsRadSclSlot);

    // Quicksurf grid scaling
    this->qsGridDelta = 2.0f;
    this->qsGridDeltaSlot.SetParameter(new core::param::FloatParam(this->qsGridDelta));
    this->MakeSlotAvailable(&this->qsGridDeltaSlot);

    // Quicksurf isovalue
    this->qsIsoVal = 0.5f;
    this->qsIsoValSlot.SetParameter(new core::param::FloatParam(this->qsIsoVal));
...
bool ProteinVariantMatch::getDiagData(core::Call& call) {
    ...
    if (this->featureList.Count() < 1) {
        ...
        this->featureList[0] = new DiagramCall::DiagramSeries(
            "RMSD", new MolecularSurfaceFeature(static_cast<float>(this->nVariants), Vec3f(0.0f, 0.0f, 0.0f)));
        ...
        this->featureList[1] = new DiagramCall::DiagramSeries("Hausdorff distance",
            new MolecularSurfaceFeature(static_cast<float>(this->nVariants), Vec3f(0.0f, 0.0f, 0.0f)));
    ...
    for (unsigned int i = 0; i < this->nVariants; ++i) {
        ms->AppendValue(
            static_cast<float>(i), this->matchRMSD[this->singleFrameIdx * this->nVariants + i] / this->maxMatchRMSDVal);
    }
    ...
    for (unsigned int i = 0; i < this->nVariants; ++i) {
        ms->AppendValue(static_cast<float>(i), this->matchSurfacePotential[this->singleFrameIdx * this->nVariants + i] /
    ...
    for (unsigned int i = 0; i < this->nVariants; ++i) {
        ms->AppendValue(
            static_cast<float>(i), this->matchSurfacePotentialSign[this->singleFrameIdx * this->nVariants + i] /
    ...
    for (unsigned int i = 0; i < this->nVariants; ++i) {
        ms->AppendValue(static_cast<float>(i),
            this->matchMeanVertexPath[this->singleFrameIdx * this->nVariants + i] / this->maxMatchMeanVertexPathVal);
    ...
    for (unsigned int i = 0; i < this->nVariants; ++i) {
        ms->AppendValue(
            static_cast<float>(i), this->matchHausdorffDistance[this->singleFrameIdx * this->nVariants + i] /
 ...
 */
bool ProteinVariantMatch::computeDensityMap(float* atomPos, size_t atomCnt, CUDAQuickSurf* cqs) {
...
#pragma omp parallel for
    ...
    int rc = cqs->calc_map(static_cast<long>(atomCnt), this->atomPosTmp.Peek(),
...
void ProteinVariantMatch::computeDensityBBox(
    const float* atomPos1, const float* atomPos2, size_t atomCnt1, size_t atomCnt2) {
...
bool ProteinVariantMatch::computeMatchSurfMapping() {
    ...
    for (unsigned int i = 0; i < this->nVariants; ++i) {
        ...
        if (!CudaSafeCall(this->potentialTex0_D.Validate(
                volCall->GetGridsize().X() * volCall->GetGridsize().Y() * volCall->GetGridsize().Z()))) {
            return false;
        }
        if (!CudaSafeCall(cudaMemcpy(this->potentialTex0_D.Peek(), (float*)(volCall->GetPointDataByIdx(0, 0)),
                sizeof(float) * volCall->GetGridsize().X() * volCall->GetGridsize().Y() * volCall->GetGridsize().Z(),
        ...
        this->getAtomPosArray(molCall, this->atomPos0, particleCnt0);
        ...
        for (unsigned int j = 0; j < this->nVariants; ++j) {
...
#if defined(VERBOSE)
            Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: matching variants %i and %i (%u variants)",
            ...
            if (!CudaSafeCall(cudaMemcpy(this->potentialTex1_D.Peek(), (float*)(volCall->GetPointDataByIdx(0, 0)),
                    sizeof(float) * volCall->GetGridsize().X() * volCall->GetGridsize().Y() *
...
//#define COMPUTE_RUNTIME
            ...
            // Get atom pos vector (dismisses solvent molecules)
            this->getAtomPosArray(molCall, this->atomPosFitted, particleCnt1);
            ...
            for (int cnt = 0; cnt < static_cast<int>(posCnt); ++cnt) {
                centroid += Vec3f(this->rmsPosVec1.Peek()[cnt * 3 + 0], this->rmsPosVec1.Peek()[cnt * 3 + 1],
...
//count (%u vs. %u))", this->ClassName(), posCnt0, posCnt1);
            ...
            //            }
            rmsVal =
                this->getRMS(this->rmsPosVec0.Peek(), this->rmsPosVec1.Peek(), posCnt, true, 2, rotation, translation);
            if (rmsVal > 100.0f) {
                Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR, "%s: Unable to perform surface matching (rms val = %f)",
...
#pragma omp parallel for
                for (int a = 0; a < static_cast<int>(particleCnt1); ++a) {
                    ...
                    Vec3f pos(this->atomPosFitted.Peek()[4 * a + 0], this->atomPosFitted.Peek()[4 * a + 1],
            ...
            this->computeDensityBBox(this->atomPos0.Peek(), this->atomPosFitted.Peek(), particleCnt0, particleCnt1);
            ...
            if (!this->computeDensityMap(this->atomPos0.Peek(), particleCnt0, (CUDAQuickSurf*)this->cudaqsurf0)) {
                return false;
            }

            // Compute density map of variant #1
            if (!this->computeDensityMap(this->atomPosFitted.Peek(), particleCnt1, (CUDAQuickSurf*)this->cudaqsurf1)) {
                return false;
            }
            ...
            if (!surfTarget.ComputeVertexPositions(((CUDAQuickSurf*)this->cudaqsurf0)->getMap(), this->volDim,
                    this->volOrg, this->volDelta, this->qsIsoVal)) {

                Log::DefaultLog.WriteMsg(
                    Log::LEVEL_ERROR, "%s: could not compute vertex positions #1", this->ClassName());

                return false;
            }

            // Get vertex positions based on the level set
            if (!surfStart.ComputeVertexPositions(((CUDAQuickSurf*)this->cudaqsurf1)->getMap(), this->volDim,
                    this->volOrg, this->volDelta, this->qsIsoVal)) {
            ...
            if (!surfStart.ComputeTriangles(((CUDAQuickSurf*)this->cudaqsurf1)->getMap(), this->volDim, this->volOrg,
                    this->volDelta, this->qsIsoVal)) {

                Log::DefaultLog.WriteMsg(
                    Log::LEVEL_ERROR, "%s: could not compute vertex triangles #1", this->ClassName());
            ...
            if (!surfStart.ComputeConnectivity(((CUDAQuickSurf*)this->cudaqsurf1)->getMap(), this->volDim, this->volOrg,
                    this->volDelta, this->qsIsoVal)) {
            ...
            if (!surfStart.ComputeTriangleNeighbors(((CUDAQuickSurf*)this->cudaqsurf1)->getMap(), this->volDim,
                    this->volOrg, this->volDelta, this->qsIsoVal)) {

                Log::DefaultLog.WriteMsg(
                    Log::LEVEL_ERROR, "%s: could not compute triangle neighbors #2", this->ClassName());
                return false;
            }
            ...
            if (!surfStart.MorphToVolumeGradient(((CUDAQuickSurf*)this->cudaqsurf1)->getMap(), this->volDim,
                    this->volOrg, this->volDelta, this->qsIsoVal, this->surfRegInterpolMode, this->surfRegMaxIt,
                    this->qsGridDelta / 100.0f * this->surfRegForcesScl, this->surfRegSpringStiffness,
                    this->surfRegForcesScl, this->surfRegExternalForcesWeight)) {
            ...
            // vector flow
            if (!surfEnd.MorphToVolumeTwoWayGVF(((CUDAQuickSurf*)this->cudaqsurf1)->getMap(),
                    ((CUDAQuickSurf*)this->cudaqsurf0)->getMap(), surfEnd.PeekCubeStates(), surfTarget.PeekCubeStates(),
                    this->volDim, this->volOrg, this->volDelta, this->qsIsoVal, this->surfMapInterpolMode,
                    this->surfMapMaxIt, this->qsGridDelta / 100.0f * this->surfMapForcesScl,
                    this->surfMapSpringStiffness, this->surfMapForcesScl, this->surfMapExternalForcesWeight, 1.0f, 50,
                    true, true)) {
                return false;
            }
            ...
            float t_map_tmp;
            float t_gvf_tmp;
            if (!surfEnd.MorphToVolumeTwoWayGVFBM(((CUDAQuickSurf*)this->cudaqsurf1)->getMap(),
                    ((CUDAQuickSurf*)this->cudaqsurf0)->getMap(), surfEnd.PeekCubeStates(), surfTarget.PeekCubeStates(),
                    this->volDim, this->volOrg, this->volDelta, this->qsIsoVal, this->surfMapInterpolMode,
                    this->surfMapMaxIt, this->qsGridDelta / 100.0f * this->surfMapForcesScl,
                    this->surfMapSpringStiffness, this->surfMapForcesScl, this->surfMapExternalForcesWeight, 1.0f, 50,
                    true, true, t_gvf_tmp, t_map_tmp)) {
                return false;
            }
            ...
            for (uint i = 0; i < maxSubdivLevel; ++i) {
                ...
                for (uint j = 0; j < subdivSubLevel; ++j) {
                    ...
                    newTris = surfEnd.RefineMesh(1, ((CUDAQuickSurf*)this->cudaqsurf1)->getMap(), this->volDim,
                        this->volOrg, this->volDelta, this->qsIsoVal,
                    ...
                    if (newTris == 0)
                        break; // No more subdivisions needed
                }

                // Perform morphing
                // Morph surface #2 to shape #1 using Two-Way-GVF

                if (!surfEnd.MorphToVolumeTwoWayGVFSubdiv(((CUDAQuickSurf*)this->cudaqsurf1)->getMap(),
                        ((CUDAQuickSurf*)this->cudaqsurf0)->getMap(), surfStart.PeekCubeStates(),
                        surfTarget.PeekCubeStates(), this->volDim, this->volOrg, this->volDelta, this->qsIsoVal,
                        this->surfMapInterpolMode, deformIt, this->qsGridDelta / 100.0f * this->surfMapForcesScl,
                        this->surfMapSpringStiffness, this->surfMapForcesScl, this->surfMapExternalForcesWeight,
            ...
            if (!surfEnd.FlagCorruptTriangles(((CUDAQuickSurf*)this->cudaqsurf0)->getMap(), surfTarget.PeekCubeStates(),
                    volDim, volOrg, volDelta, this->qsIsoVal)) {
            ...
            if (!surfEnd.TrackPathSubdivVertices(((CUDAQuickSurf*)this->cudaqsurf1)->getMap(), this->volDim,
                    this->volOrg, this->volDelta, this->surfMapForcesScl,
                    this->qsGridDelta / 100.0f * this->surfMapForcesScl, this->qsIsoVal, this->surfMapMaxIt)) {
                return false;
            }
            ...
            if (!surfEnd.ComputeSurfAttribDiff(surfStart,
                    centroid.PeekComponents(), // In case the start surface has been fitted using RMSD
                    rmsRotInv.PeekComponents(), this->rmsTranslation.PeekComponents(), this->potentialTex0_D.Peek(),
                    texDim0, texOrg0, texDelta0, this->potentialTex1_D.Peek(), texDim1, texOrg1, texDelta1)) {
                return false;
            }
            ...
            // Compute texture difference per vertex
            if (!surfEnd.ComputeSurfAttribSignDiff(surfStart,
                    centroid.PeekComponents(), // In case the start surface has been fitted using RMSD
                    rmsRotInv.PeekComponents(), this->rmsTranslation.PeekComponents(), this->potentialTex0_D.Peek(),
                    texDim0, texOrg0, texDelta0, this->potentialTex1_D.Peek(), texDim1, texOrg1, texDelta1)) {
                return false;
            ...
            this->minMatchSurfacePotentialSignVal = std::min(
                this->minMatchSurfacePotentialSignVal, this->matchSurfacePotentialSign[this->nVariants * i + j]);
            this->maxMatchSurfacePotentialSignVal = std::max(
                this->maxMatchSurfacePotentialSignVal, this->matchSurfacePotentialSign[this->nVariants * i + j]);
            ...
            Log::DefaultLog.WriteMsg(Log::LEVEL_INFO, "%s: matching surfaces %3u%%", this->ClassName(),
            ...
            if (j == this->nVariants - 1) {
                ...
                printf("*** benchmark ***\n");
                printf(
                    "  nVox        : %i-%i(%i)\n", nVoxMin, nVoxMax, (int)((float)(nVox) / static_cast<float>(nComp)));
                printf("  nVtx        : %i-%i(%i)\n", nVerticesMin, nVerticesMax,
                    (int)((float)(nVertices) / static_cast<float>(nComp)));
                printf("  t_vol       : %.1f-%.1f(%.1f)\n", t_volMin, t_volMax, t_vol / static_cast<float>(nComp));
                printf("  t_mesh      : %.1f-%.1f(%.1f)\n", t_meshMin, t_meshMax, t_mesh / static_cast<float>(nComp));
                printf("  t_gvf       : %.1f-%.1f(%.1f)\n", t_gvfMin, t_gvfMax, t_gvf / static_cast<float>(nComp));
                printf("  t_map       : %.1f-%.1f(%.1f)\n", t_mapMin, t_mapMax, t_map / static_cast<float>(nComp));
                ...
                printf("  t_met       : %.1f-%.1f(%.1f)\n", t_metMin, t_metMax, t_met / static_cast<float>(nComp));
    ...
    megamol::core::utility::log::Log::DefaultLog.WriteMsg(megamol::core::utility::log::Log::LEVEL_INFO,
        "%s: time for matching surfaces of %u variants %f sec", this->ClassName(), this->nVariants,
 ...
 */
float ProteinVariantMatch::getRMS(float* atomPos0, float* atomPos1, unsigned int cnt, bool fit, int flag,
...
#pragma omp parallel for
    ...
    float rmsValue = CalculateRMS(cnt, // Number of positions in each vector
        fit, flag,
 ...
 */
bool ProteinVariantMatch::getRMSPosArray(MolecularDataCall* mol, HostArr<float>& posArr, unsigned int& cnt) {
    cnt = 0;
        ...
        for (uint sec = 0; sec < mol->SecondaryStructureCount(); ++sec) {
            for (uint acid = 0; acid < mol->SecondaryStructures()[sec].AminoAcidCount(); ++acid) {
                const MolecularDataCall::AminoAcid* aminoAcid = dynamic_cast<const MolecularDataCall::AminoAcid*>(
                    (mol->Residues()[mol->SecondaryStructures()[sec].FirstAminoAcidIndex() + acid]));
                if (aminoAcid == NULL) {
                    Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR, "%s: Unable to perform RMS fitting using all protein\
atoms (residue mislabeled as 'amino acid')",
                        this->ClassName());
                    return false;
                }
                for (uint at = 0; at < aminoAcid->AtomCount(); ++at) {
                    posArr.Peek()[3 * cnt + 0] = mol->AtomPositions()[3 * (aminoAcid->FirstAtomIndex() + at) + 0];
                    posArr.Peek()[3 * cnt + 1] = mol->AtomPositions()[3 * (aminoAcid->FirstAtomIndex() + at) + 1];
                    posArr.Peek()[3 * cnt + 2] = mol->AtomPositions()[3 * (aminoAcid->FirstAtomIndex() + at) + 2];
                    cnt++;
                }
            }
        }
        ...
        for (uint sec = 0; sec < mol->SecondaryStructureCount(); ++sec) {
            ...
            for (uint acid = 0; acid < mol->SecondaryStructures()[sec].AminoAcidCount(); ++acid) {
                ...
                uint cAlphaIdx =
                    ((const MolecularDataCall::
                             AminoAcid*)(mol->Residues()[mol->SecondaryStructures()[sec].FirstAminoAcidIndex() + acid]))
                        ->CAlphaIndex();
                uint cCarbIdx =
                    ((const MolecularDataCall::
                             AminoAcid*)(mol->Residues()[mol->SecondaryStructures()[sec].FirstAminoAcidIndex() + acid]))
...
void ProteinVariantMatch::updatParams() {
    ...
    if (this->surfMapExternalForcesWeightSlot.IsDirty()) {
        this->surfMapExternalForcesWeight =
            this->surfMapExternalForcesWeightSlot.Param<core::param::FloatParam>()->Value();
    ...
    if (this->surfRegExternalForcesWeightSlot.IsDirty()) {
        this->surfRegExternalForcesWeight =
            this->surfRegExternalForcesWeightSlot.Param<core::param::FloatParam>()->Value();
