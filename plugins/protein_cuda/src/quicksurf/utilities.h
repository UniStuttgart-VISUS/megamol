 ...
 *  $Revision: 1.120 $  $Date: 2020/10/27 21:31:27 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
extern int analyze_selection_aligned(int n, const int* on, int* firstsel, int* lastsel, int* selected);

extern int analyze_selection_aligned_dispatch(
    wkf_cpu_caps_t* cpucaps, int n, const int* on, int* firstsel, int* lastsel, int* selected);

/// find min/max/mean values for an array of floats
extern void minmaxmean_1fv_aligned(const float* f, ptrdiff_t n, float* fmin, float* fmax, float* fmean);
...
extern int minmax_selected_3fv_aligned(const float* f, const int* on, const ptrdiff_t n3, const ptrdiff_t firstsel,
    const ptrdiff_t lastsel, float* fmin, float* fmax);
...
inline void make_spline_Q_matrix_noncontig(
    float q[4][3], float basis[4][4], const float* pts1, const float* pts2, const float* pts3, const float* pts4) {
    int i;
...
/// -180 and 180.
extern float dihedral(const float*, const float*, const float*, const float*);
...
int k_candidates(int, int, int, int, float, float*,
    float*); // generates K candidates, returns farthest away from all previous samples
