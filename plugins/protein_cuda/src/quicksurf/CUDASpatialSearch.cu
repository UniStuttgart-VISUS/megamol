...
hashAtoms(unsigned int natoms, const float4* RESTRICT xyzr, int3 numcells, float invgridspacing,
    unsigned int* RESTRICT atomIndex, unsigned int* RESTRICT atomHash) {
    ...
    unsigned int hash = (cell.z * numcells.y * numcells.x) + (cell.y * numcells.x) + cell.x;
...
__global__ static void
// __launch_bounds__ ( MAXTHRSORTHASH, MINBLOCKSORTHASH )
sortAtomsColorsGenCellLists(unsigned int natoms, const float4* RESTRICT xyzr_d, const float4* RESTRICT color_d,
    const unsigned int* atomIndex_d, const unsigned int* atomHash_d, float4* RESTRICT sorted_xyzr_d,
    float4* RESTRICT sorted_color_d, uint2* RESTRICT cellStartEnd_d) {
...
__global__ static void
// __launch_bounds__ ( MAXTHRSORTHASH, MINBLOCKSORTHASH )
sortAtomsGenCellLists(unsigned int natoms, const float4* RESTRICT xyzr_d, const unsigned int* atomIndex_d,
    const unsigned int* atomHash_d, float4* RESTRICT sorted_xyzr_d, uint2* RESTRICT cellStartEnd_d) {
    extern __shared__ unsigned int hash_s[]; // blockSize + 1 elements
    unsigned int index = (blockIdx.x * blockDim.x) + threadIdx.x;
    unsigned int hash;
...
int vmd_cuda_build_density_atom_grid(int natoms, const float4* xyzr_d, const float4* color_d, float4* sorted_xyzr_d,
    ...
    hashAtoms<<<hGsz, hBsz>>>(natoms, xyzr_d, volsz, invgridspacing, atomIndex_d, atomHash_d);

    // Sort atom indices by their grid cell address
    // XXX no pre-allocated workspace yet...
    if (dev_radix_sort_by_key(
            atomHash_d, atomIndex_d, natoms, (unsigned int*)NULL, (unsigned int*)NULL, NULL, 0, 0U, 0U) != 0) {
    ...
    sortAtomsColorsGenCellLists<<<hGsz, hBsz, smemSize>>>(
        natoms, xyzr_d, color_d, atomIndex_d, atomHash_d, sorted_xyzr_d, sorted_color_d, cellStartEnd_d);
...
int vmd_cuda_build_density_atom_grid(int natoms, const float4* xyzr_d, float4*& sorted_xyzr_d, uint2*& cellStartEnd_d,
    int3 volsz, float invgridspacing) {
    ...
    sortAtomsGenCellLists<<<hGsz, hBsz, smemSize>>>(
        natoms, xyzr_d, atomIndex_d, atomHash_d, sorted_xyzr_d, cellStartEnd_d);
