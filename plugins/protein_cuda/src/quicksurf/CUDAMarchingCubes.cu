...
inline __host__ __device__ float3 fmaf3(float x, float3 y, float3 z) {
    return make_float3(fmaf(x, y.x, z.x), fmaf(x, y.y, z.y), fmaf(x, y.z, z.z));
}
...
inline __device__ void convert_color(uchar4& cu, float3 cf) {
    ...
    cu = make_uchar4(fminf(cf.x * 255.0f, 255.0f), fminf(cf.y * 255.0f, 255.0f), fminf(cf.z * 255.0f, 255.0f), 255);
}
...
inline __device__ void convert_normal(char3& cc, float3 cf) {
    ...
    cc = make_char3(cf.x * invlen * 127.5f - 0.5f, cf.y * invlen * 127.5f - 0.5f, cf.z * invlen * 127.5f - 0.5f);
}
...
classifyVoxel(uint2* RESTRICT voxelVerts, const float* RESTRICT volume, uint3 gridSize, unsigned int numVoxels,
    float3 voxelSize, uint3 subGridStart, uint3 subGridEnd, float isoValue) {
    uint3 gridPos;
    unsigned int i;
    ...
    if (subgrid) {
        if (gridPos.x < subGridStart.x || gridPos.y < subGridStart.y || gridPos.z < subGridStart.z ||
            gridPos.x >= subGridEnd.x || gridPos.y >= subGridEnd.y || gridPos.z >= subGridEnd.z) {
...
__global__ void
// __launch_bounds__ ( KERNTHREADS, 1 )
compactVoxels(unsigned int* RESTRICT compactedVoxelArray, const uint2* RESTRICT voxelOccupied, unsigned int lastVoxel,
    unsigned int numVoxels, unsigned int numVoxelsm1) {
    unsigned int blockId = (blockIdx.y * gridDim.x) + blockIdx.x;
...
generateTriangleVerticesSMEM(float3* RESTRICT pos, const unsigned int* RESTRICT compactedVoxelArray,
    const uint2* RESTRICT numVertsScanned, const float* RESTRICT volume, uint3 gridSize, float3 voxelSize,
    float isoValue, unsigned int activeVoxels, unsigned int maxVertsM3) {
...
__global__ void
// __launch_bounds__ ( KERNTHREADS, 1 )
offsetTriangleVertices(float3* RESTRICT pos, float3 origin, unsigned int numVertsM1) {
    unsigned int zOffset = blockIdx.z * gridDim.x * gridDim.y;
    unsigned int blockId = (blockIdx.y * gridDim.x) + blockIdx.x;
...
generateTriangleNormals(
    const float3* RESTRICT pos, NORMAL* norm, float3 gridSizeInv, float3 bBoxInv, unsigned int numVerts) {
    unsigned int zOffset = blockIdx.z * gridDim.x * gridDim.y;
    unsigned int blockId = (blockIdx.y * gridDim.x) + blockIdx.x;
    unsigned int i = zOffset + (blockId * blockDim.x) + threadIdx.x;
...
__global__ void
// __launch_bounds__ ( KERNTHREADS, 1 )
generateTriangleColorNormal(const float3* RESTRICT pos, VERTEXCOL* RESTRICT col, NORMAL* RESTRICT norm,
    const VOLTEX* RESTRICT colors, uint3 gridSize, float3 gridSizeInv, float3 bBoxInv, unsigned int numVerts) {
    unsigned int zOffset = blockIdx.z * gridDim.x * gridDim.y;
    unsigned int blockId = (blockIdx.y * gridDim.x) + blockIdx.x;
    unsigned int i = zOffset + (blockId * blockDim.x) + threadIdx.x;
...
void ThrustScanWrapperUint2(uint2* output, uint2* input, unsigned int numElements) {
    ...
    thrust::exclusive_scan(thrust::device_ptr<myuint2>((myuint2*)input),
        thrust::device_ptr<myuint2>((myuint2*)input + numElements), thrust::device_ptr<myuint2>((myuint2*)output),
        (myuint2)zero);
}
...
void ThrustScanWrapperUint2(uint2* output, uint2* input, unsigned int numElements) {
    const uint2 zero = make_uint2(0, 0);
    thrust::exclusive_scan(thrust::device_ptr<uint2>(input), thrust::device_ptr<uint2>(input + numElements),
...
void ThrustScanWrapperArea(float* output, float* input, unsigned int numElements) {
    thrust::inclusive_scan(thrust::device_ptr<float>(input), thrust::device_ptr<float>(input + numElements),
...
__global__ void
// __launch_bounds__ ( KERNTHREADS, 1 )
computeTriangleAreas(const float3* RESTRICT pos, float* RESTRICT area, unsigned int maxTria) {
    unsigned int zOffset = blockIdx.z * gridDim.x * gridDim.y;
    unsigned int blockId = (blockIdx.y * gridDim.x) + blockIdx.x;
    unsigned int i = zOffset + (blockId * blockDim.x) + threadIdx.x;
...
void CUDAMarchingCubes::computeIsosurfaceVerts(float3* vertOut, unsigned int maxverts, dim3& grid3) {
    ...
    if (cudacomputemajor >= 2) {
        ...
        if (useSubGrid) {
            classifyVoxel<1, 1><<<grid3D, threads3D>>>(
                d_voxelVerts, d_volume, gridSize, numVoxels, voxelSize, subGridStart, subGridEnd, isoValue);
        } else {
            classifyVoxel<1, 0><<<grid3D, threads3D>>>(
                d_voxelVerts, d_volume, gridSize, numVoxels, voxelSize, subGridStart, subGridEnd, isoValue);
        }
    } else {
        // launch a 2-D grid for older devices
        if (useSubGrid) {
            classifyVoxel<0, 1><<<grid, threads>>>(
                d_voxelVerts, d_volume, gridSize, numVoxels, voxelSize, subGridStart, subGridEnd, isoValue);
        } else {
            classifyVoxel<0, 0><<<grid, threads>>>(
                d_voxelVerts, d_volume, gridSize, numVoxels, voxelSize, subGridStart, subGridEnd, isoValue);
        }
    }
    ...
    generateTriangleVerticesSMEM<<<grid2, NTHREADS>>>(
        vertOut, d_compVoxelArray, d_voxelVerts, d_volume, gridSize, voxelSize, isoValue, activeVoxels, maxverts - 3);
}
...
void CUDAMarchingCubes::computeIsosurface(float3* vertOut, float3* normOut, float3* colOut, unsigned int maxverts) {
    ...
    if (this->useColor) {
        switch (texformat) {
        case RGB3F:
            generateTriangleColorNormal<float3, float3, float3><<<grid3, threads>>>(
                vertOut, colOut, normOut, (float3*)d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
            break;

        case RGB4U:
            generateTriangleColorNormal<float3, uchar4, float3><<<grid3, threads>>>(
                vertOut, colOut, normOut, (uchar4*)d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
            break;
        }
    } else {
...
void CUDAMarchingCubes::computeIsosurface(float3* vertOut, float3* normOut, uchar4* colOut, unsigned int maxverts) {
    ...
    if (this->useColor) {
        switch (texformat) {
        case RGB3F:
            generateTriangleColorNormal<uchar4, float3, float3><<<grid3, threads>>>(
                vertOut, colOut, normOut, (float3*)d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
            break;

        case RGB4U:
            generateTriangleColorNormal<uchar4, uchar4, float3><<<grid3, threads>>>(
                vertOut, colOut, normOut, (uchar4*)d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
            break;
        }
    } else {
...
void CUDAMarchingCubes::computeIsosurface(float3* vertOut, char3* normOut, uchar4* colOut, unsigned int maxverts) {
    ...
    if (this->useColor) {
        switch (texformat) {
        case RGB3F:
            generateTriangleColorNormal<uchar4, float3, char3><<<grid3, threads>>>(
                vertOut, colOut, normOut, (float3*)d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
            break;

        case RGB4U:
            generateTriangleColorNormal<uchar4, uchar4, char3><<<grid3, threads>>>(
                vertOut, colOut, normOut, (uchar4*)d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
            break;
        }
    } else {
...
bool CUDAMarchingCubes::SetVolumeData(float* volume, void* colors, VolTexFormat vtexformat, uint3 gridsize,
    ...
    if (gridsize.x != gridSize.x || gridsize.y != gridSize.y || gridsize.z != gridSize.z) {
        newgridsize = true;
    ...
    origin = gridOrigin;
    bBox = boundingBox;
    voxelSize = make_float3(bBox.x / gridSize.x, bBox.y / gridSize.y, bBox.z / gridSize.z);
    ...
    copyParams.srcPtr =
        make_cudaPitchedPtr((void*)d_volume, gridExtent.width * sizeof(float), gridExtent.width, gridExtent.height);
...
bool CUDAMarchingCubes::SetVolumeData(
    float* volume, float3* colors, uint3 gridsize, float3 gridOrigin, float3 boundingBox, bool cudaArray) {
    return SetVolumeData(volume, colors, RGB3F, gridsize, gridOrigin, boundingBox, cudaArray);
}


bool CUDAMarchingCubes::SetVolumeData(
    float* volume, uchar4* colors, uint3 gridsize, float3 gridOrigin, float3 boundingBox, bool cudaArray) {
    return SetVolumeData(volume, colors, RGB4U, gridsize, gridOrigin, boundingBox, cudaArray);
}
...
bool CUDAMarchingCubes::computeIsosurface(float* volume, void* colors, VolTexFormat vtexformat, uint3 gridsize,
    float3 gridOrigin, float3 boundingBox, bool cudaArray, float3* vertOut, float3* normOut, float3* colOut,
