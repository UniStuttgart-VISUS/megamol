...
char* str_tokenize(const char* newcmd, int* argc, char* argv[]) {
    ...
    while (cmdstart != NULL &&
           (*cmdstart == ' ' || *cmdstart == ',' || *cmdstart == ';' || *cmdstart == '\t' || *cmdstart == '\n')) {
...
int k_candidates(int k, int nrays, int idx, int testpt, float minD, float* candidates, float* population) {
    ...
    for (int j = 0; j < k; j++) {
        ...
        for (int jj = 0; jj < idx; jj++) {
            float dist = arcdistance(
                population[jj * 2 + 0], candidates[j * 2 + 0], population[jj * 2 + 1], candidates[j * 2 + 1]);
...
int poisson_sample_on_sphere(float* population, int N, int k, int verbose) {
    ...
    while (converged == 0) {
        ...
        if (popul == (N + 1)) {
            ...
            if (verbose)
                printf("Poisson sampler failed at population=%d\n", popul);
            break;
        } else if (numactive >= 2) {
            while (activelist[testpt] != 1) {
            ...
            if (verbose)
                printf("Error in Poisson disk sampling procedure.\n");
        }
        ...
        if (numactive <= 2 && popul != (N + 1)) {
            ...
            fc = 0;
            numactive = N;
            testpt = 0; //failed to converge, reset
            attempts++;
        } else if (popul == (N + 1)) {
            converged = 1;
            break;
        ...
        if (attempts > 10) { //hard exit after 10 attempts
            if (verbose)
                printf("Poisson sampler failed to converge (population=%d)\n", popul);
            break;
        }
    }
    ...
    if (popul == (N + 1) && converged == 1) {
        if (verbose)
            printf("Poisson sampler done. (%d/10 attempts)\n", attempts + 1);
        return 1;
    }

    if (converged == 0) {
        if (verbose)
            printf("Poisson sampler failed -- exiting.");
        return -1;
    }
