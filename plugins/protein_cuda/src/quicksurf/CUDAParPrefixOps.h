 ...
 *      $Revision: 1.8 $        $Date: 2020/02/26 20:16:56 $
 *
 ***************************************************************************/
/**
 * \file CUDAParPrefixOps.h
 * \brief High-level CUDA parallel prefix (scan) operations (sum, min, max, ...)
 *        (wrapping CUB, Thrust, etc).
 */
...
template<typename T>
void dev_excl_scan_sum(T* in_d, long nitems, T* out_d, void* scanwork_d, long tsz, T ival);
...
template<typename T>
void dev_incl_scan_sum(T* in_d, long nitems, T* out_d, void* scanwork_d, long tsz);
