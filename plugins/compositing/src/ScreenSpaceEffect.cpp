    ...
    , m_camera_slot("Camera", "Connects a (copy of) camera state") 
{
    ...
    this->m_output_tex_slot.SetCallback(
        CallTexture2D::ClassName(), "GetData", &ScreenSpaceEffect::getDataCallback);
    this->m_output_tex_slot.SetCallback(
...
bool megamol::compositing::ScreenSpaceEffect::create() { 
    try {
        ...
        if (!instance()->ShaderSourceFactory().MakeShaderSource("Compositing::ssao", compute_ssao_src))
            return false;
    ...
    m_ssao_kernelRot_texture = std::make_shared<glowl::Texture2D>("ssao_kernel_rotation", tx_layout2, nullptr);
...
bool megamol::compositing::ScreenSpaceEffect::getDataCallback(core::Call& caller) {
    ...
    std::array<float, 2> texture_res = {
        static_cast<float>(normal_tx2D->getWidth()), static_cast<float>(normal_tx2D->getHeight())};
    ...
    if (this->m_mode.Param<core::param::EnumParam>()->Value() == 0) {
        ...
        m_ssao_prgm->Dispatch(static_cast<int>(std::ceil(std::get<0>(texture_res) / 8.0f)),
...
bool megamol::compositing::ScreenSpaceEffect::getMetaDataCallback(core::Call& caller) {
    return true;
}
