...
/**
 * CUDAVolumeRaycaster_kernel::d_render
 */
__global__ void d_render(uint * d_output, float * d_depth, cudaTextureObject_t tex, cudaTextureObject_t transferTex,
...
/**
 * CUDAVolumeRaycaster_kernel::freeCudaBuffers
 */
void CUDAVolumeRaycaster_kernel::freeCudaBuffers(void) {
...
/**
 * CUDAVolumeRaycaster_kernel::render_kernel
 */
void CUDAVolumeRaycaster_kernel::render_kernel(dim3 gridSize, dim3 blockSize, uint * d_output, float * d_depth, uint imageW, uint imageH, float fovx, float fovy, float3 camPos, float3 camDir,
    ...
    d_render << <gridSize, blockSize >> >(d_output, d_depth, this->texObj, this->customTransferTexObj, imageW, imageH, fovx, fovy, camPos, camDir, camUp, camRight, zNear, zFar, density, brightness,
        transferOffset, transferScale, minVal, maxVal, boxMin, boxMax, volSize);
}


/**
 * CUDAVolumeRaycaster_kernel::copyTransferFunction
 */
void CUDAVolumeRaycaster_kernel::copyTransferFunction(float4 * transferFunction, int functionSize) {
...
/**
 * CUDAVolumeRaycaster_kernel::transferNewVolume
 */
void CUDAVolumeRaycaster_kernel::transferNewVolume(void * h_volume, cudaExtent volumeSize) {
    if (d_volumeArray) {
        checkCudaErrors(cudaDestroyTextureObject(this->texObj));
...
/**
 * CUDAVolumeRaycaster_kernel::initCudaDevice
 */
void CUDAVolumeRaycaster_kernel::initCudaDevice(void * h_volume, cudaExtent volumeSize, float4 * transferFunction, int functionSize) {
 ...
 */
CUDAVolumeRaycaster_kernel::CUDAVolumeRaycaster_kernel(void) {
...
/**
* CUDAVolumeRaycaster_kernel::~CUDAVolumeRaycaster_kernel
*/
CUDAVolumeRaycaster_kernel::~CUDAVolumeRaycaster_kernel(void) {
}
