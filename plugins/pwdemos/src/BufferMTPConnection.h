...
namespace demos {
     ...
     */
    template<class T> class BufferMTPConnection {
    public:
    ...
    BufferMTPConnection<T>::BufferMTPConnection(unsigned int bufSize)
            : bufferStateCompleted(false), buffersConsuming(NULL),
            bufferLock(), buffersProducing(NULL), emptyBuffers(NULL),
            endOfData(false), fullBuffers(NULL), isOpen(false),
            newEmptyBuffer(false), newFullBuffer(false) {
     ...
     * BufferMTPConnection<T>::~BufferMTPConnection
     */
    template<class T> BufferMTPConnection<T>::~BufferMTPConnection(void) {
     ...
     */
    template<class T> void BufferMTPConnection<T>::AbortClose(void) {
        ...
        if (this->emptyBuffers == NULL) {
            ...
            while (leb->next != NULL) leb = leb->next;
     ...
     */
    template<class T> void BufferMTPConnection<T>::BufferConsumed(T * buf) {
     ...
     */
    template<class T> void BufferMTPConnection<T>::BufferFilled(T * buf) {
        vislib::sys::AutoLock(this->bufferLock);
     ...
     */
    template<class T> void BufferMTPConnection<T>::EndOfDataClose(void) {
        vislib::sys::AutoLock(this->bufferLock);

        this->endOfData = true;
        this->isOpen = ((this->fullBuffers != NULL)
            || (this->buffersProducing != NULL));
     ...
     */
    template<class T> T * BufferMTPConnection<T>::GetEmptyBuffer(bool wait) {
        Buffer *rv = NULL;
            ...
            if (this->emptyBuffers == NULL) {
                if (wait) {
                    while (this->emptyBuffers == NULL) {
                        ...
                        if (!this->isOpen || this->endOfData) return NULL;
     ...
     */
    template<class T> T * BufferMTPConnection<T>::GetFilledBuffer(bool wait) {
        Buffer *rv = NULL;
        ...
        if (this->isOpen) {
            ...
            if ((this->fullBuffers == NULL) && (this->buffersProducing == NULL)
                    && this->endOfData) {
                this->isOpen = false;
            }
     ...
     */
    template<class T> void BufferMTPConnection<T>::Open(void) {
        vislib::sys::AutoLock(this->bufferLock);
