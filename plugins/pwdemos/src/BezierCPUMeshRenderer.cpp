...
namespace demos {
        ...
        if (call_light != nullptr) {
            ...
            if (point_lights.size() > 1) {
                megamol::core::utility::log::Log::DefaultLog.WriteWarn(
                    "[BezierCPUMeshRenderer] Only one single 'Point Light' source is supported by this "
                    "renderer");
            } else if (point_lights.empty()) {
                megamol::core::utility::log::Log::DefaultLog.WriteWarn(
                    "[BezierCPUMeshRenderer] No 'Point Light' found");
            }
    ...
    void BezierCPUMeshRenderer::drawTube(float const* p1, float const* r1, unsigned char const* c1, float const* p2,
        ...
        col[0].Set(
            static_cast<float>(c1[0]) / 255.f, static_cast<float>(c1[1]) / 255.f, static_cast<float>(c1[2]) / 255.f);
        col[1].Set(
            static_cast<float>(c2[0]) / 255.f, static_cast<float>(c2[1]) / 255.f, static_cast<float>(c2[2]) / 255.f);
        col[2].Set(
            static_cast<float>(c3[0]) / 255.f, static_cast<float>(c3[1]) / 255.f, static_cast<float>(c3[2]) / 255.f);
        col[3].Set(
            static_cast<float>(c4[0]) / 255.f, static_cast<float>(c4[1]) / 255.f, static_cast<float>(c4[2]) / 255.f);
