 ...
 */
ParticleFortLoader::ParticleFortLoader(void) : core::Module(),
...
datahash(0), typeCnt(0), partTypes(NULL), partCnts(NULL), partDatas(NULL),
...
bboxMaxSlot("bbox::max", "The maximum values for the manual bounding box") {
    ...
    this->dataOutSlot.SetCallback(ParticleDataCall::ClassName(),
        ParticleDataCall::FunctionName(ParticleDataCall::CallForGetData),
        &ParticleFortLoader::getData);
    this->dataOutSlot.SetCallback(ParticleDataCall::ClassName(),
        ParticleDataCall::FunctionName(ParticleDataCall::CallForGetExtent),
...
bool ParticleFortLoader::getExtent(core::Call& c) {
    ...
    if ((this->autoBBoxSlot.IsDirty() || this->bboxMaxSlot.IsDirty() || this->bboxMinSlot.IsDirty()) && (this->datahash != 0)) {
        this->datahash++;
    ...
    if (this->autoBBoxSlot.Param<core::param::BoolParam>()->Value()) {
        ...
        const vislib::math::Vector<float, 3>& minval
            = this->bboxMinSlot.Param<core::param::Vector3fParam>()->Value();
        const vislib::math::Vector<float, 3>& maxval
            = this->bboxMaxSlot.Param<core::param::Vector3fParam>()->Value();
        vislib::math::Cuboid<float> mbbox(
            vislib::math::Min(minval.X(), maxval.X()),
            vislib::math::Min(minval.Y(), maxval.Y()),
            vislib::math::Min(minval.Z(), maxval.Z()),
            vislib::math::Max(minval.X(), maxval.X()),
            vislib::math::Max(minval.Y(), maxval.Y()),
...
void ParticleFortLoader::assertData(void) {
    ...
    if (this->positionFileNameSlot.IsDirty()
        ...
        try {
            ...
            if (posFileName.IsEmpty()) {
            ...
            else if (file.Open(posFileName, vislib::sys::File::READ_ONLY, vislib::sys::File::SHARE_READ, vislib::sys::File::OPEN_ONLY)) {
                unsigned int t;
                float x, y, z;
                bool typeRangeWarning = false;

                while (!file.IsEOF()) {
                    if (file.Read(&bound, 4) != 4) break; // unexpected end of file
                    if (bound != 0x10) break; // wrong block size
                    if (file.Read(&t, 4) != 4) break; // unexpected end of file
                    if (file.Read(&x, 4) != 4) break; // unexpected end of file
                    if (file.Read(&y, 4) != 4) break; // unexpected end of file
                    if (file.Read(&z, 4) != 4) break; // unexpected end of file
                    if (file.Read(&bound, 4) != 4) break; // unexpected end of file
                    if (bound != 0x10) break; // wrong block size
                ...
                if (typeRangeWarning) {
                    Log::DefaultLog.WriteWarn("At least one particle type was not in quartz type range [4000..4999] and was altered!");
                }

                file.Close();
            }
            else {
                Log::DefaultLog.WriteError("Unable to open position file \"%s\"",
                    vislib::StringA(posFileName).PeekBuffer());
            }
            ...
            if (attrFileName.IsEmpty()) {
            ...
            else if (file.Open(attrFileName, vislib::sys::File::READ_ONLY, vislib::sys::File::SHARE_READ, vislib::sys::File::OPEN_ONLY)) {
                ...
                while (!file.IsEOF()) {
                    if (file.Read(&bound, 4) != 4) break; // unexpected end of file
                    if (bound != 0x20) break; // wrong block size
                    if (file.Read(&r, 4) != 4) break; // unexpected end of file
                    if (file.Read(&d, 4) != 4) break; // unexpected end of file
                    if (file.Read(&d, 4) != 4) break; // unexpected end of file
                    if (file.Read(&d, 4) != 4) break; // unexpected end of file
                    if (file.Read(&q1, 4) != 4) break; // unexpected end of file
                    if (file.Read(&q2, 4) != 4) break; // unexpected end of file
                    if (file.Read(&q3, 4) != 4) break; // unexpected end of file
                    if (file.Read(&q4, 4) != 4) break; // unexpected end of file
                    if (file.Read(&bound, 4) != 4) break; // unexpected end of file
                    if (bound != 0x20) break; // wrong block size
                    ...
                    if (first) {
                        ...
                        if (minRad > r) minRad = r;
                        if (maxRad < r) maxRad = r;
                    }
                ...
                Log::DefaultLog.WriteError("Unable to open attribute file \"%s\"",
                    vislib::StringA(attrFileName).PeekBuffer());
            }
            ...
            for (SIZE_T i = 0; i < types.Count(); i++) {
                ...
                vislib::math::Point<float, 3> p1(v[0] - v[3] * tetGrowFac, v[1] - v[3] * tetGrowFac, v[2] - v[3] * tetGrowFac);
                vislib::math::Point<float, 3> p2(v[0] + v[3] * tetGrowFac, v[1] + v[3] * tetGrowFac, v[2] + v[3] * tetGrowFac);
                if (i == 0) {
