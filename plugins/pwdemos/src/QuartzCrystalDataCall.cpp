 ...
 */
CrystalDataCall::Crystal::Crystal(void) : baseRad(1.0f), boundRad(1.0f), faces(),
        vertices(NULL), triangleCnt(NULL), triangles(NULL) {
    // intentionally empty
}
    ...
    for (SIZE_T i = 0; i < facesCnt; i++) {
        for (SIZE_T j = i + 1; j < facesCnt; j++) {
            for (SIZE_T k = j + 1; k < facesCnt; k++) {
                if (!planes[i].CalcIntersectionPoint(planes[j], planes[k], pt)) continue; // no point.
                validPt = true;
...
bool CrystalDataCall::Crystal::operator==(const CrystalDataCall::Crystal& rhs) const {
    return vislib::math::IsEqual(this->baseRad, rhs.baseRad)
        && vislib::math::IsEqual(this->boundRad, rhs.boundRad)
