...
namespace demos {
    ...
    public:
        ...
        inline unsigned int GetCrystalType(unsigned int idx) const {
            if (this->types == NULL) throw vislib::IllegalStateException("No data", __FILE__, __LINE__);
            if (idx >= this->grpCnt) throw vislib::OutOfRangeException(idx, 0, this->grpCnt - 1, __FILE__, __LINE__);
        ...
        inline unsigned int GetParticleCount(unsigned int idx) const {
            if (this->cnt == NULL) throw vislib::IllegalStateException("No data", __FILE__, __LINE__);
            if (idx >= this->grpCnt) throw vislib::OutOfRangeException(idx, 0, this->grpCnt - 1, __FILE__, __LINE__);
            return this->cnt[idx];
        }
        ...
        inline const float* GetParticleData(unsigned int idx) const {
            if (this->part == NULL) throw vislib::IllegalStateException("No data", __FILE__, __LINE__);
            if (idx >= this->grpCnt) throw vislib::OutOfRangeException(idx, 0, this->grpCnt - 1, __FILE__, __LINE__);
         ...
         */
        inline void SetParticleData(unsigned int grpCnt, const unsigned int *types,
                const unsigned int *cnts, const float *const *parts) {
            this->grpCnt = grpCnt;
            this->types = types;
            this->cnt = cnts;
            this->part = parts;
