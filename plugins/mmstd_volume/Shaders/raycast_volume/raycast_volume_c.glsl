...
uniform sampler2D transfer_function_tx2D;

layout(rgba32f, binding = 0) writeonly uniform image2D render_target_tx2D;
...
void main()
{
    ...
    if(gID.x >= rt_resolution.x || gID.y >= rt_resolution.y)
		return;

    ivec2 pixel_coords = ivec2(gID.xy);

    vec2 clip_space_pixel_coords = vec2(
        (gID.x/rt_resolution.x) * 2.0f - 1.0f,
        (gID.y/rt_resolution.y) * 2.0f - 1.0f);
	...
	//float randomTemp = sin(dot(vec2(pixel_coords.y * rt_resolution.x + pixel_coords.x), vec2(12.9898f, 78.233f))) * 43758.5453f;
	//float random = abs(randomTemp - floor(randomTemp));
