...
namespace astro {
    ...
    bool SurfaceLICRenderer::GetExtents(core::view::CallRender3DGL& call) {
        ...
        *ci = call;
        ...
        call.AccessBoundingBoxes().SetBoundingBox(this->combineBoundingBoxes(
            {cd->GetBoundingBoxes().ObjectSpaceBBox(), ci->GetBoundingBoxes().BoundingBox()}));
        call.AccessBoundingBoxes().SetClipBox(this->combineBoundingBoxes(
            {cd->GetBoundingBoxes().ObjectSpaceClipBox(), ci->GetBoundingBoxes().ClipBox()}));
    ...
    bool SurfaceLICRenderer::Render(core::view::CallRender3DGL& call) {
        ...
        if (cd->GetComponents() != 3) {
            megamol::core::utility::log::Log::DefaultLog.WriteError(
                "Input velocities must be vectors with 3 components");
            return false;
        }
        ...
        if (this->velocity_texture == nullptr || this->velocity_texture->getWidth() != cd->GetResolution(0) ||
            ...
            this->velocity_texture =
                std::make_unique<glowl::Texture3D>("velocity_texture", velocity_layout, cd->GetData());
        }
        ...
        if (this->render_target == nullptr || this->render_target->getWidth() != cam.resolution_gate().width() ||
            ...
            glowl::TextureLayout render_tgt_layout(GL_RGBA8, cam.resolution_gate().width(),
                cam.resolution_gate().height(), 1, GL_RGBA, GL_UNSIGNED_BYTE, 1,
                {{GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER}, {GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER},
                    {GL_TEXTURE_WRAP_R, GL_CLAMP_TO_BORDER}, {GL_TEXTURE_MIN_FILTER, GL_LINEAR},
                    {GL_TEXTURE_MAG_FILTER, GL_LINEAR}},
                {});
        ...
        this->pre_compute_shdr.Dispatch(static_cast<int>(std::ceil(rt_resolution[0] / 8.0f)),
        ...
        glUniform1f(this->lic_compute_shdr.ParameterLocation("epsilon"),
            this->epsilon.Param<core::param::FloatParam>()->Value());

        glUniform1i(this->lic_compute_shdr.ParameterLocation("coloring"),
        ...
        this->lic_compute_shdr.Dispatch(static_cast<int>(std::ceil(rt_resolution[0] / 8.0f)),
            static_cast<int>(std::ceil(rt_resolution[1] / 8.0f)), 1);
    ...
    vislib::math::Cuboid<float> SurfaceLICRenderer::combineBoundingBoxes(
        std::vector<vislib::math::Cuboid<float>> boxVec) {
