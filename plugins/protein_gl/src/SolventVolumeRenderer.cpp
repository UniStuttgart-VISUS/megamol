...
protein_gl::SolventVolumeRenderer::SolventVolumeRenderer(void)
        ...
        , countSolMolParam(
              "countSolventMol", "compute number of solvent molecules (by residue type) in hydration shell")
        , currentFrameId(0)
        , atomCount(0)
        , proteinFBO(nullptr)
        , volumeTex(0)
        ...
        , lastUpdateVolumeTextureTime(0)
        , forceUpdateColoringMode(true)
        , molVisVbo(0) {
...
bool protein_gl::SolventVolumeRenderer::loadShader(
...
bool protein_gl::SolventVolumeRenderer::create(void) {
    ...
    if (!loadShader(
            this->clippedSphereShader, "protein::std::sphereClipPlaneVertex", "protein::std::sphereClipPlaneFragment"))
        return false;

    // Load cylinder shader
    if (!loadShader(
            this->cylinderSolventShader, "protein::std::cylinderSolventVertex", "protein::std::cylinderFragment"))
        return false;

    // Load shader for hydrogen bonds
    //    if( !loadShader( this->hbondLineSolventShader, "protein::std::hbondLineSolventVertex",
    //    "protein::std::hbondLineSolventFragment" ) )
    //        return false;

    // Load volume texture generation shader
    if (!loadShader(this->updateVolumeShaderMoleculeVolume, "volume::std::updateVolumeVertex",
            "volume::std::updateSolventVolumeFragmentDensity"))
        return false;
...
#pragma message( \
    "WARNING: since the user can controll density and coloing filter radius separately, this optimization might skip too many vertices!")
...
#endif
    if (!loadShader(
            this->updateVolumeShaderSolventColor, updateVolumeVertex, "volume::std::updateSolventVolumeFragmentColor"))
        return false;
    if (!loadShader(
            this->updateVolumeShaderHBondColor, updateVolumeVertex, "volume::std::updateSolventVolumeFragmentHBondClr"))
        return false;
    ...
    if (!loadShader(
            this->dualIsosurfaceShader, "volume::std::volumeDualIsoVertex", "volume::std::volumeDualIsoFragment"))
        return false;

    // Load visible solvent molecule shader
    if (!loadShader(this->visMolShader, "protein::std::visibleSolventMoleculeVertex",
            "protein::std::visibleSolventMoleculeFragment"))
        return false;

    // Load solvent type counting shader
    if (!loadShader(this->solTypeCountShader, "protein::std::solventTypeCountVertex",
            "protein::std::visibleSolventMoleculeFragment"))
        return false;
...
bool protein_gl::SolventVolumeRenderer::GetExtents(core_gl::view::CallRender3DGL& call) {
    ...
    /*    core_gl::view::CallRender3DGL *protrencr3d =
       this->protRendererCallerSlot.CallAs<core_gl::view::CallRender3DGL>(); vislib::math::Point<float, 3> protrenbbc;
        if( protrencr3d ) {
...
void protein_gl::SolventVolumeRenderer::ColorAtom(
    ...
    case ProteinColor::ColoringMode::ELEMENT: {
        ...
        vislib::math::Vector<float, 3>& c =
            this->colorLookupTable[mol->AtomTypeIndices()[atomIdx] % this->colorLookupTable.Count()];
        atomColor[0] = c.X();
        atomColor[1] = c.Y();
        atomColor[2] = c.Z();
#endif
...
bool protein_gl::SolventVolumeRenderer::Render(core_gl::view::CallRender3DGL& call) {
    ...
    if (frameID0 != frameID1 && this->interpolParam.Param<param::BoolParam>()->Value()) {
        ...
        float threshold =
            vislib::math::Min(bbox.Width(), vislib::math::Min(bbox.Height(), bbox.Depth())) * 0.4f; // 0.75f;
...
#pragma omp parallel for
        for (int atomIdx = 0; atomIdx < (int)mol->AtomCount(); atomIdx++) {
            ...
            if (interHBondFrame0Ptr)
                interHBondFrame0Ptr[atomIdx] =
                    /*inter*/ localInter < 0.5 ? interHBondFrame0Ptr[atomIdx] : interHBondFrame1Ptr[atomIdx];
        }
    }
...
void protein_gl::SolventVolumeRenderer::FindVisibleSolventMolecules(MolecularDataCall* mol) {
    ...
    glBufferData(GL_PIXEL_PACK_BUFFER_EXT, this->molVisFboWidth * this->molVisFboHeight * 3 * sizeof(float), NULL,
        GL_DYNAMIC_DRAW_ARB);
    ...
    glUniform1f(this->visMolShader.ParameterLocation("solventMolThreshold"),
    ...
    //        if( t[( i * 8192 + j) * 3 + 0] > 0 ) {
    //            std::cout << "value " << i << "," << j << " = " << t[( i * 8192 + j) * 3 + 0] << " " << t[( i * 8192 +
    //            j) * 3 + 1] << " " << t[( i * 8192 + j) * 3 + 2] << std::endl;
    //        }
    //    }
    //}
    //// ... DEBUG
    ...
    if (this->solTypeCountFbo == nullptr || this->solTypeCountFbo->getWidth() != mol->AtomSolventResidueCount()) {
        this->solTypeCountFbo = std::make_shared<glowl::FramebufferObject>(mol->AtomSolventResidueCount(), 1);
    ...
    for (unsigned int i = 0; i < mol->AtomSolventResidueCount(); i++) {
        ...
        megamol::core::utility::log::Log::DefaultLog.WriteMsg(
            megamol::core::utility::log::Log::LEVEL_INFO, "Solvent Molecule Type %i, count: %i", i, int(counter[i]));
    }
...
void protein_gl::SolventVolumeRenderer::RenderHydrogenBounds(MolecularDataCall* mol, const float* atomPos) {
    ...
    glUniform3fvARB(
        this->cylinderSolventShader.ParameterLocation("invBBoxExtend"), 1, invBBoxDimension.PeekComponents());
    glUniform1fARB(this->cylinderSolventShader.ParameterLocation("solventMolThreshold"),
        solventMolThreshold.Param<param::FloatParam>()->Value());
...
/*
 * Render the molecular data in stick mode. Special case when using solvent rendering: only render solvent molecules
 * near the isosurface between the solvent and the molecule.
 */
...
#pragma omp parallel for
        for (int atomIdx = 0; atomIdx < (int)mol->AtomCount(); atomIdx++) {
            ...
            this->vertSpheres[4 * atomIdx + 2] = atomPos[3 * atomIdx + 2];
            this->vertSpheres[4 * atomIdx + 3] =
                (spaceFilling ? mol->AtomTypes()[mol->AtomTypeIndices()[atomIdx]].Radius() : stickRadius) *
                atomRadiusFactor;
        }
    }
...
#pragma omp parallel for private(quatC, tmpVec, ortho, dir, position, angle)
        for (int atomIdx = 0; atomIdx < (int)mol->ConnectionCount(); atomIdx++) {
            ...
            // thomasbm: hotfix for jumping molecules near bounding box
            if (this->inParaCylinders[2 * atomIdx + 1] > mol->AtomTypes()[mol->AtomTypeIndices()[idx0]].Radius() +
    ...
    glUniform1fARB(this->sphereSolventShader.ParameterLocation("solventMolThreshold"),
        solventMolThreshold.Param<param::FloatParam>()->Value());
    ...
    /*if (!spaceFilling)*/ {
        ...
        glUniform1iARB(this->cylinderSolventShader.ParameterLocation("volumeSampler"), 0);
        glUniform3fvARB(
            this->cylinderSolventShader.ParameterLocation("minBBox"), 1, bbox.GetOrigin().PeekCoordinates());
        glUniform3fvARB(
            this->cylinderSolventShader.ParameterLocation("invBBoxExtend"), 1, invBBoxDimension.PeekComponents());
        glUniform1fARB(this->cylinderSolventShader.ParameterLocation("solventMolThreshold"),
            solventMolThreshold.Param<param::FloatParam>()->Value());
 ...
 */
bool protein_gl::SolventVolumeRenderer::RenderMolecularData(
    ...
    if (playSlot->Param<param::BoolParam>()->Value() || this->forceUpdateVolumeTexture ||
        fabs(call->Time() - this->lastUpdateVolumeTextureTime) > 0.01) {
...
void protein_gl::SolventVolumeRenderer::ParameterRefresh(core_gl::view::CallRender3DGL* call, MolecularDataCall* mol) {
    ...
    if (this->coloringModeSolventParam.IsDirty() || this->coloringModePolymerParam.IsDirty() ||
        this->coloringModeVolSurfParam.IsDirty()) {
    ...
    /*    // update lsit of residues that compose the solvent ...
        if (this->solventResidues.IsDirty()) {
            ...
            vislib::Array<vislib::TString> residueTokens = vislib::StringTokeniser<vislib::TCharTraits>::Split(str, ';',
       true);
...
void protein_gl::SolventVolumeRenderer::UpdateVolumeTexture(MolecularDataCall* mol) {
    ...
    if (update_vol.Count() < mol->AtomCount() * 4)
        update_vol.SetCount(mol->AtomCount() * 4);
    float* updatVolumeTextureAtoms = &update_vol[0];

    if (update_clr.Count() < mol->AtomCount() * 3)
        update_clr.SetCount(mol->AtomCount() * 3);
    float* updatVolumeTextureColors = &update_clr[0];
    ...
    #pragma omp parallel for
        for( atomCnt = 0; atomCnt < mol->AtomCount(); ++atomCnt ) {
            updatVolumeTextureAtoms[atomCnt*4+0] = ( this->atomPosInter[3*atomCnt+0] + this->translation.X()) *
    this->scale; updatVolumeTextureAtoms[atomCnt*4+1] = ( this->atomPosInter[3*atomCnt+1] + this->translation.Y()) *
    this->scale; updatVolumeTextureAtoms[atomCnt*4+2] = ( this->atomPosInter[3*atomCnt+2] + this->translation.Z()) *
    this->scale; updatVolumeTextureAtoms[atomCnt*4+3] = mol->AtomTypes()[mol->AtomTypeIndices()[atomCnt]].Radius() *
    this->scale;
        }*/
    ...
    //    "rendering %d polymer atoms", atomCntDensity );
    ...
    glUniform1f(this->updateVolumeShaderMoleculeVolume.ParameterLocation("densityScale"),
        this->volDensityScale * accumulateFactor);
    ...
    glUniform3f(this->updateVolumeShaderMoleculeVolume.ParameterLocation("invVolRes"), 1.0f / float(this->volumeSize),
    ...
    for (unsigned int z = 0; z < this->volumeSize; ++z) {
        ...
        glUniform1f(this->updateVolumeShaderMoleculeVolume.ParameterLocation("sliceDepth"),
            (float(z) + 0.5f) / float(this->volumeSize));
        // draw all atoms as points, using w for radius
    ...
    //    megamol::core::utility::log::Log::DefaultLog.WriteMsg ( megamol::core::utility::log::Log::LEVEL_INFO,
    //    "rendering %d solvent atoms", atomCntColor );
    vislib_gl::graphics::gl::GLSLShader& volumeColorShader =
        coloringByHydroBonds ? this->updateVolumeShaderHBondColor : this->updateVolumeShaderSolventColor;
    volumeColorShader.Enable();
    // set shader params
    glUniform1f(volumeColorShader.ParameterLocation("filterRadius"),
        this->colorFilterRadiusParam.Param<param::FloatParam>()->Value() /*this->volFilterRadius*/);
    glUniform1f(volumeColorShader.ParameterLocation("densityScale"),
        this->colorIntensityScaleParam.Param<param::FloatParam>()->Value() *
            accumulateFactor /*this->volDensityScale*/);
    ...
    glUniform3f(volumeColorShader.ParameterLocation("invVolRes"), 1.0f / float(this->volumeSize),
        1.0f / float(this->volumeSize), 1.0f / float(this->volumeSize));
...
void protein_gl::SolventVolumeRenderer::RenderVolume(vislib::math::Cuboid<float> boundingbox) {
    ...
    glUniform2f(
        this->volumeShader.ParameterLocation("screenResInv"), 1.0f / float(this->width), 1.0f / float(this->height));
...
void protein_gl::SolventVolumeRenderer::RayParamTextures(vislib::math::Cuboid<float> boundingbox) {
    ...
    if (this->renderIsometric) {
        glUniform3f(this->volRayStartShader.ParameterLocation("translate"), 0.0f, 0.0f, 0.0f);
    } else {
        glUniform3fv(this->volRayStartShader.ParameterLocation("translate"), 1, trans.PeekComponents());
    }
    ...
    glUniform2f(this->volRayLengthShader.ParameterLocation("zNearFar"), near_plane, far_plane);

    if (this->renderIsometric) {
        glUniform3f(this->volRayLengthShader.ParameterLocation("translate"), 0.0f, 0.0f, 0.0f);
    } else {
        glUniform3fv(this->volRayLengthShader.ParameterLocation("translate"), 1, trans.PeekComponents());
    }
    glUniform1f(this->volRayLengthShader.ParameterLocation("scale"), this->scale);
