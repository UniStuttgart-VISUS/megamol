...
void protein_gl::SolventVolumeRenderer::ColorAtom(float *atomColor, MolecularDataCall *mol, protein::Color::ColoringMode colorMode, int atomIdx, int residueIdx ) {
    switch(colorMode) {
    default:
...
/*    protein::Color::MakeColorTable( mol,
    ...
    auto solventColorMode = static_cast<protein::Color::ColoringMode>(this->coloringModeSolventParam.Param<param::EnumParam>()->Value());
...
void protein_gl::SolventVolumeRenderer::ParameterRefresh( core_gl::view::CallRender3DGL *call, MolecularDataCall *mol) {
    ...
    if( this->coloringModeSolventParam.IsDirty() || this->coloringModePolymerParam.IsDirty() || this->coloringModeVolSurfParam.IsDirty() ) {
        ...
        if (this->coloringModeVolSurfParam.Param<param::EnumParam>()->Value() == VOlCM_HydrogenBondStats)
            this->coloringModeSolventParam.Param<param::EnumParam>()->SetValue(static_cast<int>(protein::Color::ColoringMode::RESIDUE));
        this->forceUpdateVolumeTexture = true;
        this->forceUpdateColoringMode = true;
    }
