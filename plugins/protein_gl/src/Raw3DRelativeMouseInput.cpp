...
void Raw3DRelativeMouseInput::Initialize(void) {
    ...
    if ((this->hWnd = ::CreateWindowW(Raw3DRelativeMouseInput::WNDCLASSNAME, L"", WS_POPUP, CW_USEDEFAULT,
             CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, HWND_MESSAGE, 0, this->hInstance, 0)) == NULL) {
        throw SystemException(__FILE__, __LINE__);
    }
 ...
 */
LRESULT WINAPI Raw3DRelativeMouseInput::wndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
#pragma warning(disable : 4312)
    Raw3DRelativeMouseInput* thisPtr =
        reinterpret_cast<Raw3DRelativeMouseInput*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));
#pragma warning(default : 4312)
    ...
    if (msg == WM_INPUT) {
        ...
        megamol::core::utility::log::Log::DefaultLog.WriteMsg(
            megamol::core::utility::log::Log::LEVEL_INFO, "Raw Input Received");
#endif
        ...
        if (GetRawInputData(hRawInput, RID_INPUT, pRawInput, &inputSize, sizeof(RAWINPUTHEADER)) ==
            static_cast<UINT>(-1)) {
            megamol::core::utility::log::Log::DefaultLog.WriteMsg(
                megamol::core::utility::log::Log::LEVEL_ERROR, "Could not get initial raw input data.");
        }
        ...
        if (nCount == static_cast<UINT>(-1)) {
            megamol::core::utility::log::Log::DefaultLog.WriteMsg(
                megamol::core::utility::log::Log::LEVEL_ERROR, "Could not get first raw input data buffer.");
        }

#if INPUT_DEBUG
        megamol::core::utility::log::Log::DefaultLog.WriteMsg(
            megamol::core::utility::log::Log::LEVEL_INFO, "Initial buffer count: %d", nCount);
#endif
...
#if INPUT_DEBUG
            megamol::core::utility::log::Log::DefaultLog.WriteMsg(
                megamol::core::utility::log::Log::LEVEL_INFO, "Next buffer count: %d", nCount);
#endif
        }
...
bool Raw3DRelativeMouseInput::ProcessRawData(PRAWINPUT pRawInput) {
    ...
    if (GetRawInputDeviceInfo(pRawInput->header.hDevice, RIDI_DEVICEINFO, &sDeviceInfo, &cbSize) != cbSize) {
        megamol::core::utility::log::Log::DefaultLog.WriteMsg(
            megamol::core::utility::log::Log::LEVEL_ERROR, "Error while retrieving raw input device info.");
...
#if INPUT_DEBUG
                megamol::core::utility::log::Log::DefaultLog.WriteMsg(
                    megamol::core::utility::log::Log::LEVEL_INFO, "Button data received: %x", keyState);
#endif
            } else { // error of unknown nature
                megamol::core::utility::log::Log::DefaultLog.WriteMsg(
                    megamol::core::utility::log::Log::LEVEL_ERROR, "Data received was not pos, rot, or button data.");
