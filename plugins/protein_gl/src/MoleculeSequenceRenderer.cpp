    ...
    for (unsigned int chain = 0; chain < chainCount; ++chain) {
        ...
        for (unsigned int molecule = firstMolecule; molecule < firstMolecule + moleculeCount; ++molecule) {
            ...
            for (unsigned int residue = firstResidue; residue < firstResidue + residueCount; ++residue) {
                ...
                this->paintButton(static_cast<float>(residuePosition), 0.0f, buttonWidth - buttonSpacing, buttonHeight,
                    0.4f, 0.4f, 0.6f, data->ResidueTypeNames()[residues[residue]->Type()].PeekBuffer());
            ...
            this->paintButton(static_cast<float>(moleculePosition), 15.0f, residuesWidth - buttonSpacing, buttonHeight,
                1.0f, 0.8f, 0.0f, tmpStr.PeekBuffer());
        ...
        this->paintButton(static_cast<float>(chainPosition), 30.0f, static_cast<float>(moleculeWidth), buttonHeight,
            1.0f, 0.0f, 0.0f, tmpStr);
...
void MoleculeSequenceRenderer::paintButton(
    float x, float y, float w, float h, float r, float g, float b, const char* text) {
