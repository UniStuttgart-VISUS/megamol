...
namespace probe {
    ...
    ExtractSkeleton::ExtractSkeleton()
            : Module(), _getDataCall("getData", ""), _deployLineCall("deployCenterline", "") {
    ...
    bool ExtractSkeleton::compute(float* vertices, uint32_t num_vertices, uint32_t num_components) {
        ...
        if (!this->_bbox.IsBoundingBoxValid()) {
            ...
            for (uint32_t i = 0; i < num_vertices; i++) {
                ...
                points_for_triangulation.emplace_back(Point(vertices[num_components * i + 0], vertices[num_components * i + 1],vertices[num_components * i + 2]));
            }
        ...
        for (auto he : sm.halfedges()) {
            if (sm.is_border(he)) {
                core::utility::log::Log::DefaultLog.WriteError("[ExtractSkeleton] Precondition violation. Mesh has boarder.");
            }
        }
        ...
        if (cm->hasUpdate()) {
            ...
            for (auto& attrib : data->accessMeshes().begin()->second.attributes) {
                if (attrib.semantic == mesh::MeshDataAccessCollection::POSITION) {
                    ...
                    num_vertices =
                        attrib.byte_size /
                        (mesh::MeshDataAccessCollection::getByteSize(attrib.component_type) * attrib.component_cnt);
