                    ...
                    for (int k = 0; k < transformed_patch_samples.rows(); ++k) {
                        ...
                        for (int l = 0; l < transformed_fixed_points.rows(); ++l) {
                            glm::vec2 current_partner = {
                                transformed_fixed_points(l, 0), transformed_fixed_points(l, 1)};
                        ...
                        before_movement_buffer(k, 0) = 2 * transformed_patch_samples(k, 0) -
                                                       old_transfromed_patch_samples(k, 0) +
                                                       force.x * std::pow(delta_t, 2);
                        before_movement_buffer(k, 1) = 2 * transformed_patch_samples(k, 1) -
                                                       old_transfromed_patch_samples(k, 1) +
                                                       force.y * std::pow(delta_t, 2);
...
#pragma omp parallel for
    ...
    bool megamol::probe::PlaceProbes::placeProbes() {
        ...
        if (this->_method_slot.Param<core::param::EnumParam>()->Value() == 0) {
            ...
            this->forceDirectedSampling(_mesh->accessMeshes()[0]);
    ...
    bool megamol::probe::PlaceProbes::placeByCenterline(
        ...
        for (uint32_t i = 0; i < probe_count; i++) {
            ...
            min_to_vert[0] =
                vertex_accessor[vertex_step * i + 0] - centerline_accessor[centerline_step * min_index + 0];
            min_to_vert[1] =
                vertex_accessor[vertex_step * i + 1] - centerline_accessor[centerline_step * min_index + 1];
            min_to_vert[2] =
                vertex_accessor[vertex_step * i + 2] - centerline_accessor[centerline_step * min_index + 2];
