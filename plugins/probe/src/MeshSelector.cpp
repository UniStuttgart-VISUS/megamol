 ...
 #pragma omp parallel for
    ...
    while (_face_to_mesh_map.size() < _neighbor_map.size()) {
        ...
        while (not_yet_all_neighbors_detected) {
            ...
            if (open_ends.empty()) not_yet_all_neighbors_detected = false;
    ...
    auto index_ptr = reinterpret_cast<uint32_t*>(cd->getData()->accessMesh()[0].indices.data);
...
bool MeshSelector::getData(core::Call& call) {
    ...
    const auto mesh_ptr = cd->getData()->accessMesh().data();
    ...
    if (something_changed || _recalc) {
        ...
        if (_splitMeshSlot.Param<core::param::BoolParam>()->Value()) {
            if (_grouped_indices.empty() || cd->hasUpdate()) this->splitMesh(cd->getData()->accessMesh()[0]);
            ...
            for (int i = 0; i < mesh_ptr->attributes.size(); i++) {
                if (mesh_ptr->attributes.data()->semantic == mesh::MeshDataAccessCollection::POSITION) {
                    _meshNumberSlot.Param<core::param::FlexEnumParam>()->AddValue(std::to_string(num_meshes));
                    num_meshes++;
                }
            }
            _mesh_attribs.resize(1);
            _mesh_attribs[0] = mesh_ptr->attributes[_selected_mesh];

            _mesh_indices.resize(1);
            _mesh_indices[0] = mesh_ptr->indices;
        }
    ...
    if (_splitMeshSlot.Param<core::param::BoolParam>()->Value()) {
        ...
        mesh.addMesh(
            mesh_ptr->attributes, _mesh_indices[0], mesh::MeshDataAccessCollection::PrimitiveType::TRIANGLES);
        // debugging stuff
        //for (int i = 0; i < _grouped_indices.size(); ++i) {
        ...
        //    mesh.addMesh(_mesh_attribs, _mesh_indices[i], mesh::MeshDataAccessCollection::PrimitiveType::TRIANGLES);
        //}
    } else {
        mesh.addMesh(_mesh_attribs, _mesh_indices[0], mesh::MeshDataAccessCollection::PrimitiveType::TRIANGLES);
    }
