...
#pragma omp parallel for
    ...
    void ConstructHull::generateNormals(Surface_mesh& mesh) {
        ...
        CGAL::pca_estimate_normals<CGAL::Parallel_if_available_tag>(points, nb_neighbors,
            ...
            core::utility::log::Log::DefaultLog.WriteError(std::string("[ConstructHull] NormalEstimation exited with ").append(e.what()).c_str());
        }
    ...
    bool ConstructHull::getData(core::Call& call) {
        ...
        if (!_mesh_for_call) {
            ...
            _mesh_for_call->addMesh(identifier, _mesh.second, _mesh.first, mesh::MeshDataAccessCollection::PrimitiveType::TRIANGLES);
        }
    ...
    void ConstructHull::activateMesh(const Surface_mesh& shell, std::vector<std::array<float,3>>& vertices, std::vector<std::array<uint32_t,3>>& indices) {
