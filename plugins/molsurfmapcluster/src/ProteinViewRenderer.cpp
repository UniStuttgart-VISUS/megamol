...
bool ProteinViewRenderer::create(void) {
    ...
    if (!this->GetCoreInstance()->ShaderSourceFactory().MakeShaderSource("molsurfMapOverlay::vertex", texVertShader)) {
    ...
    if (!this->GetCoreInstance()->ShaderSourceFactory().MakeShaderSource(
            "molsurfMapOverlay::fragment", texFragShader)) {
        core::utility::log::Log::DefaultLog.WriteMsg(
 ...
 */
bool ProteinViewRenderer::GetExtents(view::CallRender3D_2& call) {
 ...
 */
bool ProteinViewRenderer::Render(view::CallRender3D_2& call) {
    megamol::geocalls::CallTriMeshData* ctmd = this->getDataSlot.CallAs<megamol::geocalls::CallTriMeshData>();
    if (ctmd == NULL)
        return false;
    ...
    glm::vec4 viewvew = cam.view_vector();
    glm::vec3 viewdir = glm::vec3(viewvew.x, viewvew.y, viewvew.z);
    ...
    if (this->nameSlot.Param<param::StringParam>()->Value() != vislib::TString("")) {
        std::string name = this->nameSlot.Param<param::StringParam>()->Value().PeekBuffer();
