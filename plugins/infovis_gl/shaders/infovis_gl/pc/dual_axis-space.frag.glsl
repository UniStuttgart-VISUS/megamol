...
float bilinearInterpolation(usampler2DArray sampler, vec3 coords){
    ...
    //uvec4 samples = textureGather(sampler, coords, 0);
    //float s00 = fromFixPoint(samples.w);
    //float s10 = fromFixPoint(samples.z);
    //float s01 = fromFixPoint(samples.x);
    //float s11 = fromFixPoint(samples.y);
...
void main() {
    ...
    float left_range = abs(left_max-left_min);
    float right_range = abs(right_max-right_min);
    
    float tri_area_left = 0.5 * left_range * relx;
    float tri_area_right = 0.5 * right_range * (1.0-relx);

    float dual_length = length(vec2(left_min,right_max)-vec2(left_max,right_min));
    ...
    int sample_cnt = int(float(dual_space_height) * min(dual_length,max_dual_length));
    float weight = max(1.0/float(dual_space_height), dual_length);
    
    for(int sample_idx = 0; sample_idx < sample_cnt; ++sample_idx){
        float left = left_min + (float(sample_idx)/float(sample_cnt-1)) * left_range;
        float right = right_max - (float(sample_idx)/float(sample_cnt-1)) * right_range;
    
        result += abs(bilinearInterpolation(imgRead, vec3(left,right,float(cdim))));
    }

    result = (result * weight) / float(sample_cnt);

    //fragOut = vec4(weight,0.0,0.0,1.0);
    //return;
