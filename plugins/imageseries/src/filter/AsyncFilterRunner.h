...
public:
    ...
    AsyncImageData run(Args&&... args) {
        ...
        return runFunction([filter]() { return (*filter)(); }, filter->getByteSize(), filter->getHash());
    }

    AsyncImageData runFunction(std::function<ImageData()> filter, std::size_t byteSize, AsyncImageData2D::Hash hash);
};
