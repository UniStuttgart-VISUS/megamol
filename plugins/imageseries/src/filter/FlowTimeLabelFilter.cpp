...
FlowTimeLabelFilter::ImagePtr FlowTimeLabelFilter::operator()() {
    ...
    // connect sinks to close nodes with similar time step
    {
        constexpr auto max_time_factor =
            2u; // time difference must not be larger than this factor times the incoming edge's time difference
        constexpr auto max_distance_factor =
            2.0f; // distance must not be larger than this factor times the incoming edge's implied length
        const auto max_angle =
            0.5f * (std::atan(1) * 4.0f); // enforce angle between incoming and outgoing edge to be smaller than 90°

        auto outbound = simplifiedGraph.getOutboundEdges();
        auto inbound = simplifiedGraph.getInboundEdges();

        auto dot = [](glm::vec2 lhs, glm::vec2 rhs) { return lhs.x * rhs.x + lhs.y * rhs.y; };

        for (graph::GraphData2D::NodeID sinkID = 0; sinkID < simplifiedGraph.getNodeCount(); ++sinkID) {
            if (outbound[sinkID].empty() && !inbound[sinkID].empty()) {
                const auto& sink = simplifiedGraph.getNode(sinkID);
                const auto incoming_vector =
                    sink.centerOfMass - simplifiedGraph.getNode(inbound[sinkID][0]).centerOfMass;
                const auto incoming_vector_sqr_length = dot(incoming_vector, incoming_vector);

                const auto max_time_distance =
                    max_time_factor * (sink.frameIndex - simplifiedGraph.getNode(inbound[sinkID][0]).frameIndex);
                float min_sqr_distance = max_distance_factor * incoming_vector_sqr_length;
                graph::GraphData2D::NodeID closest_node = -1;

                for (graph::GraphData2D::NodeID nodeID = 0; nodeID < simplifiedGraph.getNodeCount(); ++nodeID) {
                    if (!outbound[nodeID].empty()) {
                        const auto& node = simplifiedGraph.getNode(nodeID);

                        if (node.frameIndex + max_time_distance >= sink.frameIndex) {
                            const auto vector = node.centerOfMass - sink.centerOfMass;
                            const auto vector_sqr_length = dot(vector, vector);

                            const auto angle =
                                std::acos(dot(vector, incoming_vector) /
                                          (std::sqrt(vector_sqr_length) * std::sqrt(incoming_vector_sqr_length)));

                            if (vector_sqr_length < min_sqr_distance && std::abs(angle) < max_angle) {
                                min_sqr_distance = vector_sqr_length;
                                closest_node = nodeID;
                            }
                        }
                    }
                ...
                if (closest_node != -1) {
                    simplifiedGraph.addEdge(graph::GraphData2D::Edge{sinkID, closest_node});

                    ++simplifiedGraph.getNode(sinkID).edgeCountOut;
                    ++simplifiedGraph.getNode(closest_node).edgeCountIn;

                    simplifiedGraph.getNode(sinkID).modified = 1;
                    simplifiedGraph.getNode(closest_node).modified = 1;
                }
            }
        }
    }
