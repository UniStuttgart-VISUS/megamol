...
bool adiosDataSource::create() {
    try {
        ...
        io = std::make_shared<adios2::IO>(adiosInst.DeclareIO("Input"));
        io->SetEngine("BP3");
...
#ifdef WITH_MPI
        vislib::sys::Log::DefaultLog.WriteError("[adiosDataSource] Exception, STOPPING PROGRAM from rank %d", this->mpiRank);
#else
