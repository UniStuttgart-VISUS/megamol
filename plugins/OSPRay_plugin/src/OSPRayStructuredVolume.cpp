...
bool OSPRayStructuredVolume::readData(megamol::core::Call& call) {
    ...
    this->structureContainer.gridOrigin = gridOrigin;
    this->structureContainer.gridSpacing = gridSpacing;
    this->structureContainer.dimensions = dimensions;
    this->structureContainer.voxelCount = voxelCount;
    this->structureContainer.maxDim = maxDim;
    this->structureContainer.valueRange = {static_cast<float>(metadata->MinValues[0]), static_cast<float>(metadata->MaxValues[0])};
    ...
    this->structureContainer.clippingBoxLower = cbl;
    ...
    this->structureContainer.clippingBoxUpper = cbu;

    std::array<float,4> sData = {this->sliceNormal.Param<core::param::Vector3fParam>()->Value().GetX(),
    ...
    this->structureContainer.sliceData = sData;

    this->structureContainer.isoValue = this->IsoValue.Param<core::param::FloatParam>()->Value();
