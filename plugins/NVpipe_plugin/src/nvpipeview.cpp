...
*/
nvpipe::NVpipeView::NVpipeView(void) : AbstractView3D(), AbstractCamParamSync(), cam(), camParams(),
...
IPslot("IP", "IP to push encoded stream to"){
	...
	this->storeCameraSettingsSlot.SetUpdateCallback(&NVpipeView::onStoreCamera);
	...
	this->restoreCameraSettingsSlot.SetUpdateCallback(&NVpipeView::onRestoreCamera);
	...
	this->resetViewSlot.SetUpdateCallback(&NVpipeView::onResetView);
	...
	this->viewKeyRotLeftSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->viewKeyRotRightSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->viewKeyRotUpSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->viewKeyRotDownSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->viewKeyRollLeftSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->viewKeyRollRightSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->viewKeyZoomInSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->viewKeyZoomOutSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->viewKeyMoveLeftSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->viewKeyMoveRightSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->viewKeyMoveUpSlot.SetUpdateCallback(&NVpipeView::viewKeyPressed);
	...
	this->IPslot << new param::StringParam("");
	this->IPslot.SetUpdateCallback(&NVpipeView::onSetIP);
	this->MakeSlotAvailable(&this->IPslot);
...
void nvpipe::NVpipeView::initEncoder() {
	...
	size_t serverDeviceBufferSize = this->camParams->TileRect().Width() * this->camParams->TileRect().Height() * 4;
	...
	serverSendBufferSize = serverDeviceBufferSize; // Reserve enough space for encoded output
	serverSendBuffer = new uint8_t[serverSendBufferSize];
...
/*
* nvpipe::NVpipeView::onSetIP
*/
void nvpipe::NVpipeView::onSetIP() {
...
/*
* nvpipe::NVpipeView::GetCameraSyncNumber
*/
unsigned int nvpipe::NVpipeView::GetCameraSyncNumber(void) const {
	return this->camParams->SyncNumber();
}


/*
* nvpipe::NVpipeView::SerialiseCamera
*/
void nvpipe::NVpipeView::SerialiseCamera(vislib::Serialiser& serialiser) const {
	this->camParams->Serialise(serialiser);
}


/*
* nvpipe::NVpipeView::DeserialiseCamera
*/
void nvpipe::NVpipeView::DeserialiseCamera(vislib::Serialiser& serialiser) {
...
void nvpipe::NVpipeView::Render(const mmcRenderViewContext& context) {
	...
	*/
	size_t numBytes_ver = this->camParams->TileRect().Width() * this->camParams->TileRect().Height() * 4;
	uint8_t* rgba = new uint8_t[numBytes_ver];


	glBindFramebuffer(GL_READ_FRAMEBUFFER, FboId);
	glReadPixels(0, 0, this->camParams->TileRect().Width(), this->camParams->TileRect().Height(), GL_RGBA, GL_UNSIGNED_BYTE, rgba);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, this->camParams->TileRect().Width(), this->camParams->TileRect().Height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, rgba);
	...
	cudaMemcpy2DFromArray(this->serverDeviceBuffer, this->camParams->TileRect().Width() * 4, serverArray, 0, 0, this->camParams->TileRect().Width() * 4, this->camParams->TileRect().Height(), cudaMemcpyDeviceToDevice);
	cudaGraphicsUnmapResources(1, &serverGraphicsResource);

	size_t numBytes = this->serverSendBufferSize;
	nvp_err_t encodeStatus = nvpipe_encode(this->encoder, this->serverDeviceBuffer, this->camParams->TileRect().Width() * this->camParams->TileRect().Height() * 4, serverSendBuffer, &numBytes, this->camParams->TileRect().Width(), this->camParams->TileRect().Height(), NVPIPE_RGBA);
...
/*
* nvpipe::NVpipeView::ResetView
*/
void nvpipe::NVpipeView::ResetView(void) {
	using namespace vislib::graphics;
	VLTRACE(VISLIB_TRCELVL_INFO, "NVpipeView::ResetView\n");
...
/*
* nvpipe::NVpipeView::Resize
*/
void nvpipe::NVpipeView::Resize(unsigned int width, unsigned int height) {
...
/*
* nvpipe::NVpipeView::SetCursor2DButtonState
*/
void nvpipe::NVpipeView::SetCursor2DButtonState(unsigned int btn, bool down) {
	if (!this->toggleMouseSelection) {
		this->cursor2d.SetButtonState(btn, down);
	} else {
		// stuff from protein::NVpipeViewMouse
		switch (btn) {
		case 0: // left
			MouseFlagsSetFlag(this->mouseFlags,
				MOUSEFLAG_BUTTON_LEFT_DOWN, down);
			break;
		case 1: // right
			MouseFlagsSetFlag(this->mouseFlags,
				MOUSEFLAG_BUTTON_RIGHT_DOWN, down);
			break;
		case 2: // middle
			MouseFlagsSetFlag(this->mouseFlags,
				MOUSEFLAG_BUTTON_MIDDLE_DOWN, down);
			break;
		}
	}
}
...
/*
* nvpipe::NVpipeView::SetCursor2DPosition
*/
void nvpipe::NVpipeView::SetCursor2DPosition(float x, float y) {
	if (!this->toggleMouseSelection) {
		...
		if (cr3d) {
			...
			if ((*cr3d)(3)) {
				...
				MouseFlagsResetAllChanged(this->mouseFlags);
...
/*
* nvpipe::NVpipeView::SetInputModifier
*/
void nvpipe::NVpipeView::SetInputModifier(mmcInputModifier mod, bool down) {
...
/*
* nvpipe::NVpipeView::OnRenderView
*/
bool nvpipe::NVpipeView::OnRenderView(Call& call) {
	float overBC[3];
...
/*
* nvpipe::NVpipeView::UpdateFreeze
*/
void nvpipe::NVpipeView::UpdateFreeze(bool freeze) {
...
/*
* nvpipe::NVpipeView::unpackMouseCoordinates
*/
void nvpipe::NVpipeView::unpackMouseCoordinates(float &x, float &y) {
...
/*
* nvpipe::NVpipeView::create
*/
bool nvpipe::NVpipeView::create(void) {
...
/*
* nvpipe::NVpipeView::renderBBox
*/
void nvpipe::NVpipeView::renderBBox(void) {
...
/*
* nvpipe::NVpipeView::renderBBoxBackside
*/
void nvpipe::NVpipeView::renderBBoxBackside(void) {
...
/*
* nvpipe::NVpipeView::renderBBoxFrontside
*/
void nvpipe::NVpipeView::renderBBoxFrontside(void) {
	::glDisable(GL_LIGHTING);
	::glEnable(GL_BLEND);
	::glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	::glEnable(GL_DEPTH_TEST);
...
/*
* nvpipe::NVpipeView::renderLookAt
*/
void nvpipe::NVpipeView::renderLookAt(void) {
	const vislib::math::Cuboid<float>& boundingBox
		= this->bboxs.WorldSpaceBBox();
	...
	bool xin = true;
	if (lap.X() < minp.X()) { lap.SetX(minp.X()); xin = false; } else if (lap.X() > maxp.X()) { lap.SetX(maxp.X()); xin = false; }
	bool yin = true;
	if (lap.Y() < minp.Y()) { lap.SetY(minp.Y()); yin = false; } else if (lap.Y() > maxp.Y()) { lap.SetY(maxp.Y()); yin = false; }
	bool zin = true;
	if (lap.Z() < minp.Z()) { lap.SetZ(minp.Z()); zin = false; } else if (lap.Z() > maxp.Z()) { lap.SetZ(maxp.Z()); zin = false; }
...
/*
* nvpipe::NVpipeView::renderSoftCursor
*/
void nvpipe::NVpipeView::renderSoftCursor(void) {
	::glMatrixMode(GL_PROJECTION);
	::glLoadIdentity();
	::glMatrixMode(GL_MODELVIEW);
	::glLoadIdentity();
...
/*
* nvpipe::NVpipeView::OnGetCamParams
*/
bool nvpipe::NVpipeView::OnGetCamParams(CallCamParamSync& c) {
...
/*
* nvpipe::NVpipeView::onStoreCamera
*/
bool nvpipe::NVpipeView::onStoreCamera(param::ParamSlot& p) {
...
/*
* nvpipe::NVpipeView::onRestoreCamera
*/
bool nvpipe::NVpipeView::onRestoreCamera(param::ParamSlot& p) {
...
/*
* nvpipe::NVpipeView::onResetView
*/
bool nvpipe::NVpipeView::onResetView(param::ParamSlot& p) {
	this->ResetView();
	return true;
}
...
/*
* nvpipe::NVpipeView::viewKeyPressed
*/
bool nvpipe::NVpipeView::viewKeyPressed(param::ParamSlot& p) {
...
/*
* nvpipe::NVpipeView::onToggleButton
*/
bool nvpipe::NVpipeView::onToggleButton(param::ParamSlot& p) {
...
/*
* nvpipe::NVpipeView::renderViewCube
*/
void nvpipe::NVpipeView::renderViewCube(void) {
	::glEnable(GL_BLEND);
