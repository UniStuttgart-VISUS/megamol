        ...
        for (unsigned int y = 0; y < sy - 1; y++) {
            ...
            for (unsigned int x = 0; x < sx - 1; x++) {
                ...
                for (unsigned int j = 0; j < 8; j++) {
                    cubeValues[j] = vol[
                        (x + trisoup::volumetrics::MarchingCubeTables::a2fVertexOffset[j][0])
                ...
                for (unsigned int tetIdx = 0; tetIdx < 6; tetIdx++) {
                    ...
                    for (unsigned int j = 0; j < 8; j++) {
                        pts[j].Set(
                            p.X() + static_cast<float>(trisoup::volumetrics::MarchingCubeTables::a2fVertexOffset[j][0]) * cellSizeX,
                            p.Y() + static_cast<float>(trisoup::volumetrics::MarchingCubeTables::a2fVertexOffset[j][1]) * cellSizeY,
                            p.Z() + static_cast<float>(trisoup::volumetrics::MarchingCubeTables::a2fVertexOffset[j][2]) * cellSizeZ);
                    }
