...
#include "NGSphereBufferArrayRenderer.h"
...
void APIENTRY MyOtherFunkyDebugCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length,
 ...
 * moldyn::NGSphereRenderer::~NGSphereRenderer
 */
NGSphereBufferArrayRenderer::~NGSphereBufferArrayRenderer(void) {
    this->Release();
}
 ...
 */
bool NGSphereBufferArrayRenderer::create(void) {
#ifdef DEBUG_BLAHBLAH
    glDebugMessageCallback(MyOtherFunkyDebugCallback, NULL);
#endif
    vislib::graphics::gl::ShaderSource vert, frag;

    if (!instance()->ShaderSourceFactory().MakeShaderSource("NGsphereBufferArray::vertex", vert)) {
        return false;
    }
    if (!instance()->ShaderSourceFactory().MakeShaderSource("NGsphereBufferArray::fragment", frag)) {
        return false;
    }
...
void NGSphereBufferArrayRenderer::setPointers(MultiParticleDataCall::Particles &parts, GLuint vertBuf, const void *vertPtr, GLuint colBuf, const void *colPtr) {
 ...
 */
bool NGSphereBufferArrayRenderer::Render(Call& call) {
#ifdef DEBUG_BLAHBLAH
