...
DWORD MMSPDDataSource::buildFrameIndex(void *userdata) {
    ...
    vislib::sys::File f;
    if (!f.Open(that->filename.Param<core::param::FilePathParam>()->Value().generic_u8string().c_str(),
...
bool MMSPDDataSource::filenameChanged(core::param::ParamSlot& slot) {
    ...
    if (!this->file->Open(this->filename.Param<core::param::FilePathParam>()->Value().generic_u8string().c_str(),
