...
void io::XYZLoader::assertData(void) {
    ...
    if (!file.Open(filenameSlot.Param<core::param::FilePathParam>()->Value(), vislib::sys::File::READ_ONLY, vislib::sys::File::SHARE_READ, vislib::sys::File::OPEN_ONLY)) {
        vislib::sys::Log::DefaultLog.WriteError("Unable to open file \"%s\"", vislib::StringA(filenameSlot.Param<core::param::FilePathParam>()->Value()).PeekBuffer());
        return;
    }
    ...
    if (hasCountLineSlot.Param<core::param::BoolParam>()->Value()) {
        ...
        try {
            ...
            vislib::sys::Log::DefaultLog.WriteWarn("Unable to parse atom count from first line in \"%s\"",
    ...
    while (reader.ReadLine(l)) {
        ...
        if (parts.Count() != (hasEl ? 4 : 3)) {
            if (warning) {
                vislib::sys::Log::DefaultLog.WriteWarn("Problem parsing \"%s\":",
                    vislib::StringA(filenameSlot.Param<core::param::FilePathParam>()->Value()).PeekBuffer());
                warning = false;
            }
        }
        if (parts.Count() < (hasEl ? 4 : 3)) {
            vislib::sys::Log::DefaultLog.WriteError("Line %u has too few tokens; line will be ignored", lineNum);
            continue;
        }
        if (parts.Count() > (hasEl ? 4 : 3)) {
            vislib::sys::Log::DefaultLog.WriteWarn("Line %u has too many tokens; trailing tokens will be ignored", lineNum);
        }
        ...
        try {
            ...
            vislib::sys::Log::DefaultLog.WriteError("Failed to parse coordinates at line %u: (%f, %f, %f); line will be ignored", lineNum, x, y, z);
            continue;
        }
