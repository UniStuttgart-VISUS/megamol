...
void io::TclMolSelectionLoader::load(void) {
    ...
    if (!file.LoadFile(filenameSlot.Param<core::param::FilePathParam>()->Value().c_str(), vislib::sys::ASCIIFileBuffer::PARSING_WORDS)) {
        megamol::core::utility::log::Log::DefaultLog.WriteError("Unable to read file \"%s\"", filenameSlot.Param<core::param::FilePathParam>()->Value().c_str());
        return;
    }
