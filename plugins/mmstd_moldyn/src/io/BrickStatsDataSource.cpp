...
bool BrickStatsDataSource::create(void) {
    ...
    if (BrickStatsCall::GetTypeSize() != sizeof(BrickStatsCall::BrickStatsType)) {
        vislib::sys::Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR,
...
bool BrickStatsDataSource::filenameChanged(param::ParamSlot& slot) {
    ...
    if (!this->file->Open(this->filename.Param<param::FilePathParam>()->Value(), File::READ_ONLY, File::SHARE_READ, File::OPEN_ONLY)) {
        vislib::sys::Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR, "Unable to open MMPLD-File \"%s\".", vislib::StringA(
    ...
    while (!this->file->IsEOF()) {
        ...
        if (arr.Count() != 14) {
            vislib::sys::Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR, "Unable to parse line %lu: not enough fields (%u)", lineNum, arr.Count());
            continue;
        }
        try {
            ...
            vislib::sys::Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR, "parse error in line %lu", lineNum);
        }
    }
