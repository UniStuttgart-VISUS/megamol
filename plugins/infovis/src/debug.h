...
using debug_action = std::function<void(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length,
    GLchar const* message, GLvoid const* userParam)>;
 ...
 */
static std::function<std::string(void const*)> debug_message_user_param_handler = [](void const* userParam) {
    return std::string();
};
...
inline std::string debugSourceToString(GLenum source, std::string defaultText) {
...
inline std::string debugTypeToString(GLenum type, std::string defaultText) {
#define _GL_ENUM_TO_STRING_HELPER(theGLEnum__)                                                                         \
    case theGLEnum__:                                                                                                  \
        return #theGLEnum__
...
inline std::string debugSeverityToString(GLenum severity, std::string defaultText) {
#define _GL_ENUM_TO_STRING_HELPER(theGLEnum__)                                                                         \
    case theGLEnum__:                                                                                                  \
        return #theGLEnum__
...
inline debug_action make_debug_action_ostream(std::ostream& stream) {
...
inline void debug_action_break(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length,
    GLchar const* message, GLvoid const* userParam) {
#ifdef _MSC_VER
    __debugbreak();
#else // _MSC_VER
...
inline void debug_action_throw(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length,
    GLchar const* message, GLvoid const* userParam) {
    throw std::exception();
}

inline void debug_message_callback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length,
    GLchar const* message, GLvoid const* userParam) {
...
inline void enable_all_debug_messages() {
    glDebugMessageControl(GL_DONT_CARE, GL_DONT_CARE, GL_DONT_CARE, 0, 0, GL_TRUE);
}

inline void ignore_debug_message(GLenum source, GLenum type, GLuint id) {
    glDebugMessageControl(source, type, GL_DONT_CARE, 1, &id, GL_FALSE);
}

template <typename Iterator> void ignore_debug_messages(Iterator first, Iterator last, GLenum source, GLenum type) {
...
inline void ignore_debug_messages(std::initializer_list<debug_message_spec> ignores) {
...
inline void object_label(GLenum identifier, GLuint name, std::string label) {
