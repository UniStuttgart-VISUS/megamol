 ...
 */
std::string TableColumnScaler::ModuleName = std::string("TableColumnScaler");
...
TableColumnScaler::TableColumnScaler()
    ...
    this->dataOutSlot.SetCallback(
        TableDataCall::ClassName(), TableDataCall::FunctionName(0), &TableColumnScaler::processData);
    this->dataOutSlot.SetCallback(
        TableDataCall::ClassName(), TableDataCall::FunctionName(1), &TableColumnScaler::getExtent);
    ...
    try {
        ...
        if (this->datahash != inCall->DataHash() || this->frameID != inCall->GetFrameID()) {
            ...
            auto selectionString =
                vislib::TString(this->columnSelectorSlot.Param<core::param::StringParam>()->Value().c_str());
