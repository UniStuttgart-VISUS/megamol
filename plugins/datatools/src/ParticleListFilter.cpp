...
ParticleListFilter::ParticleListFilter(void)
    ...
    this->includeAllSlot << new core::param::ButtonParam();
    this->MakeSlotAvailable(&this->includeAllSlot);

    this->globalColorMapComputationSlot << new core::param::BoolParam(true);
    this->MakeSlotAvailable(&this->globalColorMapComputationSlot);
    this->includeHiddenInColorMapSlot << new core::param::BoolParam(true);
...
bool ParticleListFilter::getExtentCallback(core::Call& call) {
    ...
    if (outMpdc != NULL) {
        ...
        static_cast<core::AbstractGetData3DCall*>(inMpdc)->operator=(*outMpdc);
        if ((*inMpdc)(1)) {
            //*outMpdc = *inMpdc;
            static_cast<core::AbstractGetData3DCall*>(outMpdc)->operator=(*inMpdc);
            outMpdc->SetDataHash(this->datahashParticlesOut);
            return true;
        }
    }
