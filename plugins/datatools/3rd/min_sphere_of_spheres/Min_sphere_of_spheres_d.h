...
#include <Min_sphere_of_spheres_d_support_set.h>
...
template <class Traits> class Min_sphere_of_spheres_d {
...
public: // construction and destruction:
    ...
    inline Min_sphere_of_spheres_d(InputIterator begin, InputIterator end, const Traits& traits = Traits())
        : t(traits), ss(t), e(0) {
        is_up_to_date = false;
    ...
    template <typename InputIterator> inline void insert(InputIterator begin, InputIterator end) {
    ...
    template <typename InputIterator> inline void set(InputIterator begin, InputIterator end) {
        clear();
        insert(begin, end);
    }
...
public: // predicates and accessors:
    ...
    inline const Traits& traits() const { return t; }
...
private:
    ...
    bool find_farthest(int from, int to, int& i, Tag_true use_sqrt, Tag_false is_exact);
    bool find_farthest(int from, int to, int& i, Tag_true use_sqrt, Tag_true is_exact);
    bool find_farthest(int from, int to, int& i, Tag_false use_sqrt, Tag_false is_exact);
    bool find_farthest(int from, int to, int& i, Tag_false use_sqrt, Tag_true is_exact);

private:
    std::vector<Sphere> S;                                // list of the added bals
    std::vector<const Sphere*> l;                         // list of pointers to the added bals
    Min_sphere_of_spheres_d_impl::Support_set<Traits> ss; // current support set
...
Min_sphere_of_spheres_d<Traits>::Min_sphere_of_spheres_d(const Traits& traits) : t(traits), ss(t), e(0) {
    is_up_to_date = true;
    CGAL_MINIBALL_ASSERT(is_neg(ss.radius(), ss.disc())); // makes sure
                                                          // that initially no ball is enclosed (cf. contains()).
}

template <class Traits> void Min_sphere_of_spheres_d<Traits>::prepare(int size) {
    S.reserve(size);
    // (The vector l will be reserve()'d in update().)
}

template <class Traits> void Min_sphere_of_spheres_d<Traits>::insert(const Sphere& b) {
...
template <class Traits> void Min_sphere_of_spheres_d<Traits>::clear() {
    S.clear();
    l.clear();
    ss.reset();
    e = 0;
...
template <class Traits> bool Min_sphere_of_spheres_d<Traits>::is_empty() {
    if (!is_up_to_date) update();
    return is_neg(ss.radius(), ss.disc());
}

template <class Traits>
const typename Min_sphere_of_spheres_d<Traits>::Result& Min_sphere_of_spheres_d<Traits>::radius() {
    if (!is_up_to_date) update();
...
template <class Traits>
const typename Min_sphere_of_spheres_d<Traits>::FT& Min_sphere_of_spheres_d<Traits>::discriminant() {
    if (!is_up_to_date) update();
    CGAL_MINIBALL_ASSERT(!is_empty());
    return ss.disc();
}

template <class Traits> inline void Min_sphere_of_spheres_d<Traits>::update() {
    // set up the vector l containing pointers to the balls in S:
    CGAL_MINIBALL_ASSERT(l.size() == 0);
    for (typename std::vector<Sphere>::const_iterator it = S.begin(); it != S.end(); ++it) l.push_back(&(*it));
...
template <class Traits> inline bool Min_sphere_of_spheres_d<Traits>::is_valid() {
    if (!is_up_to_date) update();
    return is_valid(Is_exact());
}

template <class Traits>
inline typename Min_sphere_of_spheres_d<Traits>::Support_iterator Min_sphere_of_spheres_d<Traits>::support_begin() {
    if (!is_up_to_date) update();
    return Support_iterator(*l.begin());
}

template <class Traits>
inline typename Min_sphere_of_spheres_d<Traits>::Support_iterator Min_sphere_of_spheres_d<Traits>::support_end() {
    if (!is_up_to_date) update();
...
#    include <Min_sphere_of_spheres_d.cpp>
#    include <Min_sphere_of_spheres_d_pivot.cpp>
#endif
