...
OSPRayVelvetMaterial::OSPRayVelvetMaterial(void)
    ...
    this->velvetHorizonScatteringColor << new core::param::Vector3fParam(
        vislib::math::Vector<float, 3>(0.75f, 0.1f, 0.1f));
...
bool OSPRayVelvetMaterial::InterfaceIsDirty() {
    if (this->velvetBackScattering.IsDirty() || this->velvetHorizonScatteringColor.IsDirty() ||
        this->velvetHorizonScatteringFallOff.IsDirty() || this->velvetReflectance.IsDirty()) {
