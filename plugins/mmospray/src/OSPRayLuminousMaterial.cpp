...
void OSPRayLuminousMaterial::readParams() {
    ...
    lm.lumIntensity = this->lumIntensity.Param<core::param::FloatParam>()->Value();
    lm.lumTransparency = this->lumTransparency.Param<core::param::FloatParam>()->Value();

    materialContainer.material = lm;
}

bool OSPRayLuminousMaterial::InterfaceIsDirty() {
    if (this->lumColor.IsDirty() || this->lumIntensity.IsDirty() || this->lumTransparency.IsDirty()) {
