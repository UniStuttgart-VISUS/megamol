...
bool OSPRayTransform::getTransformationCallback(core::Call& call) {
    ...
    if (tc_in != NULL) {
        this->readParams();
        tc_in->setTransformationContainer(
            std::make_shared<OSPRayTransformationContainer>(this->_transformationContainer));
    }
...
bool OSPRayTransform::InterfaceIsDirty() {
    if (this->_pos.IsDirty() || this->_rot.IsDirty() || this->_scale.IsDirty()) {
