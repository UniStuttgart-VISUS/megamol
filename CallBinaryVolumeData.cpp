 ...
 */
CallBinaryVolumeData::CallBinaryVolumeData(void) : core::AbstractGetData3DCall(),
        sizeX(0), sizeY(0), sizeZ(0), voxSizeX(1.0f), voxSizeY(1.0f), voxSizeZ(1.0f), volume(NULL) {
    // intentionally empty
}
...
bool CallBinaryVolumeData::GetSafeVoxel(unsigned int x, unsigned int y, unsigned int z) const {
    return ((x >= this->sizeX) || (y >= this->sizeY) || (z >= this->sizeZ) || (this->volume == NULL)) ? false
