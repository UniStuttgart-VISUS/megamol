...
function(target_link_libraries NAME)
  ...
  elseif(NOT ADDING_EXTERNAL)
    message(FATAL_ERROR "Cannot specify link libraries for target \"${NAME}\" which is not built by this project.")
  endif()
endfunction()

function(set_target_properties NAME)
  if(TARGET ${NAME})
    string(REGEX REPLACE "(;([A-Z_]+);)" "###\\2###" OPTIONS "${ARGN}")
    string(REGEX REPLACE "(###([A-Z_]+);)" "###\\2###" OPTIONS "${OPTIONS}")
    string(REPLACE ";" " " OPTIONS "${OPTIONS}")
    string(REPLACE "###" ";" OPTIONS "${OPTIONS}")
    string(REPLACE ";;" ";\"\";" OPTIONS "${OPTIONS}")
    string(REGEX REPLACE ";$" ";\"\"" OPTIONS "${OPTIONS}")

    _set_target_properties(${NAME} ${OPTIONS})
  elseif(NOT ADDING_EXTERNAL)
    message(FATAL_ERROR "Cannot specify properties for target \"${NAME}\" which is not built by this project.")
  endif()
endfunction()
...
function(add_test NAME)
  if(TARGET ${NAME})
    _add_test(${NAME} ${ARGN})
  elseif(NOT ADDING_EXTERNAL)
    message(FATAL_ERROR "Cannot specify test for target \"${NAME}\" which is not built by this project.")
  endif()
endfunction()
