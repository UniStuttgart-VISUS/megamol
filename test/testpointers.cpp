...
void TestSmartPtr(void) {
	...
	::AssertEqual("NULL has no references.", int0._GetCnt(), static_cast<unsigned int>(0));
    ::AssertFalse("Smart pointer initialisation", int1.IsNull());
    ::AssertEqual("Smart pointer content check", *int1, 5);
	::AssertEqual("Refrence count of 1 is 1.", int1._GetCnt(), static_cast<unsigned int>(1));
	...
	::AssertEqual("Refrence count after assignment.", int1._GetCnt(), static_cast<unsigned int>(2));
	...
	::AssertEqual("Refrence count after copy ctor.", int2._GetCnt(), static_cast<unsigned int>(3));
	...
	::AssertEqual("Refrence count decrement after NULL asignment.", int1._GetCnt(), static_cast<unsigned int>(2));

    int2 = int0;
    ::AssertTrue("NULL smart pointer assignment", int2.IsNull());
    ::AssertFalse("No assignment cross effects.", int1.IsNull());
	::AssertEqual("Refrence count decrement after smart pointer assignment.", int1._GetCnt(), static_cast<unsigned int>(1));

    int1 = NULL;
    ::AssertTrue("Last reference destroyed", int1.IsNull());
	::AssertEqual("Reference count 0 after object destroyed.", int1._GetCnt(), static_cast<unsigned int>(0));
