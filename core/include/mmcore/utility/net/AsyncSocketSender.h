...
public:
     ...
     */
    typedef void (*CompletedFunction)(
        const DWORD result, const void* data, const SIZE_T cntBytesSent, void* userContext);
     ...
     * TODO
     */
    void Send(const void* data, const SIZE_T cntBytes, const CompletedFunction onCompleted, void* userContext,
        const bool doNotCopy = false, const INT timeout = Socket::TIMEOUT_INFINITE, const INT flags = 0,
        const bool forceSend = true);

    /**
     * TODO
     */
    inline void Send(const void* data, const SIZE_T cntBytes, sys::Event* evt, const bool doNotCopy,
        const INT timeout = Socket::TIMEOUT_INFINITE, const INT flags = 0, const bool forceSend = true) {
        this->Send(data, cntBytes, AsyncSocketSender::onSendCompletedEvt, evt, doNotCopy, timeout, flags, forceSend);
    }

    /**
     * TODO
     */
    inline void Send(const void* data, const SIZE_T cntBytes, sys::Semaphore* semaphore, const bool doNotCopy,
        const INT timeout = Socket::TIMEOUT_INFINITE, const INT flags = 0, const bool forceSend = true) {
        this->Send(
            data, cntBytes, AsyncSocketSender::onSendCompletedSem, semaphore, doNotCopy, timeout, flags, forceSend);
    }
     ...
     */
    inline void Send(const void* data, const SIZE_T cntBytes, const INT flags = 0) {
        this->Send(data, cntBytes, NULL, NULL, false, Socket::TIMEOUT_INFINITE, flags, true);
    }
...
private:
    ...
    typedef struct SendTask_t {
        ...
        inline bool operator==(const struct SendTask_t& rhs) const {
            return ((this->data0 == rhs.data0) && (this->data1 == rhs.data1) && (this->cntBytes == rhs.cntBytes) &&
                    (this->onCompleted == rhs.onCompleted) && (this->userContext == rhs.userContext) &&
                    (this->timeout == rhs.timeout) && (this->flags == rhs.flags) && (this->forceSend == rhs.forceSend));
        }
    } SendTask;
     ...
     * @param userContext
     */
    static void onSendCompletedEvt(const DWORD result, const void* data, const SIZE_T cntBytesSent, void* userContext);
     ...
     * @param userContext
     */
    static void onSendCompletedSem(const DWORD result, const void* data, const SIZE_T cntBytesSent, void* userContext);
     ...
     *                     callback.
     */
    void finaliseSendTask(SendTask& task, const DWORD result, const SIZE_T cntBytesSent);
