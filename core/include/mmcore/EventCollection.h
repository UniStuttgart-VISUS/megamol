...
public:
    struct AbstractEvent {
        AbstractEvent(size_t frame_id) : frame_id(frame_id) {}
     ...
     *
     */
    template <typename EventType> void add(std::unique_ptr<AbstractEvent>&& event);
...
private:
    ...
    std::unordered_multimap<std::type_index, std::unique_ptr<AbstractEvent>> m_events;
...
template <typename EventType> inline void EventCollection::add(std::unique_ptr<AbstractEvent>&& event) {

    m_events.emplace(std::type_index(typeid(EventType)), std::forward<std::unique_ptr<AbstractEvent>>(event));
}
