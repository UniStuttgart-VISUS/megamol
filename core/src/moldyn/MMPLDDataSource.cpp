...
bool moldyn::MMPLDDataSource::filenameChanged(param::ParamSlot& slot) {
    ...
    if (!this->file->Open(this->filename.Param<param::FilePathParam>()->Value().c_str(), File::READ_ONLY, File::SHARE_READ, File::OPEN_ONLY)) {
        megamol::core::utility::log::Log::DefaultLog.WriteMsg(Log::LEVEL_ERROR, "Unable to open MMPLD-File \"%s\".", this->filename.Param<param::FilePathParam>()->Value().c_str());
