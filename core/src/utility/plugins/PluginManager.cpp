...
PluginManager::collection_type PluginManager::ContinueLoad200(
        ...
        for (unsigned int li = 0; li < comp_info->libs_cnt; li++) {
            ...
            if (vislib::StringA("MegaMolCore").Equals(lvi.name)) {
                ...
                if ((v.GetMajorVersionNumber() != mmcoreVer.GetMajorVersionNumber())
                    ...
                    vislib::sys::Log::DefaultLog.WriteError("Plugin %s seems incompatible with MegaMolCore: core \"%s\" != plugin \"%s\" ",
                ...
                if ((v.GetMajorVersionNumber() != vislibVer.GetMajorVersionNumber())
                    ...
                    vislib::sys::Log::DefaultLog.WriteError("Plugin %s seems incompatible with vislib: vislib \"%s\" != plugin \"%s\" ",
                        vislib::StringA(path.c_str()).PeekBuffer(),
                ...
                vislib::sys::Log::DefaultLog.WriteInfo("Plugin %s compatibility with %s is not checked",
                    vislib::StringA(path.c_str()).PeekBuffer(), lvi.name);
            }
        }

        if (!MegaMolCore_compatibility_checked) {
            vislib::sys::Log::DefaultLog.WriteWarn("Plugin %s compatibility with MegaMolCore is not checked", vislib::StringA(path.c_str()).PeekBuffer());
        }
        if (!vislib_compatibility_checked) {
            vislib::sys::Log::DefaultLog.WriteWarn("Plugin %s compatibility with vislib is not checked", vislib::StringA(path.c_str()).PeekBuffer());
        }

    } else {
        vislib::sys::Log::DefaultLog.WriteWarn("Plugin %s did not provide compatibility information", vislib::StringA(path.c_str()).PeekBuffer());
    }
    ...
    if (!plugin) {
        vislib::sys::Log::DefaultLog.WriteError("Unable to create an instance of plugin %s.", vislib::StringA(path.c_str()).PeekBuffer());
        throw vislib::Exception("Plugin instantation error", __FILE__, __LINE__);
    }
    if (plugin200 == nullptr) {
        vislib::sys::Log::DefaultLog.WriteError("Plugin %s created an instance object of incompatible type version (Not 200).", vislib::StringA(path.c_str()).PeekBuffer());
        throw vislib::Exception("Plugin instantation error", __FILE__, __LINE__);
    }
    ...
    plugin200->connectStatics(
        Plugin200Instance::StaticConnectorType::Log, static_cast<void*>(&vislib::sys::Log::DefaultLog));
    ...
    // report success
    rv.push_back(plugin);
    vislib::sys::Log::DefaultLog.WriteInfo("Plugin %s loaded: %u Modules, %u Calls",
