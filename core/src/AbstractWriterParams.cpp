...
std::pair<bool, std::string> AbstractWriterParams::getNextFilename() {
    ...
    const std::string filepath = filepath_param.find_last_of("/\\") != std::string::npos
                                     ...
                                     : "";
    const std::string filename = filepath_param.find_last_of("/\\") != std::string::npos
                                     ? filepath_param.substr(filepath_param.find_last_of("/\\") + 1)
                                     : filepath_param;
    const std::string filename_we =
        filename.find_last_of('.') != std::string::npos ? filename.substr(0, filename.find_last_of('.')) : filename;
    const std::string file_ext =
        filename.find_last_of('.') != std::string::npos ? filename.substr(filename.find_last_of('.')) : "";
    ...
    if (this->writeMode.Param<param::EnumParam>()->Value() == 1 && std::ifstream(filepath_param).good()) {
        ...
        megamol::core::utility::log::Log::DefaultLog.WriteWarn(
            "Did not write file '%s'. File already exists.", filepath_param.c_str());
        ...
        postfix =
            "_" + count_string.substr(count_string.length() - this->countLength.Param<param::IntParam>()->Value());
