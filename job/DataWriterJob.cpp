...
DWORD job::DataWriterJob::Run(void *userData) {
    ...
    if (dwcc == NULL) {
        Log::DefaultLog.WriteMsg(Log::LEVEL_WARN,
            "Data writer job not connected to any data writer module\n");
        return -1;
    }

    if (!(*dwcc)(DataWriterCtrlCall::CALL_GETCAPABILITIES)) {
        Log::DefaultLog.WriteMsg(Log::LEVEL_WARN,
            "Unable to query data writer capabilities");
    } else {
    ...
    if ((*dwcc)(DataWriterCtrlCall::CALL_RUN)) {
        ...
        Log::DefaultLog.WriteMsg(Log::LEVEL_WARN, "DataWriterJob \"%s\" terminated with false", this->FullName().PeekBuffer());
        return -2;
