...
void Stride::GetChains( megamol::protein::CallProteinData *pdi)
{
	...
	//////////////////////////////////////////////////////
	// build chains from Protein Data Interface
	//////////////////////////////////////////////////////
	
	ProteinChainCnt = pdi->ProteinChainCount();
	...
	for( cntCha = 0; cntCha < ProteinChainCnt; ++cntCha)
	{
		...
		ProteinChain[cntCha]->NRes = pdi->ProteinChain( cntCha).AminoAcidCount();
		// set data for all residues
		for( cntRes = 0; cntRes < ProteinChain[cntCha]->NRes; ++cntRes )
		{
			firstAtom = pdi->ProteinChain( cntCha).AminoAcid()[cntRes].FirstAtomIndex();
			atomCount = pdi->ProteinChain( cntCha).AminoAcid()[cntRes].AtomCount();
			...
			_snprintf( r->ResType, RES_FIELD, pdi->AminoAcidName(
						 pdi->ProteinChain( cntCha).AminoAcid()[cntRes].NameIndex()).PeekBuffer());
#else /* _WIN32 */
			snprintf( r->PDB_ResNumb, RES_FIELD, "%i", cntRes);
			snprintf( r->ResType, RES_FIELD, pdi->AminoAcidName(
						 pdi->ProteinChain( cntCha).AminoAcid()[cntRes].NameIndex()).PeekBuffer());
#endif /* _WIN32 */
			// set atomic values
			...
			{
#ifdef _WIN32
				_snprintf( r->AtomType[cntAtm], AT_FIELD, pdi->AtomTypes()[
						pdi->ProteinAtomData()[cntAtm+firstAtom].TypeIndex()].Name().PeekBuffer());
#else /* _WIN32 */
				snprintf( r->AtomType[cntAtm], AT_FIELD, pdi->AtomTypes()[
						pdi->ProteinAtomData()[cntAtm+firstAtom].TypeIndex()].Name().PeekBuffer());
#endif /* _WIN32 */
				r->Coord[cntAtm][0] = pdi->ProteinAtomPositions()[3 *( firstAtom + cntAtm) + 0];
				r->Coord[cntAtm][1] = pdi->ProteinAtomPositions()[3 *( firstAtom + cntAtm) + 1];
				r->Coord[cntAtm][2] = pdi->ProteinAtomPositions()[3 *( firstAtom + cntAtm) + 2];
				r->Occupancy[cntAtm] = pdi->ProteinAtomData()[cntAtm+firstAtom].Occupancy();
				r->TempFactor[cntAtm] = pdi->ProteinAtomData()[cntAtm+firstAtom].TempFactor();
	...
	for( cntCha = 0; cntCha < ProteinChainCnt; ++ cntCha )
	{
		if( ProteinChain[cntCha] )
		{
			c = ProteinChain[cntCha];
			for( cntRes = 0; cntRes < c->NRes; ++cntRes )
			{
				if( c->Rsd[cntRes] )
				{
					printf( "RES %i (%s) in CH %i has %i atoms\n", cntRes, c->Rsd[cntRes]->ResType, cntCha, c->Rsd[cntRes]->NAtom);
				}
				else
					printf( "STRIDE_NO RES %i in CH %i\n", cntRes, cntCha);
			}
		}
		else
			printf( "STRIDE_NO CHAIN %i\n", cntCha);
	}
	*/
	// DEBUG END
...
bool Stride::WriteToInterface( megamol::protein::CallProteinData *pdi) {
	if( pdi ) {
		...
		std::vector<CallProteinData::SecStructure> sec;
		...
		for ( Cn = 0; Cn < ProteinChainCnt; ++Cn ) {
			...
			if ( !ProteinChain[Cn]->Valid )
				continue;
			// clear old secondary structures
			sec.clear();
			
			// set initial values for first sec struct elem
			firstRes = 0;
			firstAtm = pdi->ProteinChain( Cn).AminoAcid()[0].FirstAtomIndex();
			resCnt = 1;
			...
			for ( i = 1; i < ProteinChain[Cn]->NRes; i++ )
			{
				...
				else
				{
					...
					sec.push_back( CallProteinData::SecStructure());
					sec.back().SetPosition( firstAtm, atmCnt, firstRes, resCnt);
					if( type == 'G' || type == 'H' || type == 'I' )
						sec.back().SetType( CallProteinData::SecStructure::TYPE_HELIX);
					else if( type == 'E' )
						sec.back().SetType( CallProteinData::SecStructure::TYPE_SHEET);
					else
						sec.back().SetType( CallProteinData::SecStructure::TYPE_COIL);
					// start new sec struct elem
					firstRes = i;
					firstAtm = pdi->ProteinChain( Cn).AminoAcid()[i].FirstAtomIndex();
					resCnt = 1;
					atmCnt = pdi->ProteinChain( Cn).AminoAcid()[i].AtomCount();
			...
			for( i = 0; i < (int)sec.size(); ++i )
			{
				...
				/*
				printf( "CHAIN %4i RES %5i #RES %5i ASN %i\n",
						  Cn,
						  pdi->ProteinChain( Cn).SecondaryStructure()[i].FirstAminoAcidIndex(),
						  pdi->ProteinChain( Cn).SecondaryStructure()[i].AminoAcidCount(),
						  (int)pdi->ProteinChain( Cn).SecondaryStructure()[i].Type() );
				*/
			}
		}
	} else {
...
bool Stride::WriteToInterface( megamol::protein::CallProteinMovementData *pdi) {
	if( pdi ) {
        ...
        std::vector<CallProteinMovementData::SecStructure> sec;
		...
		for ( Cn = 0; Cn < ProteinChainCnt; ++Cn ) {
			...
			for ( i = 1; i < ProteinChain[Cn]->NRes; i++ ) {
				...
				if( ProteinChain[Cn]->Rsd[i]->Prop->Asn == type ) {
					...
					sec.back().SetPosition( firstAtm, atmCnt, firstRes, resCnt);
					if( type == 'G' || type == 'H' || type == 'I' )
                        sec.back().SetType( CallProteinMovementData::SecStructure::TYPE_HELIX);
					else if( type == 'E' )
						sec.back().SetType( CallProteinMovementData::SecStructure::TYPE_SHEET);
					else
						sec.back().SetType( CallProteinMovementData::SecStructure::TYPE_COIL);
					// start new sec struct elem
					firstRes = i;
					firstAtm = pdi->ProteinChain( Cn).AminoAcid()[i].FirstAtomIndex();
					resCnt = 1;
