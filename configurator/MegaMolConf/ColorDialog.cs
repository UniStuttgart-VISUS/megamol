...
namespace MegaMolConf {
    public partial class ColorDialog : Form {
        public ColorDialog(string oldValue) {
            InitializeComponent();
        ...
        public void Parse(string value) {
            ...
            var components = Array.ConvertAll(tokens, Double.Parse);

            res = (uint)components.Length / 4;
            ...
            {
                r_histo[i] = (float)components[i * 4];
                g_histo[i] = (float)components[i * 4 + 1];
                b_histo[i] = (float)components[i * 4 + 2];
                a_histo[i] = (float)components[i * 4 + 3];
            }
        }
