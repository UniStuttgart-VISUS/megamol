...
#parse user commands
while getopts "hp:dDcmirtC:" opt; do
  case $opt in
  h)
    ...
    echo
    echo "$0 -dcmi -p ~/my_inst"
    echo
    echo "  creates the build tree subdirectors for release and debug, and invokes 'cmake', 'make' and 'make install' in both build trees. Additionally the installation prefix is set to '~/my_inst'."
    echo
    exit 0
    ;;
  p)
