 ...
 */
void Color::MakeColorTable(const MolecularDataCall *mol, bool forceRecompute,
    ...
    if( atomColorTable.IsEmpty() ) {
        if( currentColoringMode == ELEMENT ) {
            ...
            for( cntSecS = 0; cntSecS < mol->SecondaryStructureCount();
                 ++cntSecS ) {
                idx = mol->SecondaryStructures()[cntSecS].
                        FirstAminoAcidIndex();
                cnt = idx + mol->SecondaryStructures()[cntSecS].
                        AminoAcidCount();
            ...
            // get the last atom of the last res of the last mol of the
            // first chain
            cntChain = 0;
            ...
            for( cnt = 0; cnt < mol->AtomCount(); ++cnt ) {
                ...
                if( cnt > cntAtom ) {
                    // get the last atom of the last res of the last mol of
                    // the next chain
                    cntChain++;
