...
bool ContourRendererDeferred::create(void) {
    ...
    if(!ci->ShaderSourceFactory().MakeShaderSource("proteinDeferred::contour::final::vertex", vertSrc)) {
        vislib::sys::Log::DefaultLog.WriteMsg(vislib::sys::Log::LEVEL_ERROR,
            "%s: Unable to load contour vertex shader source", this->ClassName() );
        return false;
    }
    if(!ci->ShaderSourceFactory().MakeShaderSource("proteinDeferred::contour::final::fragment", fragSrc)) {
        vislib::sys::Log::DefaultLog.WriteMsg(vislib::sys::Log::LEVEL_ERROR,
