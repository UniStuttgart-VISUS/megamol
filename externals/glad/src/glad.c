/*
    ...
    OpenGL loader generated by glad 0.1.14a0 on Wed Aug  9 10:24:40 2017.
...
GLAPI int isExtAvailable(const char *extensionName) {
	int res = has_ext(extensionName);
	if (res == 1) {
		return GL_TRUE;
	}
	return GL_FALSE;
...
GLAPI int areExtsAvailable(const char *extensionsNames) {
...
GLAPI int ogl_IsVersionGEQ(int testMajorVersion, int testMinorVersion) {
	if (max_loaded_major > testMajorVersion) return 1;
	if (max_loaded_major < testMajorVersion) return 0;
	if (max_loaded_minor >= testMinorVersion) return 1;
	return 0;
}
