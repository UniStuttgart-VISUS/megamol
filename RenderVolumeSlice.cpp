...
RenderVolumeSlice::RenderVolumeSlice(void) : Renderer3DModule(),
    ...
    this->getVolSlot.SetCompatibleCall<CallVolumeDataDescription>();
 ...
 * RenderVolumeSlice::RenderVolumeSlice
 */
bool RenderVolumeSlice::GetCapabilities(Call& call) {
 ...
 * RenderVolumeSlice::RenderVolumeSlice
 */
bool RenderVolumeSlice::GetExtents(Call& call) {
    view::CallRender3D *cr = dynamic_cast<view::CallRender3D*>(&call);
    if (cr == NULL) return false;

    CallVolumeData *c2 = this->getVolSlot.CallAs<CallVolumeData>();
    if ((c2 != NULL) && ((*c2)(1))) {
 ...
 * RenderVolumeSlice::RenderVolumeSlice
 */
bool RenderVolumeSlice::Render(Call& call) {
    view::CallRender3D *cr = dynamic_cast<view::CallRender3D*>(&call);
    if (cr == NULL) return false;

    // get volume data
    CallVolumeData *cvd = this->getVolSlot.CallAs<CallVolumeData>();
    ...
    float minVal = this->lowValSlot.Param<param::FloatParam>()->Value();
    float maxVal = this->highValSlot.Param<param::FloatParam>()->Value();
