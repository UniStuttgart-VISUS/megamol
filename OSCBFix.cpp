        ...
        for (UINT64 j = 0; j < parts.GetCount(); j++) {
            switch (vdt) {
                case MultiParticleDataCall::Particles::VERTDATA_FLOAT_XYZR:
                    rad = vdf[3];
                case MultiParticleDataCall::Particles::VERTDATA_FLOAT_XYZ:
                    box.Set(vdf[0] - rad, vdf[1] - rad, vdf[2] - rad,
                        vdf[0] + rad, vdf[1] + rad, vdf[2] + rad);
                    vdf = reinterpret_cast<const float*>(
                        reinterpret_cast<const char *>(vdf) + step);
                    break;
                case MultiParticleDataCall::Particles::VERTDATA_SHORT_XYZ:
                    box.Set(static_cast<float>(vds[0]) - rad,
                        static_cast<float>(vds[1]) - rad,
                        static_cast<float>(vds[2]) - rad,
                        static_cast<float>(vds[0]) + rad,
                        static_cast<float>(vds[1]) + rad,
                        static_cast<float>(vds[2]) + rad);
                    vds = reinterpret_cast<const short*>(
                        reinterpret_cast<const char *>(vds) + step);
                    break;
                default: continue;
            }
            if (!first) {
