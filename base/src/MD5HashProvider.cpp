...
void vislib::MD5HashProvider::finalise(BYTE *output, MD5_CTX *context) {
    ...
    MD5HashProvider::update(context, PADDING, padLen);
...
void vislib::MD5HashProvider::update(MD5_CTX *context, const BYTE *input, 
    ...
    if (cntInput >= partLen) {
        ...
        MD5HashProvider::transform(context->state, context->buffer);

        for (i = partLen; i + 63 < cntInput; i += 64) {
            MD5HashProvider::transform(context->state, input + i);
        }
