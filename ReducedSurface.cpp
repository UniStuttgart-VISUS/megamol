...
void ReducedSurface::ComputeReducedSurface() {
	...
	for( cnt1 = firstAtomIdx; cnt1 < ( firstAtomIdx + numberOfAtoms); ++cnt1 )
	{
		...
		radius = this->protein->AtomTypes()[this->protein->ProteinAtomData()[cnt1].TypeIndex()].Radius();
    ...
    */
	...
	//////////////////////////////////////////////////////////////////
	// check all chains if in global mode
	//////////////////////////////////////////////////////////////////
	if( globalRS )
	{
		std::cout << "global RS computation: check all chains!" << std::endl;
		// temporary variables
		...
		for( cnt1 = 0; cnt1 < this->protein->ProteinChainCount(); ++cnt1 )
		{
			// set the first atom index for the current chain
			firstAtom = this->protein->ProteinChain( cnt1).AminoAcid()[0].FirstAtomIndex();
			// set the number of atoms for the current chain
			if( ( cnt1 + 1) < this->protein->ProteinChainCount() )
			{
				atomCnt = protein->ProteinChain( cnt1 + 1).AminoAcid()[0].FirstAtomIndex() - firstAtom;
			}
			else
			{
				atomCnt = this->protein->ProteinAtomCount() - firstAtom;
			}
			...
			if( !tmpRSVertex->Treated() && tmpRSVertex->GetEdgeCount() == 0 && atomCnt > 2 )
			{
				...
				lastEdge = (unsigned int)this->rsEdge.size() - 1;
				// try to find the initial RS-face
				if( !this->FindFirstRSFace( this->rsVertex[xIdx]) ) // --> on the x-axis
					if( !this->FindFirstRSFace( this->rsVertex[yIdx]) ) // --> on the y-axis
						if( !this->FindFirstRSFace( this->rsVertex[zIdx]) ) // --> on the z-axis
...
void ReducedSurface::ComputeRSFace( unsigned int edgeIdx)
{
	...
	if( result >= 0 )
	{
		...
		if( face == NULL )
		{
			...
			face->SetProbeIndex(
				std::min( (unsigned int)this->voxelMapProbes.size()-1, (unsigned int)std::max(0, (int)floorf( ( probeCenterNewFace.GetX() - bBox.Left()) / voxelLength))),
				std::min( (unsigned int)this->voxelMapProbes[0].size()-1, (unsigned int)std::max(0, (int)floorf( ( probeCenterNewFace.GetY() - bBox.Bottom()) / voxelLength))),
				std::min( (unsigned int)this->voxelMapProbes[0][0].size()-1, (unsigned int)std::max(0, (int)floorf( ( probeCenterNewFace.GetZ() - bBox.Back()) / voxelLength))));
 ...
 */
float ReducedSurface::ComputeAngleBetweenProbes(
...
void ReducedSurface::ComputeVicinityEdge( RSEdge *edge)
{
	...
	xId = std::min( maxXId,
		(unsigned int)std::max( 0, (int)floorf( (edge->GetTorusCenter().GetX() - bBox.Left()) / voxelLength)));
	yId = std::min( maxYId,
		(unsigned int)std::max( 0, (int)floorf( (edge->GetTorusCenter().GetY() - bBox.Bottom()) / voxelLength)));
	zId = std::min( maxZId,
		(unsigned int)std::max( 0, (int)floorf( (edge->GetTorusCenter().GetZ() - bBox.Back()) / voxelLength)));
...
void ReducedSurface::ComputeProbeCutVertex( RSVertex *vertex)
{
	...
	// compute voxel indices for edge center
	xId = std::min( (unsigned int)std::max( 0,
						  (int)floorf( ( v1.GetX() - bBox.Left()) / voxelLength)), maxXId);
	yId = std::min( (unsigned int)std::max( 0,
						  (int)floorf( ( v1.GetY() - bBox.Bottom()) / voxelLength)), maxYId);
	zId = std::min( (unsigned int)std::max( 0,
											...
											const float upperThreshold)
{
	...
	for( cnt1 = firstAtomIdx; cnt1 < ( firstAtomIdx + numberOfAtoms); ++cnt1 )
	{
		...
		if( difference > upperThreshold )
		{
			...
			this->ComputeReducedSurface( protein);
 ...
 * RSEdge ctor
 */
ReducedSurface::RSEdge::RSEdge( RSVertex *v1, RSVertex *v2, 
 ...
 * RSFace ctor
 */
ReducedSurface::RSFace::RSFace( RSVertex *v1, RSVertex *v2, RSVertex *v3,
