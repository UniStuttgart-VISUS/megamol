...
vislib_gl::graphics::gl::FramebufferObject::~FramebufferObject() {
    try {
        ...
        VLTRACE(vislib::Trace::LEVEL_VL_WARN,
            "\"%s\" at line %d in \"%s\" when "
            "destroying FramebufferObject",
            e.GetMsgA(), e.GetLine(), e.GetFile());
    }
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::BindColourTexture
 */
GLenum vislib_gl::graphics::gl::FramebufferObject::BindColourTexture(const UINT which) {
    USES_GL_VERIFY;

    if (which < this->cntColourAttachments) {
        if (this->attachmentColour[which].state == ATTACHMENT_TEXTURE) {
            GL_VERIFY_RETURN(::glBindTexture(GL_TEXTURE_2D, this->attachmentColour[which].id));
        } else {
            throw vislib::IllegalStateException("The requested colour attachment "
        ...
        throw vislib::OutOfRangeException(which, 0, this->cntColourAttachments, __FILE__, __LINE__);
    }
...
GLenum vislib_gl::graphics::gl::FramebufferObject::BindDepthTexture() {
    ...
    if ((this->attachmentOther[ATTACH_IDX_DEPTH].state == ATTACHMENT_TEXTURE) ||
        ...
        GL_VERIFY_RETURN(::glBindTexture(GL_TEXTURE_2D, this->attachmentOther[ATTACH_IDX_DEPTH].id));
    } else {
        throw vislib::IllegalStateException("The depth attachment must be "
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::Create
 */
bool vislib_gl::graphics::gl::FramebufferObject::Create(const UINT width, const UINT height,
    const UINT cntColourAttachments, const ColourAttachParams* cap, const DepthAttachParams& dap,
    ...
    GL_DEFERRED_VERIFY(::glGetIntegerv(GL_FRAMEBUFFER_BINDING, &oldFb), __LINE__);

    /* Release possible old FBO before doing anything else! */
    try {
        this->Release(); // TODO: Could also return false instead of recreate.
    } catch (OpenGLException e) {
        VLTRACE(vislib::Trace::LEVEL_VL_WARN, "Release() of old FBO failed in Create(). "
    ...
    GL_DEFERRED_VERIFY(::glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, this->idFb), __LINE__);
    ...
    for (UINT i = 0; i < cntColourAttachments; i++) {
        ...
        this->createTexture(this->attachmentColour[i].id, cap[i].internalFormat, cap[i].format, cap[i].type);
        GL_DEFERRED_VERIFY(::glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT + i, GL_TEXTURE_2D,
                               this->attachmentColour[i].id, 0),
            __LINE__);
    }
    ...
    case ATTACHMENT_RENDERBUFFER:
        GL_DEFERRED_VERIFY_TRY(this->createRenderbuffer(this->attachmentOther[ATTACH_IDX_DEPTH].id, dap.format));
        GL_DEFERRED_VERIFY(::glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT,
                               GL_RENDERBUFFER_EXT, this->attachmentOther[ATTACH_IDX_DEPTH].id),
            __LINE__);
        break;

    case ATTACHMENT_TEXTURE:
        GL_DEFERRED_VERIFY_TRY(
            this->createTexture(this->attachmentOther[ATTACH_IDX_DEPTH].id, dap.format, GL_DEPTH_COMPONENT, GL_FLOAT));
        // TODO: are other formats than GL_FLOAT supported? Could not find one.
        GL_DEFERRED_VERIFY(::glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_TEXTURE_2D,
    ...
    GL_DEFERRED_VERIFY(::glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, oldFb), __LINE__);
...
GLenum vislib_gl::graphics::gl::FramebufferObject::Disable() throw() {
    ...
    if (::glBindFramebufferEXT == NULL) {
         ...
         * Disable anyway.
         */
        VLTRACE(vislib::Trace::LEVEL_VL_WARN, "glBindFramebuffer is not available.\n");
    ...
    try {
        if (!this->IsEnabled()) {
            VLTRACE(vislib::Trace::LEVEL_VL_WARN, "Trying to disable an FBO which is "
     ...
     */
    if (this->IsValid()) {
        GL_VERIFY_RETURN(::glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, this->oldFb));
        GL_VERIFY_RETURN(::glViewport(this->oldVp[0], this->oldVp[1], this->oldVp[2], this->oldVp[3]));
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::Enable
 */
GLenum vislib_gl::graphics::gl::FramebufferObject::Enable(const UINT colourAttachment) {
    USES_GL_VERIFY;

    /* Ensure that we enable only valid FBOs. */
    if (!this->IsValid()) {
        VLTRACE(vislib::Trace::LEVEL_VL_ERROR, "Cannot enable invalid FBO.\n");
        return GL_INVALID_OPERATION;
    }
     ...
     */
    if (this->IsEnabled()) {
        VLTRACE(vislib::Trace::LEVEL_VL_INFO, "FBO is already enabled.\n");
        return GL_NO_ERROR;
    }
    ...
    try {
        ...
        VLTRACE(vislib::Trace::LEVEL_VL_ERROR,
            "Could not save OpenGL state before "
            "enabling FBO (\"%s\").\n",
            e.GetMsgA());
    ...
    if (this->cntColourAttachments < 1) {
        ...
        GL_VERIFY_RETURN(::glDrawBuffer(GL_COLOR_ATTACHMENT0_EXT + colourAttachment));
        GL_VERIFY_RETURN(::glReadBuffer(GL_COLOR_ATTACHMENT0_EXT + colourAttachment));

    } else {
        /* Illegal colour attachment id. */
        ASSERT(colourAttachment >= this->cntColourAttachments);
        throw vislib::OutOfRangeException(colourAttachment, 0, this->cntColourAttachments, __FILE__, __LINE__);
    }
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::EnableMultipleV
 */
GLenum vislib_gl::graphics::gl::FramebufferObject::EnableMultipleV(
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::EnableMultiple
 */
GLenum vislib_gl::graphics::gl::FramebufferObject::EnableMultiple(UINT cntColourAttachments, ...) {
    va_list argptr;
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::GetColourTextureID
 */
GLuint vislib_gl::graphics::gl::FramebufferObject::GetColourTextureID(const UINT colourAttachment) const {

    if (this->cntColourAttachments < 1) {
        /* No colour attachment, this request is illegal. */
        throw vislib::IllegalStateException("The requested colour "
                                            "attachment is not a texture attachment.",
            __FILE__, __LINE__);
...
void vislib_gl::graphics::gl::FramebufferObject::Release() {
    ...
    if ((::glDeleteRenderbuffersEXT == NULL) || (::glDeleteFramebuffersEXT == NULL)) {
        /*
         * Extensions might not have been initialised, but dtor will call
         * Release anyway.
         */
        VLTRACE(vislib::Trace::LEVEL_VL_WARN, "glDeleteRenderbuffers or "
    ...
    for (UINT i = 0; i < 2; i++) {
        ...
        case ATTACHMENT_TEXTURE:
            if (::glIsTexture(this->attachmentOther[i].id)) {
                GL_DEFERRED_VERIFY(::glDeleteTextures(1, &this->attachmentOther[i].id), __LINE__);
            }
            break;

        case ATTACHMENT_RENDERBUFFER:
            if (::glIsRenderbufferEXT(this->attachmentOther[i].id)) {
                GL_DEFERRED_VERIFY(::glDeleteRenderbuffersEXT(1, &this->attachmentOther[i].id), __LINE__);
            }
            break;
    ...
    for (UINT i = 0; i < this->cntColourAttachments; i++) {
        ...
        case ATTACHMENT_TEXTURE:
            if (::glIsTexture(this->attachmentColour[i].id)) {
                GL_DEFERRED_VERIFY(::glDeleteTextures(1, &this->attachmentColour[i].id), __LINE__);
            }
            break;
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::ATTACH_IDX_DEPTH
 */
const UINT vislib_gl::graphics::gl::FramebufferObject::ATTACH_IDX_DEPTH = 0;


/*
 * vislib_gl::graphics::gl::FramebufferObject::ATTACH_IDX_STENCIL
 */
const UINT vislib_gl::graphics::gl::FramebufferObject::ATTACH_IDX_STENCIL = 1;


/*
 * vislib_gl::graphics::gl::FramebufferObject::FramebufferObject
 */
vislib_gl::graphics::gl::FramebufferObject::FramebufferObject(const FramebufferObject& rhs) {
    throw vislib::UnsupportedOperationException("FramebufferObject::FramebufferObject", __FILE__, __LINE__);
}


/*
 * vislib_gl::graphics::gl::FramebufferObject::createRenderbuffer
 */
void vislib_gl::graphics::gl::FramebufferObject::createRenderbuffer(GLuint& outID, const GLenum format) {
    USES_GL_DEFERRED_VERIFY;

    GL_DEFERRED_VERIFY(::glGenRenderbuffersEXT(1, &outID), __LINE__);
    GL_DEFERRED_VERIFY(::glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, outID), __LINE__);
    GL_DEFERRED_VERIFY(::glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, format, this->width, this->height), __LINE__);
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::createTexture
 */
void vislib_gl::graphics::gl::FramebufferObject::createTexture(
    GLuint& outID, const GLenum internalFormat, const GLenum format, const GLenum type) const {
    USES_GL_DEFERRED_VERIFY;
    ...
    GL_DEFERRED_VERIFY(::glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST), __LINE__);
    GL_DEFERRED_VERIFY(::glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST), __LINE__);
    GL_DEFERRED_VERIFY(
        ::glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, this->width, this->height, 0, format, type, NULL), __LINE__);
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::drawTexture
 */
GLenum vislib_gl::graphics::gl::FramebufferObject::drawTexture(
    const GLuint id, const GLint minFilter, const GLint magFilter, const double depth) const {
    USES_GL_VERIFY;
...
bool vislib_gl::graphics::gl::FramebufferObject::isComplete() const {
    ...
    case GL_FRAMEBUFFER_UNSUPPORTED_EXT:
        VLTRACE(vislib::Trace::LEVEL_ERROR, "The selected framebuffer format is "
                                            "unsupported.\n");
        /* falls through. */

    default:
        VLTRACE(vislib::Trace::LEVEL_ERROR,
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::readTexture
 */
GLenum vislib_gl::graphics::gl::FramebufferObject::readTexture(
    void* outData, const GLuint id, const GLenum format, const GLenum type) {
    USES_GL_VERIFY;
    USES_GL_DEFERRED_VERIFY;
    GLint oldTexState = 0;
    GLint oldTexId = 0;
    ...
    GL_DEFERRED_VERIFY(::glGetTexImage(GL_TEXTURE_2D, 0, format, type, outData), __LINE__);
...
/*
 * vislib_gl::graphics::gl::FramebufferObject::operator =(
 */
vislib_gl::graphics::gl::FramebufferObject& vislib_gl::graphics::gl::FramebufferObject::operator=(
