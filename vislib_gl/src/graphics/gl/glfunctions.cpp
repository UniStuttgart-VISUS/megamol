...
/*
 * vislib_gl::graphics::gl::EnableVSync
 */
bool vislib_gl::graphics::gl::EnableVSync(bool enable) {
#ifdef _WIN32
...
const vislib::VersionNumber& vislib_gl::graphics::gl::GLVersion() {
    ...
    if (number.GetMajorVersionNumber() == 0) {
        ...
        pos = verStr.Find('.');
        if (pos > 0) {
            major = vislib::CharTraitsA::ParseInt(verStr.Substring(0, pos));
        ...
        pos = verStr.Find('.');
        if (pos > 0) {
            minor = vislib::CharTraitsA::ParseInt(verStr.Substring(0, pos));
            verStr = verStr.Substring(pos + 1);

            // parse release number
            release = vislib::CharTraitsA::ParseInt(verStr);
