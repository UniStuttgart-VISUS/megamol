...
megamol::gui::AbstractWindow2::AbstractWindow2(std::string const& name) : name_(name) {}


void megamol::gui::AbstractWindow2::ApplyWindowSizePosition(bool consider_menu) {
    ...
    ImVec2 win_pos = this->win_config_.position;
    ImVec2 win_size = this->win_config_.size;
    if (this->win_config_.flags & ImGuiWindowFlags_AlwaysAutoResize) {
    ...
    this->win_config_.position = win_pos;
    // wc.config.reset_position = win_pos;
    ImGui::SetWindowPos(win_pos, ImGuiCond_Always);

    this->win_config_.size = win_size;
    // wc.config.reset_size = win_size;
    ImGui::SetWindowSize(win_size, ImGuiCond_Always);
}


void megamol::gui::AbstractWindow2::WindowContextMenu(bool menu_visible, bool& out_collapsing_changed) {
    ImGuiIO& io = ImGui::GetIO();
    ...
    bool window_maximized = (this->win_config_.size == window_viewport);
    ...
    if (ImGui::BeginPopupContextItem()) {
        ...
        if (ImGui::MenuItem(((!this->win_config_.collapsed) ? ("Collapse") : ("Expand")), "Double Left Click")) {
            this->win_config_.collapsed = !this->win_config_.collapsed;
            out_collapsing_changed = true;
        }

        if (ImGui::MenuItem("Full Width", nullptr)) {
            this->win_config_.size.x = viewport.x;
            this->win_config_.reset_pos_size = true;
        }
        ImGui::Separator();
...
/// DOCKING
        ...
        if (ImGui::ArrowButton("snap_left", ImGuiDir_Left)) {
            this->win_config_.position.x = 0.0f;
            this->win_config_.reset_pos_size = true;
            ImGui::CloseCurrentPopup();
        }
        ImGui::SameLine();
        if (ImGui::ArrowButton("snap_up", ImGuiDir_Up)) {
            this->win_config_.position.y = 0.0f;
            this->win_config_.reset_pos_size = true;
            ImGui::CloseCurrentPopup();
        }
        ImGui::SameLine();
        if (ImGui::ArrowButton("snap_down", ImGuiDir_Down)) {
            this->win_config_.position.y = viewport.y - this->win_config_.size.y;
            this->win_config_.reset_pos_size = true;
            ImGui::CloseCurrentPopup();
        }
        ImGui::SameLine();
        if (ImGui::ArrowButton("snap_right", ImGuiDir_Right)) {
            this->win_config_.position.x = viewport.x - this->win_config_.size.x;
            this->win_config_.reset_pos_size = true;
            ImGui::CloseCurrentPopup();
        }
        ImGui::Separator();
    ...
    if (toggle_window_size) {
        if (window_maximized) {
            // Window is maximized
            this->win_config_.size = this->win_config_.reset_size;
            this->win_config_.position = this->win_config_.reset_position;
            this->win_config_.reset_pos_size = true;
        } else {
            // Window is minimized
            window_viewport = ImVec2(viewport.x, viewport.y - y_offset);
            this->win_config_.reset_size = this->win_config_.size;
            this->win_config_.reset_position = this->win_config_.position;
            this->win_config_.size = window_viewport;
            this->win_config_.position = ImVec2(0.0f, y_offset);
            this->win_config_.reset_pos_size = true;
        }
    }
}
...
void megamol::gui::AbstractWindow2::StateFromJSON(const nlohmann::json& in_json) {
        ...
        if (header_item.key() == GUI_JSON_TAG_WINDOW_CONFIGS) {
            for (auto& config_item : header_item.value().items()) {
                if (config_item.key() == this->Name()) {
                    ...
                    this->win_config_.flags = static_cast<ImGuiWindowFlags>(win_flags);
                    megamol::core::utility::get_json_value<bool>(config_values, {"win_show"}, &this->win_config_.show);
                    std::array<int, 2> hotkey = {0, 0};
                    megamol::core::utility::get_json_value<int>(
                        config_values, {"win_hotkey"}, hotkey.data(), hotkey.size());
                    this->win_config_.hotkey = core::view::KeyCode(
...
void megamol::gui::AbstractWindow2::StateToJSON(nlohmann::json& inout_json) const {
    inout_json[GUI_JSON_TAG_WINDOW_CONFIGS][this->Name()]["win_show"] = this->win_config_.show;
    inout_json[GUI_JSON_TAG_WINDOW_CONFIGS][this->Name()]["win_flags"] = static_cast<int>(this->win_config_.flags);
    inout_json[GUI_JSON_TAG_WINDOW_CONFIGS][this->Name()]["win_hotkey"] = {
        static_cast<int>(this->win_config_.hotkey.key), this->win_config_.hotkey.mods.toInt()};
    inout_json[GUI_JSON_TAG_WINDOW_CONFIGS][this->Name()]["win_position"] = {
        this->win_config_.position.x, this->win_config_.position.y};
    inout_json[GUI_JSON_TAG_WINDOW_CONFIGS][this->Name()]["win_size"] = {
        this->win_config_.size.x, this->win_config_.size.y};
    inout_json[GUI_JSON_TAG_WINDOW_CONFIGS][this->Name()]["win_reset_size"] = {
        this->win_config_.reset_size.x, this->win_config_.reset_size.y};
    inout_json[GUI_JSON_TAG_WINDOW_CONFIGS][this->Name()]["win_reset_position"] = {
        this->win_config_.reset_position.x, this->win_config_.reset_position.y};
    inout_json[GUI_JSON_TAG_WINDOW_CONFIGS][this->Name()]["win_collapsed"] = this->win_config_.collapsed;
}
