...
namespace gui {
    ...
    public:
        ...
        static void DrawPlot(std::string name, ImVec2 size, const ImPlotAxisFlags& y_flags,
            megamol::gui::ProfilingUtils::MetricType display_idx,
            const megamol::core::MultiPerformanceHistory& history) {
            if (ImPlot::BeginPlot(
                    name.c_str(), nullptr, "ms", size, ImPlotFlags_None, ImPlotAxisFlags_AutoFit, y_flags)) {
